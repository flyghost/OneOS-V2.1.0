...
void os_arch_fault_exception(void *stack_frame, os_size_t *msp, os_size_t *psp)
{
    ...
    struct stack_frame_nofpu    *nofpuframe;
    ...
    {
        nofpuframe = (struct stack_frame_nofpu *)stack_common;
...
os_err_t task_stack_show(char *name, os_uint32_t context)
{
    ...
    if (task != os_task_self())
    {
        ...
        {
            float_mode = 0;
            sp_func_call =  sp_top + sizeof(struct stack_frame_nofpu) / sizeof(os_size_t);
        }
        else
        {
            float_mode = 1;
        ...
        if (1 == float_mode)
        {
            ...
            record.depth = 0;
            record.back_trace[record.depth++] = fpu_frame->pc;
            if ((fpu_frame->lr - 1) != fpu_frame->pc)
            {
                record.back_trace[record.depth++] = fpu_frame->lr - 1;
            }
        }
        /* Stack frame without floating point storage. */
        else
        {
            ...
            record.depth = 0;
            record.back_trace[record.depth++] = nofpu_frame->pc;
            if ((nofpu_frame->lr - 1) != nofpu_frame->pc)
            {
                record.back_trace[record.depth++] = nofpu_frame->lr - 1;
            }
        }
...
void _arch_exception_stack_show(void *stack_frame, os_size_t *msp, os_size_t *psp)
{
    ...
    else
    {
            ...
            || (sp_top < (os_size_t*)(os_task_self()->stack_begin)))
        {
            ...
            {
                sp_bottom = sp_top + (sizeof(struct stack_frame_nofpu) + TASK_STACK_OVERFLOW_STACK_SIZE )
                                     / sizeof(os_size_t);
            }
    ...
    {
        nofpu_frame = (struct stack_frame_nofpu *)stack_frame;
