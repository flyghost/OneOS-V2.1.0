 ...
 *
 * @file        arch_exc_nonsecure.c
 *
 * @brief       This file provides exception functions related to the Cortex-V8M nonsecure zone.
 *
 * @revision
 * Date         Author          Notes
...
void SECURE_hard_fault_track(void);
 ...
 ***********************************************************************************************************************
 */
void arch_fault_exception_nonsecure(void *stack_frame, os_size_t *msp, os_size_t *psp)
{
    if (OS_NULL != os_exception_hook)
    {
        os_err_t result;

        result = os_exception_hook(stack_frame, msp, psp);
        if (OS_EOK == result) 
        {
            return;
        }
    ...
    os_kprintf("Hard fault in nonsecure zone\r\n");
    ...
    if (!(stack_common->exc_return & EXCEPTION_STACK_FRAME_TYPE_MASK))
    {
        ...
        os_kprintf("FPU active! FPCSR is 0x%08x\r\n", fpu_frame->cpu_frame_fpu.FPSCR);
...
os_err_t task_stack_show(char *name, os_uint32_t context)
{
    ...
    if (task != os_task_self())
    {
        ...
        exc_return = ((struct stack_frame_common *)sp_top)->exc_return;

        if (exc_return & EXCEPTION_STACK_FRAME_TYPE_MASK)
        {
            float_mode = 0;
        ...
        if (1 == float_mode)
        {
            ...
            record.depth = 0;
            record.back_trace[record.depth++] = fpu_frame->cpu_frame_fpu.pc;
            
            if ((fpu_frame->cpu_frame_fpu.lr - 1) != fpu_frame->cpu_frame_fpu.pc)
            {
                record.back_trace[record.depth++] = fpu_frame->cpu_frame_fpu.lr - 1;
            }
        }
        /* Stack frame without floating point storage. */
        else
        {
            nofpu_frame = (struct stack_frame *)sp_top;
            ...
            record.depth = 0;
            record.back_trace[record.depth++] = nofpu_frame->cpu_frame.pc;
            if ((nofpu_frame->cpu_frame.lr - 1) != nofpu_frame->cpu_frame.pc)
            {
                record.back_trace[record.depth++] = nofpu_frame->cpu_frame.lr - 1;
            }
        }
 ...
 ***********************************************************************************************************************
 */ 
void _arch_exception_stack_show(void *stack_frame, os_size_t *msp, os_size_t *psp)
{
    /* The place where the function call backtrace starts after removing the registers in the stack. */
    os_size_t   *sp_func_call;
    ...
    exc_return = ((struct stack_frame_common *)stack_frame)->exc_return;
    ...
    /* According to the exc_return obtain whether to use floating point. */
    if (exc_return & EXCEPTION_STACK_FRAME_TYPE_MASK)
    {
        float_mode = 0;
        sp_func_call =  sp_top + sizeof(struct stack_frame) / sizeof(os_size_t);
    ...
    else
    {
            ...
            || (sp_top < (os_size_t*)(os_task_self()->stack_begin)))
        {
            if(1 == float_mode)
            {
                sp_bottom = sp_top + (sizeof(struct stack_frame_fpu) + TASK_STACK_OVERFLOW_STACK_SIZE )
                                     / sizeof(os_size_t);
            }
            else
            {
    ...
    {
        nofpu_frame = (struct stack_frame *)stack_frame;
