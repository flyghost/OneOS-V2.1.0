...
static inline void flush_icache_line_indexed(os_ubase_t addr)
{
    cache_op(INDEX_INVALIDATE_I, addr);
}

static inline void flush_dcache_line_indexed(os_ubase_t addr)
{
    cache_op(INDEX_WRITEBACK_INV_D, addr);
}

static inline void flush_icache_line(os_ubase_t addr)
{
    cache_op(HIT_INVALIDATE_I, addr);
}

static inline void lock_icache_line(os_ubase_t addr)
{
    cache_op(FETCH_AND_LOCK_I, addr);
}

static inline void lock_dcache_line(os_ubase_t addr)
{
    cache_op(FETCH_AND_LOCK_D, addr);
}

static inline void flush_dcache_line(os_ubase_t addr)
{
    cache_op(HIT_WRITEBACK_INV_D, addr);
}

static inline void invalidate_dcache_line(os_ubase_t addr)
{
    cache_op(HIT_INVALIDATE_D, addr);
}
static inline void blast_dcache16(void)
{
    os_ubase_t start = KSEG0BASE;
    os_ubase_t end = start + g_mips_core.dcache_size;
    os_ubase_t addr;
...
{
    os_ubase_t start = KSEG0BASE;
    os_ubase_t end = start + g_mips_core.icache_size;
    os_ubase_t addr;
