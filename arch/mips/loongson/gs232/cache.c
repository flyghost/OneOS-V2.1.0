...
int identify_cpu(void)
{
    ...
    os_kprintf("CPU configure: 0x%08x\n", read_c0_config());
    cpu_id = read_c0_prid();
    switch (cpu_id)
    {
        case PRID_LS1C:
            os_kprintf("CPU:Loongson 1C\n");
            ...
            break;
        default:
            os_kprintf("Unknown CPU type, system halted!\n");
            while (1)
            {
                ;
            }
...
void probe_cache(void)
{
    ...
    os_kprintf("DCache %2dkb, linesize %d bytes.\n", dcache_size >> 10, dcache_line_size);
    os_kprintf("ICache %2dkb, linesize %d bytes.\n", icache_size >> 10, icache_line_size);
...
void os_hw_cache_init(void)
{
    ...
    /* enable cache */
    enable_cpu_cache();
    os_kprintf("enable cpu cache done\n");
