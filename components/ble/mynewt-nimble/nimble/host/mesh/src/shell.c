...
#define MYNEWT_OS_TASK_STACK_DEFINE
...
{
	while (1) {
		ble_npl_eventq_run(&mesh_shell_queue);
	}
}

static void bt_mesh_shell_task_init(void)
{
	ble_npl_eventq_init(&mesh_shell_queue);

	nimble_task_init(&mesh_shell_task, "mesh_sh", mesh_shell_thread, NULL,
		     BLE_MESH_SHELL_TASK_PRIO, OS_WAIT_FOREVER, g_blemesh_shell_stack,
		     BLE_MESH_SHELL_STACK_SIZE);
}

const struct shell_cmd *get_mesh_shell_cmd(void) {
	return mesh_commands;
}

#include <shell.h>
extern os_err_t os_task_msleep(os_uint32_t ms);
static os_err_t nimble_mesh_shell(os_int32_t argc, char **argv)
{
    const struct shell_cmd *shell_cmd_p = &mesh_commands[0];

    if (2 > argc)
    {
        return -10;
    }
    while (shell_cmd_p->sc_cmd_func)
    {
        if (0 == strcmp(shell_cmd_p->sc_cmd, argv[1]))
        {
            shell_cmd_p->sc_cmd_func(argc - 1, &argv[1]);
            break;
        }
        shell_cmd_p++;
    }
...
SH_CMD_EXPORT(ble_mesh_shell, nimble_mesh_shell, "nimble mesh shell cmds");

static os_err_t nimble_mesh_shell_cmd_list(os_int32_t argc, char **argv)
{
    const struct shell_cmd *shell_cmd_p = &mesh_commands[0];
    ...
    while (shell_cmd_p->sc_cmd_func)
    {
        console_printf(shell_cmd_p->sc_cmd);
        if(shell_cmd_p->help) {
            console_printf("\t:%s", shell_cmd_p->help->summary);
        }
        console_printf("\n");
        os_task_msleep(1);
        shell_cmd_p++;
...
SH_CMD_EXPORT(ble_mesh_sh_list, nimble_mesh_shell_cmd_list, "nimble mesh shell cmds list");

void print_cmd_help(const struct shell_cmd *shell_cmd_p)
{
    const struct shell_cmd_help *help_stru = shell_cmd_p->help;
    const struct shell_param *param_stru;
    ...
    if(help_stru) {
        ...
        if (help_stru->params)
        {
            param_stru = help_stru->params;
            while (param_stru->param_name)
            {
                console_printf("param_name:%s\t", param_stru->param_name);
                console_printf("help:%s\n", param_stru->help);
                param_stru++;
            }
            os_task_msleep(1);
        }
...
static os_err_t nimble_mesh_shell_cmd_help(os_int32_t argc, char **argv)
{
    const struct shell_cmd *shell_cmd_p = &mesh_commands[0];
...
SH_CMD_EXPORT(ble_mesh_sh_help, nimble_mesh_shell_cmd_help, "nimble mesh shell cmds help");

#endif

// 'ble_mesh_shell_init' is only used with mynewt shell
#if MYNEWT
