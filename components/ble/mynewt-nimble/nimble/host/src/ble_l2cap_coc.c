...
ble_l2cap_coc_continue_tx(struct ble_l2cap_chan *chan)
{
    ...
    while (tx->credits) {
        ...
        if (!txom) {
            ...
            console_printf("no mem x1, len = %d\r\n", len);
            rc = BLE_HS_ENOMEM;
            goto failed;
        }
        ...
        console_printf("block_size=%d, num_blocks=%d, num_free=%d, min_free=%d\r\n", mem->mp_block_size, mem->mp_num_blocks, mem->mp_num_free, mem->mp_min_free);

        rc = os_mbuf_appendfrom(txom, tx->sdu, tx->data_offset,
                                len - sdu_size_offset);
        if (rc) {
            console_printf("no mem x3, len1=%d,len2=%d,len3=%d\r\n", len, sdu_size_offset, len - sdu_size_offset);
            rc = BLE_HS_ENOMEM;
            BLE_HS_LOG(DEBUG, "Could not append data rc=%d", rc);
            goto failed;
        }
