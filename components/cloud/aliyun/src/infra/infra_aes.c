...
#if defined(INFRA_PADLOCK_C) &&                      \
    ( defined(INFRA_HAVE_X86) || defined(INFRA_PADLOCK_ALIGN16) )
static int aes_padlock_ace = -1;
#endif

#if defined(INFRA_AES_ROM_TABLES)
/*
 * Forward S-box
 */
...
void infra_aes_free( infra_aes_context *ctx )
{
    ...
    infra_aes_zeroize( ctx, sizeof( infra_aes_context ) );
}

/*
 * AES key schedule (encryption)
 */
#if !defined(INFRA_AES_SETKEY_ENC_ALT)
                    ...
                    unsigned int keybits )
{
    ...
    switch( keybits )
    {
        ...
        default : return( INFRA_ERR_AES_INVALID_KEY_LENGTH );
    }

#if defined(INFRA_PADLOCK_C) && defined(INFRA_PADLOCK_ALIGN16)
    if( aes_padlock_ace == -1 )
        aes_padlock_ace = infra_aes_padlock_has_support( INFRA_PADLOCK_ACE );

    if( aes_padlock_ace )
        ctx->rk = RK = INFRA_PADLOCK_ALIGN16( ctx->buf );
    else
#endif
    ctx->rk = RK = ctx->buf;

#if defined(INFRA_AESNI_C) && defined(INFRA_HAVE_X86_64)
    if( infra_aesni_has_support( INFRA_AESNI_AES ) )
        return( infra_aesni_setkey_enc( (unsigned char *) ctx->rk, key, keybits ) );
#endif
                    ...
                    unsigned int keybits )
{
    ...
    /* Also checks keybits */
    if( ( ret = infra_aes_setkey_enc( &cty, key, keybits ) ) != 0 )
        goto exit;
    ...
    if( infra_aesni_has_support( INFRA_AESNI_AES ) )
    {
        infra_aesni_inverse_key( (unsigned char *) ctx->rk,
...
#endif /* !INFRA_AES_SETKEY_DEC_ALT */
                    ...
                    unsigned char output[16] )
{
#if defined(INFRA_AESNI_C) && defined(INFRA_HAVE_X86_64)
    if( infra_aesni_has_support( INFRA_AESNI_AES ) )
        return( infra_aesni_crypt_ecb( ctx, mode, input, output ) );
#endif

#if defined(INFRA_PADLOCK_C) && defined(INFRA_HAVE_X86)
    if( aes_padlock_ace )
    {
        if( infra_aes_padlock_xcryptecb( ctx, mode, input, output ) == 0 )
            return( 0 );
    ...
    if( mode == INFRA_AES_ENCRYPT )
        return( infra_aes_internal_aes_encrypt( ctx, input, output ) );
    else
        return( infra_aes_internal_aes_decrypt( ctx, input, output ) );
}
                    ...
                    unsigned char *output )
{
    ...
    if( length % 16 )
        return( INFRA_ERR_AES_INVALID_INPUT_LENGTH );

#if defined(INFRA_PADLOCK_C) && defined(INFRA_HAVE_X86)
    if( aes_padlock_ace )
    {
        if( infra_aes_padlock_xcryptcbc( ctx, mode, length, iv, input, output ) == 0 )
            return( 0 );
                       ...
                       unsigned char *output )
{
    ...
    if( mode == INFRA_AES_DECRYPT )
    {
        ...
        {
            if( n == 0 )
                infra_aes_crypt_ecb( ctx, INFRA_AES_ENCRYPT, iv, iv );
