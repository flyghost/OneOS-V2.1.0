 ...
 * @file        hal_udp.c
 *
 * @brief      a port file of udp for iotkit
 *
 * @details     
 *
 * @revision
...
int HAL_UDP_write(intptr_t p_socket, const unsigned char *p_data, unsigned int datalen)
{
    ...
    rc        = sendto(socket_id, (char *)p_data, (int)datalen, 0, &to_addr, sizeof(struct sockaddr));
    if (-1 == rc)
    {
        return -1;
    }
...
int HAL_UDP_readTimeout(intptr_t p_socket, unsigned char *p_data, unsigned int datalen, unsigned int timeout)
{
    ...
    fd_set         read_fds;
    long           socket_id = -1;
    unsigned int fromlen = sizeof(struct addrinfo);
    ...
    return recvfrom(p_socket, p_data, datalen, 0, &to_addr, &fromlen);
}
...
intptr_t HAL_UDP_create_without_connect(const char *host, unsigned short port)
{
    ...
    if (sockfd < 0)
    {
        LOG_E(DBG_EXT_TAG, "socket error");
        return -1;
    }
    if (0 == port)
    {
    ...
    {
        LOG_E(DBG_EXT_TAG, "setsockopt error");
        closesocket(sockfd);
        return -1;
    }
    ...
    else
    {
        ...
        else
        {
            ...
            if (!hp)
            {
                LOG_E(DBG_EXT_TAG, "can't resolute the host address");
                closesocket(sockfd);
                return -1;
            }
    ...
    LOG_D(DBG_EXT_TAG, "success to establish udp, fd=%d", (int)sockfd);

    return (intptr_t)sockfd;
}

int HAL_UDP_connect(intptr_t sockfd, const char *host, unsigned short port)
{
    int              rc          = -1;
    char             port_ptr[6] = {0};
    struct addrinfo  hints;
    ...
    LOG_D(DBG_EXT_TAG, "HAL_UDP_connect, host=%s, port=%d", host, port);
    sprintf(port_ptr, "%u", port);
    memset((char *)&hints, 0x00, sizeof(hints));
    hints.ai_socktype = SOCK_DGRAM;
    hints.ai_family   = AF_INET;
                   ...
                   unsigned int         timeout_ms)
{
    ...
    if (ret < 0)
    {
        LOG_E(DBG_EXT_TAG, "sendto error");
    }
