...
#include <sys/ioctl.h>
#include <errno_ext.h>
#include <signal_ext.h>
#define _DEFAULT_SOURCE
#include <azure_c_shared_utility/if.h>
...
static NETWORK_INTERFACE_DESCRIPTION *create_network_interface_description(struct iotcore_ifreq          *ifr,
                                                                           NETWORK_INTERFACE_DESCRIPTION *previous_nid)
{
    ...
    else
    {
        ...
        else if (sprintf(result->mac_address,
...
static int get_network_interface_descriptions(int socket, NETWORK_INTERFACE_DESCRIPTION **nid)
{
    ...
    else
    {
        ...
        struct iotcore_ifreq             *it  = ifc.ifc_req;
        const struct iotcore_ifreq *const end = it + (ifc.ifc_len / sizeof(struct ifreq));
        ...
        else
        {
            ...
            if (socket_io_instance->socket < SOCKET_SUCCESS)
            {
                ...
                result = __FAILURE__;
                     ...
                     set_target_network_interface(socket_io_instance->socket, socket_io_instance->target_mac_address) !=
                         0)
            {
        ...
        else
        {
            ...
            else
            {
                ...
                if (send_result != size)
                {
                    ...
                    {
                        if (errno == EAGAIN) /*send says "come back later" with EAGAIN - likely the socket buffer cannot
                                                accept more data*/
                        {
                            /*do nothing*/
                            result = 0;
                    ...
                    else
                    {
                        ...
                        if (add_pending_io(socket_io_instance,
                                           (const unsigned char *)(buffer) + send_result,
                                           size - send_result,
                                           on_send_complete,
                                           callback_context) != 0)
                        {
...
void socketio_dowork(CONCRETE_IO_HANDLE socket_io)
{
    if (socket_io != NULL)
    {
        ...
        {
            int  received = 0;
#ifndef NET_USING_MOLINK
            long n        = 1;
            ioctlsocket(socket_io_instance->socket, FIONBIO, &n);
#else
            struct timeval timeout;
            timeout.tv_sec = 0;
            timeout.tv_usec = 50 * 1000;
            setsockopt(socket_io_instance->socket, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(struct timeval));
#endif
            do
            {
                os_task_msleep(5);
                received = recv(socket_io_instance->socket, socket_io_instance->recv_bytes, RECEIVE_BYTES_VALUE, MSG_DONTWAIT);
                if (received > 0)
                {
                ...
                else if (received < 0 && errno != 0 && errno != EINTR && errno != EAGAIN)
...
int socketio_setoption(CONCRETE_IO_HANDLE socket_io, const char *optionName, const void *value)
{
    ...
    else
    {
        ...
        if (strcmp(optionName, "tcp_recvtimeo") == 0)
        {
            result =
                setsockopt(socket_io_instance->socket, SOL_SOCKET, SO_RCVTIMEO, (char *)value, sizeof(struct timeval));
            if (result == -1)
                result = errno;
        }    // 2020.11.25
