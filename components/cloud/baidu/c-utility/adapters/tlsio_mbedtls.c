...
typedef struct TLS_IO_INSTANCE_TAG
{
    ...
    mbedtls_x509_crt         x509certificate_parsed;
...
static const IO_INTERFACE_DESCRIPTION tlsio_mbedtls_interface_description = {tlsio_mbedtls_retrieveoptions,
...
static void on_underlying_io_bytes_received(void *context, const unsigned char *buffer, size_t size)
{
    ...
    unsigned char *new_socket_io_read_bytes =
        (unsigned char *)realloc(tls_io_instance->socket_io_read_bytes,
...
static int on_io_recv(void *context, unsigned char *buf, size_t sz)
{
    ...
    if (result > 0)
    {
        ...
        (void)memmove(tls_io_instance->socket_io_read_bytes,
                      tls_io_instance->socket_io_read_bytes + result,
                      tls_io_instance->socket_io_read_byte_count - result);
        tls_io_instance->socket_io_read_byte_count -= result;
        if (tls_io_instance->socket_io_read_byte_count > 0)
        {
            new_socket_io_read_bytes = (unsigned char *)realloc(tls_io_instance->socket_io_read_bytes,
                                                                tls_io_instance->socket_io_read_byte_count);
            if (new_socket_io_read_bytes != NULL)
            {
                tls_io_instance->socket_io_read_bytes = new_socket_io_read_bytes;
            }
...
static int on_io_send(void *context, const unsigned char *buf, size_t sz)
{
    ...
    if (xio_send(tls_io_instance->socket_io,
                 buf,
                 sz,
                 tls_io_instance->on_send_complete,
                 tls_io_instance->on_send_complete_callback_context) != 0)
    {
        tls_io_instance->tlsio_state = TLSIO_STATE_ERROR;
        indicate_error(tls_io_instance);
        result = 0;
...
static void mbedtls_init(void *instance, const char *host)
{
    ...
    mbedtls_x509_crt_init(&result->x509certificate_parsed);
    mbedtls_pk_init(&result->pk);
    mbedtls_entropy_add_source(&result->entropy, tlsio_entropy_poll, NULL, 128, 0);
    mbedtls_ctr_drbg_seed(&result->ctr_drbg,
                          mbedtls_entropy_func,
                          &result->entropy,
                          (const unsigned char *)pers,
                          strlen(pers));
    mbedtls_ssl_config_defaults(&result->config,
    ...
    //	  mbedtls_ssl_set_bio(&result->ssl, instance, mbedtls_net_send, mbedtls_net_recv, NULL);
...
void tlsio_mbedtls_destroy(CONCRETE_IO_HANDLE tls_io)
{
    if (tls_io != NULL)
    {
        ...
        mbedtls_x509_crt_free(&tls_io_instance->x509certificate_parsed);
                       ...
                       void               *on_io_error_context)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (xio_open(tls_io_instance->socket_io,
...
int tlsio_mbedtls_close(CONCRETE_IO_HANDLE tls_io, ON_IO_CLOSE_COMPLETE on_io_close_complete, void *callback_context)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (xio_close(tls_io_instance->socket_io, on_underlying_io_close_complete_during_close, tls_io_instance) !=
                0)
            {
                result = __FAILURE__;
            }
...
int tlsio_mbedtls_send(CONCRETE_IO_HANDLE tls_io,
                       const void        *buffer,
                       size_t             size,
                       ON_SEND_COMPLETE   on_send_complete,
    ...
    else
    {
        ...
        else
        {
            ...
            if ((underlying_io_options = xio_retrieveoptions(tls_io_instance->socket_io)) == NULL ||
                OptionHandler_AddOption(result, OPTION_UNDERLYING_IO_OPTIONS, underlying_io_options) !=
                    OPTIONHANDLER_OK)
            {
                ...
                result = NULL;
                     ...
                     OptionHandler_AddOption(result, OPTION_TRUSTED_CERT, tls_io_instance->trusted_certificates) !=
                         OPTIONHANDLER_OK)
            {
...
int tlsio_mbedtls_setoption(CONCRETE_IO_HANDLE tls_io, const char *optionName, const void *value)
{
    ...
    else
    {
        ...
        if (strcmp(OPTION_TRUSTED_CERT, optionName) == 0)
        {
            ...
            {
                int parse_result = mbedtls_x509_crt_parse(&tls_io_instance->trusted_certificates_parsed,
                ...
                {
                    mbedtls_ssl_conf_ca_chain(&tls_io_instance->config,
                                              &tls_io_instance->trusted_certificates_parsed,
                                              NULL);
                    result = 0;
                }
            }
        ...
        else if (strcmp(OPTION_X509_CERT, optionName) == 0)
        {
            ...
            else
            {
                ...
                {
                    int parse_result = mbedtls_x509_crt_parse(&tls_io_instance->x509certificate_parsed,
                                                              (const unsigned char *)value,
                                                              (int)(strlen(value) + 1));
                    if (parse_result != 0)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            int set_key_result = mbedtls_ssl_conf_own_cert(&tls_io_instance->config,
                                                                           &tls_io_instance->x509certificate_parsed,
                                                                           &tls_io_instance->pk);
        ...
        else if (strcmp(OPTION_X509_PRIVATE_KEY, optionName) == 0)
        {
            ...
            else
            {
                ...
                {
                    int parse_result = mbedtls_pk_parse_key(&tls_io_instance->pk,
                                                            (const unsigned char *)value,
                                                            (int)(strlen(value) + 1),
                                                            NULL,
                                                            0);
...
// DEPRECATED: the BAIDUIOT_CLOUD_WITH_TLS #define is deprecated.
#endif    // BAIDUIOT_CLOUD_WITH_TLS
