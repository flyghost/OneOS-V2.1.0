...
{
    return (strcmp(name, OPTION_TRUSTED_CERT) == 0) || (strcmp(name, SU_OPTION_X509_CERT) == 0) ||
           (strcmp(name, SU_OPTION_X509_PRIVATE_KEY) == 0) || (strcmp(name, OPTION_X509_ECC_CERT) == 0) ||
...
TLSIO_OPTIONS_RESULT tlsio_options_clone_option(const char *name, const void *value, void **out_value)
{
    ...
    {
        LogError("NULL parameter: name: %p, value: %p, out_value: %p", name, value, out_value);
        result = TLSIO_OPTIONS_RESULT_ERROR;
    }
    else if (is_supported_string_option(name))
    {
...
TLSIO_OPTIONS_RESULT tlsio_options_set(TLSIO_OPTIONS *options, const char *optionName, const void *value)
{
    TLSIO_OPTIONS_RESULT result;
    ...
    {
        LogError("NULL parameter: options: %p, optionName: %p, value: %p", options, optionName, value);
        result = TLSIO_OPTIONS_RESULT_ERROR;
    }
    ...
    {
        TLSIO_OPTIONS_x509_TYPE this_type =
            (strcmp(SU_OPTION_X509_CERT, optionName) == 0) ? TLSIO_OPTIONS_x509_TYPE_RSA : TLSIO_OPTIONS_x509_TYPE_ECC;
    ...
    {
        TLSIO_OPTIONS_x509_TYPE this_type = (strcmp(SU_OPTION_X509_PRIVATE_KEY, optionName) == 0)
                                                ? TLSIO_OPTIONS_x509_TYPE_RSA
                                                : TLSIO_OPTIONS_x509_TYPE_ECC;
