...
BUFFER_HANDLE Base64_Decoder(const char *source)
{
    ...
    else
    {
        ...
        {
            /*Codes_SRS_BASE64_06_011: [If the source string has an invalid length for a base 64 encoded string then
             * Base64_Decode shall return NULL.]*/
        ...
        else
        {
            ...
            {
                /*Codes_SRS_BASE64_06_010: [If there is any memory allocation failure during the decode then
                 * Base64_Decode shall return NULL.]*/
            ...
            else
            {
                ...
                /*Codes_SRS_BASE64_06_009: [If the string pointed to by source is zero length then the handle returned
                 * shall refer to a zero length buffer.]*/
...
static STRING_HANDLE Base64_Encode_Internal(const unsigned char *source, size_t size)
{
    ...
    /*Codes_SRS_BASE64_06_006: [If when allocating memory to produce the encoding a failure occurs then Base64_Encoder
     * shall return NULL.]*/
    ...
    else
    {
        ...
        */
        ...
        while (size - currentPosition >= 3)
        {
            ...
            char c2 = base64char(((source[currentPosition] & 3) << 4) | (source[currentPosition + 1] >> 4));
            char c3 =
                base64char(((source[currentPosition + 1] & 0x0F) << 2) | ((source[currentPosition + 2] >> 6) & 3));
            char c4 = base64char(source[currentPosition + 2] & 0x3F);
            currentPosition += 3;
        ...
        {
            char c1 = base64char(source[currentPosition] >> 2);
            char c2 = base64char(((source[currentPosition] & 0x03) << 4) | (source[currentPosition + 1] >> 4));
        ...
        /*Codes_SRS_BASE64_06_007: [Otherwise Base64_Encoder shall return a pointer to STRING, that string contains the
         * base 64 encoding of input.]*/
...
STRING_HANDLE Base64_Encode_Bytes(const unsigned char *source, size_t size)
{
    ...
    /*Codes_SRS_BASE64_02_002: [If source is not NULL and size is zero, then Base64_Encode_Bytes shall produce an empty
     * STRING_HANDLE.] */
    else if (size == 0)
    {
...
STRING_HANDLE Base64_Encoder(BUFFER_HANDLE input)
{
    ...
    /*1. the "data" of the binary shall be "eaten" 3 characters at a time and produce 4 base64 encoded characters for as
     * long as there are more than 3 characters still to process*/
    ...
    /*the encoding will use the optional [=] or [==] at the end of the encoded string, so that other less standard aware
     * libraries can do their work*/
    ...
    else
    {
        ...
        if ((BUFFER_content(input, &inputBinary) != 0) || (BUFFER_size(input, &inputSize) != 0))
        {
            result = NULL;
