...
BUFFER_HANDLE BUFFER_create(const unsigned char *source, size_t size)
{
    ...
    {
        /*Codes_SRS_BUFFER_02_002: [Otherwise, BUFFER_create shall allocate memory to hold size bytes and shall copy
         * from source size bytes into the newly allocated memory.] */
        ...
        {
            /* Codes_SRS_BUFFER_02_005: [If size parameter is 0 then 1 byte of memory shall be allocated yet size of the
             * buffer shall be set to 0.]*/
...
/* Codes_SRS_BUFFER_07_003: [BUFFER_delete shall delete the data associated with the BUFFER_HANDLE along with the
 * Buffer.] */
void BUFFER_delete(BUFFER_HANDLE handle)
{
    ...
    if (handle != NULL)
    {
        ...
        if (b->buffer != NULL)
        {
            /* Codes_SRS_BUFFER_07_003: [BUFFER_delete shall delete the data associated with the BUFFER_HANDLE along
             * with the Buffer.] */
            free(b->buffer);
        }
        free(b);
...
/* Codes_SRS_BUFFER_07_008: [BUFFER_build allocates size_t bytes, copies the unsigned char* into the buffer and returns
 * zero on success.] */
int BUFFER_build(BUFFER_HANDLE handle, const unsigned char *source, size_t size)
{
    ...
    /* Codes_SRS_BUFFER_01_002: [The size argument can be zero, in which case the underlying buffer held by the buffer
     * instance shall be freed.] */
    else if (size == 0)
    {
    ...
    else
    {
        ...
        {
            BUFFER *b = (BUFFER *)handle;
            /* Codes_SRS_BUFFER_07_011: [BUFFER_build shall overwrite previous contents if the buffer has been
            ...
            else
            {
                b->buffer = newBuffer;
                b->size   = size;
                /* Codes_SRS_BUFFER_01_002: [The size argument can be zero, in which case nothing shall be copied from
                 * source.] */
...
int BUFFER_append_build(BUFFER_HANDLE handle, const unsigned char *source, size_t size)
{
    ...
    {
        /* Codes_SRS_BUFFER_07_029: [ BUFFER_append_build shall return nonzero if handle or source are NULL or if size
         * is 0. ] */
        LogError("BUFFER_append_build failed invalid parameter handle: %p, source: %p, size: %uz",
                 handle,
                 source,
                 size);
        result = __FAILURE__;
    ...
    else
    {
        ...
        {
            /* Codes_SRS_BUFFER_07_030: [ if handle->buffer is NULL BUFFER_append_build shall allocate the a buffer of
             * size bytes... ] */
            if (BUFFER_safemalloc(handle, size) != 0)
            {
                /* Codes_SRS_BUFFER_07_035: [ If any error is encountered BUFFER_append_build shall return a non-null
                 * value. ] */
        ...
        {
            /* Codes_SRS_BUFFER_07_032: [ if handle->buffer is not NULL BUFFER_append_build shall realloc the buffer to
             * be the handle->size + size ] */
...
int BUFFER_pre_build(BUFFER_HANDLE handle, size_t size)
{
    ...
    else
    {
        ...
        if (b->buffer != NULL)
        {
            /* Codes_SRS_BUFFER_07_007: [BUFFER_pre_build shall return nonzero if the buffer has been previously
...
/* Codes_SRS_BUFFER_07_012: [BUFFER_unbuild shall clear the underlying unsigned char* data associated with the
 * BUFFER_HANDLE this will return zero on success.] */
extern int BUFFER_unbuild(BUFFER_HANDLE handle)
{
    ...
    else
    {
        ...
        {
            /* Codes_SRS_BUFFER_07_015: [BUFFER_unbuild shall return a nonzero value if the unsigned char* referenced by
...
int BUFFER_shrink(BUFFER_HANDLE handle, size_t decreaseSize, bool fromEnd)
{
    ...
    {
        /* Codes_SRS_BUFFER_07_038: [ If decreaseSize is less than the size of the buffer, BUFFER_shrink shall return a
         * non-null value ] */
    ...
    {
        /* Codes_SRS_BUFFER_07_039: [ BUFFER_shrink shall allocate a temporary buffer of existing buffer size minus
         * decreaseSize. ] */
        size_t alloc_size = handle->size - decreaseSize;
        if (alloc_size == 0)
        {
            /* Codes_SRS_BUFFER_07_043: [ If the decreaseSize is equal the buffer size , BUFFER_shrink shall deallocate
             * the buffer and set the size to zero. ] */
            free(handle->buffer);
            handle->buffer = NULL;
            handle->size   = 0;
            result         = 0;
        ...
        else
        {
            ...
            else
            {
                if (fromEnd)
                {
                    /* Codes_SRS_BUFFER_07_040: [ if the fromEnd variable is true, BUFFER_shrink shall remove the end of
                     * the buffer of size decreaseSize. ] */
                ...
                {
                    /* Codes_SRS_BUFFER_07_041: [ if the fromEnd variable is false, BUFFER_shrink shall remove the
                     * beginning of the buffer of size decreaseSize. ] */
...
/* Codes_SRS_BUFFER_07_021: [BUFFER_size shall place the size of the associated buffer in the size variable and return
 * zero on success.] */
...
/* Codes_SRS_BUFFER_07_024: [BUFFER_append concatenates b2 onto b1 without modifying b2 and shall return zero on
 * success.] */
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (temp == NULL)
                {
                    /* Codes_SRS_BUFFER_07_023: [BUFFER_append shall return a nonzero upon any error that is
                     * encountered.] */
                    LogError("Failure: allocating temp buffer.");
                    result = __FAILURE__;
                }
                ...
                {
                    /* Codes_SRS_BUFFER_07_024: [BUFFER_append concatenates b2 onto b1 without modifying b2 and shall
...
int BUFFER_prepend(BUFFER_HANDLE handle1, BUFFER_HANDLE handle2)
{
    ...
    else
    {
        ...
        if (b1->buffer == NULL)
        {
            /* Codes_SRS_BUFFER_01_005: [ BUFFER_prepend shall return a non-zero upon value any error that is
             * encountered. ]*/
            result = __FAILURE__;
        }
        else if (b2->buffer == NULL)
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    /* Codes_SRS_BUFFER_01_004: [ BUFFER_prepend concatenates handle1 onto handle2 without modifying
                     * handle1 and shall return zero on success. ]*/
                    // Append the BUFFER
...
int BUFFER_fill(BUFFER_HANDLE handle, unsigned char fill_char)
{
    ...
    {
        size_t index;
        /* Codes_SRS_BUFFER_07_001: [ BUFFER_fill shall fill the supplied BUFFER_HANDLE with the supplied fill
         * character. ] */
