...
CONSTMAP_HANDLE ConstMap_Create(MAP_HANDLE sourceMap)
{
    ...
    else
    {
        ...
        /*Codes_SRS_CONSTMAP_17_001: [ConstMap_Create shall create an immutable map, populated by the key, value pairs
         * in the source map.]*/
        result->map = Map_Clone(sourceMap);
        if (result->map == NULL)
        {
            free(result);
            /*Codes_SRS_CONSTMAP_17_002: [If during creation there are any errors, then ConstMap_Create shall return
             * NULL.]*/
            result = NULL;
...
void ConstMap_Destroy(CONSTMAP_HANDLE handle)
{
    ...
    {
        /*Codes_SRS_CONSTMAP_17_049: [ConstMap_Destroy shall decrement the internal reference count of the immutable
         * map.]*/
        if (DEC_REF(CONSTMAP_HANDLE_DATA, handle) == DEC_RETURN_ZERO)
        {
            /*Codes_SRS_CONSTMAP_17_004: [If the reference count is zero, ConstMap_Destroy shall release all resources
             * associated with the immutable map.]*/
...
CONSTMAP_HANDLE ConstMap_Clone(CONSTMAP_HANDLE handle)
{
    ...
    {
        /*Codes_SRS_CONSTMAP_17_039: [ConstMap_Clone shall increase the internal reference count of the immutable map
         * indicated by parameter handle]*/
...
MAP_HANDLE ConstMap_CloneWriteable(CONSTMAP_HANDLE handle)
{
    ...
    {
        /*Codes_SRS_CONSTMAP_17_052: [ConstMap_CloneWriteable shall create a new, writeable map, populated by the key,
         * value pairs in the parameter defined by handle.]*/
        /*Codes_SRS_CONSTMAP_17_053: [If during cloning, any operation fails, then ConstMap_CloneWriteableap_Clone shall
         * return NULL.]*/
        /*Codes_SRS_CONSTMAP_17_054: [Otherwise, ConstMap_CloneWriteable shall return a non-NULL handle that can be used
         * in subsequent calls.]*/
...
bool ConstMap_ContainsKey(CONSTMAP_HANDLE handle, const char *key)
{
    ...
    if (handle == NULL)
    {
        /*Codes_SRS_CONSTMAP_17_024: [If parameter handle or key are NULL then ConstMap_ContainsKey shall return
         * false.]*/
        LOG_CONSTMAP_ERROR(CONSTMAP_INVALIDARG);
    }
    else
...
bool ConstMap_ContainsValue(CONSTMAP_HANDLE handle, const char *value)
{
    ...
    if (handle == NULL)
    {
        /*Codes_SRS_CONSTMAP_17_027: [If parameter handle or value is NULL then ConstMap_ContainsValue shall return
         * false.]*/
        LOG_CONSTMAP_ERROR(CONSTMAP_INVALIDARG);
    }
    else
    {
        ...
        {
            /*Codes_SRS_CONSTMAP_17_028: [Otherwise, if a pair has its value equal to the parameter value, the
             * ConstMap_ContainsValue shall return true.]*/
            MAP_RESULT mapResult = Map_ContainsValue(((CONSTMAP_HANDLE_DATA *)handle)->map, value, &valueExists);
            if (mapResult != MAP_OK)
            {
                /*Codes_SRS_CONSTMAP_17_029: [Otherwise, if such a does not exist, then ConstMap_ContainsValue shall
                 * return false.]*/
                LOG_CONSTMAP_ERROR(ConstMap_ErrorConvert(mapResult));
            }
        }
...
CONSTMAP_RESULT
ConstMap_GetInternals(CONSTMAP_HANDLE handle, const char *const **keys, const char *const **values, size_t *count)
{
    CONSTMAP_RESULT result;
    if (handle == NULL)
    {
        /*Codes_SRS_CONSTMAP_17_046: [If parameter handle, keys, values or count is NULL then ConstMap_GetInternals
         * shall return CONSTMAP_INVALIDARG.]*/
        result = CONSTMAP_INVALIDARG;
        LOG_CONSTMAP_ERROR(result);
    }
    else
    {
        /*Codes_SRS_CONSTMAP_17_043: [ConstMap_GetInternals shall produce in *keys a pointer to an array of const char*
         *having all the keys stored so far by the map.] Codes_SRS_CONSTMAP_17_044: [ConstMap_GetInternals shall produce
         *in *values a pointer to an array of const char* having all the values stored so far by the map.]
         *Codes_SRS_CONSTMAP_17_045: [ ConstMap_GetInternals shall produce in *count the number of stored keys and
         *values.]
         */
