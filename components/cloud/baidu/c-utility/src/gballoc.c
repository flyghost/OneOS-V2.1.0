...
int gballoc_init(void)
{
    ...
    /* Codes_SRS_GBALLOC_01_026: [gballoc_Init shall create a lock handle that will be used to make the other gballoc
     * APIs thread-safe.] */
    ...
    else
    {
        ...
        /* Codes_ SRS_GBALLOC_01_002: [Upon initialization the total memory used and maximum total memory used tracked
         * by the module shall be set to 0.] */
        totalSize     = 0;
        maxSize       = 0;
        g_allocations = 0;
...
void *gballoc_malloc(size_t size)
{
    ...
    if (gballocState != GBALLOC_STATE_INIT)
    {
        /* Codes_SRS_GBALLOC_01_039: [If gballoc was not initialized gballoc_malloc shall simply call malloc without any
    ...
    else
    {
        ...
        else
        {
            ...
            if (result == NULL)
            {
                 ...
                 * and size should not be counted towards total memory used.] */
                free(allocation);
            }
            else
            {
                /* Codes_SRS_GBALLOC_01_004: [If the underlying malloc call is successful, gb_malloc shall increment the
                 * total memory used with the amount indicated by size.] */
                ...
                g_allocations++;
                totalSize += size;
                /* Codes_SRS_GBALLOC_01_011: [The maximum total memory used shall be the maximum of the total memory
                 * used at any point.] */
...
void *gballoc_calloc(size_t nmemb, size_t size)
{
    ...
    if (gballocState != GBALLOC_STATE_INIT)
    {
        /* Codes_SRS_GBALLOC_01_040: [If gballoc was not initialized gballoc_calloc shall simply call calloc without any
         * memory tracking being performed.] */
    ...
    else
    {
        ...
        else
        {
            ...
            {
                /* Codes_SRS_GBALLOC_01_021: [If the underlying calloc call is successful, gballoc_calloc shall
                 * increment the total memory used with nmemb*size.] */
                allocation->ptr  = result;
...
void *gballoc_realloc(void *ptr, size_t size)
{
    ...
    if (gballocState != GBALLOC_STATE_INIT)
    {
        /* Codes_SRS_GBALLOC_01_041: [If gballoc was not initialized gballoc_realloc shall shall simply call realloc
         * without any memory tracking being performed.] */
    ...
    else
    {
        if (ptr == NULL)
        {
            /* Codes_SRS_GBALLOC_01_017: [When ptr is NULL, gballoc_realloc shall call the underlying realloc with ptr
             * being NULL and the realloc result shall be tracked by gballoc.] */
        ...
        if (allocation == NULL)
        {
            /* Codes_SRS_GBALLOC_01_015: [When allocating memory used for tracking by gballoc_realloc fails,
             * gballoc_realloc shall return NULL and no change should be made to the counted total memory usage.] */
            /* Codes_SRS_GBALLOC_01_016: [When the ptr pointer cannot be found in the pointers tracked by gballoc,
             * gballoc_realloc shall return NULL and the underlying realloc shall not be called.] */
            result = NULL;
        }
        else
        {
            ...
            if (result == NULL)
            {
                 ...
                 * and no change should be made to the counted total memory usage.] */
                if (ptr == NULL)
                {
                    free(allocation);
                }
            ...
            else
            {
                if (ptr != NULL)
                {
                    /* Codes_SRS_GBALLOC_01_006: [If the underlying realloc call is successful, gballoc_realloc shall
                     * look up the size associated with the pointer ptr and decrease the total memory used with that
                     * size.] */
                    allocation->ptr = result;
                    totalSize -= allocation->size;
                    allocation->size = size;
                ...
                /* Codes_SRS_GBALLOC_01_007: [If realloc is successful, gballoc_realloc shall also increment the total
                 * memory used value tracked by this module.] */
                totalSize += size;
                g_allocations++;
...
void gballoc_free(void *ptr)
{
    ...
    {
        /* Codes_SRS_GBALLOC_01_009: [gballoc_free shall also look up the size associated with the ptr pointer and
         * decrease the total memory used with the associated size amount.] */
        while (curr != NULL)
        {
            if (curr->ptr == ptr)
            {
        ...
        {
            /* Codes_SRS_GBALLOC_01_019: [When the ptr pointer cannot be found in the pointers tracked by gballoc,
             * gballoc_free shall not free any memory.] */
...
size_t gballoc_getMaximumMemoryUsed(void)
{
    ...
    /* Codes_SRS_GBALLOC_01_038: [If gballoc was not initialized gballoc_getMaximumMemoryUsed shall return
     * MAX_INT_SIZE.] */
    if (gballocState != GBALLOC_STATE_INIT)
    {
    ...
    /* Codes_SRS_GBALLOC_01_034: [gballoc_getMaximumMemoryUsed shall ensure thread safety by using the lock created by
     * gballoc_Init.]  */
    else if (LOCK_OK != Lock(gballocThreadSafeLock))
    {
        /* Codes_SRS_GBALLOC_01_050: [If the lock cannot be acquired, gballoc_getMaximumMemoryUsed shall return
         * SIZE_MAX.]  */
        LogError("Failed to get the Lock.");
        result = SIZE_MAX;
    }
    ...
    {
        /* Codes_SRS_GBALLOC_01_010: [gballoc_getMaximumMemoryUsed shall return the maximum amount of total memory used
         * recorded since the module initialization.] */
        result = maxSize;
        (void)Unlock(gballocThreadSafeLock);
    }
...
size_t gballoc_getCurrentMemoryUsed(void)
{
    ...
    /* Codes_SRS_GBALLOC_01_036: [gballoc_getCurrentMemoryUsed shall ensure thread safety by using the lock created by
     * gballoc_Init.]*/
    else if (LOCK_OK != Lock(gballocThreadSafeLock))
    {
        /* Codes_SRS_GBALLOC_01_051: [If the lock cannot be acquired, gballoc_getCurrentMemoryUsed shall return
         * SIZE_MAX.] */
        LogError("Failed to get the Lock.");
        result = SIZE_MAX;
    }
...
size_t gballoc_getAllocationCount(void)
{
    ...
    /* Codes_SRS_GBALLOC_07_002: [ gballoc_getAllocationCount shall ensure thread safety by using the lock created by
     * gballoc_Init ] */
    else if (LOCK_OK != Lock(gballocThreadSafeLock))
    {
...
void gballoc_resetMetrics()
{
    ...
    /* Codes_SRS_GBALLOC_07_006: [ gballoc_resetMetrics shall ensure thread safety by using the lock created by
     * gballoc_Init ]*/
    else if (LOCK_OK != Lock(gballocThreadSafeLock))
    {
    ...
    {
        /* Codes_SRS_GBALLOC_07_008: [ gballoc_resetMetrics shall reset the total allocation size, max allocation size
         * and number of allocation to zero. ] */
        totalSize     = 0;
        maxSize       = 0;
        g_allocations = 0;
        (void)Unlock(gballocThreadSafeLock);
