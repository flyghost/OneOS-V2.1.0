...
/*Codes_SRS_MAP_02_039: [Map_Clone shall make a copy of the map indicated by parameter handle and return a non-NULL
 * handle to it.]*/
            ...
            else
            {
                ...
                if ((result->keys = Map_CloneVector((const char *const *)handleData->keys, handleData->count)) == NULL)
                {
                    ...
                    result = NULL;
                              ...
                              Map_CloneVector((const char *const *)handleData->values, handleData->count)) == NULL)
                {
                    size_t i;
                    /*Codes_SRS_MAP_02_047: [If during cloning, any operation fails, then Map_Clone shall return NULL.]
                     */
...
MAP_RESULT Map_Add(MAP_HANDLE handle, const char *key, const char *value)
{
    ...
    else
    {
        ...
        {
            /* Codes_SRS_MAP_07_009: [If the mapFilterCallback function is not NULL, then the return value will be check
             * and if it is not zero then Map_Add shall return MAP_FILTER_REJECT.] */
...
MAP_RESULT Map_AddOrUpdate(MAP_HANDLE handle, const char *key, const char *value)
{
    ...
    else
    {
        ...
        /* Codes_SRS_MAP_07_008: [If the mapFilterCallback function is not NULL, then the return value will be check and
         * if it is not zero then Map_AddOrUpdate shall return MAP_FILTER_REJECT.] */
        if (handleData->mapFilterCallback != NULL && handleData->mapFilterCallback(key, value) != 0)
        {
            result = MAP_FILTER_REJECT;
        }
        else
        {
            ...
            {
                /*Codes_SRS_MAP_02_016: [If the key already exists, then Map_AddOrUpdate shall overwrite the value of
                 * the existing key with parameter value.]*/
...
MAP_RESULT Map_Delete(MAP_HANDLE handle, const char *key)
{
    ...
    else
    {
        ...
        {
            /*Codes_SRS_MAP_02_023: [Otherwise, Map_Delete shall remove the key and its associated value from the map
             * and return MAP_OK.]*/
            size_t index = whereIsIt - handleData->keys;
            free(handleData->keys[index]);
            free(handleData->values[index]);
            memmove(handleData->keys + index,
                    handleData->keys + index + 1,
                    (handleData->count - index - 1) *
                        sizeof(char *)); /*if order doesn't matter... then this can be optimized*/
            memmove(handleData->values + index,
                    handleData->values + index + 1,
                    (handleData->count - index - 1) * sizeof(char *));
            Map_DecreaseStorageKeysValues(handleData);
            result = MAP_OK;
        }
    }
...
{
    MAP_RESULT result;
    /*Codes_SRS_MAP_02_024: [If parameter handle, key or keyExists are NULL then Map_ContainsKey shall return
     * MAP_INVALIDARG.]*/
    if ((handle == NULL) || (key == NULL) || (keyExists == NULL))
    {
        result = MAP_INVALIDARG;
        LOG_MAP_ERROR;
    }
    ...
    {
        MAP_HANDLE_DATA *handleData = (MAP_HANDLE_DATA *)handle;
        /*Codes_SRS_MAP_02_025: [Otherwise if a key exists then Map_ContainsKey shall return MAP_OK and shall write in
         * keyExists "true".]*/
        /*Codes_SRS_MAP_02_026: [If a key doesn't exist, then Map_ContainsKey shall return MAP_OK and write in keyExists
         * "false".] */
...
{
    MAP_RESULT result;
    /*Codes_SRS_MAP_02_027: [If parameter handle, value or valueExists is NULL then Map_ContainsValue shall return
     * MAP_INVALIDARG.] */
    if ((handle == NULL) || (value == NULL) || (valueExists == NULL))
    {
        result = MAP_INVALIDARG;
        LOG_MAP_ERROR;
    }
    ...
    {
        MAP_HANDLE_DATA *handleData = (MAP_HANDLE_DATA *)handle;
        /*Codes_SRS_MAP_02_028: [Otherwise, if a pair <key, value> has its value equal to the parameter value, the
         * Map_ContainsValue shall return MAP_OK and shall write in valueExists "true".]*/
        /*Codes_SRS_MAP_02_029: [Otherwise, if such a <key, value> does not exist, then Map_ContainsValue shall return
         * MAP_OK and shall write in valueExists "false".] */
...
{
    MAP_RESULT result;
    /*Codes_SRS_MAP_02_046: [If parameter handle, keys, values or count is NULL then Map_GetInternals shall return
     * MAP_INVALIDARG.] */
    if ((handle == NULL) || (keys == NULL) || (values == NULL) || (count == NULL))
    {
        result = MAP_INVALIDARG;
        LOG_MAP_ERROR;
    }
    ...
    {
        /*Codes_SRS_MAP_02_043: [Map_GetInternals shall produce in *keys an pointer to an array of const char* having
         * all the keys stored so far by the map.]*/
        /*Codes_SRS_MAP_02_044: [Map_GetInternals shall produce in *values a pointer to an array of const char* having
         * all the values stored so far by the map.]*/
    ...
    else
    {
        ...
        else
        {
            ...
            for (i = 0; (i < handleData->count) && (!breakFor); i++)
            {
                ...
                /*Codes_SRS_MAP_02_050: [If the map has properties then Map_ToJSON shall produce the following
                ...
                else
                {
                    ...
                    {
                        if (!(((i > 0) ? (STRING_concat(result, ",") == 0) : 1) &&
                              (STRING_concat_with_STRING(result, key) == 0) && (STRING_concat(result, ":") == 0) &&
