...
{
    bool result;
    /*Codes_SRS_SASTOKEN_25_025: [**SASToken_Validate shall get the SASToken value by invoking STRING_c_str on the
     * handle.**]***/
    ...
    else
    {
        ...
        {
            if (sasTokenArray[i] == 's' && sasTokenArray[i + 1] == 'e' &&
                sasTokenArray[i + 2] == '=')    // Look for se=
            {
                seStart = i + 3;
            ...
            else if (sasTokenArray[i] == 's' && sasTokenArray[i + 1] == 'r' &&
                     sasTokenArray[i + 2] == '=')    // Look for sr=
            {
                srStart = i + 3;
            ...
            else if (sasTokenArray[i] == 's' && sasTokenArray[i + 1] == 'i' && sasTokenArray[i + 2] == 'g' &&
                     sasTokenArray[i + 3] == '=')    // Look for sig=
            {
                sigStart = i + 4;
                if (srStart > 0 && srStop < 0)
                {
        ...
        /*Codes_SRS_SASTOKEN_25_027: [**If SASTOKEN does not obey the SASToken format then SASToken_Validate shall
         * return false.**]***/
        /*Codes_SRS_SASTOKEN_25_028: [**SASToken_validate shall check for the presence of sr, se and sig from the token
        ...
        else
        {
            ...
            if ((seStop <= seStart) || (srStop <= srStart) || (sigStop <= sigStart))
            {
                result = false;
            }
            else
            {
                ...
                /*Codes_SRS_SASTOKEN_25_031: [**If malloc fails during validation then SASToken_Validate shall return
                 * false.**]***/
                if (expiryASCII == NULL)
                {
                    result = false;
                ...
                else
                {
                    ...
                    /*Codes_SRS_SASTOKEN_25_029: [**SASToken_validate shall check for expiry time from token and if
                     * token has expired then would return false **]***/
                    if (expiry <= 0)
                    {
                        result = false;
                    }
                    else
                    {
                        ...
                        {
                            /*Codes_SRS_SASTOKEN_25_029: [**SASToken_validate shall check for expiry time from token and
                             * if token has expired then would return false **]***/
                            result = false;
                        }
                        else
                        {
                            /*Codes_SRS_SASTOKEN_25_030: [**SASToken_validate shall return true only if the format is
                             * obeyed and the token has not yet expired **]***/
                            result = true;
                        }
                    }
                    free(expiryASCII);
...
static STRING_HANDLE construct_sas_token(const char *key, const char *scope, const char *keyname, size_t expiry)
{
    ...
    {
        /*Codes_SRS_SASTOKEN_06_026: [If the conversion to string form fails for any reason then SASToken_Create shall
         * return NULL.]*/
        ...
        else
        {
            ...
            if (((hash = BUFFER_new()) == NULL) || ((toBeHashed = STRING_new()) == NULL) ||
            ...
            else
            {
                ...
                if ((STRING_concat(toBeHashed, scope) != 0) || (STRING_concat(toBeHashed, "\n") != 0) ||
                ...
                else
                {
                    ...
                    /*Codes_SRS_SASTOKEN_06_013: [If an error is returned from the HMAC256 function then NULL is
                     * returned from SASToken_Create.]*/
                    /*Codes_SRS_SASTOKEN_06_012: [An HMAC256 hash is calculated using the decodedKey, over
                     * toBeHashed.]*/
                    /*Codes_SRS_SASTOKEN_06_014: [If there are any errors from the following operations then NULL shall
                     * be returned.]*/
                    /*Codes_SRS_SASTOKEN_06_015: [The hash is base 64 encoded.]*/
                    /*Codes_SRS_SASTOKEN_06_028: [base64Signature shall be url encoded.]*/
                    /*Codes_SRS_SASTOKEN_06_016: [The string "SharedAccessSignature sr=" is the first part of the result
                     * of SASToken_Create.]*/
                    ...
                    if ((HMACSHA256_ComputeHash(outBuf, outLen, inBuf, inLen, hash) != HMACSHA256_OK) ||
                        ...
                        (STRING_concat(result, scope) != 0) || (STRING_concat(result, "&sig=") != 0) ||
                        (STRING_concat_with_STRING(result, urlEncodedSignature) != 0) ||
                        (STRING_concat(result, "&se=") != 0) || (STRING_concat(result, tokenExpirationTime) != 0) ||
