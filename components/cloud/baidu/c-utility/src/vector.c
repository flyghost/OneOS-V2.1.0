...
VECTOR_HANDLE VECTOR_create(size_t elementSize)
{
    ...
    else
    {
        ...
        {
            /* Codes_SRS_VECTOR_10_001: [VECTOR_create shall allocate a VECTOR_HANDLE that will contain an empty
             * vector.The size of each element is given with the parameter elementSize.] */
            result->storage     = NULL;
            result->count       = 0;
...
VECTOR_HANDLE VECTOR_move(VECTOR_HANDLE handle)
{
    ...
    else
    {
        ...
        {
            /* Codes_SRS_VECTOR_10_004: [VECTOR_move shall allocate a VECTOR_HANDLE and move the data to it from the
             * given handle.] */
...
int VECTOR_push_back(VECTOR_HANDLE handle, const void *elements, size_t numElements)
{
    ...
    else
    {
        ...
        {
            /* Codes_SRS_VECTOR_10_013: [VECTOR_push_back shall append the given elements and return 0 indicating
             * success.] */
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    /* Codes_SRS_VECTOR_10_014: [VECTOR_erase shall remove the 'numElements' starting at 'elements' and
                     * reduce its internal storage.] */
                    handle->count -= numElements;
                    if (handle->count == 0)
                    {
                        free(handle->storage);
...
void VECTOR_clear(VECTOR_HANDLE handle)
{
    ...
    {
        /* Codes_SRS_VECTOR_10_016: [VECTOR_clear shall remove all elements from the object and release internal
         * storage.] */
        free(handle->storage);
        handle->storage = NULL;
        handle->count   = 0;
...
void *VECTOR_find_if(VECTOR_HANDLE handle, PREDICATE_FUNCTION pred, const void *value)
{
    ...
    else
    {
        ...
        for (i = 0; i < handle->count; ++i)
        {
            ...
            {
                /* Codes_SRS_VECTOR_10_031: [VECTOR_find_if shall return the first element in the vector that matches
                 * `pred`.] */
                break;
            }
        }
