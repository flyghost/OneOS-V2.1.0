...
XIO_HANDLE xio_create(const IO_INTERFACE_DESCRIPTION *io_interface_description, const void *xio_create_parameters)
{
    ...
    if ((io_interface_description == NULL) ||
        ...
        (io_interface_description->concrete_io_open == NULL) || (io_interface_description->concrete_io_close == NULL) ||
    ...
    else
    {
        ...
        /* Codes_SRS_XIO_01_017: [If allocating the memory needed for the IO interface fails then xio_create shall
         * return NULL.] */
        if (xio_instance != NULL)
        {
            ...
            /* Codes_SRS_XIO_01_002: [In order to instantiate the concrete IO implementation the function
             * concrete_io_create from the io_interface_description shall be called, passing the xio_create_parameters
             * argument.] */
            xio_instance->concrete_xio_handle =
                xio_instance->io_interface_description->concrete_io_create((void *)xio_create_parameters);
...
void xio_destroy(XIO_HANDLE xio)
{
    ...
    if (xio != NULL)
    {
        ...
        /* Codes_SRS_XIO_01_006: [xio_destroy shall also call the concrete_io_destroy function that is member of the
         * io_interface_description argument passed to xio_create, while passing as argument to concrete_io_destroy the
         * result of the underlying concrete_io_create handle that was called as part of the xio_create call.] */
             ...
             void               *on_io_error_context)
{
    ...
    else
    {
        ...
        /* Codes_SRS_XIO_01_019: [xio_open shall call the specific concrete_xio_open function specified in xio_create,
         * passing callback function and context arguments for three events: open completed, bytes received, and IO
         * error.] */
        if (xio_instance->io_interface_description->concrete_io_open(xio_instance->concrete_xio_handle,
                                                                     on_io_open_complete,
                                                                     on_io_open_complete_context,
                                                                     on_bytes_received,
                                                                     on_bytes_received_context,
                                                                     on_io_error,
                                                                     on_io_error_context) != 0)
        {
...
int xio_close(XIO_HANDLE xio, ON_IO_CLOSE_COMPLETE on_io_close_complete, void *callback_context)
{
    ...
    else
    {
         ...
         */
        if (xio_instance->io_interface_description->concrete_io_close(xio_instance->concrete_xio_handle,
                                                                      on_io_close_complete,
                                                                      callback_context) != 0)
        {
            /* Codes_SRS_XIO_01_026: [If the underlying concrete_io_close fails, xio_close shall return a non-zero
             * value.] */
            result = __FAILURE__;
        }
        else
...
{
    int result;
    // char* buffer_test = (char*)buffer;
    // printf("----------start-------");
    // for(int i =0; i < size; i++){
    // printf("%02x ",*(buffer_test + i));
    //}
    // printf("----------end-------");
    ...
    else
    {
        ...
        /* Codes_SRS_XIO_01_008: [xio_send shall pass the sequence of bytes pointed to by buffer to the concrete IO
         * implementation specified in xio_create, by calling the concrete_io_send function while passing down the
         * buffer and size arguments to it.] */
        ...
        /* Codes_SRS_XIO_01_027: [xio_send shall pass to the concrete_io_send function the on_send_complete and
         * callback_context arguments.] */
        result = xio_instance->io_interface_description->concrete_io_send(xio_instance->concrete_xio_handle,
                                                                          buffer,
                                                                          size,
                                                                          on_send_complete,
                                                                          callback_context);
...
void xio_dowork(XIO_HANDLE xio)
{
    ...
    if (xio != NULL)
    {
        ...
        /* Codes_SRS_XIO_01_012: [xio_dowork shall call the concrete XIO implementation specified in xio_create, by
         * calling the concrete_io_dowork function.] */
...
int xio_setoption(XIO_HANDLE xio, const char *optionName, const void *value)
{
    ...
    /* Codes_SRS_XIO_03_030: [If the xio argument or the optionName argument is NULL, xio_setoption shall return a
     * non-zero value.] */
    ...
    else
    {
        ...
        if (strcmp(CONCRETE_OPTIONS, optionName) == 0)
        {
            ...
            if (OptionHandler_FeedOptions((OPTIONHANDLER_HANDLE)value, xio_instance->concrete_xio_handle) !=
                OPTIONHANDLER_OK)
            {
        ...
        {
            /* Codes_SRS_XIO_003_028: [xio_setoption shall pass the optionName and value to the concrete IO
             * implementation specified in xio_create by invoking the concrete_xio_setoption function.] */
            /* Codes_SRS_XIO_03_029: [xio_setoption shall return 0 upon success.] */
            /* Codes_SRS_XIO_03_031: [If the underlying concrete_xio_setoption fails, xio_setOption shall return a
             * non-zero value.] */
            result = xio_instance->io_interface_description->concrete_io_setoption(xio_instance->concrete_xio_handle,
                                                                                   optionName,
                                                                                   value);
        }
    }
...
OPTIONHANDLER_HANDLE xio_retrieveoptions(XIO_HANDLE xio)
{
    ...
    {
        XIO_INSTANCE *xio_instance = (XIO_INSTANCE *)xio;
        /*xio_retrieveoptions shall return a OPTIONHANDLER_HANDLE that has 1 option called "underlyingOptions" which is
         * of type OPTIONHANDLER_HANDLE*/
        ...
        else
        {
            OPTIONHANDLER_HANDLE concreteOptions =
                xio_instance->io_interface_description->concrete_io_retrieveoptions(xio_instance->concrete_xio_handle);
