...
/*Tests_SRS_BASE64_06_007: [Otherwise Base64_Encoder shall return a pointer to STRING, that string contains the base 64
 * encoding of inpuit.]*/
...
TEST_FUNCTION(Base64_Encode_exhaustive_succeeds)
{
    ...
    for (i = 0; i < sizeof(testVector_BINARY_with_equal_signs) / sizeof(testVector_BINARY_with_equal_signs[0]); i++)
    {
        ...
        ASSERT_ARE_EQUAL(int,
                         0,
                         BUFFER_build(input,
                                      testVector_BINARY_with_equal_signs[i].inputData,
                                      testVector_BINARY_with_equal_signs[i].inputLength));
...
/*Tests_SRS_BASE64_02_002: [If source is not NULL and size is zero, then Base64_Encode_Bytes shall produce an empty
 * STRING_HANDLE.]*/
...
/*Tests_SRS_BASE64_02_003: [Otherwise, Base64_Encode_Bytes shall produce a STRING_HANDLE containing the Base64
...
TEST_FUNCTION(Base64_Encode_Bytes_exhaustive_succeeds)
{
    ...
    for (i = 0; i < sizeof(testVector_BINARY_with_equal_signs) / sizeof(testVector_BINARY_with_equal_signs[0]); i++)
    {
        ...
        /// act
        result = Base64_Encode_Bytes(testVector_BINARY_with_equal_signs[i].inputData,
                                     testVector_BINARY_with_equal_signs[i].inputLength);
...
TEST_FUNCTION(Base64_Decoder_exhaustive_succeeds)
{
    ...
    for (i = 0; i < sizeof(testVector_BINARY_with_equal_signs) / sizeof(testVector_BINARY_with_equal_signs[0]); i++)
    {
        ...
        ASSERT_ARE_EQUAL(
            int,
            (int)0,
            memcmp(BUFFER_u_char(result), testVector_BINARY_with_equal_signs[i].inputData, BUFFER_length(result)));
...
/*Tests_SRS_BASE64_06_009: [If the string pointed to by source is zero length then the handle returned shall refer to a
 * zero length buffer.]*/
...
/*Tests_SRS_BASE64_06_011: [If the source string has an invalid length for a base 64 encoded string then Base64_Decoder
 * shall return NULL.]*/
