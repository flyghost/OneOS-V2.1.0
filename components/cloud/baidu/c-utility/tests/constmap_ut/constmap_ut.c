...
/*Tests_SRS_CONSTMAP_17_001: [ConstMap_Create shall create an immutable map, populated by the key, value pairs in the
 * source map.]*/
...
/*Tests_SRS_CONSTMAP_17_004: [If the reference count is zero, ConstMap_Destroy shall release all resources associated
 * with the immutable map.]*/
...
/*Tests_SRS_CONSTMAP_17_039: [ConstMap_Clone shall increase the internal reference count of the immutable map indicated
 * by parameter handle] */
...
/*Tests_SRS_CONSTMAP_17_052: [ConstMap_CloneWriteable shall create a new, writeable map, populated by the key, value
 * pairs in the parameter defined by handle.]*/
/*Tests_SRS_CONSTMAP_17_054: [Otherwise, ConstMap_CloneWriteable shall return a non-NULL handle that can be used in
 * subsequent calls.]*/
...
/*Tests_SRS_CONSTMAP_17_053: [If during cloning, any operation fails, then ConstMap_CloneWriteableap_Clone shall return
 * NULL.]*/
...
TEST_FUNCTION(ConstMap_ContainsKey_Success)
{
    ...
    // Call to Map
    STRICT_EXPECTED_CALL(Map_ContainsKey(IGNORED_PTR_ARG, key, IGNORED_PTR_ARG)).IgnoreArgument(1).IgnoreArgument(3);
...
{
    // Arrange
    MAP_RESULT  mapErrorList[] = {MAP_ERROR, MAP_INVALIDARG, MAP_KEYEXISTS, MAP_KEYNOTFOUND, MAP_FILTER_REJECT};
...
/*Tests_SRS_CONSTMAP_17_028: [Otherwise, if a pair has its value equal to the parameter value, the
...
TEST_FUNCTION(ConstMap_GetValue_Success)
{
    ...
    // Call to Map
    STRICT_EXPECTED_CALL(Map_GetValueFromKey(IGNORED_PTR_ARG, key)).IgnoreArgument(1);
...
/*Tests_SRS_CONSTMAP_17_043: [ConstMap_GetInternals shall produce in *keys a pointer to an array of const char* having
 * all the keys stored so far by the map.] */
/*Tests_SRS_CONSTMAP_17_044: [ConstMap_GetInternals shall produce in *values a pointer to an array of const char* having
...
TEST_FUNCTION(ConstMap_GetInternals_Success)
{
    ...
    // Call to Map
    STRICT_EXPECTED_CALL(Map_GetInternals(IGNORED_PTR_ARG, &keys, &values, &count)).IgnoreArgument(1);
...
/*Tests_SRS_CONSTMAP_17_046: [If parameter handle, keys, values or count is NULL then ConstMap_GetInternals shall return
 * CONSTMAP_INVALIDARG.]*/
...
TEST_FUNCTION(ConstMap_GetInternals_Failures)
{
    ...
    MAP_RESULT         mapErrorList[] = {MAP_ERROR, MAP_INVALIDARG, MAP_KEYEXISTS, MAP_KEYNOTFOUND, MAP_FILTER_REJECT};
    size_t             errors         = sizeof(mapErrorList) / sizeof(MAP_RESULT);
    CONSTMAP_RESULT    constErrorList[] = {CONSTMAP_ERROR,
                                        CONSTMAP_INVALIDARG,
                                        CONSTMAP_ERROR,
