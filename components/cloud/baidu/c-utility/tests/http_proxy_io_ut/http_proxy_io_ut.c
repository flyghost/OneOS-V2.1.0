...
static const HTTP_PROXY_IO_CONFIG default_http_proxy_io_config =
    {"test_host", 443, "a_proxy", 4444, "test_user", "shhhh"};

static const HTTP_PROXY_IO_CONFIG http_proxy_io_config_no_username = {"test_host", 443, "a_proxy", 4444, NULL, NULL};

static const HTTP_PROXY_IO_CONFIG http_proxy_io_config_with_username =
    {"another_test_host", 445, "another_proxy", 8888, "le_user", "le_password"};

static const HTTP_PROXY_IO_CONFIG http_proxy_io_config_with_username_cased =
    {"another_test_host", 445, "another_proxy", 8888, "lE_uSeR", "lE_pAsSwOrD"};

static const SOCKETIO_CONFIG socketio_config = {"a_proxy", 4444, NULL};
...
/* Tests_SRS_HTTP_PROXY_IO_01_005: [ `http_proxy_io_create` shall copy the `hostname`, `port`, `username` and `password`
 * values for later use when the actual CONNECT is performed. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_006: [ `hostname` and `proxy_hostname`, `username` and `password` shall be copied by
 * calling `mallocAndStrcpy_s`. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_009: [ `http_proxy_io_create` shall create a new socket IO by calling `xio_create` with
 * the arguments: ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_010: [ - `io_interface_description` shall be set to the result of
 * `socketio_get_interface_description`. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_011: [ - `xio_create_parameters` shall be set to a `SOCKETIO_CONFIG*` where `hostname` is
 * set to the `proxy_hostname` member of `io_create_parameters` and `port` is set to the `proxy_port` member of
...
/* Tests_SRS_HTTP_PROXY_IO_01_095: [ If one of the fields `username` and `password` is non-NULL, then the other has to
 * be also non-NULL, otherwise `http_proxy_io_create` shall fail and return NULL. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_002: [ If `io_create_parameters` is NULL, `http_proxy_io_create` shall fail and return
 * NULL. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_004: [ If the `hostname` or `proxy_hostname` member is NULL, then `http_proxy_io_create`
...
/* Tests_SRS_HTTP_PROXY_IO_01_051: [ If allocating memory for the new instance fails, `http_proxy_io_create` shall fail
 * and return NULL. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_007: [ If `mallocAndStrcpy_s` fails then `http_proxy_io_create` shall fail and return
 * NULL. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_050: [ If `socketio_get_interface_description` fails, `http_proxy_io_create` shall fail
 * and return NULL. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_012: [ If `xio_create` fails, `http_proxy_io_create` shall fail and return NULL. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_008: [ When `http_proxy_io_create` fails, all allocated resources up to that point shall
 * be freed. ]*/
/*
...
/* Tests_SRS_HTTP_PROXY_IO_01_013: [ `http_proxy_io_destroy` shall free the HTTP proxy IO instance indicated by
 * `http_proxy_io`. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_016: [ `http_proxy_io_destroy` shall destroy the underlying IO created in
 ...
 * ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_019: [ `http_proxy_io_open` shall open the underlying IO by calling `xio_open` on the
 * underlying IO handle created in `http_proxy_io_create`, while passing to it the callbacks
...
TEST_FUNCTION(http_proxy_io_open_opens_the_underlying_IO)
{
    ...
    // act
    result = http_proxy_io_get_interface_description()->concrete_io_open(http_io,
                                                                         test_on_io_open_complete,
                                                                         http_io,
                                                                         test_on_bytes_received,
                                                                         http_io,
                                                                         test_on_io_error,
                                                                         http_io);
...
/* Tests_SRS_HTTP_PROXY_IO_01_018: [ If any of the arguments `http_proxy_io`, `on_io_open_complete`, `on_bytes_received`
 * or `on_io_error` are NULL then `http_proxy_io_open` shall return a non-zero value. ]*/
TEST_FUNCTION(http_proxy_io_open_with_NULL_open_complete_callback_fails)
{
    ...
    // act
    result = http_proxy_io_get_interface_description()
                 ->concrete_io_open(http_io, NULL, http_io, test_on_bytes_received, http_io, test_on_io_error, http_io);
...
TEST_FUNCTION(http_proxy_io_open_with_NULL_bytes_received_callback_fails)
{
    ...
    result =
        http_proxy_io_get_interface_description()
            ->concrete_io_open(http_io, test_on_io_open_complete, http_io, NULL, http_io, test_on_io_error, http_io);
...
TEST_FUNCTION(http_proxy_io_open_with_NULL_handle_fails)
{
    ...
    // act
    result = http_proxy_io_get_interface_description()->concrete_io_open(NULL,
                                                                         test_on_io_open_complete,
                                                                         (void *)0x4242,
                                                                         test_on_bytes_received,
                                                                         (void *)0x4242,
...
/* Tests_SRS_HTTP_PROXY_IO_01_021: [ If `http_proxy_io_open` is called while the IO was already open,
 * `http_proxy_io_open` shall fail and return a non-zero value. ]*/
TEST_FUNCTION(http_proxy_io_open_after_open_fails)
{
    ...
    http_io = http_proxy_io_get_interface_description()->concrete_io_create((void *)&default_http_proxy_io_config);
    (void)http_proxy_io_get_interface_description()->concrete_io_open(http_io,
                                                                      test_on_io_open_complete,
                                                                      (void *)0x4242,
                                                                      test_on_bytes_received,
                                                                      (void *)0x4243,
...
/* Tests_SRS_HTTP_PROXY_IO_01_051: [ The arguments `on_io_open_complete_context`, `on_bytes_received_context` and
...
/* Tests_SRS_HTTP_PROXY_IO_01_022: [ `http_proxy_io_close` shall close the HTTP proxy IO and on success it shall return
 * 0. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_024: [ `http_proxy_io_close` shall close the underlying IO by calling `xio_close` on the
 * IO handle create in `http_proxy_io_create`, while passing to it the `on_underlying_io_close_complete` callback. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_026: [ The `on_io_close_complete` and `on_io_close_complete_context` arguments shall be
 * saved for later use. ]*/
TEST_FUNCTION(http_proxy_io_close_closes_the_IO)
{
    ...
    g_on_bytes_received(g_on_io_open_complete_context,
                        (const unsigned char *)connect_response,
                        sizeof(connect_response) - 1);
    umock_c_reset_all_calls();
    ...
    // act
    result = http_proxy_io_get_interface_description()->concrete_io_close(http_io,
                                                                          test_on_io_close_complete,
                                                                          (void *)0x4245);
...
/* Tests_SRS_HTTP_PROXY_IO_01_023: [ If the argument `http_proxy_io` is NULL, `http_proxy_io_close` shall fail and
...
TEST_FUNCTION(http_proxy_io_close_with_NULL_handle_fails)
{
    ...
    // act
    result =
        http_proxy_io_get_interface_description()->concrete_io_close(NULL, test_on_io_close_complete, (void *)0x4245);
...
/* Tests_SRS_HTTP_PROXY_IO_01_027: [ If `http_proxy_io_close` is called when not open, `http_proxy_io_close` shall fail
...
TEST_FUNCTION(http_proxy_io_close_when_already_closed_fails)
{
    ...
    g_on_bytes_received(g_on_io_open_complete_context,
                        (const unsigned char *)connect_response,
                        sizeof(connect_response) - 1);
    (void)http_proxy_io_get_interface_description()->concrete_io_close(http_io,
                                                                       test_on_io_close_complete,
                                                                       (void *)0x4245);
...
/* Tests_SRS_HTTP_PROXY_IO_01_053: [ `http_proxy_io_close` while OPENING shall trigger the `on_io_open_complete`
 * callback with `IO_OPEN_CANCELLED`. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_029: [ `http_proxy_io_send` shall send the `size` bytes pointed to by `buffer` and on
 * success it shall return 0. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_033: [ `http_proxy_io_send` shall send the bytes by calling `xio_send` on the underlying
 * IO created in `http_proxy_io_create` and passing `buffer` and `size` as arguments. ]*/
TEST_FUNCTION(http_proxy_io_send_calls_send_on_the_underlying_IO)
{
    ...
    STRICT_EXPECTED_CALL(
        xio_send(TEST_IO_HANDLE, IGNORED_PTR_ARG, sizeof(test_buffer), test_on_send_complete, (void *)0x4247))
        .ValidateArgumentBuffer(2, test_buffer, sizeof(test_buffer));

    // act
    result = http_proxy_io_get_interface_description()->concrete_io_send(http_io,
                                                                         test_buffer,
                                                                         sizeof(test_buffer),
                                                                         test_on_send_complete,
                                                                         (void *)0x4247);
...
/* Tests_SRS_HTTP_PROXY_IO_01_030: [ If any of the arguments `http_proxy_io` or `buffer` is NULL, `http_proxy_io_send`
...
TEST_FUNCTION(http_proxy_io_send_with_NULL_handle_fails)
{
    ...
    // act
    result = http_proxy_io_get_interface_description()->concrete_io_send(NULL,
                                                                         test_buffer,
                                                                         sizeof(test_buffer),
                                                                         test_on_send_complete,
                                                                         (void *)0x4247);
...
/* Tests_SRS_HTTP_PROXY_IO_01_034: [ If `http_proxy_io_send` is called when the IO is not open, `http_proxy_io_send`
 * shall fail and return a non-zero value. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_035: [ If the IO is in an error state (an error was reported through the `on_io_error`
 * callback), `http_proxy_io_send` shall fail and return a non-zero value. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_037: [ `http_proxy_io_dowork` shall call `xio_dowork` on the underlying IO created in
 * `http_proxy_io_create`. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_039: [ If the IO is not open (no open has been called or the IO has been closed) then
 * `http_proxy_io_dowork` shall do nothing. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_040: [ If any of the arguments `http_proxy_io` or `option_name` is NULL,
...
/* Tests_SRS_HTTP_PROXY_IO_01_042: [ If the option was handled by `http_proxy_io_set_option` or the underlying IO, then
 * `http_proxy_io_set_option` shall return 0. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_043: [ If the `option_name` argument indicates an option that is not handled by
...
/* Tests_SRS_HTTP_PROXY_IO_01_046: [ `http_proxy_io_retrieve_options` shall return an `OPTIONHANDLER_HANDLE` obtained by
 * calling `xio_retrieveoptions` on the underlying IO created in `http_proxy_io_create`. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_047: [ If the parameter `http_proxy_io` is NULL then `http_proxy_io_retrieve_options`
 * shall fail and return NULL. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_049: [ `http_proxy_io_get_interface_description` shall return a pointer to an
...
/* Tests_SRS_HTTP_PROXY_IO_01_057: [ When `on_underlying_io_open_complete` is called, the `http_proxy_io` shall send the
 * CONNECT request constructed per RFC 2817: ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_075: [ The Request-URI portion of the Request-Line is always an 'authority' as defined by
 * URI Generic Syntax [2], which is to say the host name and port number destination of the requested connection
 * separated by a colon: ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_063: [ The request shall be sent by calling `xio_send` and passing NULL as
...
/* Tests_SRS_HTTP_PROXY_IO_01_064: [ If `xio_send` fails, the `on_open_complete` callback shall be triggered with
 * `IO_OPEN_ERROR`, passing also the `on_open_complete_context` argument as `context`. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_062: [ If any failure is encountered while constructing the request, the
 * `on_open_complete` callback shall be triggered with `IO_OPEN_ERROR`, passing also the `on_open_complete_context`
 * argument as `context`. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_059: [ - If `username` and `password` have been specified in the arguments passed to
 * `http_proxy_io_create`, then the header `Proxy-Authorization` shall be added to the request. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_061: [ Encoding to Base64 shall be done by calling `Base64_Encode_Bytes`. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_060: [ - The value of `Proxy-Authorization` shall be the constructed according to RFC
 * 2617. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_091: [ To receive authorization, the client sends the userid and password, separated by a
 * single colon (":") character, within a base64 [7] encoded string in the credentials. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_092: [ A client MAY preemptively send the corresponding Authorization header with requests
 * for resources in that space without receipt of another challenge from the server. ]*/
...
TEST_FUNCTION(when_the_underlying_io_open_complete_is_called_the_CONNECT_request_with_auth_is_sent)
{
    ...
    const char         connect_request[] =
        "CONNECT another_test_host:445 HTTP/1.1\r\nHost:another_test_host:445\r\nProxy-authorization: Basic "
        "__encoded_base64__\r\n\r\n";
    const char plain_auth_string[] = "le_user:le_password";
    const char base64encoded[]     = "__encoded_base64__";

    http_io =
        http_proxy_io_get_interface_description()->concrete_io_create((void *)&http_proxy_io_config_with_username);
    (void)http_proxy_io_get_interface_description()->concrete_io_open(http_io,
                                                                      test_on_io_open_complete,
                                                                      (void *)0x4242,
                                                                      test_on_bytes_received,
    ...
    STRICT_EXPECTED_CALL(STRING_c_str(TEST_STRING_HANDLE)).SetReturn(base64encoded);
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(xio_send(TEST_IO_HANDLE, IGNORED_PTR_ARG, sizeof(connect_request) - 1, IGNORED_PTR_ARG, NULL))
        .ValidateArgumentBuffer(2, connect_request, sizeof(connect_request) - 1);
    EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG));
...
TEST_FUNCTION(when_the_underlying_io_open_complete_is_called_the_CONNECT_request_with_auth_is_sent_cased)
{
    ...
    http_io = http_proxy_io_get_interface_description()->concrete_io_create(
        (void *)&http_proxy_io_config_with_username_cased);
    (void)http_proxy_io_get_interface_description()->concrete_io_open(http_io,
                                                                      test_on_io_open_complete,
                                                                      (void *)0x4242,
                                                                      test_on_bytes_received,
...
TEST_FUNCTION(when_allocating_memory_for_the_plain_auth_string_fails_on_open_complete_is_triggered_with_IO_OPEN_ERROR)
{
    ...
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).SetReturn(NULL);    // auth
    STRICT_EXPECTED_CALL(xio_close(TEST_IO_HANDLE, NULL, NULL));
    STRICT_EXPECTED_CALL(test_on_io_open_complete((void *)0x4242, IO_OPEN_ERROR));
...
/* Tests_SRS_HTTP_PROXY_IO_01_078: [ When `on_underlying_io_open_complete` is called with `IO_OPEN_ERROR`, the
 * `on_open_complete` callback shall be triggered with `IO_OPEN_ERROR`, passing also the `on_open_complete_context`
 * argument as `context`. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_079: [ When `on_underlying_io_open_complete` is called with `IO_OPEN_CANCELLED`, the
 * `on_open_complete` callback shall be triggered with `IO_OPEN_CANCELLED`, passing also the `on_open_complete_context`
 * argument as `context`. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_076: [ When `on_underlying_io_open_complete` is called while waiting for the CONNECT
 * reply, the `on_open_complete` callback shall be triggered with `IO_OPEN_ERROR`, passing also the
...
/* Tests_SRS_HTTP_PROXY_IO_01_077: [ When `on_underlying_io_open_complete` is called in after OPEN has completed, the
 * `on_io_error` callback shall be triggered passing the `on_io_error_context` argument as `context`. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_065: [ When bytes are received and the response to the CONNECT request was not yet
 * received, the bytes shall be accumulated until a double new-line is detected. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_066: [ When a double new-line is detected the response shall be parsed in order to extract
 * the status code. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_069: [ Any successful (2xx) response to a CONNECT request indicates that the proxy has
 * established a connection to the requested host and port, and has switched to tunneling the current connection to that
 * server connection. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_070: [ When a success status code is parsed, the `on_open_complete` callback shall be
 * triggered with `IO_OPEN_OK`, passing also the `on_open_complete_context` argument as `context`. ]*/
...
TEST_FUNCTION(on_underlying_io_bytes_received_with_a_good_reply_in_2_chunks_indicates_OPEN_OK)
{
    ...
    // act
    g_on_bytes_received(g_on_bytes_received_context,
                        (const unsigned char *)connect_response + sizeof(connect_response) - 2,
                        1);
...
/* Tests_SRS_HTTP_PROXY_IO_01_067: [ If allocating memory for the buffered bytes fails, the `on_open_complete` callback
 * shall be triggered with `IO_OPEN_ERROR`, passing also the `on_open_complete_context` argument as `context`. ]*/
...
TEST_FUNCTION(on_underlying_io_bytes_received_with_a_good_reply_status_code_201_indicates_OPEN_OK)
{
    ...
    // act
    g_on_bytes_received(g_on_bytes_received_context,
                        (const unsigned char *)connect_response_201,
                        sizeof(connect_response_201) - 1);
...
TEST_FUNCTION(on_underlying_io_bytes_received_with_a_good_reply_status_code_299_indicates_OPEN_OK)
{
    ...
    // act
    g_on_bytes_received(g_on_bytes_received_context,
                        (const unsigned char *)connect_response_299,
                        sizeof(connect_response_299) - 1);
 ...
 * triggered with `IO_OPEN_OK`, passing also the `on_open_complete_context` argument as `context`. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_073: [ Once a success status code was parsed, the IO shall be OPEN. ]*/
/* Tests_SRS_HTTP_PROXY_IO_01_090: [ Any successful (2xx) response to a CONNECT request indicates that the proxy has
 * established a connection to the requested host and port, and has switched to tunneling the current connection to that
 * server connection. ]*/
TEST_FUNCTION(on_underlying_io_bytes_received_with_a_good_reply_status_code_200_and_some_text_indicates_OPEN_OK)
{
    ...
    // act
    g_on_bytes_received(g_on_bytes_received_context,
                        (const unsigned char *)connect_response_200,
                        sizeof(connect_response_200) - 1);
...
/* Tests_SRS_HTTP_PROXY_IO_01_071: [ If the status code is not successful, the `on_open_complete` callback shall be
 * triggered with `IO_OPEN_ERROR`, passing also the `on_open_complete_context` argument as `context`. ]*/
TEST_FUNCTION(on_underlying_io_bytes_received_with_a_199_code_indicates_an_error)
{
    ...
    // act
    g_on_bytes_received(g_on_bytes_received_context,
                        (const unsigned char *)connect_response_199,
                        sizeof(connect_response_199) - 1);
...
/* Tests_SRS_HTTP_PROXY_IO_01_072: [ Any bytes that are extra (not consumed by the CONNECT response), shall be indicated
 * as received by calling the `on_bytes_received` callback and passing the `on_bytes_received_context` as context
 * argument. ]*/
TEST_FUNCTION(one_extra_byte_gets_indicated_as_received)
{
    ...
    // act
    g_on_bytes_received(g_on_bytes_received_context,
                        (const unsigned char *)connect_response_with_byte,
                        sizeof(connect_response_with_byte) - 1);
...
/* Tests_SRS_HTTP_PROXY_IO_01_074: [ If `on_underlying_io_bytes_received` is called while OPEN, all bytes shall be
 * indicated as received by calling the `on_bytes_received` callback and passing the `on_bytes_received_context` as
 * context argument. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_080: [ If `on_underlying_io_bytes_received` is called while the underlying IO is being
 * opened, the `on_open_complete` callback shall be triggered with `IO_OPEN_ERROR`, passing also the
 * `on_open_complete_context` argument as `context`. ]*/
TEST_FUNCTION(
    if_bytes_are_indicated_as_received_while_opening_the_underlying_IO_an_error_is_indicated_in_the_open_complete_callback)
{
    // arrange
    CONCRETE_IO_HANDLE http_io;
...
/* Tests_SRS_HTTP_PROXY_IO_01_068: [ If parsing the CONNECT response fails, the `on_open_complete` callback shall be
 * triggered with `IO_OPEN_ERROR`, passing also the `on_open_complete_context` argument as `context`. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_083: [ `on_underlying_io_close_complete` while CLOSING shall call the
 * `on_io_close_complete` callback, passing to it the `on_io_close_complete_context` as `context` argument. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_086: [ If the `on_io_close_complete` callback passed to `http_proxy_io_close` was NULL, no
 * callback shall be triggered. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_089: [ If the `on_underlying_io_error` callback is called while the IO is OPEN, the
 * `on_io_error` callback shall be called with the `on_io_error_context` argument as `context`. ]*/
...
/* Tests_SRS_HTTP_PROXY_IO_01_087: [ If the `on_underlying_io_error` callback is called while OPENING, the
 * `on_open_complete` callback shall be triggered with `IO_OPEN_ERROR`, passing also the `on_open_complete_context`
 * argument as `context`. ]*/
TEST_FUNCTION(
    when_on_underlying_io_error_is_called_while_waiting_for_CONNECT_response_the_error_is_indicated_via_open_complete)
{
    // arrange
    CONCRETE_IO_HANDLE http_io;
 ...
 * argument as `context`. ]*/
TEST_FUNCTION(
    when_on_underlying_io_error_is_called_while_waiting_for_underlying_IO_to_open_the_error_is_indicated_via_open_complete)
{
    // arrange
    CONCRETE_IO_HANDLE http_io;
