...
#define TEST_EXECUTE_REQUEST_RELATIVE_PATH                                                                             \
    (const char *)"/devices/Huzzah_w_DHT22/messages/events?api-version=2016-11-14"
#define TEST_EXECUTE_REQUEST_CONTENT                                                                                   \
    (const unsigned char                                                                                               \
         *)"{\"ObjectType\":\"DeviceInfo\", \"Version\":\"1.0\", \"IsSimulatedDevice\":false, "                        \
           "\"DeviceProperties\":{\"DeviceID\":\"Huzzah_w_DHT22\", \"HubEnabledState\":true}, \"Commands\":[{ "        \
           "\"Name\":\"SetHumidity\", \"Parameters\":[{\"Name\":\"humidity\",\"Type\":\"int\"}]},{ "                   \
           "\"Name\":\"SetTemperature\", \"Parameters\":[{\"Name\":\"temperature\",\"Type\":\"int\"}]}]}"
...
static const xio_dowork_job doworkjob_4none_oe[6]   = {XIO_DOWORK_JOB_NONE,
                                                     XIO_DOWORK_JOB_NONE,
                                                     XIO_DOWORK_JOB_NONE,
                                                     XIO_DOWORK_JOB_NONE,
                                                     XIO_DOWORK_JOB_OPEN,
                                                     XIO_DOWORK_JOB_END};
static const xio_dowork_job doworkjob_4none_ee[6]   = {XIO_DOWORK_JOB_NONE,
                                                     XIO_DOWORK_JOB_NONE,
                                                     XIO_DOWORK_JOB_NONE,
                                                     XIO_DOWORK_JOB_NONE,
                                                     XIO_DOWORK_JOB_ERROR,
                                                     XIO_DOWORK_JOB_END};
static const xio_dowork_job doworkjob_o_3none_ee[6] = {XIO_DOWORK_JOB_OPEN,
                                                       XIO_DOWORK_JOB_NONE,
                                                       XIO_DOWORK_JOB_NONE,
                                                       XIO_DOWORK_JOB_NONE,
                                                       XIO_DOWORK_JOB_ERROR,
...
static const xio_dowork_job doworkjob_o_rce[8] = {XIO_DOWORK_JOB_OPEN,
                                                  XIO_DOWORK_JOB_RECEIVED,
                                                  XIO_DOWORK_JOB_RECEIVED,
                                                  XIO_DOWORK_JOB_RECEIVED,
                                                  XIO_DOWORK_JOB_RECEIVED,
                                                  ...
                                                  XIO_DOWORK_JOB_CLOSE,
                                                  XIO_DOWORK_JOB_END};
static const xio_dowork_job doworkjob_o_rc_error[9] = {XIO_DOWORK_JOB_OPEN,
                                                       XIO_DOWORK_JOB_RECEIVED,
                                                       XIO_DOWORK_JOB_RECEIVED,
                                                       XIO_DOWORK_JOB_RECEIVED,
                                                       XIO_DOWORK_JOB_RECEIVED,
                                                       ...
                                                       XIO_DOWORK_JOB_ERROR,
                                                       XIO_DOWORK_JOB_END};
static const xio_dowork_job doworkjob_o_rre[4]      = {XIO_DOWORK_JOB_OPEN,
                                                  XIO_DOWORK_JOB_RECEIVED,
                                                  XIO_DOWORK_JOB_RECEIVED,
                                                  XIO_DOWORK_JOB_END};
...
static const IO_SEND_RESULT sendresult_7ok[7] =
    {IO_SEND_OK, IO_SEND_OK, IO_SEND_OK, IO_SEND_OK, IO_SEND_OK, IO_SEND_OK, IO_SEND_OK};
static const IO_SEND_RESULT sendresult_6ok_error[7] =
    {IO_SEND_OK, IO_SEND_OK, IO_SEND_OK, IO_SEND_OK, IO_SEND_OK, IO_SEND_OK, IO_SEND_ERROR};
...
{
    int result;
    if ((xio == NULL) || (on_io_open_complete == NULL) || (on_io_open_complete_context == NULL) ||
        (on_bytes_received == NULL) || (on_bytes_received_context == NULL) || (on_io_error == NULL) ||
        (on_io_error_context == NULL))
    {
        result = __FAILURE__;
    }
    else
...
{
    int result;
    if ((xio == NULL) || (on_io_close_complete == NULL) || (on_io_close_complete_context == NULL))
    {
        result = __FAILURE__;
    }
    else
...
int my_xio_send(XIO_HANDLE       xio,
                const void      *buffer,
                size_t           size,
...
void my_xio_dowork(XIO_HANDLE xio)
{
    if (xio != NULL)
    {
        ...
        case XIO_DOWORK_JOB_RECEIVED:
            ...
            {
                my_on_bytes_received(my_on_bytes_received_context,
                                     DoworkJobsReceivedBuffer,
                                     DoworkJobsReceivedBuffer_size[DoworkJobsReceivedBuffer_counter]);
            }
            DoworkJobs++;
...
static void createHttpObjects(HTTP_HEADERS_HANDLE *requestHttpHeaders, HTTP_HEADERS_HANDLE *responseHttpHeaders)
{
    ...
    *responseHttpHeaders = HTTPHeaders_Alloc();
    if ((*requestHttpHeaders == NULL) || (*responseHttpHeaders == NULL))
    {
...
static void setHttpCertificate(HTTP_HANDLE httpHandle)
{
    ...
    /*Tests_SRS_HTTPAPI_COMPACT_21_057: [ The HTTPAPI_SetOption shall receive a handle that identiry the HTTP
     * connection. ]*/
...
static void setHttpx509ClientCertificateAndKey(HTTP_HANDLE httpHandle)
{
    ...
    HTTPAPI_SetOption(httpHandle,
                      SU_OPTION_X509_PRIVATE_KEY,
                      TEST_SETOPTIONS_X509PRIVATEKEY); /* currentmalloc_call += 1 */
}

static void
setupAllCallBeforeOpenHTTPsequence(HTTP_HEADERS_HANDLE requestHttpHeaders, int numberOfDoWork, bool useClientCert)
{
    int i;

    STRICT_EXPECTED_CALL(HTTPHeaders_GetHeaderCount(requestHttpHeaders, IGNORED_PTR_ARG)).IgnoreArgument(2);
    ...
    STRICT_EXPECTED_CALL(xio_open(IGNORED_PTR_ARG,
                                  IGNORED_PTR_ARG,
                                  IGNORED_PTR_ARG,
                                  IGNORED_PTR_ARG,
                                  IGNORED_PTR_ARG,
    ...
    {
        STRICT_EXPECTED_CALL(xio_dowork(IGNORED_NUM_ARG)).IgnoreArgument(1);
        if (i > 0)
        {
...
static void
PrepareReceiveHead(HTTP_HEADERS_HANDLE requestHttpHeaders, size_t bufferSize[], int doworkReduction[], int countSizes)
{
    int countBuffer;
...
/*Tests_SRS_HTTPAPI_COMPACT_21_007: [ If there is not enough memory to control the http protocol, the HTTPAPI_Init shall
 * return HTTPAPI_ALLOC_FAILED. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_006: [ If HTTPAPI_Init succeed allocating all the needed memory, it shall return
 * HTTPAPI_OK. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_014: [ If the hostName is NULL, the HTTPAPI_CreateConnection shall return NULL as the
 * handle. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_015: [ If the hostName is empty, the HTTPAPI_CreateConnection shall return NULL as the
 * handle. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_011: [ The HTTPAPI_CreateConnection shall create an http connection to the host specified
...
/*Tests_SRS_HTTPAPI_COMPACT_21_013: [ If there is not enough memory to control the http connection, the
...
/*Tests_SRS_HTTPAPI_COMPACT_21_016: [ If the HTTPAPI_CreateConnection failed to create the connection, it shall return
 * NULL as the handle. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_017: [ The HTTPAPI_CloseConnection shall close the connection previously created in
 * HTTPAPI_ExecuteRequest. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_076: [ After close the connection, The HTTPAPI_CloseConnection shall destroy the
 * connection previously created in HTTPAPI_CreateConnection. ]*/
TEST_FUNCTION(HTTPAPI_CloseConnection__valid_hostName_Succeed)
{
    /// arrange
    HTTP_HANDLE httpHandle = createHttpConnection();
    STRICT_EXPECTED_CALL(xio_close(IGNORED_PTR_ARG, IGNORED_PTR_ARG, IGNORED_PTR_ARG)).IgnoreAllArguments();
    STRICT_EXPECTED_CALL(xio_destroy(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
...
/*Tests_SRS_HTTPAPI_COMPACT_21_020: [ If the connection handle is NULL, the HTTPAPI_CloseConnection shall not do
 * anything. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_018: [ If there is a certificate associated to this connection, the
...
/*Tests_SRS_HTTPAPI_COMPACT_06_001: [ If there is a x509 client certificate associated to this connection, the
 * HTTAPI_CloseConnection shall free all allocated memory for the certificate. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_06_002: [ If there is a x509 client private key associated to this connection, then
...
/*Tests_SRS_HTTPAPI_COMPACT_21_087: [ If the xio return anything different than 0, the HTTPAPI_CloseConnection shall
 * destroy the connection anyway. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_084: [ The HTTPAPI_CloseConnection shall wait, at least, 10 seconds for the SSL close
 * process. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_086: [ The HTTPAPI_CloseConnection shall wait, at least, 100 milliseconds between
 * retries. ]*/
TEST_FUNCTION(HTTPAPI_CloseConnection__close_on_dowork_succeed)
{
    ...
    STRICT_EXPECTED_CALL(xio_close(IGNORED_PTR_ARG, IGNORED_PTR_ARG, IGNORED_PTR_ARG)).IgnoreAllArguments();
    STRICT_EXPECTED_CALL(xio_dowork(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(xio_destroy(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
...
/*Tests_SRS_HTTPAPI_COMPACT_21_085: [ If the HTTPAPI_CloseConnection retries 10 seconds to close the connection without
 * success, it shall destroy the connection anyway. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_034: [ If there is no previous connection, the HTTPAPI_ExecuteRequest shall return
 * HTTPAPI_INVALID_ARG. ]*/
TEST_FUNCTION(HTTPAPI_ExecuteRequest__NULL_handle_failed)
{
    ...
    /// act
    result = HTTPAPI_ExecuteRequest((HTTP_HANDLE)NULL,
                                    HTTPAPI_REQUEST_GET,
                                    TEST_EXECUTE_REQUEST_RELATIVE_PATH,
                                    requestHttpHeaders,
                                    TEST_EXECUTE_REQUEST_CONTENT,
...
/*Tests_SRS_HTTPAPI_COMPACT_21_037: [ If the request type is unknown, the HTTPAPI_ExecuteRequest shall return
 * HTTPAPI_INVALID_ARG. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_039: [ If the relativePath is NULL or invalid, the HTTPAPI_ExecuteRequest shall return
 * HTTPAPI_INVALID_ARG. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_041: [ If the httpHeadersHandle is NULL or invalid, the HTTPAPI_ExecuteRequest shall
...
/*Tests_SRS_HTTPAPI_COMPACT_21_054: [ If Http header maker cannot provide the number of headers, the
...
/*Tests_SRS_HTTPAPI_COMPACT_21_023: [ If the transport failed setting the Certificate, the HTTPAPI_ExecuteRequest shall
 * not send any request and return HTTPAPI_SET_OPTION_FAILED. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_06_005: [ If the transport failed setting the client certificate, the HTTPAPI_ExecuteRequest
 * shall not send any request and return HTTPAPI_SET_OPTION_FAILED. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_06_006: [ If the transport failed setting the client certificate private key, the
...
/*Tests_SRS_HTTPAPI_COMPACT_21_052: [ If any memory allocation get fail, the HTTPAPI_ExecuteRequest shall return
 * HTTPAPI_ALLOC_FAILED. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_062: [ If any memory allocation get fail, the HTTPAPI_SetOption shall return
 * HTTPAPI_ALLOC_FAILED. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_064: [ If the HTTPAPI_SetOption get success setting the option, it shall return
 * HTTPAPI_OK. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_063: [ If the HTTP do not support the optionName, the HTTPAPI_SetOption shall return
 * HTTPAPI_INVALID_ARG. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_052: [ If any memory allocation get fail, the HTTPAPI_ExecuteRequest shall return
 * HTTPAPI_ALLOC_FAILED. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_070: [ If any memory allocation get fail, the HTTPAPI_CloneOption shall return
 * HTTPAPI_ALLOC_FAILED. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_066: [ The HTTPAPI_CloneOption shall return a clone of the value identified by the
 * optionName. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_072: [ If the HTTPAPI_CloneOption get success setting the option, it shall return
 * HTTPAPI_OK. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_067: [ If the optionName is NULL, the HTTPAPI_CloneOption shall return
 * HTTPAPI_INVALID_ARG. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_069: [ If the savedValue is NULL, the HTTPAPI_CloneOption shall return
 * HTTPAPI_INVALID_ARG. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_071: [ If the HTTP do not support the optionName, the HTTPAPI_CloneOption shall return
 * HTTPAPI_INVALID_ARG. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_025: [ If the open process failed, the HTTPAPI_ExecuteRequest shall not send any request
 * and return HTTPAPI_OPEN_REQUEST_FAILED. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_077: [ The HTTPAPI_ExecuteRequest shall wait, at least, 10 seconds for the SSL open
 * process. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_078: [ If the HTTPAPI_ExecuteRequest cannot open the connection in 10 seconds, it shall
 * fail and return HTTPAPI_OPEN_REQUEST_FAILED. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_027: [ If the HTTPAPI_ExecuteRequest cannot create a buffer to send the request, it shall
 * not send any request and return HTTPAPI_STRING_PROCESSING_ERROR. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_026: [ If the open process succeed, the HTTPAPI_ExecuteRequest shall send the request
 * message to the host. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_024: [ The HTTPAPI_ExecuteRequest shall open the transport connection with the host to
 * send the request. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_022: [ If a Certificate was provided, the HTTPAPI_ExecuteRequest shall set this option on
 * the transport layer. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_06_003: [ If the x509 client certificate is provided, the HTTPAPI_ExecuteRequest shall set
 * this option on the transport layer. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_06_004: [ If the x509 client certificate private key is provided, the HTTPAPI_ExecuteRequest
 * shall set this optionon the transport layer. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_028: [ If the HTTPAPI_ExecuteRequest cannot send the request header, it shall return
...
/*Tests_SRS_HTTPAPI_COMPACT_21_079: [ The HTTPAPI_ExecuteRequest shall wait, at least, 20 seconds to send a buffer using
 * the SSL connection. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_080: [ If the HTTPAPI_ExecuteRequest retries to send the message for 20 seconds without
 * success, it shall fail and return HTTPAPI_SEND_REQUEST_FAILED. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_029: [ If the HTTPAPI_ExecuteRequest cannot send the buffer with the request, it shall
...
/*Tests_SRS_HTTPAPI_COMPACT_21_030: [ At the end of the transmission, the HTTPAPI_ExecuteRequest shall receive the
 * response from the host. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_032: [ If the HTTPAPI_ExecuteRequest cannot read the message with the request result, it
 * shall return HTTPAPI_READ_DATA_FAILED. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_055: [ If the HTTPAPI_ExecuteRequest cannot parser the received message, it shall return
...
/*Tests_SRS_HTTPAPI_COMPACT_21_031: [ After receive the response, the HTTPAPI_ExecuteRequest shall close the transport
 * connection with the host. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_033: [ If the whole process succeed, the HTTPAPI_ExecuteRequest shall retur HTTPAPI_OK.
 * ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_021: [ The HTTPAPI_ExecuteRequest shall execute the http communtication with the provided
...
/*Tests_SRS_HTTPAPI_COMPACT_21_046: [ The HTTPAPI_ExecuteRequest shall return the http status reported by the host in
 * the received response. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_047: [ The HTTPAPI_ExecuteRequest shall report the status in the statusCode parameter.
 * ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_050: [ If there is a content in the response, the HTTPAPI_ExecuteRequest shall copy it in
 * the responseContent buffer. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_073: [ The message received by the HTTPAPI_ExecuteRequest shall starts with a valid
 * header. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_074: [ After the header, the message received by the HTTPAPI_ExecuteRequest can contain
 * addition information about the content. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_035: [ The HTTPAPI_ExecuteRequest shall execute resquest for types `GET`, `POST`, `PUT`,
 * `DELETE`, `PATCH`. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_038: [ The HTTPAPI_ExecuteRequest shall execute the resquest for the path in relativePath
 * parameter. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_043: [ If the content is NULL, the HTTPAPI_ExecuteRequest shall send the request without
 * content. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_044: [ If the content is not NULL, the number of bytes in the content shall be provided
 * in contentLength parameter. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_075: [ The message received by the HTTPAPI_ExecuteRequest can contain a body with the
 * message content. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_045: [ If the contentLength is lower than one, the HTTPAPI_ExecuteRequest shall send the
...
/*Tests_SRS_HTTPAPI_COMPACT_21_048: [ If the statusCode is NULL, the HTTPAPI_ExecuteRequest shall report not report any
 * status. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_049: [ If responseHeadersHandle is provide, the HTTPAPI_ExecuteRequest shall prepare a
 * Response Header usign the HTTPHeaders_AddHeaderNameValuePair. ]*/
...
/*Tests_SRS_HTTPAPI_COMPACT_21_051: [ If the responseContent is NULL, the HTTPAPI_ExecuteRequest shall ignore any
...
/*Tests_SRS_HTTPAPI_COMPACT_21_081: [ The HTTPAPI_ExecuteRequest shall try to read the message with the response up to
 * 20 seconds. ]*/
/*Tests_SRS_HTTPAPI_COMPACT_21_082: [ If the HTTPAPI_ExecuteRequest retries 20 seconds to receive the message without
 * success, it shall fail and return HTTPAPI_READ_DATA_FAILED. ]*/
