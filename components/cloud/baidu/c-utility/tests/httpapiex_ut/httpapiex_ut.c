...
/*this function takes care of setting expected call for httpapiex_executerequest assuming all the parameters are
 * non-null*/
static void setupAllCallBeforeHTTPsequence(void)
{
    /*this is building the host and content-length for the http request headers*/
    STRICT_EXPECTED_CALL(BUFFER_length(IGNORED_PTR_ARG)).IgnoreArgument(1).SetReturn(TEST_BUFFER_SIZE);
    ...
    STRICT_EXPECTED_CALL(
        HTTPHeaders_ReplaceHeaderNameValuePair(IGNORED_PTR_ARG, "Content-Length", TOSTRING(TEST_BUFFER_SIZE)))
        .IgnoreArgument(1);
}

static void setupAllCallForHTTPsequence(const char         *relativePath,
                                        ...
                                        BUFFER_HANDLE       responseHttpBody)
{
    ...
    STRICT_EXPECTED_CALL(VECTOR_size(IGNORED_PTR_ARG))
        /*this is passing the options*/ /*there are none saved in the regular sequences*/
        .IgnoreArgument(1);

    STRICT_EXPECTED_CALL(BUFFER_length(requestHttpBody)).SetReturn(TEST_BUFFER_SIZE);

    STRICT_EXPECTED_CALL(BUFFER_u_char(requestHttpBody)).SetReturn(TEST_BUFFER);
...
static void prepareHTTPAPIEX_ExecuteRequest(unsigned int       *asGivenByHttpApi,
                                            HTTP_HEADERS_HANDLE requestHttpHeaders,
                                            HTTP_HEADERS_HANDLE responseHttpHeaders,
                                            BUFFER_HANDLE       responseHttpBody,
                                            HTTPAPI_RESULT      resultToBeUsed)
{
    ...
    STRICT_EXPECTED_CALL(BUFFER_length(IGNORED_PTR_ARG)).IgnoreArgument(1).SetReturn(TEST_BUFFER_SIZE);
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_PTR_ARG)).IgnoreArgument(1).SetReturn(TEST_BUFFER);
    STRICT_EXPECTED_CALL(HTTPAPI_ExecuteRequest(IGNORED_PTR_ARG,
                                                HTTPAPI_REQUEST_PATCH,
                                                TEST_RELATIVE_PATH,
                                                requestHttpHeaders,
...
TEST_FUNCTION_INITIALIZE(TestMethodInitialize)
{
    ...
    for (i = 0; i < N_MAX_FAILS; i++)
        whenShallHTTPAPI_CreateConnection_fail[i] = 0;

    currentHTTPAPI_Init_call = 0;
    for (i = 0; i < N_MAX_FAILS; i++)
        whenShallHTTPAPI_Init_fail[i] = 0;
...
/*Tests_SRS_HTTPAPIEX_02_004: [Otherwise, HTTPAPIEX_Create shall return a HTTAPIEX_HANDLE suitable for further calls to
 * the module.]*/
TEST_FUNCTION(HTTPAPIEX_Create_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(VECTOR_create(IGNORED_NUM_ARG)).IgnoreArgument(1);
...
TEST_FUNCTION(HTTPAPIEX_Destroy_frees_resources_1) /*this is destroy after created*/
{
    ...
    STRICT_EXPECTED_CALL(VECTOR_size(IGNORED_PTR_ARG)).IgnoreArgument(1);
...
TEST_FUNCTION(HTTPAPIEX_Destroy_frees_resources_2) /*this is destroy after setting options*/
{
    ...
    STRICT_EXPECTED_CALL(VECTOR_element(IGNORED_PTR_ARG, 0)).IgnoreArgument(1);
...
TEST_FUNCTION(HTTPAPIEX_Destroy_frees_resources_3) /*this is destroy after having a sequence build*/
{
    ...
    setupAllCallForHTTPsequence(TEST_RELATIVE_PATH,
                                requestHttpHeaders,
                                requestHttpBody,
                                responseHttpHeaders,
                                responseHttpBody);
    (void)HTTPAPIEX_ExecuteRequest(httpapiexhandle,
                                   HTTPAPI_REQUEST_PATCH,
                                   TEST_RELATIVE_PATH,
                                   requestHttpHeaders,
                                   requestHttpBody,
...
TEST_FUNCTION(HTTPAPIEX_Create_fails_when_malloc_fails)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_malloc(0)).IgnoreArgument(1);
...
/*Tests_SRS_HTTPAPIEX_02_003: [If saving the parameter hostName fails for any reason, then HTTPAPIEX_Create shall return
 * NULL.] */
TEST_FUNCTION(HTTPAPIEX_Create_fails_when_STRING_construct_fails)
{
    ...
    STRICT_EXPECTED_CALL(STRING_construct(TEST_HOSTNAME)).SetReturn(NULL);
...
TEST_FUNCTION(HTTPAPIEX_Create_fails_when_VECTOR_create_fails)
{
    ...
    STRICT_EXPECTED_CALL(VECTOR_create(IGNORED_NUM_ARG)).IgnoreArgument(1).SetReturn(NULL);
...
/*Tests_SRS_HTTPAPIEX_02_006: [If parameter handle is NULL then HTTPAPIEX_ExecuteRequest shall fail and return
...
TEST_FUNCTION(HTTPAPIEX_ExecuteRequest_with_NULL_handle_fails)
{
    ...
    /// act
    HTTPAPIEX_RESULT result = HTTPAPIEX_ExecuteRequest(NULL,
                                                       HTTPAPI_REQUEST_POST,
                                                       TEST_RELATIVE_PATH,
                                                       TEST_REQUEST_HTTP_HEADERS,
                                                       TEST_REQUEST_BODY,
                                                       &httpStatusCode,
                                                       TEST_RESPONSE_HTTP_HEADERS,
                                                       TEST_RESPONSE_BODY);
...
/*Tests_SRS_HTTPAPIEX_02_007: [If parameter requestType does not indicate a valid request, HTTPAPIEX_ExecuteRequest
 * shall fail and return HTTPAPIEX_INVALID_ARG.] */
TEST_FUNCTION(HTTPAPIEX_ExecuteRequest_fails_with_invalid_request_type)
{
    ...
    /// act
    result = HTTPAPIEX_ExecuteRequest(httpapiexhandle,
...
/*Tests_SRS_HTTPAPIEX_02_023: [HTTPAPIEX_ExecuteRequest shall try to execute the HTTP call by ensuring the following API
 * call sequence is respected:]*/
...
/*Tests_SRS_HTTPAPIEX_02_008: [If parameter relativePath is NULL then HTTPAPIEX_INVALID_ARG shall not assume a relative
 * path - that is, it will assume an empty path ("").] */
...
/*Tests_SRS_HTTPAPIEX_02_009: [If parameter requestHttpHeadersHandle is NULL then HTTPAPIEX_ExecuteRequest shall
   allocate a temporary internal instance of HTTPHEADERS, shall add to that instance the following headers
    Host:{hostname} - as it was indicated by the call to HTTPAPIEX_Create API call
    Content-Length:the size of the requestContent parameter, and use this instance to all the subsequent calls to
   HTTPAPI_ExecuteRequest as parameter httpHeadersHandle.]
*/
/*Tests_SRS_HTTPAPIEX_02_013: [If requestContent is NULL then HTTPAPIEX_ExecuteRequest shall behave as if a buffer of
 * zero size would have been used, that is, it shall call HTTPAPI_ExecuteRequest with parameter content = NULL and
 * contentLength = 0.]*/
TEST_FUNCTION(HTTPAPIEX_ExecuteRequest_with_NULL_request_headers_and_NULL_requestBody_succeeds)
{
    ...
    /*this is building the host and content-length for the http request headers*/
    STRICT_EXPECTED_CALL(BUFFER_length(IGNORED_PTR_ARG)).IgnoreArgument(1).SetReturn(0);
    STRICT_EXPECTED_CALL(size_tToString(IGNORED_PTR_ARG, IGNORED_NUM_ARG, 0)).IgnoreArgument(1).IgnoreArgument(2);
    STRICT_EXPECTED_CALL(STRING_c_str(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(HTTPHeaders_ReplaceHeaderNameValuePair(IGNORED_PTR_ARG, "Host", TEST_HOSTNAME))
        .IgnoreArgument(1);
    ...
    STRICT_EXPECTED_CALL(BUFFER_length(IGNORED_PTR_ARG)).IgnoreArgument(1).SetReturn(0);
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_PTR_ARG)).IgnoreArgument(1).SetReturn(NULL);
    ...
    STRICT_EXPECTED_CALL(BUFFER_delete(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(HTTPHeaders_Free(IGNORED_PTR_ARG)).IgnoreArgument(1);
...
/*Tests_SRS_HTTPAPIEX_02_010: [If any of the operations in SRS_HTTAPIEX_02_009 fails, then HTTPAPIEX_ExecuteRequest
 * shall return HTTPAPIEX_ERROR.]*/
TEST_FUNCTION(
    HTTPAPIEX_ExecuteRequest_with_NULL_request_headers_and_NULL_requestBody_fails_when_HTTPHeaders_ReplaceHeaderNameValuePair_fails)
{
    /// arrange
    HTTPAPIEX_HANDLE httpapiexhandle = HTTPAPIEX_Create(TEST_HOSTNAME);
    HTTPAPIEX_RESULT result;
 ...
 * shall return HTTPAPIEX_ERROR.]*/
TEST_FUNCTION(
    HTTPAPIEX_ExecuteRequest_with_NULL_request_headers_and_NULL_requestBody_fails_when_HTTPHeaders_ReplaceHeaderNameValuePair_fails_2)
{
    /// arrange
    HTTPAPIEX_HANDLE httpapiexhandle = HTTPAPIEX_Create(TEST_HOSTNAME);
    HTTPAPIEX_RESULT result;
 ...
 * shall return HTTPAPIEX_ERROR.]*/
TEST_FUNCTION(
    HTTPAPIEX_ExecuteRequest_with_NULL_request_headers_and_NULL_requestBody_fails_when_HTTPHeaders_ReplaceHeaderNameValuePair_fails_3)
{
    /// arrange
    HTTPAPIEX_HANDLE httpapiexhandle = HTTPAPIEX_Create(TEST_HOSTNAME);
    HTTPAPIEX_RESULT result;
    ...
    STRICT_EXPECTED_CALL(HTTPHeaders_Alloc()).SetReturn(NULL); /*because it makes fakes request headers*/
...
/*Tests_SRS_HTTPAPIEX_02_011: [If parameter requestHttpHeadersHandle is not NULL then HTTPAPIEX_ExecuteRequest shall
   create or update the following headers of the request: Host:{hostname} Content-Length:the size of the requestContent
   parameter, and shall use the so constructed HTTPHEADERS object to all calls to HTTPAPI_ExecuteRequest as parameter
   httpHeadersHandle.]
*/
...
/*Tests_SRS_HTTPAPIEX_02_012: [If any of the operations required for SRS_HTTAPIEX_02_011 fails, then
 * HTTPAPIEX_ExecuteRequest shall return HTTPAPIEX_ERROR.] */
TEST_FUNCTION(
    HTTPAPIEX_ExecuteRequest_with_non_NULL_request_headers_and_NULL_requestBody_fails_when_HTTPHeaders_ReplaceHeaderNameValuePair_fails_1)
{
    /// arrange
    HTTPAPIEX_HANDLE httpapiexhandle = HTTPAPIEX_Create(TEST_HOSTNAME);
    HTTPAPIEX_RESULT result;
 ...
 * HTTPAPIEX_ExecuteRequest shall return HTTPAPIEX_ERROR.] */
TEST_FUNCTION(
    HTTPAPIEX_ExecuteRequest_with_non_NULL_request_headers_and_NULL_requestBody_fails_when_HTTPHeaders_ReplaceHeaderNameValuePair_fails_2)
{
    /// arrange
    HTTPAPIEX_HANDLE httpapiexhandle = HTTPAPIEX_Create(TEST_HOSTNAME);
    HTTPAPIEX_RESULT result;
    ...
    /*this is building the host and content-length for the http request headers*/
    STRICT_EXPECTED_CALL(BUFFER_length(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(size_tToString(IGNORED_PTR_ARG, IGNORED_NUM_ARG, TEST_BUFFER_SIZE))
        .IgnoreArgument(1)
        .IgnoreArgument(2);
    STRICT_EXPECTED_CALL(STRING_c_str(IGNORED_PTR_ARG)).IgnoreArgument(1);
 ...
 * HTTPAPIEX_ExecuteRequest shall return HTTPAPIEX_ERROR.] */
TEST_FUNCTION(
    HTTPAPIEX_ExecuteRequest_with_non_NULL_request_headers_and_NULL_requestBody_fails_when_fake_requestbody_creation_fails)
{
    /// arrange
    HTTPAPIEX_HANDLE httpapiexhandle = HTTPAPIEX_Create(TEST_HOSTNAME);
    HTTPAPIEX_RESULT result;
    ...
    STRICT_EXPECTED_CALL(BUFFER_new()).SetReturn(NULL); /*because it makes a fake buffer*/
...
/*Tests_SRS_HTTPAPIEX_02_014: [If requestContent is not NULL then its content and its size shall be used for parameters
 * content and contentLength of HTTPAPI_ExecuteRequest.] */
TEST_FUNCTION(HTTPAPIEX_ExecuteRequest_with_non_NULL_request_headers_and_non_NULL_requestBody_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(BUFFER_length(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_PTR_ARG)).IgnoreArgument(1);
 ...
 * content and contentLength of HTTPAPI_ExecuteRequest.] */
TEST_FUNCTION(
    HTTPAPIEX_ExecuteRequest_with_non_NULL_request_headers_and_non_NULL_requestBody_fails_when_HTTPHeaders_ReplaceHeaderNameValuePair_fails_1)
{
    /// arrange
    HTTPAPIEX_HANDLE httpapiexhandle = HTTPAPIEX_Create(TEST_HOSTNAME);
    HTTPAPIEX_RESULT result;
 ...
 * content and contentLength of HTTPAPI_ExecuteRequest.] */
TEST_FUNCTION(
    HTTPAPIEX_ExecuteRequest_with_non_NULL_request_headers_and_non_NULL_requestBody_fails_when_HTTPHeaders_ReplaceHeaderNameValuePair_fails_2)
{
    /// arrange
    HTTPAPIEX_HANDLE httpapiexhandle = HTTPAPIEX_Create(TEST_HOSTNAME);
    HTTPAPIEX_RESULT result;
...
/*Tests_SRS_HTTPAPIEX_02_015: [If statusCode is NULL then HTTPAPIEX_ExecuteRequest shall not write in statusCode the
 * HTTP status code, and it will use a temporary internal int for parameter statusCode to the calls of
...
/*Tests_SRS_HTTPAPIEX_02_016: [If statusCode is not NULL then If statusCode is NULL then HTTPAPIEX_ExecuteRequest shall
 * use it for parameter statusCode to the calls of HTTPAPI_ExecuteRequest.] */
...
/*Tests_SRS_HTTPAPIEX_02_017: [If responseHeaders handle is NULL then HTTPAPIEX_ExecuteRequest shall create a temporary
 * internal instance of HTTPHEADERS object and use that for responseHeaders parameter of HTTPAPI_ExecuteRequest call.]
 */
...
/*Tests_SRS_HTTPAPIEX_02_018: [If creating the temporary http headers in SRS_HTTPAPIEX_02_017 fails then
 * HTTPAPIEX_ExecuteRequest shall return HTTPAPIEX_ERROR.] */
...
/*Tests_SRS_HTTPAPIEX_02_019: [If responseHeaders is not NULL, then then HTTPAPIEX_ExecuteRequest shall use that object
 * as parameter responseHeaders of HTTPAPI_ExecuteRequest call.] */
...
/*Tests_SRS_HTTPAPIEX_02_020: [If responseContent is NULL then HTTPAPIEX_ExecuteRequest shall create a temporary
 * internal BUFFER object and use that as parameter responseContent of HTTPAPI_ExecuteRequest call.] */
...
/*Tests_SRS_HTTPAPIEX_02_021: [If creating the BUFFER_HANDLE in SRS_HTTPAPIEX_02_020 fails, then
 * HTTPAPIEX_ExecuteRequest shall return HTTPAPIEX_ERROR.] */
TEST_FUNCTION(HTTPAPIEX_ExecuteRequest_with_NULL_fails_when_BUFFER_new_fails)
{
    ...
    STRICT_EXPECTED_CALL(BUFFER_new()).SetReturn(NULL); /*because it makes a fake response buffer*/
...
/*Tests_SRS_HTTPAPIEX_02_022: [If responseContent is not NULL then HTTPAPIEX_ExecuteRequest use that as parameter
 * responseContent of HTTPAPI_ExecuteRequest call.] */
...
/*Tests_SRS_HTTPAPIEX_02_023: [HTTPAPIEX_ExecuteRequest shall try to execute the HTTP call by ensuring the following API
 * call sequence is respected:]*/
/*Tests_SRS_HTTPAPIEX_02_024: [If any point in the sequence fails, HTTPAPIEX_ExecuteRequest shall attempt to recover by
 * going back to the previous step and retrying that step.]*/
...
/*Tests_SRS_HTTPAPIEX_02_028: [HTTPAPIEX_ExecuteRequest shall return HTTPAPIEX_OK when a call to HTTPAPI_ExecuteRequest
...
TEST_FUNCTION(HTTPAPIEX_ExecuteRequest_TestCase_S1) /*refer to httpapiex_retry_mechanism.vsdx*/
{
    ...
    prepareHTTPAPIEX_ExecuteRequest(&asGivenByHttpApi,
                                    requestHttpHeaders,
                                    responseHttpHeaders,
                                    responseHttpBody,
                                    HTTPAPI_OK);
...
TEST_FUNCTION(HTTPAPIEX_ExecuteRequest_TestCase_S2) /*refer to httpapiex_retry_mechanism.vsdx*/
{
    ...
    {
        STRICT_EXPECTED_CALL(HTTPAPI_CloseConnection(IGNORED_PTR_ARG)).IgnoreArgument(1);
...
/*Tests_SRS_HTTPAPIEX_02_033: [If parameter optionName is NULL then HTTPAPIEX_SetOption shall return
 * HTTPAPIEX_INVALID_ARG.] */
TEST_FUNCTION(HTTPAPIEX_SetOption_fails_with_NULL_optionName_fails)
{
    ...
    /// act
    result = HTTPAPIEX_SetOption(httpapiexhandle, NULL, (void *)42);
...
TEST_FUNCTION(HTTPAPIEX_SetOption_fails_with_NULL_value_fails)
{
    ...
    /// act
    result = HTTPAPIEX_SetOption(httpapiexhandle, "someOption", NULL);
...
/*Tests_SRS_HTTPAPIEX_02_037: [HTTPAPIEX_SetOption shall attempt to save the value of the option by calling
 * HTTPAPI_CloneOption passing optionName and value, irrespective of the existence of a HTTPAPI_HANDLE] */
/*Tests_SRS_HTTPAPIEX_02_039: [If HTTPAPI_CloneOption returns HTTPAPI_OK then HTTPAPIEX_SetOption shall create or update
 * the pair optionName/value.]*/
TEST_FUNCTION(HTTPAPIEX_SetOption_happy_path_without_httpapi_handle_succeeds)
{
    ...
    EXPECTED_CALL(HTTPAPI_CloneOption("someOption",
                                      "333",
                                      IGNORED_PTR_ARG)); /*this asks lower HTTPAPI to create a clone of the option*/
    ...
    /// act
    result = HTTPAPIEX_SetOption(httpapiexhandle, "someOption", "333");
...
TEST_FUNCTION(HTTPAPIEX_SetOption_happy_path_without_httpapi_handle_can_save_2_options_succeeds)
{
    ...
    EXPECTED_CALL(HTTPAPI_CloneOption("someOption1",
                                      (void *)"3",
                                      IGNORED_PTR_ARG)); /*this asks lower HTTPAPI to create a clone of the option*/

    EXPECTED_CALL(VECTOR_find_if(IGNORED_PTR_ARG,
                                 IGNORED_PTR_ARG,
                                 "someOption1")); /*this is looking for the option to device between update / create*/

    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "someOption1"))
        .IgnoreArgument(1); /*this is creating a clone of the optionName*/

    STRICT_EXPECTED_CALL(
        VECTOR_push_back(IGNORED_PTR_ARG, IGNORED_PTR_ARG, 1)) /*this is increasing the array of options by 1*/
        .IgnoreArgument(1)
        .IgnoreArgument(2);

    EXPECTED_CALL(HTTPAPI_CloneOption("someOption2",
                                      (void *)"33",
                                      IGNORED_PTR_ARG)); /*this asks lower HTTPAPI to create a clone of the option*/

    EXPECTED_CALL(VECTOR_find_if(IGNORED_PTR_ARG,
                                 IGNORED_PTR_ARG,
                                 "someOption2")); /*this is looking for the option to device between update / create*/
    ...
    /// act
    result1 = HTTPAPIEX_SetOption(httpapiexhandle, "someOption1", (void *)"3");
    result2 = HTTPAPIEX_SetOption(httpapiexhandle, "someOption2", (void *)"33");
...
/*Tests_SRS_HTTPAPIEX_02_035: [HTTPAPIEX_ExecuteRequest shall pass all the saved options (see HTTPAPIEX_SetOption) to
 * the newly create HTTPAPI_HANDLE in step 2 by calling HTTPAPI_SetOption.]*/
TEST_FUNCTION(HTTPAPIEX_SetOption_passes_saved_options_to_httpapi_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(VECTOR_element(IGNORED_PTR_ARG, 1)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(HTTPAPI_SetOption(IGNORED_PTR_ARG, "someOption2", (void *)"33"))
        .IgnoreArgument(1)
        .IgnoreArgument(3);

    STRICT_EXPECTED_CALL(BUFFER_length(requestHttpBody)).SetReturn(TEST_BUFFER_SIZE);
    requestHttpBodyLength = TEST_BUFFER_SIZE;

    STRICT_EXPECTED_CALL(BUFFER_u_char(requestHttpBody)).SetReturn(TEST_BUFFER);
    requestHttpBodyContent = TEST_BUFFER;
...
/*Tests_SRS_HTTPAPIEX_02_041: [If creating or updating the pair optionName/value fails then shall return
 * HTTPAPIEX_ERROR.] */
TEST_FUNCTION(HTTPAPIEX_SetOption_fails_when_VECTOR_push_back_fails)
{
    ...
    /// act
    result = HTTPAPIEX_SetOption(httpapiexhandle, "someOption2", (void *)"33");
...
TEST_FUNCTION(HTTPAPIEX_SetOption_fails_when_gballoc_fails_1)
{
    ...
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "someOption2"))
        .IgnoreArgument(1)
        .SetReturn(1); /*this is creating a clone of the optionName*/
...
/*Tests_SRS_HTTPAPIEX_02_041: [If creating or updating the pair optionName/value fails then shall return
 * HTTPAPIEX_ERROR.] */
/*Tests_SRS_HTTPAPIEX_02_040: [For all other return values of HTTPAPI_SetOption, HTTPIAPIEX_SetOption shall return
 * HTTPAPIEX_ERROR.] */
...
/*Tests_SRS_HTTPAPIEX_02_038: [If HTTPAPI_CloneOption returns HTTPAPI_INVALID_ARG then HTTPAPIEX shall return
 * HTTPAPIEX_INVALID_ARG.]*/
...
/*Tests_SRS_HTTPAPIEX_02_031: [If HTTPAPI_HANDLE exists then HTTPAPIEX_SetOption shall call HTTPAPI_SetOption passing
 * the same optionName and value and shall return a value conforming to the below table:] */
TEST_FUNCTION(HTTPAPIEX_SetOption_passes_saved_options_to_existing_httpapi_handle_succeeds)
{
    ...
    EXPECTED_CALL(VECTOR_find_if(IGNORED_PTR_ARG,
                                 IGNORED_PTR_ARG,
                                 "someOption")); /*this is looking for the option to device between update / create*/
...
TEST_FUNCTION(HTTPAPIEX_SetOption_passes_saved_options_to_existing_httpapi_handle_fails_1)
{
    ...
    EXPECTED_CALL(HTTPAPI_SetOption(IGNORED_PTR_ARG, "someOption", "3")).SetReturn(HTTPAPI_INVALID_ARG);
...
TEST_FUNCTION(HTTPAPIEX_SetOption_passes_saved_options_to_existing_httpapi_handle_fails_2)
{
    ...
    EXPECTED_CALL(HTTPAPI_SetOption(IGNORED_PTR_ARG, "someOption", "3")).SetReturn(HTTPAPI_ALLOC_FAILED);
...
TEST_FUNCTION(HTTPAPIEX_SetOption_happy_path_on_update_option_value_succeeds)
{
    ...
    EXPECTED_CALL(HTTPAPI_CloneOption(OPTION_NAME,
                                      "4",
                                      IGNORED_PTR_ARG)); /*this asks lower HTTPAPI to create a clone of the option*/

    EXPECTED_CALL(VECTOR_find_if(IGNORED_PTR_ARG,
                                 IGNORED_PTR_ARG,
                                 OPTION_NAME)); /*this is looking for the option to device between update / create*/
