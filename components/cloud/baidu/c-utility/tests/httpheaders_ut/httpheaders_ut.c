...
/*Below tags exists for traceability reasons only, they canot be really tested by automated means, except "this file
 * compiles"*/
...
/*Tests_SRS_HTTP_HEADERS_99_002:[ This API shall produce a HTTP_HANDLE that can later be used in subsequent calls to the
 * module.]*/
...
/*Tests_SRS_HTTP_HEADERS_99_005:[ Calling this API shall de-allocate the data structures allocated by previous API calls
 * to the same handle.]*/
TEST_FUNCTION(HTTPHeaders_Free_with_valid_handle_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(Map_Destroy(IGNORED_PTR_ARG)).IgnoreArgument(1);
...
TEST_FUNCTION(HTTPHeaders_Alloc_fails_when_Map_Create_fails)
{
    ...
    HTTP_HEADERS_HANDLE httpHandle;
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(Map_Create(IGNORED_PTR_ARG)).SetReturn(NULL);
...
/*Tests_SRS_HTTP_HEADERS_99_004:[ After a successful init, HTTPHeaders_GetHeaderCount shall report 0 existing
 * headers.]*/
...
TEST_FUNCTION(HTTPHeaders_AddHeaderNameValuePair_happy_path_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(Map_AddOrUpdate(IGNORED_PTR_ARG, NAME1, VALUE1)).IgnoreArgument(1);
...
/*Codes_SRS_HTTP_HEADERS_99_015:[ The function shall return HTTP_HEADERS_ALLOC_FAILED when an internal request to
...
TEST_FUNCTION(HTTPHeaders_AddHeaderNameValuePair_fails_when_Map_AddOrUpdate_fails)
{
    ...
    STRICT_EXPECTED_CALL(Map_AddOrUpdate(IGNORED_PTR_ARG, NAME1, VALUE1)).IgnoreArgument(1).SetReturn(MAP_ERROR);
...
/*Tests_SRS_HTTP_HEADERS_99_016:[ The function shall store the name:value pair in such a way that when later retrieved
 * by a call to GetHeader it will return a string that shall strcmp equal to the name+": "+value.]*/
...
/*Tests_SRS_HTTP_HEADERS_99_014:[ The function shall return when the handle is not valid or when name parameter is NULL
 * or when value parameter is NULL.]*/
...
/*Tests_SRS_HTTP_HEADERS_99_017:[ If the name already exists in the collection of headers, the function shall
 * concatenate the new value after the existing value, separated by a comma and a space as in: old-value+",
 * "+new-value.]*/
TEST_FUNCTION(HTTPHeaders_AddHeaderNameValuePair_with_same_Name_appends_to_existing_value_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(Map_GetValueFromKey(IGNORED_PTR_ARG, NAME1)).IgnoreArgument(1).SetReturn(VALUE1);

    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).IgnoreArgument(1);

    STRICT_EXPECTED_CALL(Map_AddOrUpdate(IGNORED_PTR_ARG, NAME1, VALUE1 ", " VALUE1)).IgnoreArgument(1);
...
/*Tests_SRS_HTTP_HEADERS_99_015:[ The function shall return HTTP_HEADERS_ALLOC_FAILED when an internal request to
 * allocate memory fails.]*/
...
TEST_FUNCTION(HTTPHeaders_AddHeaderNameValuePair_add_two_headers_produces_two_headers)
{
    ...
    STRICT_EXPECTED_CALL(Map_AddOrUpdate(IGNORED_PTR_ARG, NAME2, VALUE2)).IgnoreArgument(1);
...
TEST_FUNCTION(HTTPHeaders_When_Second_Added_Header_Is_A_Substring_Of_An_Existing_Header_2_Headers_Are_Added)
{
    ...
    STRICT_EXPECTED_CALL(Map_AddOrUpdate(IGNORED_PTR_ARG, "a", VALUE1)).IgnoreArgument(1);
...
/*Tests_SRS_HTTP_HEADERS_99_022:[ The return value shall be NULL if name parameter is NULL or if httpHeadersHandle is
 * NULL]*/
...
/*Tests_SRS_HTTP_HEADERS_99_021:[ In this case the return value shall point to a string that shall strcmp equal to the
...
TEST_FUNCTION(HTTPHeaders_FindHeaderValue_retrieves_previously_stored_value_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(Map_GetValueFromKey(IGNORED_PTR_ARG, NAME1)).IgnoreArgument(1);
...
TEST_FUNCTION(HTTPHeaders_FindHeaderValue_retrieves_previously_stored_value_for_two_headers_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(Map_GetValueFromKey(IGNORED_PTR_ARG, NAME1)).IgnoreArgument(1).SetReturn(VALUE1);
    STRICT_EXPECTED_CALL(Map_GetValueFromKey(IGNORED_PTR_ARG, NAME2)).IgnoreArgument(1).SetReturn(VALUE2);
...
TEST_FUNCTION(HTTPHeaders_FindHeaderValue_retrieves_concatenation_of_previously_stored_values_for_header_name_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(Map_GetValueFromKey(IGNORED_PTR_ARG, NAME1)).IgnoreArgument(1).SetReturn(VALUE1 ", " VALUE2);
...
/*Tests_SRS_HTTP_HEADERS_99_020:[ The return value shall be different than NULL when the name matches the name of a
 * previously stored name:value pair.]*/
...
TEST_FUNCTION(HTTPHeaders_FindHeaderValue_returns_NULL_for_nonexistent_value)
{
    ...
    STRICT_EXPECTED_CALL(Map_GetValueFromKey(IGNORED_PTR_ARG, NAME2)).IgnoreArgument(1).SetReturn((const char *)NULL);
...
/* Tests_SRS_HTTP_HEADERS_06_001: [This API will perform exactly as HTTPHeaders_AddHeaderNameValuePair except that if
...
TEST_FUNCTION(HTTPHeaders_ReplaceHeaderNameValuePair_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(Map_AddOrUpdate(IGNORED_PTR_ARG, NAME1, VALUE2)).IgnoreArgument(1);
...
TEST_FUNCTION(HTTPHeaders_ReplaceHeaderNameValuePair_for_none_existing_header_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(Map_GetValueFromKey(IGNORED_PTR_ARG, NAME1)).IgnoreArgument(1).SetReturn((const char *)NULL);
...
/*Tests_SRS_HTTP_HEADERS_99_024:[ The function shall return HTTP_HEADERS_INVALID_ARG when an invalid handle is
 * passed.]*/
...
/*Tests_SRS_HTTP_HEADERS_99_026:[ The function shall write in *headersCount the number of currently stored headers and
 * shall return HTTP_HEADERS_OK]*/
...
/*Tests_SRS_HTTP_HEADERS_99_029:[ The function shall return HTTP_HEADERS_INVALID_ARG if index is not valid (for example,
 * out of range) for the currently stored headers.]*/
...
/*Tests_SRS_HTTP_HEADERS_99_027:[ Calling this API shall produce the string value+": "+pair) for the index header in the
...
/*Tests_SRS_HTTP_HEADERS_99_031:[ If name contains the character ":" then the return value shall be
...
/*Tests_SRS_HTTP_HEADERS_99_036:[ If name contains the characters outside character codes 33 to 126 then the return
 * value shall be HTTP_HEADERS_INVALID_ARG]*/
...
TEST_FUNCTION(HTTPHeaders_AddHeaderNameValuePair_with_LWS_value_stores_without_LWS_characters_succeeds)
{
    ...
    /// act
    res =
        HTTPHeaders_AddHeaderNameValuePair(httpHandle,
                                           NAME1,
...
TEST_FUNCTION(HTTPHEADERS_Clone_happy_path)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(Map_Clone(IGNORED_PTR_ARG)).IgnoreArgument(1);
...
TEST_FUNCTION(HTTPHEADERS_Clone_fails_when_map_clone_fails)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(Map_Clone(IGNORED_PTR_ARG)).IgnoreArgument(1).SetReturn(NULL);
