...
/*Tests_SRS_MAP_02_010: [Otherwise, Map_Add shall add the pair <key,value> to the map.] */
/*Tests_SRS_MAP_02_012: [Otherwise, Map_Add shall return MAP_OK.] */
/*Tests_SRS_MAP_02_043: [Map_GetInternals shall produce in *keys an pointer to an array of const char* having all the
 * keys stored so far by the map.]*/
/*Tests_SRS_MAP_02_044: [Map_GetInternals shall produce in *values a pointer to an array of const char* having all the
 * values stored so far by the map.]*/
...
/*Tests_SRS_MAP_02_016: [If the key already exists, then Map_AddOrUpdate shall overwrite the value of the existing key
...
TEST_FUNCTION(Map_AddOrUpdate_with_2_pair_overwrites_firstValue_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(
        gballoc_realloc(IGNORED_PTR_ARG, strlen(TEST_YELLOWVALUE) + 1)) /*changing redkey value to yellow*/
        .ValidateArgumentBuffer(1, TEST_REDVALUE, strlen(TEST_REDVALUE) + 1);
...
TEST_FUNCTION(Map_AddOrUpdate_with_2_pair_overwrites_secondValue)
{
    ...
    STRICT_EXPECTED_CALL(
        gballoc_realloc(IGNORED_PTR_ARG, strlen(TEST_YELLOWVALUE) + 1)) /*changing bluekey value to yellow*/
...
/*Tests_SRS_MAP_02_023: [Otherwise, Map_Delete shall remove the key and its associated value from the map and return
 * MAP_OK.] */
...
/*Tests_SRS_MAP_02_024: [If parameter handle, key or keyExists are NULL then Map_ContainsKey shall return
 * MAP_INVALIDARG.]*/
...
/*Tests_SRS_MAP_02_025: [Otherwise if a key exists then Map_ContainsKey shall return MAP_OK and shall write in keyExists
 * "true".]*/
/*Tests_SRS_MAP_02_026: [If a key doesn't exist, then Map_ContainsKey shall return MAP_OK and write in keyExists
 * "false".] */
...
/*Tests_SRS_MAP_02_027: [If parameter handle, value or valueExists is NULL then Map_ContainsValue shall return
 * MAP_INVALIDARG.]*/
...
/*Tests_SRS_MAP_02_028: [Otherwise, if a pair <key, value> has its value equal to the parameter value, the
 * Map_ContainsValue shall return MAP_OK and shall write in valueExists "true".]*/
/*Tests_SRS_MAP_02_029: [Otherwise, if such a <key, value> does not exist, then Map_ContainsValue shall return MAP_OK
 * and shall write in valueExists "false".] */
...
/*Tests_SRS_MAP_02_046: [If parameter handle, keys, values or count is NULL then Map_GetInternals shall return
 * MAP_INVALIDARG.] */
...
/*Tests_SRS_MAP_02_039: [Map_Clone shall make a copy of the map indicated by parameter handle and return a non-NULL
 * handle to it.]*/
...
/* Tests_SRS_MAP_07_009: [If the mapFilterCallback function is not NULL, then the return value will be check and if it
 * is not zero then Map_Add shall return MAP_FILTER_REJECT.] */
...
/* Tests_SRS_MAP_07_008: [If the mapFilterCallback function is not NULL, then the return value will be check and if it
 * is not zero then Map_AddOrUpdate shall return MAP_FILTER_REJECT.] */
...
TEST_FUNCTION(Map_ToJSON_with_empty_MAP_produces_empty_JSON)
{
    ...
    STRICT_EXPECTED_CALL(STRING_construct("{")).IgnoreArgument(1);

    STRICT_EXPECTED_CALL(STRING_concat(IGNORED_PTR_ARG, "}")).IgnoreArgument(1);
...
TEST_FUNCTION(Map_ToJSON_with_empty_MAP_produces_empty_JSON_fails_when_STRING_concat_fails)
{
    ...
    STRICT_EXPECTED_CALL(STRING_concat(IGNORED_PTR_ARG, "}")).IgnoreArgument(1).SetReturn(1);
...
TEST_FUNCTION(Map_ToJSON_with_empty_MAP_produces_empty_JSON_fails_when_STRING_construct_fails)
{
    ...
    STRICT_EXPECTED_CALL(STRING_construct("{")).IgnoreArgument(1).SetReturn(NULL);
...
/*Tests_SRS_MAP_02_050: [If the map has properties then Map_ToJSON shall produce the following string:{"name1":"value1",
 * "name2":"value2" ...}] */
TEST_FUNCTION(Map_ToJSON_with_1_MAP_element_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(STRING_concat(IGNORED_PTR_ARG, ":")) /*now JSON is {"redkey":*/
        .IgnoreArgument(1);
    STRICT_EXPECTED_CALL(
        STRING_concat_with_STRING(IGNORED_PTR_ARG, IGNORED_PTR_ARG)) /*now JSON is {"redkey":"reddoor"*/
        .IgnoreArgument(1)
        .IgnoreArgument(2);
