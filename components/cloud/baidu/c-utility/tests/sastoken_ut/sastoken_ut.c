...
/*Tests_SRS_SASTOKEN_25_025: [**SASToken_Validate shall get the SASToken value by invoking STRING_c_str on the
 * handle.**]***/
...
/*Tests_SRS_SASTOKEN_25_027: [**If SASTOKEN does not obey the SASToken format then SASToken_Validate shall return
 * false.**]*/
...
/*Tests_SRS_SASTOKEN_25_028: [**SASToken_validate shall check for the presence of sr, se and sig from the token and
...
/*Tests_SRS_SASTOKEN_25_030: [**SASToken_validate shall return true only if the format is obeyed and the token has not
 * yet expired **]*/
...
TEST_FUNCTION(SASToken_validate_proper_format_with_skn_1_pass)
{
    ...
    const char *TEST_INVALID_SE =
        "SharedAccessSignature sr=devices.net/devices/tmp_device&sig=TESTSIG&se=0123456789&skn=";
    size_t        TEST_INVALID_SE_LENGTH = strlen(TEST_INVALID_SE);
    STRING_HANDLE handle                 = TEST_STRING_HANDLE;
    bool          result;
    umock_c_reset_all_calls();
...
/*Tests_SRS_SASTOKEN_25_029: [**SASToken_validate shall check for expiry time from token and if token has expired then
 * would return false **]*/
...
TEST_FUNCTION(SASToken_Create_null_keyName_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(size_tToString(IGNORED_PTR_ARG, sizeof(TEST_TOKEN_EXPIRATION_TIME), TEST_EXPIRY))
        .IgnoreArgument(1)
        .CopyOutArgumentBuffer(1, TEST_TOKEN_EXPIRATION_TIME, sizeof(TEST_TOKEN_EXPIRATION_TIME));
    ...
    STRICT_EXPECTED_CALL(HMACSHA256_ComputeHash(IGNORED_PTR_ARG,
                                                TEST_LENGTH_DECODEDKEY,
                                                IGNORED_PTR_ARG,
                                                TEST_LENGTH_TOBEHASHED,
                                                TEST_HASH_HANDLE))
        .IgnoreArgument(1)
        .IgnoreArgument(3);
...
/*Tests_SRS_SASTOKEN_06_026: [If the conversion to string form fails for any reason then SASToken_Create shall return
 * NULL.]*/
...
/*Tests_SRS_SASTOKEN_06_024: [The size_t value ((size_t) (difftime(get_time(NULL),0) + 3600)) is converted to a string
 * form.]*/
...
/*Tests_SRS_SASTOKEN_06_013: [If an error is returned from the HMAC256 function then NULL is returned from
 * SASToken_Create.]*/
...
TEST_FUNCTION(SASToken_Create_HMAC256_passes_signature_encode_fails)
{
    ...
    STRICT_EXPECTED_CALL(HMACSHA256_ComputeHash(IGNORED_PTR_ARG,
        ...
        .IgnoreArgument(1)
        .IgnoreArgument(3);
    STRICT_EXPECTED_CALL(Base64_Encoder(TEST_HASH_HANDLE)).SetReturn(NULL);
...
/*Tests_SRS_SASTOKEN_06_016: [The string "SharedAccessSignature sr=" is the first part of the result of
 * SASToken_Create.]*/
/*Tests_SRS_SASTOKEN_06_014: [If there are any errors from the following operations then NULL shall be returned.]*/
