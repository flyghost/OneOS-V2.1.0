...
char *umocktypes_stringify_const_ADDRINFOA_ptr(const ADDRINFOA **value)
{
    ...
    length = sprintf(
        temp_buffer,
        "{ ai_flags = %d, ai_family = %d, ai_socktype = %d, ai_protocol = %d, ai_addrlen = %u, ai_canonname = %s",
        (*value)->ai_flags,
        (*value)->ai_family,
        (*value)->ai_socktype,
        (*value)->ai_protocol,
        (unsigned int)((*value)->ai_addrlen),
        (*value)->ai_canonname);
    if (length > 0)
    {
...
{
    int result = 1;
    if (((*left)->ai_flags != (*right)->ai_flags) || ((*left)->ai_family != (*right)->ai_family) ||
        ((*left)->ai_socktype != (*right)->ai_socktype) || ((*left)->ai_protocol != (*right)->ai_protocol) ||
        ((((*left)->ai_canonname == NULL) || ((*right)->ai_canonname == NULL)) &&
         ((*left)->ai_canonname != (*right)->ai_canonname)) ||
...
TEST_FUNCTION(socketio_destroy_socket_succeeds)
{
    ...
    EXPECTED_CALL(send(IGNORED_NUM_ARG, IGNORED_PTR_ARG, IGNORED_NUM_ARG, IGNORED_NUM_ARG)).SetReturn(0);
...
TEST_FUNCTION(socketio_open_socket_fails)
{
    ...
    EXPECTED_CALL(socket(IGNORED_NUM_ARG, IGNORED_NUM_ARG, IGNORED_NUM_ARG)).SetReturn(INVALID_SOCKET);
...
TEST_FUNCTION(socketio_open_connect_fails)
{
    ...
    EXPECTED_CALL(socket(IGNORED_NUM_ARG, IGNORED_NUM_ARG, IGNORED_NUM_ARG));
    EXPECTED_CALL(getaddrinfo(IGNORED_PTR_ARG, IGNORED_PTR_ARG, &TEST_ADDR_INFO, IGNORED_PTR_ARG));
    EXPECTED_CALL(connect(IGNORED_NUM_ARG, &test_sock_addr, IGNORED_NUM_ARG)).SetReturn(WSAECONNREFUSED);
...
TEST_FUNCTION(socketio_open_ioctlsocket_fails)
{
    ...
    static ADDRINFO    addrInfo =
        {AI_PASSIVE, AF_INET, SOCK_STREAM, IPPROTO_TCP, 128, NULL, (struct sockaddr *)0x11, NULL};
    ...
    EXPECTED_CALL(ioctlsocket(IGNORED_NUM_ARG, IGNORED_NUM_ARG, IGNORED_PTR_ARG)).SetReturn(WSAENETDOWN);
...
TEST_FUNCTION(socketio_close_Succeeds)
{
    ...
    int result = socketio_open(ioHandle,
                               test_on_io_open_complete,
                               &callbackContext,
                               test_on_bytes_received,
                               &callbackContext,
...
TEST_FUNCTION(socketio_send_socket_io_fails)
{
    ...
    // act
    int result = socketio_send(NULL,
                               (const void *)TEST_BUFFER_VALUE,
                               TEST_BUFFER_SIZE,
                               OnSendComplete,
                               (void *)TEST_CALLBACK_CONTEXT);
...
static CONCRETE_IO_HANDLE setup_socket_and_expect_WSAIoctl()
{
    ...
    STRICT_EXPECTED_CALL(WSAIoctl(*(SOCKET *)ioHandle,
                                  SIO_KEEPALIVE_VALS,
                                  IGNORED_PTR_ARG,
...
TEST_FUNCTION(socketio_setoption_does_not_persist_keepalive_values_if_WSAIoctl_fails)
{
    ...
    result = socketio_setoption(
        ioHandle,
        "tcp_keepalive_time",
        &irrelevant);    // use different option for 2nd call so we don't overwrite the value from the 1st
    ASSERT_ARE_EQUAL(int, 0, result);
