...
/* Tests_SRS_STRING_07_008: [STRING_new_quoted shall return a valid STRING_HANDLE Copying the supplied const char* value
...
/* Tests_SRS_STRING_07_045: [STRING_construct_sprintf shall allocate a new string with the value of the specified printf
 * formated const char. ] */
/* Tests_SRS_STRING_07_041: [STRING_construct_sprintf shall determine the size of the resulting string and allocate the
 * necessary memory.] */
...
/* Tests_SRS_STRING_07_034: [String_Concat_with_STRING shall concatenate a given STRING_HANDLE variable with a source
 * STRING_HANDLE.] */
...
TEST_FUNCTION(STRING_Copy_Succeed)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_realloc(IGNORED_PTR_ARG, strlen(TEST_STRING_VALUE) + 1)).IgnoreArgument(1);
...
TEST_FUNCTION(STRING_Copy_n_Succeed)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_realloc(IGNORED_PTR_ARG, NUMBER_OF_CHAR_TOCOPY + 1)).IgnoreArgument(1);
...
TEST_FUNCTION(STRING_Copy_n_With_Size_0_Succeed)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_realloc(IGNORED_PTR_ARG, 1)).IgnoreArgument(1);
...
TEST_FUNCTION(STRING_quote_Succeed)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_realloc(IGNORED_PTR_ARG, 2 + strlen(TEST_STRING_VALUE) + 1)).IgnoreArgument(1);
...
/*Tests_SRS_STRING_02_007: [STRING_construct_n shall construct a STRING_HANDLE from first "n" characters of the string
...
TEST_FUNCTION(STRING_construct_n_succeeds_with_2_char)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_malloc(3)).IgnoreArgument(1);
...
TEST_FUNCTION(STRING_construct_n_succeeds_with_3_char_out_of_five)
{
    ...
    STRING_HANDLE result;
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_malloc(4)).IgnoreArgument(1);
...
/* Tests_SRS_STRING_07_034: [STRING_compare returns an integer greater than, equal to, or less than zero, accordingly as
...
/*Tests_SRS_STRING_02_013: [The string shall copy the characters of source "as they are" (until the '\0' character) with
 * the following exceptions:] */
/*Tests_SRS_STRING_02_014: [If any character has the value outside [1...127] then STRING_new_JSON shall fail and return
 * NULL.] */
...
/*Tests_SRS_STRING_02_019: [If the character code is less than 0x20 then it shall be represented as \\u00xx, where xx is
 * the hex representation of the character code.]*/
...
/*Tests_SRS_STRING_02_021: [If the complete JSON representation cannot be produced, then STRING_new_JSON shall fail and
 * return NULL.] */
...
/*Tests_SRS_STRING_02_023: [ Otherwise, STRING_from_BUFFER shall build a string that has the same content (byte-by-byte)
 * as source and return a non-NULL handle. ]*/
...
TEST_FUNCTION(STRING_from_byte_array_fails_1)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_malloc(1 + 1)).SetReturn(NULL);
