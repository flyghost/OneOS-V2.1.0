...
/* Tests_SRS_TICKCOUNTER_FREERTOS_30_004: [ If allocation of the internally-defined TICK_COUNTER_INSTANCE structure
 * fails, tickcounter_create shall return NULL. (Initialization failure is not possible for FreeRTOS.) ] */
TEST_FUNCTION(tickcounter_freertos_create_fails)
{
    /// arrange
    TICK_COUNTER_HANDLE tickHandle;
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).IgnoreArgument(1).SetReturn((void *)NULL);
...
/* Tests_SRS_TICKCOUNTER_FREERTOS_30_003: [ tickcounter_create shall allocate and initialize an internally-defined
 * TICK_COUNTER_INSTANCE structure and return its pointer on success. ] */
TEST_FUNCTION(tickcounter_freertos_create_succeed)
{
    /// arrange
    TICK_COUNTER_HANDLE tickHandle;
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(xTaskGetTickCount()).SetReturn(FAKE_TICK_NO_OVERFLOW);
...
/* Tests_SRS_TICKCOUNTER_FREERTOS_30_005: [ tickcounter_destroy shall delete the internally-defined
 * TICK_COUNTER_INSTANCE structure specified by the tick_counter parameter. (This call has no failure case.) ] */
...
/* Tests_SRS_TICKCOUNTER_FREERTOS_30_007: [ If the tick_counter parameter is NULL, tickcounter_get_current_ms shall
...
/* Tests_SRS_TICKCOUNTER_FREERTOS_30_008: [ If the current_ms parameter is NULL, tickcounter_get_current_ms shall return
 * a non-zero value to indicate error. ] */
...
/* Tests_SRS_TICKCOUNTER_FREERTOS_30_009: [ tickcounter_get_current_ms shall set *current_ms to the number of
 * milliseconds elapsed since the tickcounter_create call for the specified tick_counter and return 0 to indicate
 * success (In FreeRTOS this call has no failure case.) ] */
TEST_FUNCTION(tickcounter_freertos_get_current_ms_succeed)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(xTaskGetTickCount()).SetReturn(FAKE_TICK_NO_OVERFLOW);
    STRICT_EXPECTED_CALL(xTaskGetTickCount()).SetReturn((FAKE_TICK_NO_OVERFLOW + FAKE_TICK_INTERVAL));
...
/* Tests_SRS_TICKCOUNTER_FREERTOS_30_010: [ If the FreeRTOS call xTaskGetTickCount experiences a single overflow between
 * the calls to tickcounter_create and tickcounter_get_current_ms, the tickcounter_get_current_ms call shall still
...
TEST_FUNCTION(tickcounter_freertos_get_current_ms_succeed_despite_overflow)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(xTaskGetTickCount()).SetReturn(FAKE_TICK_BEFORE_OVERFLOW);
    STRICT_EXPECTED_CALL(xTaskGetTickCount()).SetReturn((FAKE_TICK_AFTER_OVERFLOW));
