...
static ADDRINFO              TEST_ADDR_INFO =
    {AI_PASSIVE, AF_INET, SOCK_STREAM, IPPROTO_TCP, 128, NULL, (struct sockaddr *)&test_sock_addr, NULL};
...
MOCK_FUNCTION_END(0)
MOCK_FUNCTION_WITH_CODE(WSAAPI,
                        int,
                        getaddrinfo,
                        PCSTR,
                        pNodeName,
...
/* Tests_SRS_TLSIO_CYCLONESSL_SOCKET_BSD_01_001: [ tlsio_cyclonessl_socket_create shall create a new socket to be used
 * by CycloneSSL. ]*/
/* Tests_SRS_TLSIO_CYCLONESSL_SOCKET_BSD_01_008: [ On success tlsio_cyclonessl_socket_create shall return 0 and fill in
 * the socket handle in the new_socket out argument. ]*/
/* Tests_SRS_TLSIO_CYCLONESSL_SOCKET_BSD_01_003: [ tlsio_cyclonessl_socket_create shall call socket to create a TCP
 * socket. ]*/
/* Tests_SRS_TLSIO_CYCLONESSL_SOCKET_BSD_01_004: [ tlsio_cyclonessl_socket_create shall call getaddrinfo to obtain a
 * hint ADDRINFO. ]*/
/* Tests_SRS_TLSIO_CYCLONESSL_SOCKET_BSD_01_006: [ tlsio_cyclonessl_socket_create shall call connect and pass the
 * constructed address in order to connect the socket. ]*/
TEST_FUNCTION(tlsio_cyclonessl_socket_create_succeeds)
{
    ...
    EXPECTED_CALL(connect(TEST_SOCKET, &test_sock_addr, IGNORED_NUM_ARG)).ValidateArgument_s();
...
/* Tests_SRS_TLSIO_CYCLONESSL_SOCKET_BSD_01_002: [ If hostname or new_socket is NULL, then
 * tlsio_cyclonessl_socket_create shall fail and it shall return a non-zero value. ]*/
...
/* Tests_SRS_TLSIO_CYCLONESSL_SOCKET_BSD_01_007: [ If any of the socket calls fails, then tlsio_cyclonessl_socket_create
...
TEST_FUNCTION(when_a_failure_occurs_for_tlsio_cyclonessl_socket_create_then_create_fails)
{
    ...
    STRICT_EXPECTED_CALL(socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)).SetFailReturn((SOCKET)-1);
    EXPECTED_CALL(getaddrinfo(IGNORED_PTR_ARG, IGNORED_PTR_ARG, &TEST_ADDR_INFO, IGNORED_PTR_ARG)).SetFailReturn(-1);
    EXPECTED_CALL(connect(TEST_SOCKET, &test_sock_addr, IGNORED_NUM_ARG)).ValidateArgument_s().SetFailReturn(-1);
...
/* Tests_SRS_TLSIO_CYCLONESSL_SOCKET_BSD_01_009: [ tlsio_cyclonessl_socket_destroy shall close the socket passed as
 * argument by calling the function close. ]*/
...
/* Tests_SRS_TLSIO_CYCLONESSL_SOCKET_BSD_01_010: [ If socket is INVALID_SOCKET (-1), tlsio_cyclonessl_socket_destroy
 * shall do nothing. ]*/
