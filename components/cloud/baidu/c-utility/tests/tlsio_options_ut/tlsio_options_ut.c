...
TEST_FUNCTION(tlsio_options_initialize__succeeds)
{
    ...
    /// act
    tlsio_options_initialize(&options,
    ...
    ASSERT_ARE_EQUAL(
        ...
        (int)(TLSIO_OPTION_BIT_TRUSTED_CERTS | TLSIO_OPTION_BIT_x509_RSA_CERT | TLSIO_OPTION_BIT_x509_ECC_CERT));
...
TEST_FUNCTION(tlsio_options__set_parameter_validation__fails)
{
    ...
    p0[k] = NULL;
    p1[k] = OPTION_TRUSTED_CERT;
    p2[k] = fake_x509_key;
    fm[k] = "Unexpected tlsio_options_initialize success when options is NULL"; /* */
    k++;
    p0[k] = &options;
    p1[k] = NULL; /*          */
    p2[k] = fake_x509_key;
    fm[k] = "Unexpected tlsio_options_initialize success when option_name is NULL";
    k++;
    p0[k] = &options;
    p1[k] = OPTION_TRUSTED_CERT;
    p2[k] = NULL;
    fm[k] = "Unexpected tlsio_options_initialize success when option_value is NULL";
    k++;
...
TEST_FUNCTION(tlsio_options__set_x509_bad_combos__fails)
{
    ...
    p0[k] = SU_OPTION_X509_CERT; /*  */
    p1[k] = OPTION_X509_ECC_CERT;
    k++;
    p0[k] = SU_OPTION_X509_CERT; /*  */
    p1[k] = OPTION_X509_ECC_KEY;
    k++;
    p0[k] = SU_OPTION_X509_PRIVATE_KEY;
    p1[k] = OPTION_X509_ECC_CERT;
    k++;
    p0[k] = SU_OPTION_X509_PRIVATE_KEY;
    p1[k] = OPTION_X509_ECC_KEY;
    k++;
    p0[k] = OPTION_X509_ECC_CERT; /* */
    p1[k] = SU_OPTION_X509_CERT;
    k++;
    p0[k] = OPTION_X509_ECC_CERT; /* */
    p1[k] = SU_OPTION_X509_PRIVATE_KEY;
    k++;
    p0[k] = OPTION_X509_ECC_KEY; /*  */
    p1[k] = SU_OPTION_X509_CERT;
    k++;
    p0[k] = OPTION_X509_ECC_KEY; /*  */
    p1[k] = SU_OPTION_X509_PRIVATE_KEY;
    ...
    for (i = 0; i < SET_INCONSISTENT_x509_COUNT; i++)
    {
        ...
        /// assert
        ASSERT_ARE_EQUAL_WITH_MSG(int,
                                  (int)result,
                                  (int)TLSIO_OPTIONS_RESULT_ERROR,
                                  "Unexpected success with inconsistent x509 settings");
...
TEST_FUNCTION(tlsio_options__set_not_supported__fails)
{
    ...
    for (i = 0; i < SET_NOT_SUPPORTED_COUNT; i++)
    {
        ...
        ASSERT_ARE_EQUAL_WITH_MSG(int,
                                  (int)result,
                                  (int)TLSIO_OPTIONS_RESULT_ERROR,
                                  "Unexpected success with unsupported option");
...
//                                   (int)result,
//                                   (int)TLSIO_OPTIONS_RESULT_ERROR,
//                                   "Unexpected success with malloc failure");
...
TEST_FUNCTION(tlsio_options__clone_parameter_validation__fails)
{
    ...
    p1[k] = fake_x509_key;
    p2[k] = &out_result;
    fm[k] = "Unexpected clone_option success when name is NULL"; /* */
    k++;
    p0[k] = OPTION_TRUSTED_CERT;
    p1[k] = NULL; /*    */
    p2[k] = &out_result;
    fm[k] = "Unexpected clone_option success when option value is NULL";
    k++;
    p0[k] = OPTION_TRUSTED_CERT;
    p1[k] = fake_x509_key;
    p2[k] = NULL; /*  */
    fm[k] = "Unexpected clone_option success when out_status is NULL";
    k++;
    ...
    for (i = 0; i < SET_PV_COUNT; i++)
    {
        ...
        ASSERT_ARE_EQUAL_WITH_MSG(int,
                                  (int)result,
                                  (int)TLSIO_OPTIONS_RESULT_ERROR,
                                  "Unexpected success with bad clone parameter");
...
TEST_FUNCTION(tlsio_options__retrieve_ex_parameter_validation__fails)
{
    ...
    p0[k] = NULL;
    p1[k] = pfCloneOption_impl;
    p2[k] = pfDestroyOption_impl;
    p3[k] = pfSetOption_impl;
    k++;
    p0[k] = &options;
    p1[k] = NULL; /*         */
    p2[k] = pfDestroyOption_impl;
...
//         /// act
//         result =
//             tlsio_options_retrieve_options_ex(&options, pfCloneOption_impl, pfDestroyOption_impl, pfSetOption_impl);
