...
// Tests_SRS_UUID_09_004: [ The UUID string shall be parsed into an UUID type (16 unsigned char array) and filled in
// `uuid` ] Tests_SRS_UUID_09_006: [ If no failures occur, UUID_generate shall return zero ]
TEST_FUNCTION(UUID_generate_succeed)
{
    ...
    umock_c_reset_all_calls();
    STRICT_EXPECTED_CALL(malloc(UUID_STRING_SIZE)).SetReturn(uuid_string);
...
// Tests_SRS_UUID_09_003: [ If the UUID string fails to be obtained, UUID_generate shall fail and return a non-zero
// value ] Tests_SRS_UUID_09_005: [ If `uuid` fails to be set, UUID_generate shall fail and return a non-zero value ]
...
// Tests_SRS_UUID_09_014: [ Each character in `uuid` shall be written in the respective positions of `uuid_string` as a
// 2-digit HEX value ] Tests_SRS_UUID_09_016: [ If no failures occur, UUID_to_string shall return `uuid_string` ]
TEST_FUNCTION(UUID_to_string_succeed)
{
    ...
    umock_c_reset_all_calls();
    STRICT_EXPECTED_CALL(malloc(UUID_STRING_SIZE * sizeof(char))).SetReturn(buffer);
...
// Tests_SRS_UUID_09_008: [ Each pair of digits in `uuid_string`, excluding dashes, shall be read as a single HEX value
// and saved on the respective position in `uuid` ] Tests_SRS_UUID_09_010: [ If no failures occur, UUID_from_string
// shall return zero ]
