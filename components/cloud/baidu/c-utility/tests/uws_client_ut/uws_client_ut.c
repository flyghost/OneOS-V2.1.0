...
Tests_SRS_UWS_CLIENT_01_254: [ If an endpoint receives a Ping frame and has not yet sent Pong frame(s) in response to
previous Ping frame(s), the endpoint MAY elect to send a Pong frame for only the most recently processed Ping frame. ]
...
Tests_SRS_UWS_CLIENT_01_362: [ To achieve reasonable levels of protection, clients should use only Strong TLS
algorithms. ] Tests_SRS_UWS_CLIENT_01_289: [ An endpoint SHOULD use a method that cleanly closes the TCP connection, as
well as the TLS session, if applicable, discarding any trailing bytes that may have been received. ]
Tests_SRS_UWS_CLIENT_01_078: [ Otherwise, all further communication on this channel MUST run through the encrypted
tunnel [RFC5246]. ] Tests_SRS_UWS_CLIENT_01_141: [ masking is done whether or not the WebSocket Protocol is running over
TLS. ]
*/

/* Requirements satisfied by the way the APIs are defined
Tests_SRS_UWS_CLIENT_01_324: [ 1000 indicates a normal closure, meaning that the purpose for which the connection was
established has been fulfilled. ] Tests_SRS_UWS_CLIENT_01_325: [ 1001 indicates that an endpoint is "going away", such
as a server going down or a browser having navigated away from a page. ] Tests_SRS_UWS_CLIENT_01_326: [ 1002 indicates
that an endpoint is terminating the connection due to a protocol error. ] Tests_SRS_UWS_CLIENT_01_327: [ 1003 indicates
that an endpoint is terminating the connection because it has received a type of data it cannot accept (e.g., an
endpoint that understands only text data MAY send this if it receives a binary message). ] Tests_SRS_UWS_CLIENT_01_328:
[ Reserved.  The specific meaning might be defined in the future. ] Tests_SRS_UWS_CLIENT_01_329: [ 1005 is a reserved
value and MUST NOT be set as a status code in a Close control frame by an endpoint. ] Tests_SRS_UWS_CLIENT_01_330: [
1006 is a reserved value and MUST NOT be set as a status code in a Close control frame by an endpoint. ]
Tests_SRS_UWS_CLIENT_01_331: [ 1007 indicates that an endpoint is terminating the connection because it has received
data within a message that was not consistent with the type of the message (e.g., non-UTF-8 [RFC3629] data within a text
message). ] Tests_SRS_UWS_CLIENT_01_332: [ 1008 indicates that an endpoint is terminating the connection because it has
received a message that violates its policy. ] Tests_SRS_UWS_CLIENT_01_333: [ 1009 indicates that an endpoint is
terminating the connection because it has received a message that is too big for it to process. ]
Tests_SRS_UWS_CLIENT_01_334: [ 1010 indicates that an endpoint (client) is terminating the connection because it has
expected the server to negotiate one or more extension, but the server didn't return them in the response message of the
WebSocket handshake. ] Tests_SRS_UWS_CLIENT_01_336: [ 1011 indicates that a server is terminating the connection because
it encountered an unexpected condition that prevented it from fulfilling the request. ] Tests_SRS_UWS_CLIENT_01_337: [
1015 is a reserved value and MUST NOT be set as a status code in a Close control frame by an endpoint. ]
Tests_SRS_UWS_CLIENT_01_238: [ As the data is not guaranteed to be human readable, clients MUST NOT show it to end
users. ] Tests_SRS_UWS_CLIENT_01_211: [ One implication of this is that in absence of extensions, senders and receivers
must not depend on the presence of specific frame boundaries. ]
*/
...
TEST_DEFINE_ENUM_TYPE(WS_FRAME_TYPE, WS_FRAME_TYPE_VALUES);
...
static int my_singlylinkedlist_remove(SINGLYLINKEDLIST_HANDLE list, LIST_ITEM_HANDLE item)
{
    ...
    (void)list;
    (void)memmove((void *)&list_items[index],
                  &list_items[index + 1],
                  sizeof(const void *) * (list_item_count - index - 1));
    list_item_count--;
...
my_singlylinkedlist_find(SINGLYLINKEDLIST_HANDLE handle, LIST_MATCH_FUNCTION match_function, const void *match_context)
{
    size_t      i;
...
static char *umocktypes_stringify_const_SOCKETIO_CONFIG_ptr(const SOCKETIO_CONFIG **value)
{
    ...
    int   length;
    length = sprintf(temp_buffer,
                     "{ hostname = %s, port = %d, accepted_socket = %p }",
                     (*value)->hostname,
                     (*value)->port,
...
MOCK_FUNCTION_WITH_CODE(,
                        void,
                        test_on_ws_send_frame_complete,
                        void *,
                        context,
...
static int
my_xio_send(XIO_HANDLE xio, const void *buffer, size_t size, ON_SEND_COMPLETE on_send_complete, void *callback_context)
{
    (void)xio;
    (void)buffer;
    (void)size;
...
static OPTIONHANDLER_HANDLE
my_OptionHandler_Create(pfCloneOption cloneOption, pfDestroyOption destroyOption, pfSetOption setOption)
{
...
BUFFER_HANDLE my_uws_frame_encoder_encode(WS_FRAME_TYPE        opcode,
...
/* Tests_SRS_UWS_CLIENT_01_001: [`uws_client_create` shall create an instance of uws and return a non-NULL handle to
 * it.]*/
/* Tests_SRS_UWS_CLIENT_01_017: [ `uws_client_create` shall create a pending send IO list that is to be used to queue
 * send packets by calling `singlylinkedlist_create`. ]*/
/* Tests_SRS_UWS_CLIENT_01_005: [ If `use_ssl` is false then `uws_client_create` shall obtain the interface used to
 * create a socketio instance by calling `socketio_get_interface_description`. ]*/
/* Tests_SRS_UWS_CLIENT_01_008: [ The obtained interface shall be used to create the IO used as underlying IO by the
 * newly created uws instance. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_413: [ The protocol information indicated by `protocols` and `protocol_count` shall be copied
 * for later use (for constructing the upgrade request). ]*/
/* Tests_SRS_UWS_CLIENT_01_063: [ A client will need to supply a /host/, /port/, /resource name/, and a /secure/ flag,
 * which are the components of a WebSocket URI as discussed in Section 3, along with a list of /protocols/ and
 * /extensions/ to be used. ]*/
/* Tests_SRS_UWS_CLIENT_01_076: [ If /secure/ is true, the client MUST perform a TLS handshake over the connection after
 * opening the connection and before sending the handshake data [RFC2818]. ]*/
TEST_FUNCTION(uws_client_create_with_valid_args_no_ssl_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "111")).IgnoreArgument_destination();
    ...
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "test_protocol")).IgnoreArgument_destination();
...
/* Tests_SRS_UWS_CLIENT_01_002: [ If any of the arguments `hostname` and `resource_name` is NULL then
 * `uws_client_create` shall return NULL. ]*/
TEST_FUNCTION(uws_client_create_with_NULL_hostname_fails)
{
    ...
    // act
    UWS_CLIENT_HANDLE uws_client =
        uws_client_create(NULL, 80, "222", false, protocols, sizeof(protocols) / sizeof(protocols[0]));
...
TEST_FUNCTION(uws_client_create_with_NULL_resource_name_fails)
{
    ...
    // act
    UWS_CLIENT_HANDLE uws_client =
        uws_client_create("testhost", 80, NULL, false, protocols, sizeof(protocols) / sizeof(protocols[0]));
...
TEST_FUNCTION(uws_client_create_with_valid_args_no_ssl_port_different_than_80_succeeds)
{
    ...
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "test_host")).IgnoreArgument_destination();
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "333")).IgnoreArgument_destination();
    STRICT_EXPECTED_CALL(singlylinkedlist_create());
    STRICT_EXPECTED_CALL(socketio_get_interface_description());
    STRICT_EXPECTED_CALL(xio_create(TEST_SOCKET_IO_INTERFACE_DESCRIPTION, &socketio_config))
        .IgnoreArgument_io_create_parameters();
...
/* Tests_SRS_UWS_CLIENT_01_410: [ The `protocols` argument shall be allowed to be NULL, in which case no protocol is to
 * be specified by the client in the upgrade request. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_411: [ If `protocol_count` is non zero and `protocols` is NULL then `uws_client_create` shall
...
/* Tests_SRS_UWS_CLIENT_01_412: [ If the `protocol` member of any of the items in the `protocols` argument is NULL, then
 * `uws_client_create` shall fail and return NULL. ]*/
TEST_FUNCTION(uws_client_create_with_the_first_protocol_name_NULL_fails)
{
    ...
    // act
    uws_client = uws_client_create("test_host",
                                   81,
                                   "333",
                                   false,
                                   NULL_test_protocol,
                                   sizeof(NULL_test_protocol) / sizeof(NULL_test_protocol[0]));
...
/* Tests_SRS_UWS_CLIENT_01_003: [ If allocating memory for the new uws instance fails then `uws_client_create` shall
 * return NULL. ]*/
TEST_FUNCTION(when_allocating_memory_for_the_new_uws_instance_fails_then_uws_client_create_fails)
{
    ...
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).SetReturn(NULL);
...
/* Tests_SRS_UWS_CLIENT_01_392: [ If allocating memory for the copy of the `hostname` argument fails, then
 * `uws_client_create` shall return NULL. ]*/
TEST_FUNCTION(when_allocating_memory_for_the_hostname_copy_fails_then_uws_client_create_fails)
{
    ...
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "test_host")).IgnoreArgument_destination().SetReturn(1);
...
/* Tests_SRS_UWS_CLIENT_01_405: [ If allocating memory for the copy of the `resource_name` argument fails, then
 * `uws_client_create` shall return NULL. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_018: [ If `singlylinkedlist_create` fails then `uws_client_create` shall fail and return
 * NULL. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_007: [ If obtaining the underlying IO interface fails, then `uws_client_create` shall fail
 * and return NULL. ]*/
TEST_FUNCTION(when_getting_the_socket_interface_description_fails_then_uws_client_create_fails)
{
    ...
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "test_resource/1")).IgnoreArgument_destination();
    STRICT_EXPECTED_CALL(singlylinkedlist_create());
    STRICT_EXPECTED_CALL(socketio_get_interface_description()).SetReturn(NULL);
...
/* Tests_SRS_UWS_CLIENT_01_414: [ If allocating memory for the copied protocol information fails then
 * `uws_client_create` shall fail and return NULL. ]*/
...
TEST_FUNCTION(when_allocating_memory_for_the_first_proitocol_name_fails_then_uws_client_create_fails)
{
    ...
    STRICT_EXPECTED_CALL(xio_create(TEST_SOCKET_IO_INTERFACE_DESCRIPTION, &socketio_config))
        .IgnoreArgument_io_create_parameters();
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "test_protocol")).IgnoreArgument_destination().SetReturn(1);
    EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG));
    STRICT_EXPECTED_CALL(xio_destroy(TEST_IO_HANDLE));
    STRICT_EXPECTED_CALL(singlylinkedlist_destroy(TEST_SINGLYLINKEDSINGLYLINKEDLIST_HANDLE));
    EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG));
...
TEST_FUNCTION(when_allocating_memory_for_the_second_protocol_name_fails_then_uws_client_create_fails)
{
    ...
    STRICT_EXPECTED_CALL(xio_create(TEST_SOCKET_IO_INTERFACE_DESCRIPTION, &socketio_config))
        .IgnoreArgument_io_create_parameters();
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "test_protocol1")).IgnoreArgument_destination();
    ...
    uws_client = uws_client_create("test_host",
                                   ...
                                   false,
                                   two_protocols,
                                   sizeof(two_protocols) / sizeof(two_protocols[0]));
...
/* Tests_SRS_UWS_CLIENT_01_006: [ If `use_ssl` is true then `uws_client_create` shall obtain the interface used to
 * create a tlsio instance by calling `platform_get_default_tlsio`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_360: [ Connection confidentiality and integrity is provided by running the WebSocket Protocol
 * over TLS (wss URIs). ]*/
/* Tests_SRS_UWS_CLIENT_01_361: [ WebSocket implementations MUST support TLS and SHOULD employ it when communicating
 * with their peers. ]*/
TEST_FUNCTION(uws_client_create_with_valid_args_ssl_succeeds)
{
    ...
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "test_host")).IgnoreArgument_destination();
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "test_resource/23")).IgnoreArgument_destination();
    STRICT_EXPECTED_CALL(singlylinkedlist_create());
...
/* Tests_SRS_UWS_CLIENT_01_515: [ `uws_client_create_with_io` shall create an instance of uws and return a non-NULL
 * handle to it. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_521: [ The underlying IO shall be created by calling `xio_create`, while passing as arguments
 * the `io_interface` and `io_create_parameters` argument values. ]*/
/* Tests_SRS_UWS_CLIENT_01_523: [ The argument `resource_name` shall be copied for later use. ]*/
/* Tests_SRS_UWS_CLIENT_01_530: [ `uws_client_create_with_io` shall create a pending send IO list that is to be used to
 * queue send packets by calling `singlylinkedlist_create`. ]*/
/* Tests_SRS_UWS_CLIENT_01_527: [ The protocol information indicated by `protocols` and `protocol_count` shall be copied
 * for later use (for constructing the upgrade request). ]*/
TEST_FUNCTION(uws_client_create_with_io_valid_args_succeeds)
{
    ...
    // act
    uws_client = uws_client_create_with_io(TEST_SOCKET_IO_INTERFACE_DESCRIPTION,
                                           &socketio_config,
                                           "test_host",
                                           80,
                                           "111",
...
/* Tests_SRS_UWS_CLIENT_01_516: [ If any of the arguments `io_interface`, `hostname` and `resource_name` is NULL then
 * `uws_client_create_with_io` shall return NULL. ]*/
TEST_FUNCTION(uws_client_create_with_io_with_NULL_io_interface_description_fails)
{
    ...
    // act
    uws_client = uws_client_create_with_io(NULL,
                                           &socketio_config,
                                           "test_host",
                                           80,
                                           "111",
...
/* Tests_SRS_UWS_CLIENT_01_517: [ If allocating memory for the new uws instance fails then `uws_client_create_with_io`
 * shall return NULL. ]*/
/* Tests_SRS_UWS_CLIENT_01_519: [ If allocating memory for the copy of the `hostname` argument fails, then
 * `uws_client_create` shall return NULL. ]*/
/* Tests_SRS_UWS_CLIENT_01_522: [ If `xio_create` fails, then `uws_client_create_with_io` shall fail and return NULL.
 * ]*/
/* Tests_SRS_UWS_CLIENT_01_529: [ If allocating memory for the copy of the `resource_name` argument fails, then
 * `uws_client_create_with_io` shall return NULL. ]*/
/* Tests_SRS_UWS_CLIENT_01_531: [ If `singlylinkedlist_create` fails then `uws_client_create_with_io` shall fail and
 * return NULL. ]*/
/* Tests_SRS_UWS_CLIENT_01_528: [ If allocating memory for the copied protocol information fails then
 * `uws_client_create_with_io` shall fail and return NULL. ]*/
TEST_FUNCTION(when_any_call_fails_uws_client_create_with_io_fails)
{
    ...
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).SetFailReturn(NULL);
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "test_host")).IgnoreArgument_destination().SetFailReturn(1);
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, "111")).IgnoreArgument_destination().SetFailReturn(1);
    STRICT_EXPECTED_CALL(singlylinkedlist_create()).SetFailReturn(NULL);
    STRICT_EXPECTED_CALL(xio_create(TEST_SOCKET_IO_INTERFACE_DESCRIPTION, &socketio_config))
        .IgnoreArgument_io_create_parameters()
        .SetFailReturn(NULL);
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).SetFailReturn(NULL);
...
/* Tests_SRS_UWS_CLIENT_01_524: [ The `protocols` argument shall be allowed to be NULL, in which case no protocol is to
 * be specified by the client in the upgrade request. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_525: [ If `protocol_count` is non zero and `protocols` is NULL then
 * `uws_client_create_with_io` shall fail and return NULL. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_526: [ If the `protocol` member of any of the items in the `protocols` argument is NULL, then
 * `uws_client_create_with_io` shall fail and return NULL. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_023: [ `uws_client_destroy` shall ensure the underlying IO created in `uws_client_open_async`
 * is destroyed by calling `xio_destroy`. ]*/
/* Tests_SRS_UWS_CLIENT_01_024: [ `uws_client_destroy` shall free the list used to track the pending sends by calling
 * `singlylinkedlist_destroy`. ]*/
/* Tests_SRS_UWS_CLIENT_01_424: [ `uws_client_destroy` shall free the buffer allocated in `uws_client_create` by calling
 * `BUFFER_delete`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_021: [ `uws_client_destroy` shall perform a close action if the uws instance has already been
 * open. ]*/
/* Tests_SRS_UWS_CLIENT_01_034: [ `uws_client_close_async` shall obtain all the pending send frames by repetitively
 * querying for the head of the pending IO list and freeing that head item. ]*/
/* Tests_SRS_UWS_CLIENT_01_035: [ Obtaining the head of the pending send frames list shall be done by calling
...
TEST_FUNCTION(uws_client_destroy_also_performs_a_close)
{
    ...
    uws_client = uws_client_create("test_host", 444, "/aaa", true, protocols, sizeof(protocols) / sizeof(protocols[0]));
    (void)uws_client_open_async(uws_client,
                                test_on_ws_open_complete,
                                (void *)0x4242,
                                test_on_ws_frame_received,
                                (void *)0x4243,
                                test_on_ws_peer_closed,
    ...
    g_on_bytes_received(g_on_bytes_received_context,
                        (const unsigned char *)test_upgrade_response,
                        sizeof(test_upgrade_response));
    umock_c_reset_all_calls();
...
/* Tests_SRS_UWS_CLIENT_01_025: [ `uws_client_open_async` shall open the underlying IO by calling `xio_open` and
 * providing the IO handle created in `uws_client_create` as argument. ]*/
/* Tests_SRS_UWS_CLIENT_01_367: [ The callbacks `on_underlying_io_open_complete`, `on_underlying_io_bytes_received` and
 * `on_underlying_io_error` shall be passed as arguments to `xio_open`. ]*/
/* Tests_SRS_UWS_CLIENT_01_026: [ On success, `uws_client_open_async` shall return 0. ]*/
/* Tests_SRS_UWS_CLIENT_01_061: [ To _Establish a WebSocket Connection_, a client opens a connection and sends a
 * handshake as defined in this section. ]*/
TEST_FUNCTION(uws_client_open_async_opens_the_underlying_IO)
{
    ...
    STRICT_EXPECTED_CALL(xio_open(TEST_IO_HANDLE,
                                  IGNORED_PTR_ARG,
                                  IGNORED_PTR_ARG,
                                  IGNORED_PTR_ARG,
                                  IGNORED_PTR_ARG,
    ...
    // act
    result = uws_client_open_async(uws_client,
                                   test_on_ws_open_complete,
                                   (void *)0x4242,
                                   test_on_ws_frame_received,
                                   (void *)0x4243,
...
/* Tests_SRS_UWS_CLIENT_01_027: [ If `uws_client`, `on_ws_open_complete`, `on_ws_frame_received`, `on_ws_peer_closed` or
 * `on_ws_error` is NULL, `uws_client_open_async` shall fail and return a non-zero value. ]*/
TEST_FUNCTION(uws_client_open_async_with_NULL_handle_fails)
{
    ...
    // act
    result = uws_client_open_async(NULL,
                                   test_on_ws_open_complete,
                                   (void *)0x4242,
                                   test_on_ws_frame_received,
                                   (void *)0x4243,
...
/* Tests_SRS_UWS_CLIENT_01_028: [ If opening the underlying IO fails then `uws_client_open_async` shall fail and return
 * a non-zero value. ]*/
/* Tests_SRS_UWS_CLIENT_01_075: [ If the connection could not be opened, either because a direct connection failed or
 * because any proxy used returned an error, then the client MUST _Fail the WebSocket Connection_ and abort the
 * connection attempt. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_394: [ `uws_client_open_async` while the uws instance is already OPEN or OPENING shall fail
...
TEST_FUNCTION(uws_client_open_async_while_waiting_for_CLOSE_frame_fails)
{
    ...
    g_on_bytes_received(g_on_bytes_received_context,
                        (const unsigned char *)test_upgrade_response,
                        sizeof(test_upgrade_response) - 1);
...
/* Tests_SRS_UWS_CLIENT_01_029: [ `uws_client_close_async` shall close the uws instance connection if an open action is
 * either pending or has completed successfully (if the IO is open). ]*/
/* Tests_SRS_UWS_CLIENT_01_031: [ `uws_client_close_async` shall close the connection by calling `xio_close` while
 * passing as argument the IO handle created in `uws_client_create`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_399: [ `on_ws_close_complete` and `on_ws_close_complete_context` shall be saved and the
 * callback `on_ws_close_complete` shall be triggered when the close is complete. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_397: [ The `on_ws_close_complete` argument shall be allowed to be NULL, in which case no
 * callback shall be called when the close is complete. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_032: [ `uws_client_close_async` when no open action has been issued shall fail and return a
 * non-zero value. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_033: [ `uws_client_close_async` after a `uws_client_close_async` shall fail and return a
 * non-zero value. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_035: [ Obtaining the head of the pending send frames list shall be done by calling
 * `singlylinkedlist_get_head_item`. ]*/
/* Tests_SRS_UWS_CLIENT_01_036: [ For each pending send frame the send complete callback shall be called with
 * `UWS_SEND_FRAME_CANCELLED`. ]*/
/* Tests_SRS_UWS_CLIENT_01_037: [ When indicating pending send frames as cancelled the callback context passed to the
 * `on_ws_send_frame_complete` callback shall be the context given to `uws_client_send_frame_async`. ]*/
TEST_FUNCTION(uws_client_close_async_with_1_pending_send_frames_indicates_the_frames_as_cancelled)
{
    ...
    g_on_bytes_received(g_on_bytes_received_context,
                        (const unsigned char *)test_upgrade_response,
                        sizeof(test_upgrade_response) - 1);
    (void)uws_client_send_frame_async(uws_client,
...
/* Tests_SRS_UWS_CLIENT_01_465: [ `uws_client_close_handshake_async` shall initiate the close handshake by sending a
 * close frame to the peer. ]*/
/* Tests_SRS_UWS_CLIENT_01_466: [ On success `uws_client_close_handshake_async` shall return 0. ]*/
/* Tests_SRS_UWS_CLIENT_01_468: [ `on_ws_close_complete` and `on_ws_close_complete_context` shall be saved and the
 * callback `on_ws_close_complete` shall be triggered when the close is complete. ]*/
...
TEST_FUNCTION(uws_client_close_handshake_async_sends_the_close_frame)
{
    ...
    STRICT_EXPECTED_CALL(
        uws_frame_encoder_encode(WS_CLOSE_FRAME, IGNORED_PTR_ARG, sizeof(close_frame_payload), true, true, 0))
    ...
    STRICT_EXPECTED_CALL(BUFFER_delete(IGNORED_PTR_ARG)).ValidateArgumentValue_handle(&buffer_handle);
    STRICT_EXPECTED_CALL(singlylinkedlist_get_head_item(TEST_SINGLYLINKEDSINGLYLINKEDLIST_HANDLE));
...
/* Tests_SRS_UWS_CLIENT_01_467: [ if `uws_client` is NULL, `uws_client_close_handshake_async` shall return a non-zero
 * value. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_469: [ The `on_ws_close_complete` argument shall be allowed to be NULL, in which case no
 * callback shall be called when the close is complete. ]*/
/* Tests_SRS_UWS_CLIENT_01_471: [ The callback `on_underlying_io_close_sent` shall be passed as argument to `xio_send`.
 * ]*/
...
/* Tests_SRS_UWS_CLIENT_01_472: [ If `xio_send` fails, `uws_client_close_handshake_async` shall fail and return a
 * non-zero value. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_473: [ `uws_client_close_handshake_async` when no open action has been issued shall fail and
...
/* Tests_SRS_UWS_CLIENT_01_474: [ `uws_client_close_handshake_async` when already CLOSING shall fail and return a
 * non-zero value. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_369: [ When `on_underlying_io_open_complete` is called with `IO_OPEN_ERROR` while uws is
 * OPENING (`uws_client_open_async` was called), uws shall report that the open failed by calling the
 * `on_ws_open_complete` callback passed to `uws_client_open_async` with `WS_OPEN_ERROR_UNDERLYING_IO_OPEN_FAILED`. ]*/
TEST_FUNCTION(
...
/* Tests_SRS_UWS_CLIENT_01_409: [ After any error is indicated by `on_ws_open_complete`, a subsequent
 * `uws_client_open_async` shall be possible. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_401: [ If `on_underlying_io_open_complete` is called with a NULL context,
 * `on_underlying_io_open_complete` shall do nothing. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_402: [ When `on_underlying_io_open_complete` is called with `IO_OPEN_CANCELLED` while uws is
 * OPENING (`uws_client_open_async` was called), uws shall report that the open failed by calling the
 * `on_ws_open_complete` callback passed to `uws_client_open_async` with `WS_OPEN_ERROR_UNDERLYING_IO_OPEN_CANCELLED`.
 * ]*/
TEST_FUNCTION(
...
/* Tests_SRS_UWS_CLIENT_01_371: [ When `on_underlying_io_open_complete` is called with `IO_OPEN_OK` while uws is OPENING
 * (`uws_client_open_async` was called), uws shall prepare the WebSockets upgrade request. ]*/
/* Tests_SRS_UWS_CLIENT_01_372: [ Once prepared the WebSocket upgrade request shall be sent by calling `xio_send`. ]*/
/* Tests_SRS_UWS_CLIENT_01_080: [ Once a connection to the server has been established (including a connection via a
 * proxy or over a TLS-encrypted tunnel), the client MUST send an opening handshake to the server. ]*/
/* Tests_SRS_UWS_CLIENT_01_081: [ The handshake consists of an HTTP Upgrade request, along with a list of required and
...
/* Tests_SRS_UWS_CLIENT_01_084: [ The "Request-URI" part of the request MUST match the /resource name/ defined in
 * Section 3 (a relative URI) or be an absolute http/https URI that, when parsed, has a /resource name/, /host/, and
 * /port/ that match the corresponding ws/wss URI. ]*/
/* Tests_SRS_UWS_CLIENT_01_085: [ The request MUST contain a |Host| header field whose value contains /host/ plus
 * optionally ":" followed by /port/ (when not using the default port). ]*/
/* Tests_SRS_UWS_CLIENT_01_086: [ The request MUST contain an |Upgrade| header field whose value MUST include the
 * "websocket" keyword. ]*/
/* Tests_SRS_UWS_CLIENT_01_087: [ The request MUST contain a |Connection| header field whose value MUST include the
 * "Upgrade" token. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_089: [ The value of this header field MUST be a nonce consisting of a randomly selected
...
/* Tests_SRS_UWS_CLIENT_01_497: [ The nonce needed for the upgrade request shall be Base64 encoded with
 * `Base64_Encode_Bytes`. ]*/
TEST_FUNCTION(on_underlying_io_open_complete_with_OK_prepares_and_sends_the_WebSocket_upgrade_request)
{
    ...
    STRICT_EXPECTED_CALL(Base64_Encode_Bytes(IGNORED_PTR_ARG, 16)).ValidateArgumentBuffer(1, expected_nonce, 16);
...
/* Tests_SRS_UWS_CLIENT_01_498: [ If Base64 encoding the nonce for the upgrade request fails, then the uws client shall
 * report that the open failed by calling the `on_ws_open_complete` callback passed to `uws_client_open_async` with
 * `WS_OPEN_ERROR_BASE64_ENCODE_FAILED`. ]*/
TEST_FUNCTION(
    when_base64_encode_fails_on_underlying_io_open_complete_triggers_the_error_WS_OPEN_ERROR_BASE64_ENCODE_FAILED)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
...
/* Tests_SRS_UWS_CLIENT_01_406: [ If not enough memory can be allocated to construct the WebSocket upgrade request, uws
 * shall report that the open failed by calling the `on_ws_open_complete` callback passed to `uws_client_open_async`
 * with `WS_OPEN_ERROR_NOT_ENOUGH_MEMORY`. ]*/
TEST_FUNCTION(
    when_allocating_memory_for_the_websocket_upgrade_request_fails_the_error_WS_OPEN_ERROR_NOT_ENOUGH_MEMORY_is_indicated_via_the_open_complete_callback)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
...
/* Tests_SRS_UWS_CLIENT_01_373: [ If `xio_send` fails then uws shall report that the open failed by calling the
 * `on_ws_open_complete` callback passed to `uws_client_open_async` with `WS_OPEN_ERROR_CANNOT_SEND_UPGRADE_REQUEST`.
 * ]*/
...
/* Tests_SRS_UWS_CLIENT_01_407: [ When `on_underlying_io_open_complete` is called when the uws instance has send the
 * upgrade request but it is waiting for the response, an error shall be reported to the user by calling the
 * `on_ws_open_complete` with `WS_OPEN_ERROR_MULTIPLE_UNDERLYING_IO_OPEN_EVENTS`. ]*/
TEST_FUNCTION(
    when_sending_the_upgrade_request_fails_the_error_WS_OPEN_ERROR_MULTIPLE_UNDERLYING_IO_OPEN_EVENTS_is_indicated)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
...
/* Tests_SRS_UWS_CLIENT_01_378: [ When `on_underlying_io_bytes_received` is called while the uws is OPENING, the
 * received bytes shall be accumulated in order to attempt parsing the WebSocket Upgrade response. ]*/
/* Tests_SRS_UWS_CLIENT_01_380: [ If an WebSocket Upgrade request can be parsed from the accumulated bytes, the status
 * shall be read from the WebSocket upgrade response. ]*/
/* Tests_SRS_UWS_CLIENT_01_381: [ If the status is 101, uws shall be considered OPEN and this shall be indicated by
 * calling the `on_ws_open_complete` callback passed to `uws_client_open_async` with `IO_OPEN_OK`. ]*/
/* Tests_SRS_UWS_CLIENT_01_065: [ When the client is to _Establish a WebSocket Connection_ given a set of (/host/,
 * /port/, /resource name/, and /secure/ flag), along with a list of /protocols/ and /extensions/ to be used, and an
 * /origin/ in the case of web browsers, it MUST open a connection, send an opening handshake, and read the server's
 * handshake in response. ]*/
/* Tests_SRS_UWS_CLIENT_01_102: [ Once the client's opening handshake has been sent, the client MUST wait for a response
 * from the server before sending any further data. ]*/
/* Tests_SRS_UWS_CLIENT_01_115: [ If the server's response is validated as provided for above, it is said that _The
 * WebSocket Connection is Established_ and that the WebSocket Connection is in the OPEN state. ]*/
/* Tests_SRS_UWS_CLIENT_01_478: [ A Status-Line with a 101 response code as per RFC 2616 [RFC2616]. ]*/
TEST_FUNCTION(
    on_underlying_io_bytes_received_with_a_full_reply_after_the_upgrade_request_was_sent_indicates_open_complete)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
...
/* Tests_SRS_UWS_CLIENT_01_382: [ If a negative status is decoded from the WebSocket upgrade request, an error shall be
 * indicated by calling the `on_ws_open_complete` callback passed to `uws_client_open_async` with
 * `WS_OPEN_ERROR_BAD_RESPONSE_STATUS`. ]*/
/* Tests_SRS_UWS_CLIENT_01_478: [ A Status-Line with a 101 response code as per RFC 2616 [RFC2616]. ]*/
TEST_FUNCTION(
    on_underlying_io_bytes_received_with_a_reply_with_a_status_code_different_than_101_indicates_an_open_complete_with_error)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
...
TEST_FUNCTION(after_a_bad_status_code_a_subsequent_open_completes)
{
    ...
    g_on_io_open_complete(g_on_io_open_complete_context, IO_OPEN_OK);
    g_on_bytes_received(g_on_bytes_received_context,
                        (const unsigned char *)test_bad_upgrade_response,
                        sizeof(test_bad_upgrade_response) - 1);
...
/* Tests_SRS_UWS_CLIENT_01_383: [ If the WebSocket upgrade request cannot be decoded an error shall be indicated by
 * calling the `on_ws_open_complete` callback passed to `uws_client_open_async` with
...
/* Tests_SRS_UWS_CLIENT_01_478: [ A Status-Line with a 101 response code as per RFC 2616 [RFC2616]. ]*/
TEST_FUNCTION(
    on_underlying_io_bytes_received_with_a_complete_HTTP_version_but_no_status_code_indicates_an_open_complete_with_error)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
...
/* Tests_SRS_UWS_CLIENT_01_379: [ If allocating memory for accumulating the bytes fails, uws shall report that the open
 * failed by calling the `on_ws_open_complete` callback passed to `uws_client_open_async` with
 * `WS_OPEN_ERROR_NOT_ENOUGH_MEMORY`. ]*/
TEST_FUNCTION(
    when_allocating_memory_for_the_received_bytes_fails_on_underlying_io_bytes_received_indicates_open_complete_with_error)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
    ...
    EXPECTED_CALL(gballoc_realloc(IGNORED_PTR_ARG, IGNORED_NUM_ARG)).SetReturn(NULL);
    STRICT_EXPECTED_CALL(xio_close(TEST_IO_HANDLE, NULL, NULL));
    STRICT_EXPECTED_CALL(test_on_ws_open_complete((void *)0x4242, WS_OPEN_ERROR_NOT_ENOUGH_MEMORY));
...
/* Tests_SRS_UWS_CLIENT_01_415: [ If called with a NULL `context` argument, `on_underlying_io_bytes_received` shall do
 * nothing. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_416: [ If called with NULL `buffer` or zero `size` and the state of the iws is OPENING, uws
 * shall report that the open failed by calling the `on_ws_open_complete` callback passed to `uws_client_open_async`
 * with `WS_OPEN_ERROR_INVALID_BYTES_RECEIVED_ARGUMENTS`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_417: [ When `on_underlying_io_bytes_received` is called while OPENING but before the
 * `on_underlying_io_open_complete` has been called, uws shall report that the open failed by calling the
 * `on_ws_open_complete` callback passed to `uws_client_open_async` with
...
void when_only_n_bytes_are_received_from_the_response_no_open_complete_is_indicated(const char *test_upgrade_response,
                                                                                    size_t      n)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
...
/* Tests_SRS_UWS_CLIENT_01_384: [ Any extra bytes that are left unconsumed after decoding a succesfull WebSocket upgrade
 * response shall be used for decoding WebSocket frames by passing them to `uws_frame_decoder_decode`. ]*/
TEST_FUNCTION(
    when_1_extra_byte_is_received_the_open_complete_is_properly_indicated_and_the_extra_byte_is_saved_for_decoding_frames)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
...
/* Tests_SRS_UWS_CLIENT_01_386: [ When a WebSocket data frame is decoded succesfully it shall be indicated via the
 * callback `on_ws_frame_received`. ]*/
/* Tests_SRS_UWS_CLIENT_01_385: [ If the state of the uws instance is OPEN, the received bytes shall be used for
...
/* Tests_SRS_UWS_CLIENT_01_169: [ The payload length is the length of the "Extension data" + the length of the
 * "Application data". ]*/
/* Tests_SRS_UWS_CLIENT_01_173: [ The "Payload data" is defined as "Extension data" concatenated with "Application
 * data". ]*/
...
/* Tests_SRS_UWS_CLIENT_01_280: [ Upon receiving a data frame (Section 5.6), the endpoint MUST note the /type/ of the
 * data as defined by the opcode (frame-opcode) from Section 5.2. ]*/
/* Tests_SRS_UWS_CLIENT_01_281: [ The "Application data" from this frame is defined as the /data/ of the message. ]*/
/* Tests_SRS_UWS_CLIENT_01_282: [ If the frame comprises an unfragmented message (Section 5.4), it is said that _A
 * WebSocket Message Has Been Received_ with type /type/ and data /data/. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_163: [ The length of the "Payload data", in bytes: ]*/
/* Tests_SRS_UWS_CLIENT_01_164: [ if 0-125, that is the payload length. ]*/
/* Tests_SRS_UWS_CLIENT_01_264: [ The "Payload data" is arbitrary binary data whose interpretation is solely up to the
 * application layer. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_165: [ If 126, the following 2 bytes interpreted as a 16-bit unsigned integer are the payload
 * length. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_166: [ If 127, the following 8 bytes interpreted as a 64-bit unsigned integer (the most
 ...
 * for example, the length of a 124-byte-long string can't be encoded as the sequence 126, 0, 124. ]*/
/* Tests_SRS_UWS_CLIENT_01_419: [ If there is an error decoding the WebSocket frame, an error shall be indicated by
 * calling the `on_ws_error` callback with `WS_ERROR_BAD_FRAME_RECEIVED`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_418: [ If allocating memory for the bytes accumulated for decoding WebSocket frames fails, an
 * error shall be indicated by calling the `on_ws_error` callback with `WS_ERROR_NOT_ENOUGH_MEMORY`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_384: [ Any extra bytes that are left unconsumed after decoding a succesfull WebSocket upgrade
 * response shall be used for decoding WebSocket frames ]*/
TEST_FUNCTION(
    when_1_byte_is_received_together_with_the_upgrade_request_and_one_byte_with_a_separate_call_decoding_frame_succeeds)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
    ...
    g_on_io_open_complete(g_on_io_open_complete_context, IO_OPEN_OK);
    g_on_bytes_received(g_on_bytes_received_context,
                        (const unsigned char *)upgrade_response_frame,
                        sizeof(test_upgrade_response));
    umock_c_reset_all_calls();
 ...
 * response shall be used for decoding WebSocket frames ]*/
TEST_FUNCTION(
    when_a_1_byte_complete_frame_is_received_together_with_the_upgrade_request_the_frame_is_indicated_as_received)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
    ...
    EXPECTED_CALL(gballoc_realloc(IGNORED_PTR_ARG, IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(test_on_ws_open_complete((void *)0x4242, WS_OPEN_OK));
    STRICT_EXPECTED_CALL(test_on_ws_frame_received((void *)0x4243,
                                                   WS_FRAME_TYPE_BINARY,
 ...
 * response shall be used for decoding WebSocket frames ]*/
TEST_FUNCTION(
    when_2_complete_frames_are_received_together_with_the_upgrade_request_the_frames_are_indicated_as_received)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
...
/* Tests_SRS_UWS_CLIENT_01_145: [ In this case, it MAY use the status code 1002 (protocol error) as defined in
 * Section 7.4.1. (These rules might be relaxed in a future specification.) ]*/
/* Tests_SRS_UWS_CLIENT_01_160: [ Defines whether the "Payload data" is masked. ]*/
/* Tests_SRS_UWS_CLIENT_01_140: [ To avoid confusing network intermediaries (such as intercepting proxies) and for
 * security reasons that are further discussed in Section 10.3, a client MUST mask all frames that it sends to the
 * server (see Section 5.3 for further details). ]*/
...
TEST_FUNCTION(when_a_masked_frame_is_received_and_encoding_the_close_frame_fails_an_error_is_indicated_anyhow)
{
    ...
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(BUFFER_new()).CaptureReturn(&buffer_handle);
    uws_client = uws_client_create("test_host", 444, "/aaa", true, protocols, sizeof(protocols) / sizeof(protocols[0]));
    (void)uws_client_open_async(uws_client,
                                test_on_ws_open_complete,
                                (void *)0x4242,
...
/* Tests_SRS_UWS_CLIENT_01_460: [ When a CLOSE frame is received the callback `on_ws_peer_closed` passed to
 * `uws_client_open_async` shall be called, while passing to it the argument `on_ws_peer_closed_context`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_235: [ The Close frame MAY contain a body (the "Application data" portion of the frame) that
 * indicates a reason for closing, such as an endpoint shutting down, an endpoint having received a frame too large, or
 * an endpoint having received a frame that does not conform to the format expected by the endpoint. ]*/
/* Tests_SRS_UWS_CLIENT_01_236: [ If there is a body, the first two bytes of the body MUST be a 2-byte unsigned integer
 * (in network byte order) representing a status code with value /code/ defined in Section 7.4. ]*/
/* Tests_SRS_UWS_CLIENT_01_461: [ The argument `close_code` shall be set to point to the code extracted from the CLOSE
 * frame. ]*/
/* Tests_SRS_UWS_CLIENT_01_296: [ Upon either sending or receiving a Close control frame, it is said that _The WebSocket
 * Closing Handshake is Started_ and that the WebSocket connection is in the CLOSING state. ]*/
/* Tests_SRS_UWS_CLIENT_01_241: [ If an endpoint receives a Close frame and did not previously send a Close frame, the
 * endpoint MUST send a Close frame in response. ]*/
...
TEST_FUNCTION(when_a_CLOSE_frame_is_received_while_in_open_the_code_is_reported_to_the_user)
{
    ...
    STRICT_EXPECTED_CALL(
        xio_send(TEST_IO_HANDLE, sent_close_frame, sizeof(sent_close_frame), IGNORED_PTR_ARG, IGNORED_PTR_ARG))
...
/* Tests_SRS_UWS_CLIENT_01_463: [ The extra bytes (besides the close code) shall be passed to the `on_ws_peer_closed`
 * callback by using `extra_data` and `extra_data_length`. ]*/
/* Tests_SRS_UWS_CLIENT_01_241: [ If an endpoint receives a Close frame and did not previously send a Close frame, the
 * endpoint MUST send a Close frame in response. ]*/
/* Tests_SRS_UWS_CLIENT_01_140: [ To avoid confusing network intermediaries (such as intercepting proxies) and for
 * security reasons that are further discussed in Section 10.3, a client MUST mask all frames that it sends to the
...
TEST_FUNCTION(when_a_CLOSE_frame_is_received_with_extra_bytes_the_bytes_are_passed_to_the_callback)
{
    ...
    EXPECTED_CALL(gballoc_realloc(IGNORED_PTR_ARG, IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(utf8_checker_is_valid_utf8(IGNORED_PTR_ARG, 2)).ValidateArgumentBuffer(1, &close_frame[4], 2);
    STRICT_EXPECTED_CALL(uws_frame_encoder_encode(WS_CLOSE_FRAME, NULL, 0, true, true, 0))
        .CaptureReturn(&buffer_handle);
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_PTR_ARG))
        .ValidateArgumentValue_handle(&buffer_handle)
    ...
    STRICT_EXPECTED_CALL(BUFFER_delete(IGNORED_PTR_ARG)).ValidateArgumentValue_handle(&buffer_handle);
    STRICT_EXPECTED_CALL(
        test_on_ws_peer_closed((void *)0x4301, IGNORED_PTR_ARG, IGNORED_PTR_ARG, sizeof(expected_extra_data)))
        .ValidateArgumentBuffer(2, &expected_close_code, sizeof(expected_close_code))
...
/* Tests_SRS_UWS_CLIENT_01_237: [ Following the 2-byte integer, the body MAY contain UTF-8-encoded data with value
 * /reason/, the interpretation of which is not defined by this specification. ]*/
 ...
 * ]*/
/* Tests_SRS_UWS_CLIENT_01_290: [ An endpoint MAY close the connection via any means available when necessary, such as
 * when under attack. ]*/
/* Tests_SRS_UWS_CLIENT_01_140: [ To avoid confusing network intermediaries (such as intercepting proxies) and for
 * security reasons that are further discussed in Section 10.3, a client MUST mask all frames that it sends to the
 * server (see Section 5.3 for further details). ]*/
TEST_FUNCTION(
    when_a_CLOSE_frame_is_received_while_in_open_and_encoding_the_outgoing_CLOSE_fails_the_connection_is_closed)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
    ...
    EXPECTED_CALL(gballoc_realloc(IGNORED_PTR_ARG, IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(uws_frame_encoder_encode(WS_CLOSE_FRAME, NULL, 0, true, true, 0)).SetReturn(NULL);
    STRICT_EXPECTED_CALL(xio_close(TEST_IO_HANDLE, IGNORED_PTR_ARG, IGNORED_PTR_ARG))
        .IgnoreArgument_callback_context()
        .IgnoreArgument_on_io_close_complete();
    STRICT_EXPECTED_CALL(test_on_ws_peer_closed((void *)0x4301, IGNORED_PTR_ARG, NULL, 0))
 ...
 * server (see Section 5.3 for further details). ]*/
TEST_FUNCTION(
    when_a_CLOSE_frame_is_received_while_in_open_and_sending_the_outgoing_CLOSE_fails_the_connection_is_closed)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
...
TEST_FUNCTION(sending_after_a_close_is_received_does_not_send_anything)
{
    ...
    // act
    result = uws_client_send_frame_async(uws_client,
                                         WS_FRAME_TYPE_BINARY,
                                         test_payload,
                                         sizeof(test_payload),
                                         true,
...
/* Tests_SRS_UWS_CLIENT_01_044: [ If any the arguments `uws_client` is NULL, `uws_client_send_frame_async` shall fail
 * and return a non-zero value. ]*/
TEST_FUNCTION(uws_client_send_frame_async_with_NULL_handle_fails)
{
    ...
    // act
    result = uws_client_send_frame_async(NULL,
                                         WS_FRAME_TYPE_BINARY,
                                         test_payload,
                                         sizeof(test_payload),
                                         true,
...
/* Tests_SRS_UWS_CLIENT_01_045: [ If `size` is non-zero and `buffer` is NULL then `uws_client_send_frame_async` shall
...
/* Tests_SRS_UWS_CLIENT_01_043: [ If the uws instance is not OPEN (open has not been called or is still in progress)
 * then `uws_client_send_frame_async` shall fail and return a non-zero value. ]*/
/* Tests_SRS_UWS_CLIENT_01_146: [ A data frame MAY be transmitted by either the client or the server at any time after
 * opening handshake completion and before that endpoint has sent a Close frame (Section 5.5.1). ]*/
...
/* Tests_SRS_UWS_CLIENT_01_039: [ - the encoded websocket frame, so that the frame can be later sent when
 * `uws_client_dowork` is called ]*/
...
/* Tests_SRS_UWS_CLIENT_01_425: [ Encoding shall be done by calling `uws_frame_encoder_encode` and passing to it the
 * `buffer` and `size` argument for payload, the `is_final` flag and setting `is_masked` to true. ]*/
/* Tests_SRS_UWS_CLIENT_01_428: [ The encoded frame buffer memory shall be obtained by calling `BUFFER_u_char` on the
 * encode buffer. ]*/
/* Tests_SRS_UWS_CLIENT_01_429: [ The encoded frame size shall be obtained by calling `BUFFER_length` on the encode
 * buffer. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_146: [ A data frame MAY be transmitted by either the client or the server at any time after
 * opening handshake completion and before that endpoint has sent a Close frame (Section 5.5.1). ]*/
/* Tests_SRS_UWS_CLIENT_01_270: [ An endpoint MUST encapsulate the /data/ in a WebSocket frame as defined in
 * Section 5.2. ]*/
/* Tests_SRS_UWS_CLIENT_01_274: [ If the data is being sent by the client, the frame(s) MUST be masked as defined in
 * Section 5.3. ]*/
/* Tests_SRS_UWS_CLIENT_01_276: [ The frame(s) that have been formed MUST be transmitted over the underlying network
 * connection. ]*/
TEST_FUNCTION(uws_client_send_frame_async_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(singlylinkedlist_add(TEST_SINGLYLINKEDSINGLYLINKEDLIST_HANDLE, IGNORED_PTR_ARG))
        .IgnoreArgument_item();
    STRICT_EXPECTED_CALL(
        xio_send(TEST_IO_HANDLE, IGNORED_PTR_ARG, sizeof(encoded_frame), IGNORED_PTR_ARG, IGNORED_PTR_ARG))
        .IgnoreArgument_on_send_complete()
        .IgnoreArgument_callback_context()
...
/* Tests_SRS_UWS_CLIENT_01_272: [ The opcode (frame-opcode) of the first frame containing the data MUST be set to the
 * appropriate value from Section 5.2 for data that is to be interpreted by the recipient as text or binary data. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_047: [ If allocating memory for the newly queued item fails, `uws_client_send_frame_async`
...
/* Tests_SRS_UWS_CLIENT_01_426: [ If `uws_frame_encoder_encode` fails, `uws_client_send_frame_async` shall fail and
 * return a non-zero value. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_058: [ If `xio_send` fails, `uws_client_send_frame_async` shall fail and return a non-zero
 * value. ]*/
/* Tests_SRS_UWS_CLIENT_09_001: [ If `xio_send` fails and the message is still queued, it shall be de-queued and
 * destroyed. ] */
TEST_FUNCTION(when_xio_send_fails_uws_client_send_frame_async_fails)
{
    ...
    STRICT_EXPECTED_CALL(
        ...
        .SetReturn(1);
    STRICT_EXPECTED_CALL(
        singlylinkedlist_find(TEST_SINGLYLINKEDSINGLYLINKEDLIST_HANDLE, IGNORED_PTR_ARG, IGNORED_PTR_ARG))
...
/* Tests_SRS_UWS_CLIENT_01_049: [ If `singlylinkedlist_add` fails, `uws_client_send_frame_async` shall fail and return a
 * non-zero value. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_050: [ The argument `on_ws_send_frame_complete` shall be optional, if NULL is passed by the
 * caller then no send complete callback shall be triggered. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_389: [ When `on_underlying_io_send_complete` is called with `IO_SEND_OK` as a result of
 * sending a WebSocket frame to the underlying IO, the send shall be indicated to the uws user by calling
 * `on_ws_send_frame_complete` with `WS_SEND_FRAME_OK`. ]*/
/* Tests_SRS_UWS_CLIENT_01_432: [ The indicated sent frame shall be removed from the list by calling
...
/* Tests_SRS_UWS_CLIENT_01_433: [ If `singlylinkedlist_remove` fails an error shall be indicated by calling the
 * `on_ws_error` callback with `WS_ERROR_CANNOT_REMOVE_SENT_ITEM_FROM_LIST`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_390: [ When `on_underlying_io_send_complete` is called with `IO_SEND_ERROR` as a result of
 * sending a WebSocket frame to the underlying IO, the send shall be indicated to the uws user by calling
 * `on_ws_send_frame_complete` with `WS_SEND_FRAME_ERROR`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_391: [ When `on_underlying_io_send_complete` is called with `IO_SEND_CANCELLED` as a result
 * of sending a WebSocket frame to the underlying IO, the send shall be indicated to the uws user by calling
 * `on_ws_send_frame_complete` with `WS_SEND_FRAME_CANCELLED`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_435: [ When `on_underlying_io_send_complete` is called with a NULL `context`, it shall do
 * nothing. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_436: [ When `on_underlying_io_send_complete` is called with any other error code, the send
 * shall be indicated to the uws user by calling `on_ws_send_frame_complete` with `WS_SEND_FRAME_ERROR`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_430: [ `uws_client_dowork` shall call `xio_dowork` with the IO handle argument set to the
 * underlying IO created in `uws_client_create`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_375: [ When `on_underlying_io_error` is called while uws is OPENING, uws shall report that
 * the open failed by calling the `on_ws_open_complete` callback passed to `uws_client_open_async` with
 * `WS_OPEN_ERROR_UNDERLYING_IO_ERROR`. ]*/
/* Tests_SRS_UWS_CLIENT_01_077: [ If this fails (e.g., the server's certificate could not be verified), then the client
 * MUST _Fail the WebSocket Connection_ and abort the connection. ]*/
TEST_FUNCTION(
    on_underlying_io_error_while_opening_underlying_io_indicates_an_open_complete_with_WS_OPEN_ERROR_UNDERLYING_IO_ERROR)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
 ...
 * `WS_OPEN_ERROR_UNDERLYING_IO_ERROR`. ]*/
TEST_FUNCTION(
    on_underlying_io_error_while_waiting_for_upgrade_response_indicates_an_open_complete_with_WS_OPEN_ERROR_UNDERLYING_IO_ERROR)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
...
/* Tests_SRS_UWS_CLIENT_01_376: [ When `on_underlying_io_error` is called while the uws instance is OPEN, an error shall
 * be reported to the user by calling the `on_ws_error` callback that was passed to `uws_client_open_async` with the
 * argument `WS_ERROR_UNDERLYING_IO_ERROR`. ]*/
/* Tests_SRS_UWS_CLIENT_01_318: [ Servers MAY close the WebSocket connection whenever desired. ]*/
/* Tests_SRS_UWS_CLIENT_01_269: [ If at any point the state of the WebSocket connection changes, the endpoint MUST abort
 * the following steps. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_377: [ When `on_underlying_io_error` is called while the uws instance is CLOSING the
 * underlying IO shall be closed by calling `xio_close`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_377: [ When `on_underlying_io_error` is called while the uws instance is CLOSING the
 * underlying IO shall be closed by calling `xio_close`. ]*/
/* Tests_SRS_UWS_CLIENT_01_499: [ If the CLOSE was due to the peer closing, the callback `on_ws_close_complete` shall
 * not be called. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_500: [ If `xio_close` fails then the callback `on_ws_close_complete` shall be called, while
 * passing the `on_ws_close_complete_context` argument to it. ]*/
/* Tests_SRS_UWS_CLIENT_01_500: [ The callback `on_ws_close_complete` shall be called, while passing the
 * `on_ws_close_complete_context` argument to it. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_490: [ When `on_underlying_io_close_sent` is called while the uws client is CLOSING,
 * `on_underlying_io_close_sent` shall close the underlying IO by calling `xio_close`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_493: [ If calling `xio_close` fails, the state of the uws client shall be considered CLOSED
 * and the `on_ws_close_complete` shall be called if it was specified. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_496: [ If the close was initiated by the peer no `on_ws_close_complete` shall be called. ]*/
TEST_FUNCTION(
    when_xio_close_fails_in_on_underlying_io_close_sent_and_CLOSE_initiated_by_peer_no_callback_is_triggered_and_next_open_succeeds)
{
    // arrange
    TLSIO_CONFIG        tlsio_config;
    UWS_CLIENT_HANDLE   uws_client;
...
/* Tests_SRS_UWS_CLIENT_01_253: [ A Pong frame sent in response to a Ping frame must have identical "Application data"
...
/* Tests_SRS_UWS_CLIENT_01_251: [ An endpoint MAY send a Ping frame any time after the connection is established and
...
TEST_FUNCTION(when_a_PING_frame_was_received_with_some_payload_a_PONG_frame_is_sent_with_the_Same_payload)
{
    ...
    EXPECTED_CALL(gballoc_realloc(IGNORED_PTR_ARG, IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(
        uws_frame_encoder_encode(WS_PONG_FRAME, pong_frame_payload, sizeof(pong_frame_payload), true, true, 0))
...
/* Tests_SRS_UWS_CLIENT_01_440: [ If any of the arguments `uws_client` or `option_name` is NULL `uws_client_set_option`
...
/* Tests_SRS_UWS_CLIENT_01_510: [ If the option name is `uWSClientOptions` then `uws_client_set_option` shall call
 * `OptionHandler_FeedOptions` and pass to it the underlying IO handle and the `value` argument. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_511: [ If `OptionHandler_FeedOptions` fails, `uws_client_set_option` shall fail and return a
 * non-zero value. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_441: [ Otherwise all options shall be passed as they are to the underlying IO by calling
 * `xio_setoption`. ]*/
/* Tests_SRS_UWS_CLIENT_01_442: [ On success, `uws_client_set_option` shall return 0. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_443: [ If `xio_setoption` fails, `uws_client_set_option` shall fail and return a non-zero
 * value. ]*/
TEST_FUNCTION(when_xio_setoption_fails_then_uws_set_option_fails)
{
    ...
    STRICT_EXPECTED_CALL(xio_setoption(TEST_IO_HANDLE, "option1", (void *)0x4242)).SetReturn(1);
...
/* Tests_SRS_UWS_CLIENT_01_444: [ If parameter `uws_client` is `NULL` then `uws_client_retrieve_options` shall fail and
 * return NULL. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_445: [ `uws_client_retrieve_options` shall call `OptionHandler_Create` to produce an
 * `OPTIONHANDLER_HANDLE` and on success return the new `OPTIONHANDLER_HANDLE` handle. ]*/
/* Tests_SRS_UWS_CLIENT_01_501: [ `uws_client_retrieve_options` shall add to the option handler one option, whose name
 * shall be `uWSClientOptions` and the value shall be queried by calling `xio_retrieveoptions`. ]*/
...
TEST_FUNCTION(
    uws_retrieve_options_calls_the_underlying_xio_retrieve_options_and_returns_the_a_new_option_handler_instance)
{
    // arrange
    TLSIO_CONFIG         tlsio_config;
    UWS_CLIENT_HANDLE    uws_client;
    ...
    STRICT_EXPECTED_CALL(
        OptionHandler_AddOption(TEST_OPTIONHANDLER_HANDLE, "uWSClientOptions", TEST_IO_OPTIONHANDLER_HANDLE));
...
/* Tests_SRS_UWS_CLIENT_01_446: [ If `OptionHandler_Create` fails then `uws_client_retrieve_options` shall fail and
 * return NULL. ]*/
TEST_FUNCTION(when_OptionHandler_Create_fails_then_uws_retrieve_options_fails)
{
    ...
    EXPECTED_CALL(OptionHandler_Create(IGNORED_PTR_ARG, IGNORED_PTR_ARG, IGNORED_PTR_ARG)).SetReturn(NULL);
...
/* Tests_SRS_UWS_CLIENT_01_503: [ If `xio_retrieveoptions` fails, `uws_client_retrieve_options` shall fail and return
 * NULL. ]*/
TEST_FUNCTION(when_xio_retrieveoptions_fails_then_uws_retrieve_options_fails)
{
    ...
    EXPECTED_CALL(OptionHandler_Create(IGNORED_PTR_ARG, IGNORED_PTR_ARG, IGNORED_PTR_ARG));
    STRICT_EXPECTED_CALL(xio_retrieveoptions(TEST_IO_HANDLE)).SetReturn(NULL);
...
/* Tests_SRS_UWS_CLIENT_01_505: [ If `OptionHandler_AddOption` fails, `uws_client_retrieve_options` shall fail and
 * return NULL. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_507: [ `uws_client_clone_option` called with `name` being `uWSClientOptions` shall return the
 * same value. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_512: [ `uws_client_clone_option` called with any other option name than `uWSClientOptions`
 * shall return NULL. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_506: [ If `uws_client_clone_option` is called with NULL `name` or `value` it shall return
 * NULL. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_509: [ If `uws_client_destroy_option` is called with NULL `name` or `value` it shall do
 * nothing. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_508: [ `uws_client_destroy_option` called with the option `name` being `uWSClientOptions`
 * shall destroy the value by calling `OptionHandler_Destroy`. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_475: [ When `on_underlying_io_close_complete` is called while closing the underlying IO a
 * subsequent `uws_client_open_async` shall succeed. ]*/
TEST_FUNCTION(
    underlying_io_close_after_a_send_close_frame_failed_puts_the_uws_in_closed_state_and_a_new_open_is_allowed)
{
    // arrange
    TLSIO_CONFIG      tlsio_config;
    UWS_CLIENT_HANDLE uws_client;
...
/* Tests_SRS_UWS_CLIENT_01_477: [ When `on_underlying_io_close_complete` is called with a NULL context, it shall do
 * nothing. ]*/
...
/* Tests_SRS_UWS_CLIENT_01_387: [ When `on_underlying_io_close_complete` is called when the uws instance is closing, the
 * `on_ws_close_complete` callback passed to `uws_client_close_async` shall be called. ]*/
