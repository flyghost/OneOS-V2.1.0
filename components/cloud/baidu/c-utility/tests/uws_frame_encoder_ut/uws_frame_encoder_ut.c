...
static void
stringify_bytes(const unsigned char *bytes, size_t byte_count, char *output_string, size_t string_buffer_size)
{
    size_t i;
    size_t pos = 0;
...
/* Tests_SRS_UWS_FRAME_ENCODER_01_054: [ If `length` is greater than 0 and payload is NULL, then
 * `uws_frame_encoder_encode` shall fail and return NULL. ]*/
...
/* Tests_SRS_UWS_FRAME_ENCODER_01_001: [ `uws_frame_encoder_encode` shall encode the information given in `opcode`,
 * `payload`, `length`, `is_masked`, `is_final` and `reserved` according to the RFC6455 into a new buffer.]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_044: [ On success `uws_frame_encoder_encode` shall return a non-NULL handle to the
 * result buffer. ]*/
...
TEST_FUNCTION(uws_frame_encoder_encode_encodes_a_zero_length_binary_frame)
{
    ...
    STRICT_EXPECTED_CALL(BUFFER_new()).CaptureReturn(&newly_created_buffer);
    STRICT_EXPECTED_CALL(BUFFER_enlarge(IGNORED_PTR_ARG, 2)).ValidateArgumentValue_handle(&newly_created_buffer);
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_PTR_ARG)).ValidateArgumentValue_handle(&newly_created_buffer);
    ...
    stringify_bytes(real_BUFFER_u_char(result),
                    real_BUFFER_length(result),
                    actual_encoded_str,
                    sizeof(actual_encoded_str));
...
/* Tests_SRS_UWS_FRAME_ENCODER_01_049: [ If `BUFFER_new` fails then `uws_frame_encoder_encode` shall fail and return
 * NULL. ]*/
...
/* Tests_SRS_UWS_FRAME_ENCODER_01_047: [ If `BUFFER_enlarge` fails then `uws_frame_encoder_encode` shall fail and return
 * NULL. ]*/
TEST_FUNCTION(when_BUFFER_enlarge_fails_then_uws_frame_encoder_encode_fails)
{
    ...
    STRICT_EXPECTED_CALL(BUFFER_enlarge(IGNORED_PTR_ARG, 2))
        ...
        .SetReturn(1);
    STRICT_EXPECTED_CALL(BUFFER_delete(IGNORED_PTR_ARG)).ValidateArgumentValue_handle(&newly_created_buffer);
...
/* Tests_SRS_UWS_FRAME_ENCODER_01_051: [ If `BUFFER_u_char` fails then `uws_frame_encoder_encode` shall fail and return
 * a NULL. ]*/
...
/* Tests_SRS_UWS_FRAME_ENCODER_01_004: [ MUST be 0 unless an extension is negotiated that defines meanings for non-zero
 * values. ]*/
...
/* Tests_SRS_UWS_FRAME_ENCODER_01_052: [ If `reserved` has any bits set except the lowest 3 then
 * `uws_frame_encoder_encode` shall fail and return NULL. ]*/
...
/* Tests_SRS_UWS_FRAME_ENCODER_01_006: [ If an unknown opcode is received, the receiving endpoint MUST _Fail the
...
/* Tests_SRS_UWS_FRAME_ENCODER_01_015: [ Defines whether the "Payload data" is masked. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_053: [ In order to obtain a 32 bit value for masking, `gb_rand` shall be used 4 times
 * (for each byte). ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_016: [ If set to 1, a masking key is present in masking-key, and this is used to
 * unmask the "Payload data" as per Section 5.3. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_026: [ This field is present if the mask bit is set to 1 and is absent if the mask bit
 * is set to 0. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_042: [ The payload length, indicated in the framing as frame-payload-length, does NOT
 * include the length of the masking key. ]*/
TEST_FUNCTION(uws_frame_encoder_encode_encodes_a_masked_zero_length_binary_frame)
{
    ...
    STRICT_EXPECTED_CALL(BUFFER_enlarge(IGNORED_PTR_ARG, sizeof(expected_bytes)))
        .ValidateArgumentValue_handle(&newly_created_buffer);
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_PTR_ARG)).ValidateArgumentValue_handle(&newly_created_buffer);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0xFF);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0xFF);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0xFF);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0xFF);
...
TEST_FUNCTION(uws_frame_encoder_encode_encodes_a_masked_zero_length_binary_frame_different_mask)
{
    ...
    STRICT_EXPECTED_CALL(BUFFER_enlarge(IGNORED_PTR_ARG, sizeof(expected_bytes)))
        .ValidateArgumentValue_handle(&newly_created_buffer);
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_PTR_ARG)).ValidateArgumentValue_handle(&newly_created_buffer);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0x42);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0x43);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0x44);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0x45);
...
/* Tests_SRS_UWS_FRAME_ENCODER_01_043: [ if 0-125, that is the payload length. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_023: [ The payload length is the length of the "Extension data" + the length of the
 * "Application data". ]*/
...
/* Tests_SRS_UWS_FRAME_ENCODER_01_043: [ if 0-125, that is the payload length. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_022: [ Note that in all cases, the minimal number of bytes MUST be used to encode the
 * length, for example, the length of a 124-byte-long string can't be encoded as the sequence 126, 0, 124. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_023: [ The payload length is the length of the "Extension data" + the length of the
 * "Application data". ]*/
TEST_FUNCTION(uws_frame_encoder_encode_encodes_a_125_byte_long_binary_frame)
{
    ...
    STRICT_EXPECTED_CALL(BUFFER_new()).CaptureReturn(&newly_created_buffer);
    STRICT_EXPECTED_CALL(BUFFER_enlarge(IGNORED_PTR_ARG, 125 + 2)).ValidateArgumentValue_handle(&newly_created_buffer);
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_PTR_ARG)).ValidateArgumentValue_handle(&newly_created_buffer);
...
/* Tests_SRS_UWS_FRAME_ENCODER_01_019: [ If 126, the following 2 bytes interpreted as a 16-bit unsigned integer are the
 * payload length. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_023: [ The payload length is the length of the "Extension data" + the length of the
 * "Application data". ]*/
TEST_FUNCTION(uws_frame_encoder_encode_encodes_a_126_byte_long_binary_frame)
{
    ...
    STRICT_EXPECTED_CALL(BUFFER_new()).CaptureReturn(&newly_created_buffer);
    STRICT_EXPECTED_CALL(BUFFER_enlarge(IGNORED_PTR_ARG, 126 + 4)).ValidateArgumentValue_handle(&newly_created_buffer);
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_PTR_ARG)).ValidateArgumentValue_handle(&newly_created_buffer);
...
TEST_FUNCTION(uws_frame_encoder_encode_encodes_a_65535_byte_long_binary_frame)
{
    ...
    ASSERT_ARE_EQUAL_WITH_MSG(int, 0, memcmp(expected_bytes, real_BUFFER_u_char(result), real_BUFFER_length(result)),
"Memory compare failed"); ASSERT_ARE_EQUAL(char_ptr, umock_c_get_expected_calls(), umock_c_get_actual_calls());
...
*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_020: [ If 127, the following 8 bytes interpreted as a 64-bit unsigned integer (the
...
*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_033: [ A masked frame MUST have the field frame-masked set to 1, as defined in
 * Section 5.2. **]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_034: [ The masking key is contained completely within the frame, as defined in
 * Section 5.2 as frame-masking-key. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_035: [ It is used to mask the "Payload data" defined in the same section as
 * frame-payload-data, which includes "Extension data" and "Application data". ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_036: [ The masking key is a 32-bit value chosen at random by the client. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_037: [ When preparing a masked frame, the client MUST pick a fresh masking key from
 * the set of allowed 32-bit values. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_038: [ The masking key needs to be unpredictable; thus, the masking key MUST be
 * derived from a strong source of entropy, and the masking key for a given frame MUST NOT make it simple for a
 * server/proxy to predict the masking key for a subsequent frame. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_039: [ To convert masked data into unmasked data, or vice versa, the following
 * algorithm is applied. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_040: [ The same algorithm applies regardless of the direction of the translation,
 * e.g., the same steps are applied to mask the data as to unmask the data. ]*/
/* Tests_SRS_UWS_FRAME_ENCODER_01_041: [ Octet i of the transformed data ("transformed-octet-i") is the XOR of octet i
 * of the original data ("original-octet-i") with octet at index i modulo 4 of the masking key ("masking-key-octet-j"):
 * ]*/
TEST_FUNCTION(uws_frame_encoder_encode_masks_a_1_byte_frame_with_0_as_mask)
{
    ...
    STRICT_EXPECTED_CALL(BUFFER_enlarge(IGNORED_PTR_ARG, sizeof(expected_bytes)))
        .ValidateArgumentValue_handle(&newly_created_buffer);
    STRICT_EXPECTED_CALL(BUFFER_u_char(IGNORED_PTR_ARG)).ValidateArgumentValue_handle(&newly_created_buffer);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0x00);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0x00);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0x00);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0x00);
...
TEST_FUNCTION(uws_frame_encoder_encode_masks_a_8_byte_frame_with_different_mask_bytes)
{
    ...
    unsigned char expected_bytes[] =
        {0x82, 0x88, 0x00, 0xFF, 0xAA, 0x42, 0x42, 0xBC, 0xEE, 0x07, 0x01, 0xFD, 0x55, 0xE8};
    ...
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0x00);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0xFF);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0xAA);
    STRICT_EXPECTED_CALL(gb_rand()).SetReturn(0x42);
