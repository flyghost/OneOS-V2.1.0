...
/* Tests_SRS_VECTOR_10_001: [VECTOR_create shall allocate a VECTOR_HANDLE that will contain an empty vector.The size of
 * each element is given with the parameter elementSize.] */
...
TEST_FUNCTION(VECTOR_push_back_fails_if_realloc_fails)
{
    ...
    /// act
    STRICT_EXPECTED_CALL(gballoc_realloc(NULL, sizeof(VECTOR_UNITTEST))).SetReturn(NULL);
    result = VECTOR_push_back(handle, &sItem, 1);
...
/* Tests_SRS_VECTOR_10_014: [VECTOR_erase shall remove the `numElements` starting at `elements` and reduce its internal
 * storage.] */
TEST_FUNCTION(VECTOR_erase_succeeds_case_1)
{
    ...
    umock_c_reset_all_calls();
    STRICT_EXPECTED_CALL(gballoc_realloc(IGNORED_PTR_ARG, sizeof(VECTOR_UNITTEST))).IgnoreArgument_ptr();
