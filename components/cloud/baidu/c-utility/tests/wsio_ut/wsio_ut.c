...
static int my_uws_open_async(UWS_CLIENT_HANDLE    uws,
...
static int
my_uws_close_async(UWS_CLIENT_HANDLE uws, ON_WS_CLOSE_COMPLETE on_ws_close_complete, void *on_ws_close_complete_context)
{
    (void)uws;
...
static int my_uws_send_frame_async(UWS_CLIENT_HANDLE         uws,
...
/* Tests_SRS_WSIO_01_185: [ - `underlying_io_interface` shall be set to the `underlying_io_interface` field in the
 * `io_create_parameters` passed to `wsio_create`. ]*/
/* Tests_SRS_WSIO_01_186: [ - `underlying_io_parameters` shall be set to the `underlying_io_parameters` field in the
 * `io_create_parameters` passed to `wsio_create`. ]*/
/* Tests_SRS_WSIO_01_072: [ - `hostname` set to the `hostname` field in the `io_create_parameters` passed to
 * `wsio_create`. ]*/
/* Tests_SRS_WSIO_01_130: [ - `port` set to the `port` field in the `io_create_parameters` passed to `wsio_create`. ]*/
/* Tests_SRS_WSIO_01_128: [ - `resource_name` set to the `resource_name` field in the `io_create_parameters` passed to
 * `wsio_create`. ]*/
/* Tests_SRS_WSIO_01_129: [ - `protocols` shall be filled with only one structure, that shall have the `protocol` set to
 * the value of the `protocol` field in the `io_create_parameters` passed to `wsio_create`. ]*/
/* Tests_SRS_WSIO_01_076: [ `wsio_create` shall create a pending send IO list that is to be used to queue send packets
 * by calling `singlylinkedlist_create`. ]*/
TEST_FUNCTION(wsio_create_for_secure_connection_with_valid_args_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(uws_client_create_with_io(TEST_UNDERLYING_IO_INTERFACE,
                                                   TEST_UNDERLYING_IO_PARAMETERS,
                                                   TEST_HOST_ADDRESS,
                                                   443,
                                                   TEST_RESOURCE_NAME,
                                                   IGNORED_PTR_ARG,
...
/* Tests_SRS_WSIO_01_067: [ If any of the members `hostname`, `resource_name` or `protocol` is NULL in `WSIO_CONFIG`
 * then `wsio_create` shall return NULL. ]*/
...
TEST_FUNCTION(wsio_create_for_secure_connection_with_valid_args_succeeds_2)
{
    ...
    STRICT_EXPECTED_CALL(
        uws_client_create_with_io(TEST_UNDERLYING_IO_INTERFACE, NULL, "another.com", 80, "haga", IGNORED_PTR_ARG, 1));
    STRICT_EXPECTED_CALL(singlylinkedlist_create());
...
/* Tests_SRS_WSIO_01_080: [ `wsio_destroy` shall destroy the uws instance created in `wsio_create` by calling
 * `uws_client_destroy`. ]*/
/* Tests_SRS_WSIO_01_081: [ `wsio_destroy` shall free the list used to track the pending send IOs by calling
...
/* Tests_SRS_WSIO_01_082: [ `wsio_open` shall open the underlying uws instance by calling `uws_client_open_async` and
 * providing the uws handle created in `wsio_create` as argument. ]*/
...
TEST_FUNCTION(wsio_open_opens_the_underlying_uws_instance)
{
    ...
    STRICT_EXPECTED_CALL(uws_client_open_async(TEST_UWS_HANDLE,
                                               IGNORED_PTR_ARG,
                                               IGNORED_PTR_ARG,
                                               IGNORED_PTR_ARG,
                                               IGNORED_PTR_ARG,
    ...
    // act
    result = wsio_get_interface_description()->concrete_io_open(wsio,
...
/* Tests_SRS_WSIO_01_084: [ If opening the underlying uws instance fails then `wsio_open` shall fail and return a
 * non-zero value. ]*/
...
TEST_FUNCTION(wsio_open_when_already_opening_fails)
{
    ...
    int                result;
    wsio = wsio_get_interface_description()->concrete_io_create(&default_wsio_config);
    (void)wsio_get_interface_description()->concrete_io_open(wsio,
                                                             test_on_io_open_complete,
                                                             (void *)0x4242,
                                                             test_on_bytes_received,
                                                             (void *)0x4243,
...
/* Tests_SRS_WSIO_01_132: [ If any of the arguments `ws_io`, `on_io_open_complete`, `on_bytes_received`, `on_io_error`
 * is NULL, `wsio_open` shall fail and return a non-zero value. ]*/
TEST_FUNCTION(wsio_open_with_NULL_handle_fails)
{
    ...
    // act
    result = wsio_get_interface_description()->concrete_io_open(NULL,
                                                                test_on_io_open_complete,
                                                                (void *)0x4242,
                                                                test_on_bytes_received,
                                                                (void *)0x4243,
...
/* Tests_SRS_WSIO_01_085: [ `wsio_close` shall close the websockets IO if an open action is either pending or has
 * completed successfully (if the IO is open).  ]*/
/* Tests_SRS_WSIO_01_133: [ On success `wsio_close` shall return 0. ]*/
/* Tests_SRS_WSIO_01_091: [ `wsio_close` shall obtain all the pending IO items by repetitively querying for the head of
 * the pending IO list and freeing that head item. ]*/
/* Tests_SRS_WSIO_01_087: [ `wsio_close` shall call `uws_client_close_async` while passing as argument the IO handle
 * created in `wsio_create`.  ]*/
/* Tests_SRS_WSIO_01_092: [ Obtaining the head of the pending IO list shall be done by calling
 * `singlylinkedlist_get_head_item`. ]*/
/* Tests_SRS_WSIO_01_094: [ The callback context passed to the `on_send_complete` callback shall be the context given to
 * `wsio_send`.  ]*/
...
/* Tests_SRS_WSIO_01_090: [ The argument `on_io_close_complete` shall be optional, if NULL is passed by the caller then
 * no close complete callback shall be triggered.  ]*/
...
/* Tests_SRS_WSIO_01_093: [ For each pending item the send complete callback shall be called with
 * `IO_SEND_CANCELLED`.]*/
TEST_FUNCTION(wsio_close_indicates_a_pending_send_as_CANCELLED)
{
    ...
    (void)wsio_get_interface_description()->concrete_io_open(wsio,
                                                             ...
                                                             (void *)0x4244);
    g_on_ws_open_complete(g_on_ws_open_complete_context, WS_OPEN_OK);
    (void)wsio_get_interface_description()->concrete_io_send(wsio,
                                                             test_buffer,
                                                             sizeof(test_buffer),
...
/* Tests_SRS_WSIO_01_095: [ `wsio_send` shall call `uws_client_send_frame_async`, passing the `buffer` and `size`
...
TEST_FUNCTION(wsio_send_with_1_byte_calls_uws_send_frame)
{
    ...
    STRICT_EXPECTED_CALL(uws_client_send_frame_async(TEST_UWS_HANDLE,
    ...
    // act
    result = wsio_get_interface_description()->concrete_io_send(wsio,
                                                                test_buffer,
                                                                sizeof(test_buffer),
                                                                test_on_send_complete,
                                                                (void *)0x4343);
...
/* Tests_SRS_WSIO_01_099: [ If the wsio is not OPEN (open has not been called or is still in progress) then `wsio_send`
...
/* Tests_SRS_WSIO_01_100: [ If any of the arguments `ws_io` or `buffer` are NULL, `wsio_send` shall fail and return a
 * non-zero value. ]*/
TEST_FUNCTION(wsio_send_with_NULL_wsio_fails)
{
    ...
    // act
    result = wsio_get_interface_description()->concrete_io_send(NULL,
                                                                test_buffer,
                                                                sizeof(test_buffer),
                                                                test_on_send_complete,
                                                                (void *)0x4343);
...
TEST_FUNCTION(wsio_send_with_zero_size_fails)
{
    ...
    // act
    result =
        wsio_get_interface_description()->concrete_io_send(wsio, test_buffer, 0, test_on_send_complete, (void *)0x4343);
...
/* Tests_SRS_WSIO_01_134: [ If allocating memory for the pending IO data fails, `wsio_send` shall fail and return a
 * non-zero value. ]*/
...
/* Tests_SRS_WSIO_01_105: [ The argument `on_send_complete` shall be optional, if NULL is passed by the caller then no
 * send complete callback shall be triggered. ]*/
...
/* Tests_SRS_WSIO_01_121: [ When `on_underlying_ws_error` is called while the IO is OPEN the wsio instance shall be set
 * to ERROR and an error shall be indicated via the `on_io_error` callback passed to `wsio_open`. ]*/
/* Tests_SRS_WSIO_01_123: [ When calling `on_io_error`, the `on_io_error_context` argument given in `wsio_open` shall be
 * passed to the callback `on_io_error`. ]*/
...
/* Tests_SRS_WSIO_01_122: [ When `on_underlying_ws_error` is called while the IO is OPENING, the `on_io_open_complete`
 * callback passed to `wsio_open` shall be called with `IO_OPEN_ERROR`. ]*/
...
/* Tests_SRS_WSIO_01_136: [ When `on_underlying_ws_open_complete` is called with `WS_OPEN_OK` while the IO is opening,
 * the callback `on_io_open_complete` shall be called with `IO_OPEN_OK`. ]*/
TEST_FUNCTION(
...
/* Tests_SRS_WSIO_01_149: [ When `on_underlying_ws_open_complete` is called with `WS_OPEN_CANCELLED` while the IO is
 * opening, the callback `on_io_open_complete` shall be called with `IO_OPEN_CANCELLED`. ]*/
TEST_FUNCTION(
    when_on_underlying_ws_open_complete_is_called_with_CANCELLED_while_OPENING_the_io_open_complete_callback_is_called_with_OPEN_CANCELLED)
{
    // arrange
    CONCRETE_IO_HANDLE wsio;
...
/* Tests_SRS_WSIO_01_137: [ When `on_underlying_ws_open_complete` is called with any other error code while the IO is
 * opening, the callback `on_io_open_complete` shall be called with `IO_OPEN_ERROR`. ]*/
TEST_FUNCTION(
    when_on_underlying_ws_open_complete_is_called_with_an_error_while_OPENING_the_io_open_complete_callback_is_called_with_OPEN_ERROR)
{
    // arrange
    CONCRETE_IO_HANDLE wsio;
...
/* Tests_SRS_WSIO_01_139: [ When `on_underlying_ws_open_complete` is called while in OPEN state it shall indicate an
 * error by calling the `on_io_error` callback passed to `wsio_open` and switch to the ERROR state. ]*/
/* Tests_SRS_WSIO_01_140: [ When calling `on_io_error`, the `on_io_error_context` argument given in `wsio_open` shall be
 * passed to the callback `on_io_error`. ]*/
...
/* Tests_SRS_WSIO_01_141: [ When `on_underlying_ws_open_complete` is called while in the ERROR state it shall indicate
 * an error by calling the `on_io_error` callback passed to `wsio_open`. ]*/
/* Tests_SRS_WSIO_01_140: [ When calling `on_io_error`, the `on_io_error_context` argument given in `wsio_open` shall be
 * passed to the callback `on_io_error`. ]*/
...
/* Tests_SRS_WSIO_01_142: [ When `on_underlying_ws_open_complete` is called while in the CLOSING state an error shall be
 * indicated by calling the `on_io_error` callback passed to `wsio_open`. ]*/
...
/* Tests_SRS_WSIO_01_124: [ When `on_underlying_ws_frame_received` is called the bytes in the frame shall be indicated
 * by calling the `on_bytes_received` callback passed to `wsio_open`. ]*/
/* Tests_SRS_WSIO_01_125: [ When calling `on_bytes_received`, the `on_bytes_received_context` argument given in
 * `wsio_open` shall be passed to the callback `on_bytes_received`. ]*/
...
/* Tests_SRS_WSIO_01_126: [ If `on_underlying_ws_frame_received` is called while the IO is in any state other than OPEN,
 * it shall do nothing. ]*/
...
/* Tests_SRS_WSIO_01_151: [ If the WebSocket frame type is not binary then an error shall be indicated by calling the
 * `on_io_error` callback passed to `wsio_open`. ]*/
/* Tests_SRS_WSIO_01_152: [ When calling `on_io_error`, the `on_io_error_context` argument given in `wsio_open` shall be
 * passed to the callback `on_io_error`. ]*/
...
/* Tests_SRS_WSIO_01_153: [ When `on_underlying_ws_frame_received` is called with zero `size`, no bytes shall be
 * indicated up as received. ]*/
...
/* Tests_SRS_WSIO_01_154: [ When `on_underlying_ws_frame_received` is called with a positive `size` and a NULL `buffer`,
 * an error shall be indicated by calling the `on_io_error` callback passed to `wsio_open`. ]*/
...
/* Tests_SRS_WSIO_01_143: [ When `on_underlying_ws_send_frame_complete` is called after sending a WebSocket frame, the
 * pending IO shall be removed from the list. ]*/
...
/* Tests_SRS_WSIO_01_146: [ When `on_underlying_ws_send_frame_complete` is called with `WS_SEND_OK`, the callback
 * `on_send_complete` shall be called with `IO_SEND_OK`. ]*/
...
/* Tests_SRS_WSIO_01_147: [ When `on_underlying_ws_send_frame_complete` is called with `WS_SEND_CANCELLED`, the callback
 * `on_send_complete` shall be called with `IO_SEND_CANCELLED`. ]*/
...
/* Tests_SRS_WSIO_01_148: [ When `on_underlying_ws_send_frame_complete` is called with any other error code, the
 * callback `on_send_complete` shall be called with `IO_SEND_ERROR`. ]*/
...
/* Tests_SRS_WSIO_01_155: [ When `on_underlying_ws_send_frame_complete` is called with a NULL context it shall do
 * nothing. ]*/
...
/* Tests_SRS_WSIO_01_109: [ If any of the arguments `ws_io` or `option_name` is NULL `wsio_setoption` shall return a
 * non-zero value. ]*/
...
/* Tests_SRS_WSIO_01_183: [ If the option name is `WSIOOptions` then `wsio_setoption` shall call
...
TEST_FUNCTION(wsio_setoption_with_WSIOOptions_feeds_the_options_to_the_underlying_layer)
{
    ...
    // act
    result =
        wsio_get_interface_description()->concrete_io_setoption(wsio, "WSIOOptions", (void *)TEST_OPTIONHANDLER_HANDLE);
...
/* Tests_SRS_WSIO_01_184: [ If `OptionHandler_FeedOptions` fails, `wsio_setoption` shall fail and return a non-zero
 * value. ]*/
...
TEST_FUNCTION(when_uws_set_option_fails_wsio_setoption_fails)
{
    ...
    STRICT_EXPECTED_CALL(uws_client_set_option(TEST_UWS_HANDLE, "option1", (void *)0x4242)).SetReturn(1);
...
/* Tests_SRS_WSIO_01_119: [ `wsio_retrieveoptions` shall call `OptionHandler_Create` to produce an
...
/* Tests_SRS_WSIO_01_178: [ `uws_client_retrieve_options` shall add to the option handler one option, whose name shall
 * be `uWSCLientOptions` and the value shall be queried by calling `uws_client_retrieve_options`. ]*/
/* Tests_SRS_WSIO_01_181: [ Adding the option shall be done by calling `OptionHandler_AddOption`. ]*/
TEST_FUNCTION(
    wsio_retrieveoptions_creates_an_option_handler_with_the_value_obtained_from_the_underlying_retrieve_options)
{
    // arrange
    OPTIONHANDLER_HANDLE result;
    CONCRETE_IO_HANDLE   wsio;
    ...
    STRICT_EXPECTED_CALL(
        OptionHandler_AddOption(TEST_OPTIONHANDLER_HANDLE, "WSIOOptions", TEST_UWS_CLIENT_OPTIONHANDLER_HANDLE));
...
/* Tests_SRS_WSIO_01_180: [ If `uws_client_retrieve_options` fails, `uws_client_retrieve_options` shall fail and return
 * NULL. ]*/
TEST_FUNCTION(when_uws_client_retrieve_options_fails_then_wsio_retrieveoptions_fails)
{
    ...
    EXPECTED_CALL(OptionHandler_Create(IGNORED_PTR_ARG, IGNORED_PTR_ARG, IGNORED_PTR_ARG));
    STRICT_EXPECTED_CALL(uws_client_retrieve_options(TEST_UWS_HANDLE)).SetReturn(NULL);
...
/* Tests_SRS_WSIO_01_182: [ If `OptionHandler_AddOption` fails, `uws_client_retrieve_options` shall fail and return
 * NULL. ]*/
...
/* Tests_SRS_WSIO_01_175: [ `wsio_destroy_option` called with the option `name` being `WSIOOptions` shall destroy the
 * value by calling `OptionHandler_Destroy`. ]*/
...
/* Tests_SRS_WSIO_01_159: [ When `on_underlying_ws_close_complete` while the IO is closing (after `wsio_close`), the
 * close shall be indicated up by calling the `on_io_close_complete` callback passed to `wsio_close`. ]*/
/* Tests_SRS_WSIO_01_163: [ When `on_io_close_complete` is called, the context passed to `wsio_close` shall be passed as
 * argument to `on_io_close_complete`. ]*/
...
/* Tests_SRS_WSIO_01_161: [ If the context passed to `on_underlying_ws_close_complete` is NULL,
 * `on_underlying_ws_close_complete` shall do nothing. ]*/
...
/* Tests_SRS_WSIO_01_170: [ When `on_underlying_ws_peer_closed` and the state of the IO is OPENING an error shall be
 * indicated by calling the `on_io_open_complete` callback passed to `wsio_open` with the error code `WS_OPEN_ERROR`.
 * ]*/
...
/* Tests_SRS_WSIO_01_166: [ When `on_underlying_ws_peer_closed` and the state of the IO is OPEN an error shall be
 * indicated by calling the `on_io_error` callback passed to `wsio_open`. ]*/
/* Tests_SRS_WSIO_01_168: [ The `close_code`, `extra_data` and `extra_data_length` arguments shall be ignored. ]*/
...
// Tests_SRS_WSIO_07_001: [When `on_underlying_ws_peer_closed` and the state of the IO is NOT_OPEN an error will be
// raised and the io_state will remain as NOT_OPEN]
