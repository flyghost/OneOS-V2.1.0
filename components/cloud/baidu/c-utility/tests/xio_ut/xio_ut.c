...
const IO_INTERFACE_DESCRIPTION test_io_description = {test_xio_retrieveoptions,
                                                      test_xio_create,
                                                      test_xio_destroy,
                                                      test_xio_open,
                                                      test_xio_close,
...
/* Tests_SRS_XIO_01_002: [In order to instantiate the concrete IO implementation the function concrete_io_create from
 * the io_interface_description shall be called, passing the xio_create_parameters argument.] */
...
TEST_FUNCTION(when_concrete_xio_create_fails_then_xio_create_fails)
{
    // arrange
    XIO_HANDLE result;
    STRICT_EXPECTED_CALL(test_xio_create(NULL)).SetReturn((CONCRETE_IO_HANDLE)NULL);
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG));
...
{
    // arrange
    const IO_INTERFACE_DESCRIPTION io_description_null = {NULL,
                                                          test_xio_create,
                                                          test_xio_destroy,
                                                          test_xio_open,
                                                          test_xio_close,
...
{
    // arrange
    const IO_INTERFACE_DESCRIPTION io_description_null = {test_xio_retrieveoptions,
                                                          NULL,
                                                          test_xio_destroy,
                                                          test_xio_open,
                                                          test_xio_close,
...
TEST_FUNCTION(when_allocating_memory_Fails_then_xio_create_fails)
{
    ...
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).SetReturn((void *)NULL);
...
/* Tests_SRS_XIO_01_006: [xio_destroy shall also call the concrete_xio_destroy function that is member of the
 * io_interface_description argument passed to xio_create, while passing as argument to concrete_xio_destroy the result
 * of the underlying concrete_xio_create handle that was called as part of the xio_create call.] */
...
/* Tests_SRS_XIO_01_019: SRS_XIO_01_019: [xio_open shall call the specific concrete_xio_open function specified in
 * xio_create, passing callback function and context arguments for three events: open completed, bytes received, and IO
 * error.] */
...
/* Tests_SRS_XIO_01_008: [xio_send shall pass the sequence of bytes pointed to by buffer to the concrete IO
 * implementation specified in xio_create, by calling the concrete_xio_send function while passing down the buffer and
 * size arguments to it.] */
...
TEST_FUNCTION(xio_send_calls_the_underlying_concrete_xio_send_and_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(
        test_xio_send(TEST_CONCRETE_IO_HANDLE, send_data, sizeof(send_data), test_on_send_complete, (void *)0x4242));
...
/* Tests_SRS_XIO_01_012: [xio_dowork shall call the concrete IO implementation specified in xio_create, by calling the
 * concrete_xio_dowork function.] */
...
/* Tests_SRS_XIO_03_030: [If the xio argument or the optionName argument is NULL, xio_setoption shall return a non-zero
 * value.] */
...
/* Tests_SRS_XIO_003_028: [xio_setoption shall pass the optionName and value to the concrete IO implementation specified
 * in xio_create by invoking the concrete_xio_setoption function.] */
...
TEST_FUNCTION(xio_setoption_fails_when_concrete_xio_setoption_fails)
{
    ...
    STRICT_EXPECTED_CALL(test_xio_setoption(TEST_CONCRETE_IO_HANDLE, optionName, optionValue)).SetReturn(42);
...
static void xio_retrieveoptions_inert_path(void)
{
    ...
    STRICT_EXPECTED_CALL(test_xio_retrieveoptions(IGNORED_PTR_ARG)).IgnoreArgument_handle();
...
/*Tests_SRS_XIO_02_002: [ xio_retrieveoptions shall create a OPTIONHANDLER_HANDLE by calling OptionHandler_Create
 * passing xio_setoption as setOption argument and xio_CloneOption and xio_DestroyOption for cloneOption and
 * destroyOption. ]*/
/*Tests_SRS_XIO_02_003: [ xio_retrieveoptions shall retrieve the concrete handle's options by a call to
 * concrete_io_retrieveoptions. ]*/
/*Tests_SRS_XIO_02_004: [ xio_retrieveoptions shall add a hardcoded option named concreteOptions having the same content
 * as the concrete handle's options. ]*/
