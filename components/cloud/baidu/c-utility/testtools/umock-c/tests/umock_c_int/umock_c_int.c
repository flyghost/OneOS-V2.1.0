...
Tests_SRS_UMOCK_C_LIB_01_010: [If on_umock_c_error is non-NULL it shall be saved for later use (to be invoked whenever
an umock_c error needs to be signaled to the user).] Tests_SRS_UMOCK_C_LIB_01_011: [umock_c_deinit shall free all
umock_c used resources.] Tests_SRS_UMOCK_C_LIB_01_012: [If umock_c was not initialized, umock_c_deinit shall do
nothing.]
*/

/* Tested by unittests of umocktypes and umocktypename:
Tests_SRS_UMOCK_C_LIB_01_145: [ Since umock_c needs to maintain a list of registered types, the following rules shall be
applied: ] Tests_SRS_UMOCK_C_LIB_01_146: [ Each type shall be normalized to a form where all extra spaces are removed. ]
Tests_SRS_UMOCK_C_LIB_01_147: [ Type names are case sensitive. ]
*/

/* Tested by unittests of umockcallpairs:
Tests_SRS_UMOCK_C_LIB_01_194: [ If the first argument passed to destroy_call is not found in the list of tracked handles
(returned by create_call) then umock_c shall raise an error with the code UMOCK_C_INVALID_PAIRED_CALLS. ]
*/
...
/* Tests_SRS_UMOCK_C_LIB_01_067: [char\* and const char\* shall be supported out of the box through a separate header,
 * umockvalue_charptr.h.]*/
...
{
    test_on_umock_c_error_CALL *new_calls = (test_on_umock_c_error_CALL *)realloc(
        test_on_umock_c_error_calls,
        sizeof(test_on_umock_c_error_CALL)  *(test_on_umock_c_error_call_count + 1));
    if (new_calls != NULL)
    {
...
int umocktypes_are_equal_TEST_STRUCT_WITH_2_MEMBERS(const TEST_STRUCT_WITH_2_MEMBERS *left,
                                                    const TEST_STRUCT_WITH_2_MEMBERS *right)
{
    (void)left;
    (void)right;
    return 1;
...
int umocktypes_copy_TEST_STRUCT_WITH_2_MEMBERS(TEST_STRUCT_WITH_2_MEMBERS       *destination,
                                               const TEST_STRUCT_WITH_2_MEMBERS *source)
{
    (void)source;
    (void)destination;
    return 0;
...
/* Tests_SRS_UMOCK_C_LIB_01_150: [ MOCK_FUNCTION_WITH_CODE shall define a mock function and allow the user to embed code
 * between this define and a MOCK_FUNCTION_END call. ]*/
...
TEST_SUITE_INITIALIZE(suite_init)
{
    ...
    /* Tests_SRS_UMOCK_C_LIB_01_070: [umockvalue_charptr_register_types shall return 0 on success and non-zero on
     * failure.]*/
    result = umocktypes_charptr_register_types();
    ASSERT_ARE_EQUAL(int, 0, result);
    /* Tests_SRS_UMOCK_C_LIB_01_065: [REGISTER_UMOCK_VALUE_TYPE shall register the type identified by value_type to be
     * usable by umock_c for argument and return types and instruct umock_c which functions to use for getting the
     * stringify, are_equal, copy and free.]*/
    REGISTER_UMOCK_VALUE_TYPE(int *, stringify_func_intptr, are_equal_func_intptr, copy_func_intptr, free_func_intptr);
    REGISTER_UMOCK_VALUE_TYPE(unsigned char *,
                              stringify_func_unsignedcharptr,
                              are_equal_func_unsignedcharptr,
                              copy_func_unsignedcharptr,
                              free_func_unsignedcharptr);
    REGISTER_UMOCK_VALUE_TYPE(TEST_STRUCT_COPY_FAILS,
                              stringify_func_TEST_STRUCT_COPY_FAILS,
                              are_equal_func_TEST_STRUCT_COPY_FAILS,
                              copy_func_TEST_STRUCT_COPY_FAILS,
                              free_func_TEST_STRUCT_COPY_FAILS);
    /* Tests_SRS_UMOCK_C_LIB_01_066: [If only the value_type is specified in the macro invocation then the stringify,
     * are_equal, copy and free function names shall be automatically derived from the type as:
     * umockvalue_stringify_value_type, umockvalue_are_equal_value_type, umockvalue_copy_value_type,
     * umockvalue_free_value_type.]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_014: [For each argument the argument value shall be stored for later comparison with actual
 * calls.] */
...
TEST_FUNCTION(
    two_different_STRICT_EXPECTED_CALL_instances_without_an_actual_call_yields_2_missing_calls_with_order_preserved)
{
    // arrange
...
/* Tests_SRS_UMOCK_C_LIB_01_074: [When an expected call is recorded a call modifier interface in the form of a structure
 * containing function pointers shall be returned to the caller.] */
TEST_FUNCTION(STRICT_EXPECTED_CALL_allows_call_modifiers)
{
    // arrange

    // act
    STRICT_EXPECTED_CALL(test_dependency_2_args(42, 43)).ValidateAllArguments();
...
TEST_FUNCTION(STRICT_EXPECTED_CALL_with_ignore_all_arguments_and_then_validate_all_args_still_validates_args)
{
    ...
    // act
    STRICT_EXPECTED_CALL(test_dependency_2_args(42, 43)).IgnoreAllArguments().ValidateAllArguments();
...
TEST_FUNCTION(EXPECTED_CALL_with_validate_all_arguments_and_then_ignore_all_args_still_ignores_args)
{
    ...
    // act
    EXPECTED_CALL(test_dependency_2_args(42, 43)).ValidateAllArguments().IgnoreAllArguments();
...
TEST_FUNCTION(STRICT_EXPECTED_CALL_with_validate_ignore_validate_all_arguments_validates_args)
{
    ...
    // act
    EXPECTED_CALL(test_dependency_2_args(42, 43)).ValidateAllArguments().IgnoreAllArguments().ValidateAllArguments();
...
/* Tests_SRS_UMOCK_C_LIB_01_076: [The IgnoreAllArguments call modifier shall record that for that specific call all
 * arguments will be ignored for that specific call.] */
TEST_FUNCTION(IgnoreAllArguments_ignores_args_on_a_STRICT_EXPECTED_CALL)
{
    // arrange

    // act
    STRICT_EXPECTED_CALL(test_dependency_2_args(42, 43)).IgnoreAllArguments();
...
/* Tests_SRS_UMOCK_C_LIB_01_077: [The ValidateAllArguments call modifier shall record that for that specific call all
...
TEST_FUNCTION(ValidateAllArguments_validates_all_args_on_an_EXPECTED_CALL)
{
    ...
    // act
    EXPECTED_CALL(test_dependency_2_args(42, 43)).ValidateAllArguments();
...
/* Tests_SRS_UMOCK_C_LIB_01_078: [The IgnoreArgument_{arg_name} call modifier shall record that the argument identified
 * by arg_name will be ignored for that specific call.] */
TEST_FUNCTION(IgnoreArgument_by_name_ignores_only_that_argument_on_a_STRICT_EXPECTED_CALL)
{
    // arrange

    // act
    STRICT_EXPECTED_CALL(test_dependency_2_args(42, 43)).IgnoreArgument_a();
...
{
    // arrange
    STRICT_EXPECTED_CALL(test_dependency_2_args(42, 43)).IgnoreArgument_b();
...
/* Tests_SRS_UMOCK_C_LIB_01_079: [The ValidateArgument_{arg_name} call modifier shall record that the argument
 * identified by arg_name will be validated for that specific call.]*/
TEST_FUNCTION(ValidateArgument_by_name_validates_only_that_argument_on_an_EXPECTED_CALL)
{
    // arrange
    EXPECTED_CALL(test_dependency_2_args(42, 43)).ValidateArgument_a();
...
{
    // arrange
    EXPECTED_CALL(test_dependency_2_args(42, 43)).ValidateArgument_b();
...
/* Tests_SRS_UMOCK_C_LIB_01_080: [The IgnoreArgument call modifier shall record that the indexth argument will be
 * ignored for that specific call.]*/
TEST_FUNCTION(IgnoreArgument_by_index_for_first_arg_ignores_the_first_argument)
{
    // arrange
    STRICT_EXPECTED_CALL(test_dependency_2_args(42, 43)).IgnoreArgument(1);
...
{
    // arrange
    STRICT_EXPECTED_CALL(test_dependency_2_args(41, 42)).IgnoreArgument(2);
...
/* Tests_SRS_UMOCK_C_LIB_01_081: [If the index is out of range umock_c shall raise an error with the code
...
TEST_FUNCTION(IgnoreArgument_by_index_with_index_0_triggers_the_on_error_callback)
{
    ...
    // act
    STRICT_EXPECTED_CALL(test_dependency_2_args(41, 42)).IgnoreArgument(0);
...
TEST_FUNCTION(IgnoreArgument_by_index_with_index_greater_than_arg_count_triggers_the_on_error_callback)
{
    ...
    // act
    STRICT_EXPECTED_CALL(test_dependency_2_args(41, 42)).IgnoreArgument(3);
...
/* Tests_SRS_UMOCK_C_LIB_01_082: [The ValidateArgument call modifier shall record that the indexth argument will be
 * validated for that specific call.]*/
TEST_FUNCTION(ValidateArgument_by_index_for_first_arg_ignores_the_first_argument)
{
    // arrange
    EXPECTED_CALL(test_dependency_2_args(42, 43)).ValidateArgument(1);
...
{
    // arrange
    EXPECTED_CALL(test_dependency_2_args(42, 42)).ValidateArgument(2);
...
/* Tests_SRS_UMOCK_C_LIB_01_083: [If the index is out of range umock_c shall raise an error with the code
 * UMOCK_C_ARG_INDEX_OUT_OF_RANGE.]*/
TEST_FUNCTION(ValidateArgument_by_index_with_0_index_triggers_the_on_error_callback)
{
    ...
    // act
    EXPECTED_CALL(test_dependency_2_args(42, 42)).ValidateArgument(0);
...
TEST_FUNCTION(ValidateArgument_by_index_with_index_greater_than_arg_count_triggers_the_on_error_callback)
{
    ...
    // act
    EXPECTED_CALL(test_dependency_2_args(42, 42)).ValidateArgument(3);
...
/* Tests_SRS_UMOCK_C_LIB_01_084: [The SetReturn call modifier shall record that when an actual call is matched with the
 * specific expected call, it shall return the result value to the code under test.] */
TEST_FUNCTION(SetReturn_sets_the_return_value_for_a_strict_expected_call)
{
    // arrange
    STRICT_EXPECTED_CALL(test_dependency_no_args()).SetReturn(42);
...
{
    // arrange
    EXPECTED_CALL(test_dependency_no_args()).SetReturn(42);
...
{
    // arrange
    STRICT_EXPECTED_CALL(test_dependency_1_arg(42)).SetReturn(42);
...
{
    // arrange
    STRICT_EXPECTED_CALL(test_dependency_1_arg(42)).SetReturn(142);
    STRICT_EXPECTED_CALL(test_dependency_1_arg(43)).SetReturn(143);
...
/* Tests_SRS_UMOCK_C_LIB_01_087: [The CopyOutArgumentBuffer call modifier shall copy the memory pointed to by bytes and
 * being length bytes so that it is later injected as an out argument when the code under test calls the mock function.]
 */
...
/* Tests_SRS_UMOCK_C_LIB_01_133: [ If several calls to CopyOutArgumentBuffer are made, only the last buffer shall be
 * kept. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_091: [If the index is out of range umock_c shall raise an error with the code
 * UMOCK_C_ARG_INDEX_OUT_OF_RANGE.]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_092: [If bytes is NULL or length is 0, umock_c shall raise an error with the code
...
TEST_FUNCTION(CopyOutArgumentBuffer_with_NULL_bytes_triggers_the_error_callback)
{
    ...
    // act
    EXPECTED_CALL(test_dependency_1_out_arg(IGNORED_PTR_ARG)).CopyOutArgumentBuffer(1, NULL, sizeof(int));
...
TEST_FUNCTION(CopyOutArgumentBuffer_with_0_length_triggers_the_error_callback)
{
    ...
    // act
    EXPECTED_CALL(test_dependency_1_out_arg(IGNORED_PTR_ARG)).CopyOutArgumentBuffer(1, &injected_int, 0);
...
/* Tests_SRS_UMOCK_C_LIB_01_154: [ The CopyOutArgumentBuffer_{arg_name} call modifier shall copy the memory pointed to
 * by bytes and being length bytes so that it is later injected as an out argument when the code under test calls the
 * mock function. ] */
/* Tests_SRS_UMOCK_C_LIB_01_159: [ The argument targetted by CopyOutArgumentBuffer_{arg_name} shall also be marked as
 * ignored. ] */
...
/* Tests_SRS_UMOCK_C_LIB_01_156: [ If several calls to CopyOutArgumentBuffer are made, only the last buffer shall be
 * kept. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_158: [ If bytes is NULL or length is 0, umock_c shall raise an error with the code
 * UMOCK_C_INVALID_ARGUMENT_BUFFER. ] */
TEST_FUNCTION(CopyOutArgumentBuffer_arg_name_with_NULL_bytes_triggers_the_error_callback)
{
    ...
    // act
    EXPECTED_CALL(test_dependency_1_out_arg(IGNORED_PTR_ARG)).CopyOutArgumentBuffer_a(NULL, sizeof(int));
...
TEST_FUNCTION(CopyOutArgumentBuffer_arg_name_with_0_length_triggers_the_error_callback)
{
    ...
    // act
    EXPECTED_CALL(test_dependency_1_out_arg(IGNORED_PTR_ARG)).CopyOutArgumentBuffer_a(&injected_int, 0);
...
/* Tests_SRS_UMOCK_C_LIB_01_095: [The ValidateArgumentBuffer call modifier shall copy the memory pointed to by bytes and
 * being length bytes so that it is later compared against a pointer type argument when the code under test calls the
 * mock function.] */
/* Tests_SRS_UMOCK_C_LIB_01_096: [If the content of the code under test buffer and the buffer supplied to
 * ValidateArgumentBuffer does not match then this should be treated as a mismatch in argument comparison for that
 * argument.]*/
/* Tests_SRS_UMOCK_C_LIB_01_097: [ValidateArgumentBuffer shall implicitly perform an IgnoreArgument on the indexth
 * argument.]*/
 ...
 * argument.]*/
TEST_FUNCTION(
    ValidateArgumentBuffer_checks_the_argument_buffer_and_mismatch_is_detected_when_content_does_not_match_for_expected_call)
{
    // arrange
    unsigned char expected_buffer[] = {0x42};
    unsigned char actual_buffer[]   = {0x43};
...
/* Tests_SRS_UMOCK_C_LIB_01_099: [If the index is out of range umock_c shall raise an error with the code
 * UMOCK_C_ARG_INDEX_OUT_OF_RANGE.]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_100: [If bytes is NULL or length is 0, umock_c shall raise an error with the code
 * UMOCK_C_INVALID_ARGUMENT_BUFFER.] */
TEST_FUNCTION(ValidateArgumentBuffer_with_NULL_buffer_triggers_the_error_callback)
{
    ...
    // act
    EXPECTED_CALL(test_dependency_buffer_arg(IGNORED_PTR_ARG)).ValidateArgumentBuffer(1, NULL, 1);
...
TEST_FUNCTION(ValidateArgumentBuffer_with_0_length_triggers_the_error_callback)
{
    ...
    // act
    EXPECTED_CALL(test_dependency_buffer_arg(IGNORED_PTR_ARG)).ValidateArgumentBuffer(1, expected_buffer, 0);
...
/* Tests_SRS_UMOCK_C_LIB_01_132: [ If several calls to ValidateArgumentBuffer are made, only the last buffer shall be
 * kept. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_104: [The REGISTER_GLOBAL_MOCK_HOOK shall register a mock hook to be called every time the
 * mocked function is called by production code.]*/
/* Tests_SRS_UMOCK_C_LIB_01_105: [The hook's result shall be returned by the mock to the production code.]*/
/* Tests_SRS_UMOCK_C_LIB_01_106: [The signature for the hook shall be assumed to have exactly the same arguments and
 * return as the mocked function.]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_107: [If there are multiple invocations of REGISTER_GLOBAL_MOCK_HOOK, the last one shall
 * take effect over the previous ones.] */
...
/* Tests_SRS_UMOCK_C_LIB_01_134: [ REGISTER_GLOBAL_MOCK_HOOK called with a NULL hook unregisters a previously registered
 * hook. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_108: [The REGISTER_GLOBAL_MOCK_RETURN shall register a return value to always be returned by
 * a mock function.]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_109: [If there are multiple invocations of REGISTER_GLOBAL_MOCK_RETURN, the last one shall
 * take effect over the previous ones.]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_111: [The REGISTER_GLOBAL_MOCK_FAIL_RETURN shall register a fail return value to be returned
 * by a mock function when marked as failed in the expected calls.]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_112: [If there are multiple invocations of REGISTER_GLOBAL_FAIL_MOCK_RETURN, the last one
 * shall take effect over the previous ones.]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_142: [ If any error occurs during REGISTER_GLOBAL_MOCK_FAIL_RETURN, umock_c shall raise an
 * error with the code UMOCK_C_ERROR. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_113: [The REGISTER_GLOBAL_MOCK_RETURNS shall register both a success and a fail return value
 * associated with a mock function.]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_114: [If there are multiple invocations of REGISTER_GLOBAL_MOCK_RETURNS, the last one shall
 * take effect over the previous ones.]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_145: [ Since umock_c needs to maintain a list of registered types, the following rules shall
 * be applied: ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_144: [ Out of the box umock_c shall support the following types through the header
 * umocktypes_c.h: ]*/
...
{
    // arrange
    STRICT_EXPECTED_CALL(test_dependency_all_types(-42,
                                                   42, /* char */
                                                   -43,
                                                   43, /* short */
                                                   -44,
...
/* Tests_SRS_UMOCK_C_LIB_01_148: [ If call comparison fails an error shall be indicated by calling the error callback
 * with UMOCK_C_COMPARE_CALL_ERROR. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_136: [ When multiple return values are set for a mock function by using different means
 * (such as SetReturn), the following order shall be in effect: ]*/
/* Tests_SRS_UMOCK_C_LIB_01_137: [ - If a return value has been specified for an expected call then that value shall be
 * returned. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_137: [ - If a return value has been specified for an expected call then that value shall be
 * returned. ]*/
/* Tests_SRS_UMOCK_C_LIB_01_138: [ - If a global mock hook has been specified then it shall be called and its result
 * returned. ]*/
TEST_FUNCTION(
    when_the_return_value_is_given_by_SetReturn_for_a_function_with_a_global_return_hook_the_SetReturn_value_is_returned)
{
    REGISTER_GLOBAL_MOCK_HOOK(test_dependency_with_global_mock_hook, my_hook_test_dependency_with_global_mock_hook);

    // arrange
    STRICT_EXPECTED_CALL(test_dependency_with_global_mock_hook()).SetReturn(42);
 ...
 * returned. ]*/
TEST_FUNCTION(
    when_the_return_value_is_not_given_by_SetReturn_for_a_function_with_a_global_return_hook_the_mock_hook_return_value_is_returned)
{
    REGISTER_GLOBAL_MOCK_HOOK(test_dependency_with_global_mock_hook, my_hook_test_dependency_with_global_mock_hook);
...
TEST_FUNCTION(
    when_the_return_value_is_given_by_SetReturn_for_a_function_with_a_global_return_hook_and_global_return_the_SetReturn_value_is_returned)
{
    REGISTER_GLOBAL_MOCK_HOOK(test_dependency_with_global_mock_hook, my_hook_test_dependency_with_global_mock_hook);
...
/* Tests_SRS_UMOCK_C_LIB_01_139: [ - If a global return value has been specified then it shall be returned. ]*/
TEST_FUNCTION(
    when_the_return_value_is_not_given_by_SetReturn_for_a_function_with_a_global_return_hook_and_global_return_the_global_mock_hook_value_is_returned)
{
    REGISTER_GLOBAL_MOCK_HOOK(test_dependency_with_global_mock_hook, my_hook_test_dependency_with_global_mock_hook);
    REGISTER_GLOBAL_MOCK_RETURN(test_dependency_with_global_mock_hook, 44);
...
/* Tests_SRS_UMOCK_C_LIB_01_140: [ - Otherwise the value of a static variable of the same type as the return type shall
 * be returned. ]*/
...
{
    // arrange
    EXPECTED_CALL(test_mock_function_returning_string()).SetReturn("a");
...
TEST_FUNCTION(registering_an_alias_type_works)
{
    ...
    STRICT_EXPECTED_CALL(test_mock_function_with_funkytype(42)).SetReturn(42);
...
/* Tests_SRS_UMOCK_C_LIB_01_153: [ If no custom handler has beed registered for a pointer type, it shall be trated as
 * void*. ] */
...
/* Tests_SRS_UMOCK_C_LIB_01_179: [ The CaptureReturn call modifier shall copy the return value that is being returned to
 * the code under test when an actual call is matched with the expected call. ]*/
TEST_FUNCTION(capture_return_captures_the_return_value)
{
    // arrange
    int captured_return;

    STRICT_EXPECTED_CALL(test_dependency_for_capture_return()).CaptureReturn(&captured_return);
...
/* Tests_SRS_UMOCK_C_LIB_01_180: [ If CaptureReturn is called multiple times for the same call, an error shall be
 * indicated with the code UMOCK_C_CAPTURE_RETURN_ALREADY_USED. ]*/
TEST_FUNCTION(capture_return_twice_captures_the_return_value_in_the_pointer_indicated_by_the_second_call)
{
    ...
    // assert
    // TFS661968 ASSERT_ARE_EQUAL(size_t, 1, test_on_umock_c_error_call_count);
    // TFS661968 ASSERT_ARE_EQUAL(UMOCK_C_ERROR_CODE, UMOCK_C_CAPTURE_RETURN_ALREADY_USED,
    // test_on_umock_c_error_calls[0].error_code);
}

/* Tests_SRS_UMOCK_C_LIB_01_182: [ If captured_return_value is NULL, umock_c shall raise an error with the code
 * UMOCK_C_NULL_ARGUMENT. ]*/
TEST_FUNCTION(capture_return_with_NULL_argument_indicates_an_error)
{
    // arrange
    STRICT_EXPECTED_CALL(test_dependency_for_capture_return()).CaptureReturn(NULL);
...
TEST_FUNCTION(capture_return_when_no_matching_actual_call_does_not_capture_anything)
{
    ...
    STRICT_EXPECTED_CALL(test_dependency_for_capture_return_with_arg(42)).CaptureReturn(&captured_return);
...
TEST_FUNCTION(capture_return_takes_into_account_a_set_return_call)
{
    ...
    STRICT_EXPECTED_CALL(test_dependency_for_capture_return()).SetReturn(42).CaptureReturn(&captured_return);
...
/* Tests_SRS_UMOCK_C_LIB_01_183: [ The ValidateArgumentValue_{arg_name} shall validate that the value of an argument
 * matches the value pointed by arg_value. ]*/
TEST_FUNCTION(validate_argument_value_validates_the_value_pointed_by_arg_value)
{
    // arrange
    int arg_value = 0;

    STRICT_EXPECTED_CALL(test_dependency_1_arg(0)).ValidateArgumentValue_a(&arg_value);
...
TEST_FUNCTION(validate_argument_value_validates_the_value_pointed_by_arg_value_for_a_char_star)
{
    ...
    STRICT_EXPECTED_CALL(test_dependency_char_star_arg(NULL)).ValidateArgumentValue_s(&arg_value);
...
/* Tests_SRS_UMOCK_C_LIB_01_184: [ If arg_value is NULL, umock_c shall raise an error with the code
 * UMOCK_C_NULL_ARGUMENT. ]*/
TEST_FUNCTION(validate_argument_value_with_NULL_value_triggers_an_error)
{
    ...
    // act
    STRICT_EXPECTED_CALL(test_dependency_char_star_arg(NULL)).ValidateArgumentValue_s(NULL);
...
/* Tests_SRS_UMOCK_C_LIB_01_185: [ The ValidateArgumentValue_{arg_name} modifier shall inhibit comparing with any value
 * passed directly as an argument in the expected call. ]*/
TEST_FUNCTION(validate_argument_value_overrides_existing_arg_value)
{
    // arrange
    char *arg_value = "42";

    STRICT_EXPECTED_CALL(test_dependency_char_star_arg("42")).ValidateArgumentValue_s(&arg_value);
...
/* Tests_SRS_UMOCK_C_LIB_01_186: [ The ValidateArgumentValue_{arg_name} shall implicitly do a ValidateArgument for the
 * arg_name argument, making sure the argument is not ignored. ]*/
TEST_FUNCTION(validate_argument_value_shall_implicitly_validate_the_argument)
{
    // arrange
    char *arg_value = "42";

    STRICT_EXPECTED_CALL(test_dependency_char_star_arg("42")).IgnoreArgument_s().ValidateArgumentValue_s(&arg_value);
...
/* Tests_SRS_UMOCK_C_LIB_01_187: [ REGISTER_UMOCKC_PAIRED_CREATE_DESTROY_CALLS shall register with umock two calls that
 * are expected to be paired. ]*/
/* Tests_SRS_UMOCK_C_LIB_01_188: [ The create call shall have a non-void return type. ]*/
/* Tests_SRS_UMOCK_C_LIB_01_191: [ At each create_call a memory block shall be allocated so that it can be reported as a
 * leak by any memory checker. ]*/
/* Tests_SRS_UMOCK_C_LIB_01_193: [ When a destroy_call happens the memory block associated with the argument passed to
 * it shall be freed. ] */
...
/* Tests_SRS_UMOCK_C_LIB_01_190: [ If create_call or destroy_call do not obey these rules, at the time of calling
 * REGISTER_UMOCKC_PAIRED_CREATE_DESTROY_CALLS umock_c shall raise an error with the code UMOCK_C_INVALID_PAIRED_CALLS.
 * ]*/
/* Tests_SRS_UMOCK_C_LIB_01_188: [ The create call shall have a non-void return type. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_189: [ The destroy call shall take as argument at least one argument. The type of the first
 * argument shall be of the same type as the return type for the create_call. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_196: [ The type used for the return of create_call and first argument of destroy_call shall
 * be allowed to be any type registered with umock. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_199: [ `ValidateArgumentValue_{arg_name}_AsType` shall ensure that validation of the
 * argument `arg_name` is done as if the argument is of type `type_name`. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_200: [ If `type_name` is NULL, umock_c shall raise an error with the code
 * UMOCK_C_NULL_ARGUMENT. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_203: [ If `ValidateArgumentValue_{arg_name}_AsType` is used multiple times on the same
 * argument, the last call shall apply. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_205: [ If `IGNORED_PTR_ARG` or `IGNORED_NUM_ARG` is used as an argument value with
 * `STRICT_EXPECTED_CALL`, the argument shall be automatically ignored. ]*/
/* Tests_SRS_UMOCK_C_LIB_01_207: [ `IGNORED_NUM_ARG` shall be defined to 0 so that it can be used for numeric type
 * arguments. ]*/
...
/* Tests_SRS_UMOCK_C_LIB_01_205: [ If `IGNORED_PTR_ARG` or `IGNORED_NUM_ARG` is used as an argument value with
 * `STRICT_EXPECTED_CALL`, the argument shall be automatically ignored. ]*/
/* Tests_SRS_UMOCK_C_LIB_01_206: [ `IGNORED_PTR_ARG` shall be defined as NULL so that it can be used for pointer type
 * arguments. ]*/
...
TEST_FUNCTION(auto_ignore_when_first_arg_is_a_struct_succeeds_for_2nd_arg)
{
    ...
    // arrange
    STRICT_EXPECTED_CALL(
        test_dependency_struct_with_2_members((struct TEST_STRUCT_WITH_2_MEMBERS_TAG){2, 3}, IGNORED_NUM_ARG));
...
/* Tests_SRS_UMOCK_C_LIB_01_101: [The IgnoreAllCalls call modifier shall record that all calls matching the expected
 * call shall be ignored. ]*/
...
{
    // arrange
    STRICT_EXPECTED_CALL(test_dependency_no_args()).IgnoreAllCalls();
...
/* Tests_SRS_UMOCK_C_LIB_01_102: [If multiple matching actual calls occur no unexpected calls shall be reported.]*/
/* Tests_SRS_UMOCK_C_LIB_01_103: [The call matching shall be done taking into account arguments and call modifiers
 * referring to arguments.]*/
TEST_FUNCTION(IgnoreAllCalls_ignores_2_calls_with_matching_1_arg)
{
    // arrange
    STRICT_EXPECTED_CALL(test_dependency_1_arg(42)).IgnoreAllCalls();
