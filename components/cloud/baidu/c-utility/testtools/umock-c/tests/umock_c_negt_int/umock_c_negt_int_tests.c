...
/* Tests_SRS_UMOCK_C_LIB_01_167: [ umock_c_negative_tests_snapshot shall take a snapshot of the current setup of
 * expected calls (a.k.a happy path). ]*/
/* Tests_SRS_UMOCK_C_LIB_01_170: [ umock_c_negative_tests_reset shall bring umock_c expected and actual calls to the
 * state recorded when umock_c_negative_tests_snapshot was called. ]*/
/* Tests_SRS_UMOCK_C_LIB_01_173: [ umock_c_negative_tests_fail_call shall instruct the negative tests module to fail a
 * specific call. ]*/
/* Tests_SRS_UMOCK_C_LIB_01_176: [ umock_c_negative_tests_call_count shall provide the number of expected calls, so that
 * the test code can iterate through all negative cases. ]*/
TEST_FUNCTION(negative_tests_with_one_call)
{
    size_t i;
    STRICT_EXPECTED_CALL(function_1()).SetReturn(0).SetFailReturn(1);
...
TEST_FUNCTION(negative_tests_with_2_calls)
{
    size_t i;
    STRICT_EXPECTED_CALL(function_1()).SetReturn(0).SetFailReturn(1);
    STRICT_EXPECTED_CALL(function_2()).SetReturn(0).SetFailReturn(1);
    umock_c_negative_tests_snapshot();
...
{
    size_t i;
    STRICT_EXPECTED_CALL(function_4_void_ptr_return_non_NULL(IGNORED_PTR_ARG)).SetFailReturn(NULL).IgnoreArgument(1);
    umock_c_negative_tests_snapshot();
...
/* Tests_SRS_UMOCK_C_LIB_01_204: [ Tracking of paired calls shall not be done if the actual call to the `create_call` is
 * using the `SetFailReturn` call modifier. ]*/
TEST_FUNCTION(SetFailReturn_suppresses_paired_calls_tracking)
{
    ...
    STRICT_EXPECTED_CALL(some_create(42)).SetFailReturn(NULL);
...
TEST_FUNCTION(SetFailReturn_suppresses_paired_calls_tracking_for_mockable_functions)
{
    ...
    STRICT_EXPECTED_CALL(some_other_create(42)).SetFailReturn(NULL);
