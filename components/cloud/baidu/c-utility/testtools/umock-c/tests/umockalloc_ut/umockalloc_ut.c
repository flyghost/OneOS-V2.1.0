...
{
    test_realloc_CALL *new_calls =
        (test_realloc_CALL *)realloc(test_realloc_calls, sizeof(test_realloc_CALL) * (test_realloc_call_count + 1));
    if (new_calls != NULL)
    {
...
{
    test_free_CALL *new_calls =
        (test_free_CALL *)realloc(test_free_calls, sizeof(test_free_CALL) * (test_free_call_count + 1));
    if (new_calls != NULL)
    {
...
/* Tests_SRS_UMOCKALLOC_01_003: [ umockalloc_realloc shall call realloc, while passing the ptr and size arguments to
 * realloc. ] */
