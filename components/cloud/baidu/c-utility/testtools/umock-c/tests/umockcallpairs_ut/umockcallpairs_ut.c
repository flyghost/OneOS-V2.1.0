...
int umocktypes_are_equal(const char *type, const void *left, const void *right)
{
    ...
    umocktypes_are_equal_CALL *new_calls =
        (umocktypes_are_equal_CALL *)realloc(umocktypes_are_equal_calls,
                                             sizeof(umocktypes_are_equal_CALL)  *(umocktypes_are_equal_call_count + 1));
    if (new_calls != NULL)
    {
...
int umocktypes_copy(const char *type, void *destination, const void *source)
{
    ...
    umocktypes_copy_CALL *new_calls =
        (umocktypes_copy_CALL *)realloc(umocktypes_copy_calls,
                                        sizeof(umocktypes_copy_CALL)  *(umocktypes_copy_call_count + 1));
    if (new_calls != NULL)
    {
        size_t typename_length                                 = strlen(type);
...
{
    umocktypes_free_CALL *new_calls =
        (umocktypes_free_CALL *)realloc(umocktypes_free_calls,
                                        sizeof(umocktypes_free_CALL)  *(umocktypes_free_call_count + 1));
    if (new_calls != NULL)
    {
        size_t typename_length                                 = strlen(type);
...
/* Tests_SRS_UMOCKCALLPAIRS_01_001: [ umockcallpairs_track_create_paired_call shall add a new entry to the
 * PAIRED_HANDLES array and on success it shall return 0. ]*/
/* Tests_SRS_UMOCKCALLPAIRS_01_002: [ umockcallpairs_track_create_paired_call shall copy the handle_value to the
 * handle_value member of the new entry. ] */
/* Tests_SRS_UMOCKCALLPAIRS_01_003: [ umockcallpairs_track_create_paired_call shall allocate a memory block and store a
 * pointer to it in the memory field of the new entry. ]*/
...
/* Tests_SRS_UMOCKCALLPAIRS_01_004: [ If any of the arguments paired_handles, handle or handle_type is NULL,
...
/* Tests_SRS_UMOCKCALLPAIRS_01_005: [ If allocating memory fails, umockcallpairs_track_create_paired_call shall fail and
...
/* Tests_SRS_UMOCKCALLPAIRS_01_007: [ If umocktypes_copy fails, umockcallpairs_track_create_paired_call shall fail and
 * return a non-zero value. ]*/
...
/* Tests_SRS_UMOCKCALLPAIRS_01_008: [ umockcallpairs_track_destroy_paired_call shall remove from the paired handles
 * array pointed by the paired_handles field the entry that is associated with the handle passed in the handle argument.
 * ]*/
/* Tests_SRS_UMOCKCALLPAIRS_01_009: [ On success umockcallpairs_track_destroy_paired_call shall return 0. ]*/
/* Tests_SRS_UMOCKCALLPAIRS_01_011: [ umockcallpairs_track_destroy_paired_call shall free the memory pointed by the
 * memory field in the PAIRED_HANDLES array entry associated with handle. ]*/
/* Tests_SRS_UMOCKCALLPAIRS_01_012: [ If the array paired handles array is empty after removing the entry, the
 * paired_handles field shall be freed and set to NULL. ]*/
/* Tests_SRS_UMOCKCALLPAIRS_01_013: [ When looking up which entry to remove, the comparison of the handle values shall
 * be done by calling umocktypes_are_equal. ]*/
...
/* Tests_SRS_UMOCKCALLPAIRS_01_010: [ If any of the arguments is NULL, umockcallpairs_track_destroy_paired_call shall
...
/* Tests_SRS_UMOCKCALLPAIRS_01_014: [ If umocktypes_are_equal fails, umockcallpairs_track_destroy_paired_call shall fail
...
/* Tests_SRS_UMOCKCALLPAIRS_01_015: [ If the handle is not found in the array then
