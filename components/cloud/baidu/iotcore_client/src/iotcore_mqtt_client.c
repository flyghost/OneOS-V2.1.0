...
static void on_mqtt_operation_complete(MQTT_CLIENT_HANDLE       handle,
                                       MQTT_CLIENT_EVENT_RESULT action_result,
                                       const void              *msg_info,
                                       void                    *callback_ctx)
{
    (void)msg_info;
    ...
    case MQTT_CLIENT_ON_CONNACK:
    {
        ...
        if (connack != NULL)
        {
            ...
            else
            {
                ...
                LogError("Connection Not Accepted: 0x%x: %s",
                         connack->returnCode,
                         retrieve_mqtt_return_codes(connack->returnCode));
    ...
    case MQTT_CLIENT_ON_SUBSCRIBE_ACK:
    {
        ...
        if (suback != NULL)
        {
            ...
            {
                PMQTT_SUB_CALLBACK_INFO _sub_handle_entry =
                ...
                if (suback->packetId == _sub_handle_entry->packet_id)
                {
                    (void)DList_RemoveEntryList(
                        _current_list_entry);    // First remove the item from Waiting for Ack List.
                    _sub_handle_entry->sub_callback((IOTCORE_MQTT_QOS *)(suback->qosReturn),
                                                    suback->qosCount,
                                                    _sub_handle_entry->context);
    ...
    case MQTT_CLIENT_ON_PUBLISH_ACK:
    {
        ...
        if (puback != NULL)
        {
            ...
            {
                PMQTT_PUB_CALLBACK_INFO _mqtt_msg_entry =
                    containingRecord(_current_list_entry, MQTT_PUB_CALLBACK_INFO, entry);
                DLIST_ENTRY _save_list_entry;
                _save_list_entry.Flink = _current_list_entry->Flink;
                         ...
                         void                      *context)
{
    ...
    MQTT_MESSAGE_HANDLE mqtt_get_msg =
        mqttmessage_create(packet_id, pub_topic_name, (QOS_VALUE)qos_value, pub_msg, pub_msg_length);
...
int unsubscribe_mqtt_topics(IOTCORE_MQTT_CLIENT_HANDLE iotcore_client, const char *unsubscribe)
{
    ...
    do
    {
        ...
        if (mqtt_client_unsubscribe(iotcore_client->mqtt_client,
                                    get_next_packet_id(iotcore_client),
                                    unsubscribe_list,
                                    1))
        {
...
static MQTT_CLIENT_OPTIONS *_construct_mqtt_options(const char      *client_id,
                                                    const char      *username,
                                                    const char      *passwd,
...
static XIO_HANDLE create_tls_connection(const char *endpoint, const char *certificates)
{
...
static XIO_HANDLE create_mutual_tls_connection(const char *endpoint,
                                               const char *certificates,
                                               const char *client_cert_in_option,
                                               const char *client_key_in_option)
{
    TLSIO_CONFIG tlsio_config = {endpoint, MQTT_CONNECTION_TLS_PORT};
    // enable wolfssl by set certificates
    // tlsio_config.certificate = certificates;
...
static int send_mqtt_connect_message(IOTCORE_MQTT_CLIENT_HANDLE iotcore_client)
{
    ...
    if (!iotcore_client->xio_transport)
    {
        ...
        case MQTT_CONNECTION_TLS:
            iotcore_client->xio_transport =
                create_tls_connection(iotcore_client->endpoint, iotcore_client->certificates);
            break;
        case MQTT_CONNECTION_MUTUAL_TLS:
            iotcore_client->xio_transport = create_mutual_tls_connection(iotcore_client->endpoint,
                                                                         iotcore_client->certificates,
    ...
    {
        if (mqtt_client_connect(iotcore_client->mqtt_client, iotcore_client->xio_transport, iotcore_client->options) !=
            0)
        {
...
static iot_bool _is_iotcore_connect_param_valid(const IOTCORE_CONNECT_MQTT_PARAM *param)
{
    ...
    if (param && param->broker_addr && param->broker_addr[0] && param->client_id && param->client_id[0] &&
        param->passwd && param->passwd[0] && param->user_name && param->user_name[0])
    {
        _is_valid = IOT_TRUE;
    }
    return _is_valid;
...
static void on_recv_callback(MQTT_MESSAGE_HANDLE msg_handle, void *context)
{
    ...
    _client_handle->recv_callback(mqttmessage_getPacketId(msg_handle),
                                                         ...
                                                         size_t               retry_timeout_limit_in_sec)
{
    ...
    _iotcore_client->options = _construct_mqtt_options(param.client_id,
                                                       param.user_name,
                                                       param.passwd,
                                                       will_topic,
                                                       will_payload,
                                                       ...
                                                       false,
                                                       QOS_1_AT_LEAST_ONCE);
    free(param.client_id);
    free(param.user_name);
    free(param.passwd);
    ...
    _iotcore_client->mqtt_client = mqtt_client_init(on_recv_callback,
                                                    on_mqtt_operation_complete,
                                                    _iotcore_client,
                                                    on_mqtt_error_complete,
                                                    _iotcore_client);
...
{
    iotcore_client->certificates = (char *)certificates;
}
...
static int initialize_mqtt_connection(IOTCORE_MQTT_CLIENT_HANDLE iotcore_client)
{
    int _result = 0;
    ...
    if (!iotcore_client->mqtt_client_status.is_destroy_called)
    {
            ...
            IOT_TRUE == can_retry(iotcore_client->retry_logic))
        {
            ...
            else
            {
                    ...
                    iotcore_client->mqtt_client_status.is_recoverable_error)
                {
                    ...
                    iotcore_client->mqtt_client = mqtt_client_init(on_recv_callback,
                                                                   on_mqtt_operation_complete,
                                                                   iotcore_client,
                                                                   on_mqtt_error_complete,
                                                                   iotcore_client);
        ...
        // Codes_SRS_IOTCORE_TRANSPORT_MQTT_COMMON_09_001: [ iotcoreTransport_MQTT_Common_DoWork shall trigger
        // reconnection if the mqtt_client_connect does not complete within `keepalive` seconds]
        else if (iotcore_client->mqtt_client_status.connect_status == MQTT_CLIENT_CONNECT_STATUS_CONNECTING)
        {
            ...
            else if ((current_time - iotcore_client->mqtt_connect_time) / 1000 >
                     iotcore_client->options->keepAliveInterval)
            {
...
int iotcore_mqtt_doconnect(IOTCORE_MQTT_CLIENT_HANDLE iotcore_client, size_t timeout)
{
    ...
    do
    {
        ...
        tickcounter_get_current_ms(iotcore_client->msg_tick_counter, &currentTime);
             ...
             iotcore_client->mqtt_client_status.is_recoverable_error && (currentTime - lastSendTime) / 1000 <= timeout);
