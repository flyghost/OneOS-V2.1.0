...
#ifndef INFINITY
#define INFINITY                                                                                                       \
    ((float)(_HUGE_ENUF * _HUGE_ENUF)) /* causes warning C4756: overflow in constant arithmetic (by design) */
#endif                                 /* INFINITY */
...
AGENT_DATA_TYPES_RESULT Create_EDM_BOOLEAN_from_int(AGENT_DATA_TYPE *agentData, int v)
{
    ...
    /*Codes_SRS_AGENT_TYPE_SYSTEM_99_013:[All the Create_... functions shall check their parameters for validity.When an
     * invalid parameter is detected, a code of AGENT_DATA_TYPES_INVALID_ARG shall be returned]*/
    if (agentData == NULL)
    {
...
{
    AGENT_DATA_TYPES_RESULT result;
    /*Codes_SRS_AGENT_TYPE_SYSTEM_99_013:[ All the Create_... functions shall check their parameters for validity. When
     * an invalid parameter is detected, a code of AGENT_DATA_TYPES_INVALID_ARG shall be returned ].*/
    if (agentData == NULL)
    {
        result = AGENT_DATA_TYPES_INVALID_ARG;
        LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
...
/*Codes_SRS_AGENT_TYPE_SYSTEM_99_091:[Creates an AGENT_DATA_TYPE containing an Edm.DateTimeOffset from an
 * EDM_DATE_TIME_OFFSET.]*/
AGENT_DATA_TYPES_RESULT Create_AGENT_DATA_TYPE_from_EDM_DATE_TIME_OFFSET(AGENT_DATA_TYPE     *agentData,
                                                                         EDM_DATE_TIME_OFFSET v)
{
    AGENT_DATA_TYPES_RESULT result;
    /*Codes_SRS_AGENT_TYPE_SYSTEM_99_013:[ All the Create_... functions shall check their parameters for validity. When
     * an invalid parameter is detected, a code of AGENT_DATA_TYPES_INVALID_ARG shall be returned ]*/
    ...
    {
        /*Codes_SRS_AGENT_TYPE_SYSTEM_99_092:[ The structure shall be validated to be conforming to OData specifications
         * (odata-abnf-construction-rules, 2013), and if found invalid, AGENT_DATA_TYPES_INVALID_ARG shall be
         * returned.]*/
        result = AGENT_DATA_TYPES_INVALID_ARG;
    }
    else if ((v.dateTime.tm_hour > 23) || (v.dateTime.tm_hour < 0) || (v.dateTime.tm_min > 59) ||
             (v.dateTime.tm_min < 0) || (v.dateTime.tm_sec > 59) || (v.dateTime.tm_sec < 0))
    {
        /*Codes_SRS_AGENT_TYPE_SYSTEM_99_092:[ The structure shall be validated, and if found invalid,
         * AGENT_DATA_TYPES_INVALID_ARG shall be returned.]*/
        result = AGENT_DATA_TYPES_INVALID_ARG;
    }
    ...
    else if ((v.hasTimeZone) && ((v.timeZoneHour < -23) || (v.timeZoneHour > 23) || (v.timeZoneMinute > 59)))
    {
        /*Codes_SRS_AGENT_TYPE_SYSTEM_99_092:[ The structure shall be validated to be conforming to OData specifications
         * (odata-abnf-construction-rules, 2013), and if found invalid, AGENT_DATA_TYPES_INVALID_ARG shall be
         * returned.]*/
...
{
    AGENT_DATA_TYPES_RESULT result;
    /*Codes_SRS_AGENT_TYPE_SYSTEM_99_013:[ All the functions shall check their parameters for validity. When an invalid
     * parameter is detected, the value AGENT_DATA_TYPES_INVALID_ARG shall be returned ].*/
    if (agentData == NULL)
    {
        result = AGENT_DATA_TYPES_INVALID_ARG;
        LogError("result = %s ", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
...
static int scanAndReadNDigitsInt(const char *source, size_t *position, int *value, size_t N)
{
    N++;
    *value = 0;
    while ((IS_DIGIT(source[*position])) && (N > 0))
    {
        *value *= 10;
...
static int ValidateDecimal(const char *v, size_t vlen)
{
    ...
    else
    {
        ...
        {
            /*Codes_SRS_AGENT_TYPE_SYSTEM_99_067:[ If the string indicated by the parameter v does not match exactly an
             * ODATA string representation, AGENT_DATA_TYPES_INVALID_ARG shall be returned.]*/
            result = 1;
        }
...
static int scan4base64char(const char    *source,
                           size_t         sourceSize,
                           ...
                           unsigned char *destination2)
{
    ...
    else
    {
        ...
        if ((base64toValue(source[0], &b0) == 0) && (base64toValue(source[1], &b1) == 0) &&
            (base64toValue(source[2], &b2) == 0) && (base64toValue(source[3], &b3) == 0))
        {
...
/*return 0 if the character is one of ( 'A' / 'E' / 'I' / 'M' / 'Q' / 'U' / 'Y' / 'c' / 'g' / 'k' / 'o' / 's' / 'w' /
 * '0' / '4' / '8' )*/
...
int scanbase64b16(const char    *source,
                  size_t         sourceSize,
                  size_t        *consumed,
                  unsigned char *destination0,
                  unsigned char *destination1)
{
    ...
    else
    {
        ...
        *consumed = 0;
        if ((base64toValue(source[0], &c0) == 0) && (base64toValue(source[1], &c1) == 0) &&
...
int scanbase64b8(const char *source, size_t sourceSize, size_t *consumed, unsigned char *destination0)
{
    ...
    {
        unsigned char c0, c1;
        if ((base64toValue(source[0], &c0) == 0) && (base64b8toValue(source[1], &c1) == 0))
        {
            *consumed     = 2 + (((sourceSize >= 4) && (source[2] == '=') && (source[3] == '='))
                                 ? 2
                                 : 0); /*== produce 1 or 0 ( in C )*/
...
/*Codes_SRS_AGENT_TYPE_SYSTEM_99_039:[ Creates an AGENT_DATA_TYPE containing an EDM_DECIMAL from a null-terminated
 * string.]*/
AGENT_DATA_TYPES_RESULT Create_EDM_DECIMAL_from_charz(AGENT_DATA_TYPE *agentData, const char *v)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                /*Codes_SRS_AGENT_TYPE_SYSTEM_99_067:[ If the string indicated by the parameter v does not match exactly
                 * an ODATA string representation, AGENT_DATA_TYPES_INVALID_ARG shall be returned.]*/
                result = AGENT_DATA_TYPES_INVALID_ARG;
                LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
            }
...
static int ValidateDate(int year, int month, int day)
{
    ...
    {
        /*Codes_SRS_AGENT_TYPE_SYSTEM_99_070:[ If year-month-date does not indicate a valid day (for example 31 Feb
         * 2013), then AGENT_DATA_TYPES_INVALID_ARG shall be returned.]*/
        if (day <= 0)
        {
            result = 1;
        }
        else
        {
            ...
            /*leap years are those that can be divided by 4. But if the year can be divided by 100, it is not leap. But
             * if they year can be divided by 400 it is leap*/
            if ((month == 1) || /*these months have always 31 days*/
                (month == 3) || (month == 5) || (month == 7) || (month == 8) || (month == 10) || (month == 12))
            {
                ...
                {
                    /*Codes_SRS_AGENT_TYPE_SYSTEM_99_070:[If year - month - date does not indicate a valid day(for
                     * example 31 Feb 2013), then AGENT_DATA_TYPES_INVALID_ARG shall be returned.]*/
                    result = 1;
                }
            }
            ...
            {
                /*Codes_SRS_AGENT_TYPE_SYSTEM_99_070:[If year - month - date does not indicate a valid day(for example
                 * 31 Feb 2013), then AGENT_DATA_TYPES_INVALID_ARG shall be returned.]*/
                result = 1;
            }
        }
    }
...
/*Codes_SRS_AGENT_TYPE_SYSTEM_99_069:[ Creates an AGENT_DATA_TYPE containing an EDM_DATE from a year, a month and a day
 * of the month.]*/
AGENT_DATA_TYPES_RESULT
Create_AGENT_DATA_TYPE_from_date(AGENT_DATA_TYPE *agentData, int16_t year, uint8_t month, uint8_t day)
{
    AGENT_DATA_TYPES_RESULT result;
    /*Codes_SRS_AGENT_TYPE_SYSTEM_99_013:[ All the Create_... functions shall check their parameters for validity. When
     * an invalid parameter is detected, a code of AGENT_DATA_TYPES_INVALID_ARG shall be returned ].*/
    ...
    {
        /*Codes_SRS_AGENT_TYPE_SYSTEM_99_069:[ Creates an AGENT_DATA_TYPE containing an EDM_DATE from a year, a month
...
AGENT_DATA_TYPES_RESULT Create_AGENT_DATA_TYPE_from_charz(AGENT_DATA_TYPE *agentData, const char *v)
{
    ...
    {
        /*Codes_SRS_AGENT_TYPE_SYSTEM_99_049:[ Creates an AGENT_DATA_TYPE containing an EDM_STRING from an ASCII zero
         * terminated string.]*/
...
AGENT_DATA_TYPES_RESULT Create_AGENT_DATA_TYPE_from_charz_no_quotes(AGENT_DATA_TYPE *agentData, const char *v)
{
    ...
    {
        /* Codes_SRS_AGENT_TYPE_SYSTEM_01_001: [Creates an AGENT_DATA_TYPE containing an EDM_STRING from an ASCII zero
         * terminated string.] */
...
void Destroy_AGENT_DATA_TYPE(AGENT_DATA_TYPE *agentData)
{
    ...
    else
    {
        ...
        {
            /*Codes_SRS_AGENT_TYPE_SYSTEM_99_050:[Destroy_AGENT_DATA_TYPE shall deallocate all allocated resources used
...
AGENT_DATA_TYPES_RESULT AgentDataTypes_ToString(STRING_HANDLE destination, const AGENT_DATA_TYPE *value)
{
    ...
    /*Codes_SRS_AGENT_TYPE_SYSTEM_99_015:[If destination parameter is NULL, AgentDataTypes_ToString shall return
    ...
    /*Codes_SRS_AGENT_TYPE_SYSTEM_99_053:[ If value is NULL or has been destroyed or otherwise doesn't contain valid
     * data, AGENT_DATA_TYPES_INVALID_ARG shall be returned.]*/
    else if (value == NULL)
    {
        result = AGENT_DATA_TYPES_INVALID_ARG;
        LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
    ...
    else
    {
        ...
        case (EDM_NULL_TYPE):
        {
            ...
            {
                /*Codes_SRS_AGENT_TYPE_SYSTEM_99_016:[ When the value cannot be converted to a string
                 * AgentDataTypes_ToString shall return AGENT_DATA_TYPES_ERROR.]*/
                result = AGENT_DATA_TYPES_ERROR;
                LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
            }
            else
            {
                /*Codes_SRS_AGENT_TYPE_SYSTEM_99_014:[ All functions shall return AGENT_DATA_TYPES_OK when the
        ...
        case (EDM_BOOLEAN_TYPE):
        {
            /*Codes_SRS_AGENT_TYPE_SYSTEM_99_017:[EDM_BOOLEAN:as in(odata - abnf - construction - rules, 2013),
             * booleanValue = "true" / "false"]*/
            if (value->value.edmBoolean.value == EDM_TRUE)
            {
                /*SRS_AGENT_TYPE_SYSTEM_99_030:[If v is different than 0 then the AGENT_DATA_TYPE shall have the value
                 * "true".]*/
            ...
            {
                /*Codes_SRS_AGENT_TYPE_SYSTEM_99_029:[ If v 0 then the AGENT_DATA_TYPE shall have the value "false"
                 * Boolean.]*/
            ...
            {
                /*Codes_SRS_AGENT_TYPE_SYSTEM_99_053:[ If value contains invalid data, AgentDataTypes_ToString shall
        ...
        case (EDM_BYTE_TYPE):
        {
            ...
            size_t pos = 0;
            if (value->value.edmByte.value >= 100)
                tempbuffer2[pos++] = '0' + (value->value.edmByte.value / 100);
            if (value->value.edmByte.value >= 10)
                tempbuffer2[pos++] = '0' + (value->value.edmByte.value % 100) / 10;
        ...
        case (EDM_DATE_TIME_OFFSET_TYPE):
        {
            /*Codes_SRS_AGENT_TYPE_SYSTEM_99_019:[ EDM_DATETIMEOFFSET: dateTimeOffsetValue = year "-" month "-" day "T"
             * hour ":" minute [ ":" second [ "." fractionalSeconds ] ] ( "Z" / sign hour ":" minute )]*/
            ...
            if (value->value.edmDateTimeOffset.hasTimeZone)
            {
                if (value->value.edmDateTimeOffset.hasFractionalSecond)
                {
                    ...
                    {
                        if (sprintf_s(tempBuffer,
                                      tempBufferSize,
                                      "\"%.4d-%.2d-%.2dT%.2d:%.2d:%.2d.%.12llu%+.2d:%.2d\"", /*+ in printf forces the
                                                                                                sign to appear*/
                ...
                else
                {
                    ...
                    else
                    {
                        if (sprintf_s(
                                tempBuffer,
                                tempBufferSize,
                                "\"%.4d-%.2d-%.2dT%.2d:%.2d:%.2d%+.2d:%.2d\"", /*+ in printf forces the sign to appear*/
                                value->value.edmDateTimeOffset.dateTime.tm_year + 1900,
                                value->value.edmDateTimeOffset.dateTime.tm_mon + 1,
                                value->value.edmDateTimeOffset.dateTime.tm_mday,
                                value->value.edmDateTimeOffset.dateTime.tm_hour,
                ...
                if (value->value.edmDateTimeOffset.hasFractionalSecond)
                {
                    ...
                    else
                    {
                        if (sprintf_s(
                                tempBuffer,
                                tempBufferSize,
                                "\"%.4d-%.2d-%.2dT%.2d:%.2d:%.2d.%.12lluZ\"", /*+ in printf forces the sign to appear*/
                                value->value.edmDateTimeOffset.dateTime.tm_year + 1900,
                                value->value.edmDateTimeOffset.dateTime.tm_mon + 1,
                                value->value.edmDateTimeOffset.dateTime.tm_mday,
                                value->value.edmDateTimeOffset.dateTime.tm_hour,
        ...
        case (EDM_SBYTE_TYPE):
        {
            ...
            size_t pos      = 0;
            /*Codes_SRS_AGENT_TYPE_SYSTEM_99_026:[ EDM_SBYTE: sbyteValue = [ sign ] 1*3DIGIT  ; numbers in the range
             * from -128 to 127]*/
            ...
            if (absValue >= 100)
                tempbuffer2[pos++] = '0' + (char)(absValue / 100);
            if (absValue >= 10)
                tempbuffer2[pos++] = '0' + (absValue % 100) / 10;
        ...
        case (EDM_STRING_TYPE):
        {
            ...
            size_t nControlCharacters =
                0; /*counts how many characters are to be expanded from 1 character to \uxxxx (6 characters)*/
...
#ifdef _MSC_VER
#pragma warning(suppress : 6386) /* The test Create_AGENT_DATA_TYPE_from_charz_With_2_Slashes_Succeeds verifies that   \
                                    Code Analysis is wrong here */
#endif
                    tempBuffer[w] = '"';
                    /*zero terminating it*/
        ...
        /* Codes_SRS_AGENT_TYPE_SYSTEM_01_003: [EDM_STRING_no_quotes: the string is copied as given when the
         * AGENT_DATA_TYPE was created.] */
        case (EDM_STRING_NO_QUOTES_TYPE):
        {
        ...
        case (EDM_SINGLE_TYPE):
        {
            /*C89 standard says: When a float is promoted to double or long double, or a double is promoted to long
             * double, its value is unchanged*/
            /*I read that as : when a float is NaN or Inf, it will stay NaN or INF in double representation*/

            /*The sprintf function returns the number of characters written in the array, not counting the terminating
             * null character.*/
            ...
            else
            {
                ...
                {
                    if (sprintf_s(tempBuffer, tempBufferSize, "%.*f", FLT_DIG, (double)(value->value.edmSingle.value)) <
                        0)
                    {
                        result = AGENT_DATA_TYPES_ERROR;
                        LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
        ...
        case (EDM_DOUBLE_TYPE):
        {
            ...
            /*C99 ... does*/
            /*C11 is same as C99*/
            /*Codes_SRS_AGENT_TYPE_SYSTEM_99_022:[ EDM_DOUBLE: doubleValue = decimalValue [ "e" [SIGN] 1*DIGIT ] /
             * nanInfinity ; IEEE 754 binary64 floating-point number (15-17 decimal digits). The representation shall
             * use DBL_DIG C #define*/
        ...
        case (EDM_COMPLEX_TYPE_TYPE):
        {
            ...
            /*uses JSON encoder*/
            /*Codes_SRS_AGENT_TYPE_SYSTEM_99_062:[ If the AGENT_DATA_TYPE represents a "complex type", then the JSON
             * marshaller shall produce the following JSON value:[...]*/
            MULTITREE_HANDLE treeHandle;
            result = AGENT_DATA_TYPES_OK;
            /*SRS_AGENT_TYPE_SYSTEM_99_016:[ When the value cannot be converted to a string AgentDataTypes_ToString
             * shall return AGENT_DATA_TYPES_ERROR.]*/
            ...
            else
            {
                ...
                {
                    if (MultiTree_AddLeaf(treeHandle,
                                          value->value.edmComplexType.fields[i].fieldName,
                                          value->value.edmComplexType.fields[i].value) != MULTITREE_OK)
                    {
                        /*SRS_AGENT_TYPE_SYSTEM_99_016:[ When the value cannot be converted to a string
                         * AgentDataTypes_ToString shall return AGENT_DATA_TYPES_ERROR.]*/
                        result = AGENT_DATA_TYPES_ERROR;
                        LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
                        break;
                ...
                if (result == AGENT_DATA_TYPES_OK)
                {
                    /*SRS_AGENT_TYPE_SYSTEM_99_016:[ When the value cannot be converted to a string
                     * AgentDataTypes_ToString shall return AGENT_DATA_TYPES_ERROR.]*/
                    if (JSONEncoder_EncodeTree(treeHandle,
                                               destination,
                                               (JSON_ENCODER_TOSTRING_FUNC)AgentDataTypes_ToString) != JSON_ENCODER_OK)
                    {
                        result = AGENT_DATA_TYPES_ERROR;
                        LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
                    }
        ...
        case EDM_BINARY_TYPE:
        {
            ...
            /*1. the "data" of the binary shall be "eaten" 3 characters at a time and produce 4 base64 encoded
             * characters for as long as there are more than 3 characters still to process*/
            ...
            /*the encoding will use the optional [=] or [==] at the end of the encoded string, so that other less
             * standard aware libraries can do their work*/
            /*these are the bits of the 3 normal bytes to be encoded*/
            size_t neededSize = 2; /*2 because starting and ending quotes */
            neededSize +=
                (value->value.edmBinary.size == 0) ? (0) : ((((value->value.edmBinary.size - 1) / 3) + 1) * 4);
            ...
            else
            {
                ...
                */
                ...
                while (value->value.edmBinary.size - currentPosition >= 3)
                {
                    ...
                    char c2 = base64char(((value->value.edmBinary.data[currentPosition] & 3) << 4) |
                                         (value->value.edmBinary.data[currentPosition + 1] >> 4));
                    char c3 = base64char(((value->value.edmBinary.data[currentPosition + 1] & 0x0F) << 2) |
                                         ((value->value.edmBinary.data[currentPosition + 2] >> 6) & 3));
                    char c4 = base64char(value->value.edmBinary.data[currentPosition + 2] & 0x3F);
                    currentPosition += 3;
                ...
                {
                    char c1 = base64char(value->value.edmBinary.data[currentPosition] >> 2);
                    char c2 = base64char(((value->value.edmBinary.data[currentPosition] & 0x03) << 4) |
                                         (value->value.edmBinary.data[currentPosition + 1] >> 4));
...
AGENT_DATA_TYPES_RESULT Create_AGENT_DATA_TYPE_from_AGENT_DATA_TYPE(AGENT_DATA_TYPE *dest, const AGENT_DATA_TYPE *src)
{
    ...
    else
    {
        ...
        case (EDM_STRING_NO_QUOTES_TYPE):
        {
            ...
            if (mallocAndStrcpy_s((char **)&(dest->value.edmStringNoQuotes.chars),
                                  (char *)src->value.edmStringNoQuotes.chars) != 0)
            {
                result = AGENT_DATA_TYPES_ERROR;
                LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
            }
        ...
        case (EDM_COMPLEX_TYPE_TYPE):
        {
            ...
            else
            {
                ...
                dest->value.edmComplexType.fields   = (COMPLEX_TYPE_FIELD_TYPE *)malloc(
                    dest->value.edmComplexType.nMembers * sizeof(COMPLEX_TYPE_FIELD_TYPE));
                ...
                else
                {
                    ...
                    for (i = 0; i < dest->value.edmComplexType.nMembers; i++)
                    {
                        ...
                        if (mallocAndStrcpy_s((char **)(&(dest->value.edmComplexType.fields[i].fieldName)),
                                              src->value.edmComplexType.fields[i].fieldName) != 0)
                        {
                            result = AGENT_DATA_TYPES_ERROR;
                            LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
                            break;
                        ...
                        else
                        {
                            ...
                            dest->value.edmComplexType.fields[i].value =
                                (AGENT_DATA_TYPE *)malloc(sizeof(AGENT_DATA_TYPE));
                            ...
                            {
                                if (Create_AGENT_DATA_TYPE_from_AGENT_DATA_TYPE(
                                        dest->value.edmComplexType.fields[i].value,
                                        src->value.edmComplexType.fields[i].value) != AGENT_DATA_TYPES_OK)
                                {
                                    result = AGENT_DATA_TYPES_ERROR;
                                    LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
                                    break;
...
AGENT_DATA_TYPES_RESULT Create_AGENT_DATA_TYPE_from_MemberPointers(AGENT_DATA_TYPE        *agentData,
                                                                   const char             *typeName,
                                                                   size_t                  nMembers,
                                                                   ...
                                                                   const AGENT_DATA_TYPE **memberPointerValues)
{
    ...
    /* Codes_SRS_AGENT_TYPE_SYSTEM_99_109:[ AGENT_DATA_TYPES_INVALID_ARG shall be returned if memberPointerValues
     * parameter is NULL.] */
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if (Create_AGENT_DATA_TYPE_from_AGENT_DATA_TYPE(values + i, memberPointerValues[i]) !=
                    AGENT_DATA_TYPES_OK)
                {
                    size_t j;
                    for (j = 0; j < i; j++)
                                                            ...
                                                            const AGENT_DATA_TYPE *memberValues)
{
    ...
    /*Codes_SRS_AGENT_TYPE_SYSTEM_99_057:[ If memberNames is NULL, the function shall return
     * AGENT_DATA_TYPES_INVALID_ARG .]*/
    ...
    /*Codes_SRS_AGENT_TYPE_SYSTEM_99_058:[ If any of the memberNames[i] is NULL, the function shall return
     * AGENT_DATA_TYPES_INVALID_ARG .]*/
    ...
    /*Codes_SRS_AGENT_TYPE_SYSTEM_99_059:[ If memberValues is NULL, the function shall return
     * AGENT_DATA_TYPES_INVALID_ARG .]*/
    ...
    /*Codes_SRS_AGENT_TYPE_SYSTEM_99_063:[ If there are two memberNames with the same name, then the function shall
     * return  AGENT_DATA_TYPES_INVALID_ARG.]*/
    ...
    else
    {
        ...
        agentData->value.edmComplexType.fields =
            (COMPLEX_TYPE_FIELD_TYPE *)malloc(nMembers * sizeof(COMPLEX_TYPE_FIELD_TYPE));
        if (agentData->value.edmComplexType.fields == NULL)
        {
            result = AGENT_DATA_TYPES_ERROR;
            LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
        ...
        else
        {
            ...
            {
                /*copy the name*/
                if (mallocAndStrcpy_s((char **)(&(agentData->value.edmComplexType.fields[i].fieldName)),
                                      memberNames[i]) != 0)
                {
                    result = AGENT_DATA_TYPES_ERROR;
                    LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
                ...
                else
                {
                    ...
                    /*copy the rest*/
                    agentData->value.edmComplexType.fields[i].value =
                        (AGENT_DATA_TYPE *)malloc(sizeof(AGENT_DATA_TYPE));
                    if (agentData->value.edmComplexType.fields[i].value == NULL)
                    {
                        /*deallocate the name*/
                    ...
                    {
                        /*copy the values*/
                        if (Create_AGENT_DATA_TYPE_from_AGENT_DATA_TYPE(agentData->value.edmComplexType.fields[i].value,
                                                                        &(memberValues[i])) != AGENT_DATA_TYPES_OK)
                        {
                            result = AGENT_DATA_TYPES_ERROR;
                            LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
                            break;
...
static void fill_tm_yday_and_tm_wday(struct tm *source)
{
    ...
    source->tm_yday                  = (daysInAllPreviousMonths(source->tm_mon)) + (source->tm_mday - 1) +
                      ((source->tm_mon > 1 /*1 is Feb*/) && isLeapYear(year));
...
static int sscanf2d(const char *pos2, int *sec)
{
    int    result;
    size_t position = 1;
    if ((pos2[0] == ':') && (scanAndReadNDigitsInt(pos2, &position, sec, 2) == 0))
    {
        result = 1;
    }
    else
...
static int sscanf3d2d(const char *src, int *hourOffset, int *minOffset)
{
    ...
    if ((scanAndReadNDigitsInt(src, &position, hourOffset, (3 - position)) == 0) && (src[position++] == ':') &&
...
AGENT_DATA_TYPES_RESULT
CreateAgentDataType_From_String(const char *source, AGENT_DATA_TYPE_TYPE type, AGENT_DATA_TYPE *agentData)
{

    AGENT_DATA_TYPES_RESULT result;

    /* Codes_SRS_AGENT_TYPE_SYSTEM_99_073:[ CreateAgentDataType_From_String shall return AGENT_DATA_TYPES_INVALID_ARG if
     * source is NULL.] */
    if ((source == NULL) ||
        /* Codes_SRS_AGENT_TYPE_SYSTEM_99_074:[ CreateAgentDataType_From_String shall return
           AGENT_DATA_TYPES_INVALID_ARG if agentData is NULL.] */
        (agentData == NULL))
    {
        result = AGENT_DATA_TYPES_INVALID_ARG;
        LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
    ...
    {
        /* Codes_SRS_AGENT_TYPE_SYSTEM_99_071:[ CreateAgentDataType_From_String shall create an AGENT_DATA_TYPE from a
         * char* representation of the type indicated by type parameter.] */
        /* Codes_SRS_AGENT_TYPE_SYSTEM_99_072:[ The implementation for the transformation of the char* source into
         * AGENT_DATA_TYPE shall be type specific.] */
        switch (type)
        {
        default:
            /* Codes_SRS_AGENT_TYPE_SYSTEM_99_075:[ CreateAgentDataType_From_String shall return
             * AGENT_DATA_TYPES_NOT_IMPLEMENTED if type is not a supported type.] */
            result = AGENT_DATA_TYPES_NOT_IMPLEMENTED;
            LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
            break;
        ...
        case EDM_BOOLEAN_TYPE:
        {
            ...
            {
                /* Codes_SRS_AGENT_TYPE_SYSTEM_99_087:[ CreateAgentDataType_From_String shall return
                 * AGENT_DATA_TYPES_INVALID_ARG if source is not a valid string for a value of type type.] */
                result = AGENT_DATA_TYPES_INVALID_ARG;
                LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
            }
        ...
        {
            int sByteValue;
            if ((sscanfd(source, &sByteValue) != 1) || (sByteValue < -128) || (sByteValue > 127))
            {
                /* Codes_SRS_AGENT_TYPE_SYSTEM_99_087:[ CreateAgentDataType_From_String shall return
                 * AGENT_DATA_TYPES_INVALID_ARG if source is not a valid string for a value of type type.] */
                result = AGENT_DATA_TYPES_INVALID_ARG;
        ...
        {
            int byteValue;
            if ((sscanfd(source, &byteValue) != 1) || (byteValue < 0) || (byteValue > 255))
            {
                /* Codes_SRS_AGENT_TYPE_SYSTEM_99_087:[ CreateAgentDataType_From_String shall return
                 * AGENT_DATA_TYPES_INVALID_ARG if source is not a valid string for a value of type type.] */
                result = AGENT_DATA_TYPES_INVALID_ARG;
        ...
        {
            int int16Value;
            if ((sscanfd(source, &int16Value) != 1) || (int16Value < -32768) || (int16Value > 32767))
            {
                /* Codes_SRS_AGENT_TYPE_SYSTEM_99_087:[ CreateAgentDataType_From_String shall return
                 * AGENT_DATA_TYPES_INVALID_ARG if source is not a valid string for a value of type type.] */
                result = AGENT_DATA_TYPES_INVALID_ARG;
        ...
        case EDM_INT32_TYPE:
        {
            ...
            if ((sscanfu(pos, &uint32Value) != 1) || (strLength > 11) || ((uint32Value > 2147483648UL) && isNegative) ||
        ...
        case EDM_INT64_TYPE:
        {
            ...
            if ((sscanfllu(&pos, &ullValue) != 1) || (strLength > 20) ||
        ...
        case EDM_DATE_TYPE:
        {
            ...
            if ((strLength < 2) || (source[0] != '"') || (source[strLength - 1] != '"'))
            {
                /* Codes_SRS_AGENT_TYPE_SYSTEM_99_087:[ CreateAgentDataType_From_String shall return
                 * AGENT_DATA_TYPES_INVALID_ARG if source is not a valid string for a value of type type.] */
                result = AGENT_DATA_TYPES_INVALID_ARG;
            ...
            else
            {
                ...
                if ((scanAndReadNDigitsInt(source, &pos, &year, 4) != 0) || (source[pos++] != '-') ||
                    (scanAndReadNDigitsInt(source, &pos, &month, 2) != 0) || (source[pos++] != '-') ||
                    (scanAndReadNDigitsInt(source, &pos, &day, 2) != 0) ||
                    (Create_AGENT_DATA_TYPE_from_date(agentData,
                                                      (int16_t)(sign * year),
                                                      (uint8_t)month,
                                                      (uint8_t)day) != AGENT_DATA_TYPES_OK))
                {
                    /* Codes_SRS_AGENT_TYPE_SYSTEM_99_087:[ CreateAgentDataType_From_String shall return
                     * AGENT_DATA_TYPES_INVALID_ARG if source is not a valid string for a value of type type.] */
                    result = AGENT_DATA_TYPES_INVALID_ARG;
        ...
        case EDM_DATE_TIME_OFFSET_TYPE:
        {
            ...
            else
            {
                ...
                if ((scanAndReadNDigitsInt(source, &pos, &year, 4) != 0) || (source[pos++] != '-') ||
                    (scanAndReadNDigitsInt(source, &pos, &month, 2) != 0) || (source[pos++] != '-') ||
                    (scanAndReadNDigitsInt(source, &pos, &day, 2) != 0) || (source[pos++] != 'T') ||
                    (scanAndReadNDigitsInt(source, &pos, &hour, 2) != 0) || (source[pos++] != ':') ||
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if ((strcmp(pos2, "Z\"") == 0) || agentData->value.edmDateTimeOffset.hasTimeZone)
                            {
                                if ((ValidateDate(year, month, day) != 0) || (hour < 0) || (hour > 23) || (min < 0) ||
                                    (min > 59) || (sec < 0) || (sec > 59) || (fractionalSeconds > 999999999999) ||
                                    (hourOffset < -23) || (hourOffset > 23) || (minOffset < 0) || (minOffset > 59))
                                {
                                    /* Codes_SRS_AGENT_TYPE_SYSTEM_99_087:[ CreateAgentDataType_From_String shall return
                                     * AGENT_DATA_TYPES_INVALID_ARG if source is not a valid string for a value of type
                                     * type.] */
                                    result = AGENT_DATA_TYPES_INVALID_ARG;
                                    LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
                                }
...
#pragma warning(pop)
        ...
        case EDM_DECIMAL_TYPE:
        {
            ...
            else
            {
                ...
                {
                    /* Codes_SRS_AGENT_TYPE_SYSTEM_99_088:[ CreateAgentDataType_From_String shall return
                     * AGENT_DATA_TYPES_ERROR if any other error occurs.] */
                    result = AGENT_DATA_TYPES_ERROR;
                    LogError("(result = %s)", ENUM_TO_STRING(AGENT_DATA_TYPES_RESULT, result));
                }
                else
        ...
        case EDM_BINARY_TYPE:
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    agentData->value.edmBinary.size =
                        (((sourceLength - 2) + 4) / 4) * 3; /*this is overallocation, shall be trimmed later*/
                    agentData->value.edmBinary.data = (unsigned char *)malloc(
                        agentData->value.edmBinary.size); /*this is overallocation, shall be trimmed later*/
                    ...
                    else
                    {
                        ...
                        while (scan4base64char(source + sourcePosition,
                                               sourceLength - sourcePosition,
                                               agentData->value.edmBinary.data + destinationPosition,
                                               agentData->value.edmBinary.data + destinationPosition + 1,
                                               agentData->value.edmBinary.data + destinationPosition + 2) == 0)
                        {
                            sourcePosition += 4;
                            destinationPosition += 3;
                        }

                        if (scanbase64b16(source + sourcePosition,
                                          sourceLength - sourcePosition,
                                          &consumed,
                                          agentData->value.edmBinary.data + destinationPosition,
                                          agentData->value.edmBinary.data + destinationPosition + 1) == 0)
                        {
                            sourcePosition += consumed;
                            destinationPosition += 2;
                        }
                        else if (scanbase64b8(source + sourcePosition,
                                              sourceLength - sourcePosition,
                                              &consumed,
                                              agentData->value.edmBinary.data + destinationPosition) == 0)
                        {
                            sourcePosition += consumed;
...
// extern AGENT_DATA_TYPES_RESULT AgentDataType_GetComplexTypeField(AGENT_DATA_TYPE* agentData, size_t index,
// COMPLEX_TYPE_FIELD_TYPE* complexField);
