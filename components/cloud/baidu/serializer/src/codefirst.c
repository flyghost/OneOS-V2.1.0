...
#define LOG_CODEFIRST_ERROR LogError("(result = %s)", ENUM_TO_STRING(CODEFIRST_RESULT, result))
...
#define CODEFIRST_STATE_VALUES CODEFIRST_STATE_NOT_INIT, CODEFIRST_STATE_INIT_BY_INIT, CODEFIRST_STATE_INIT_BY_API
...
static void DestroyDevice(DEVICE_HEADER_DATA *deviceHeader)
{
    ...
    /* Codes_SRS_CODEFIRST_99_087:[In order to release the device handle, CodeFirst_DestroyDevice shall call
     * Device_Destroy.] */
...
static CODEFIRST_RESULT buildStructTypes(SCHEMA_HANDLE                           schemaHandle,
                                         const REFLECTED_DATA_FROM_DATAPROVIDER *reflectedData)
{
            ...
            else
            {
                ...
                for (maybeField = reflectedData->reflectedData; maybeField != NULL; maybeField = maybeField->next)
                {
                    if (maybeField->type == REFLECTION_FIELD_TYPE)
                    {
                        ...
                        {
                            if (Schema_AddStructTypeProperty(structTypeHandle,
                                                             maybeField->what.field.fieldName,
                                                             maybeField->what.field.fieldType) != SCHEMA_OK)
                            {
                                /*Codes_SRS_CODEFIRST_99_076:[If any Schema APIs fail, CODEFIRST_SCHEMA_ERROR shall be
                                 * returned.]*/
                                result = CODEFIRST_SCHEMA_ERROR;
...
static CODEFIRST_RESULT buildModel(SCHEMA_HANDLE                           schemaHandle,
                                   const REFLECTED_DATA_FROM_DATAPROVIDER *reflectedData,
                                   const REFLECTED_SOMETHING              *modelReflectedData)
{
    CODEFIRST_RESULT           result = CODEFIRST_OK;
    const REFLECTED_SOMETHING *something;
    ...
    if (modelTypeHandle == NULL)
    {
        /*Codes_SRS_CODEFIRST_99_076:[If any Schema APIs fail, CODEFIRST_SCHEMA_ERROR shall be returned.]*/
        result = CODEFIRST_SCHEMA_ERROR;
        LogError("cannot get model %s %s",
                 modelReflectedData->what.model.name,
    ...
    for (something = reflectedData->reflectedData; something != NULL; something = something->next)
    {
        ...
        {
            SCHEMA_MODEL_TYPE_HANDLE childModelHande =
                Schema_GetModelByName(schemaHandle, something->what.property.type);

            /* if this is a model type use the appropriate APIs for it */
            if (childModelHande != NULL)
            {
                if (Schema_AddModelModel(modelTypeHandle,
                                         something->what.property.name,
                                         childModelHande,
                                         something->what.property.offset,
                                         NULL) != SCHEMA_OK)
                {
                    /*Codes_SRS_CODEFIRST_99_076:[If any Schema APIs fail, CODEFIRST_SCHEMA_ERROR shall be returned.]*/
                    result = CODEFIRST_SCHEMA_ERROR;
            ...
            {
                if (Schema_AddModelProperty(modelTypeHandle,
                                            something->what.property.name,
                                            something->what.property.type) != SCHEMA_OK)
                {
                    /*Codes_SRS_CODEFIRST_99_076:[If any Schema APIs fail, CODEFIRST_SCHEMA_ERROR shall be returned.]*/
                    result = CODEFIRST_SCHEMA_ERROR;
        ...
        {
            SCHEMA_MODEL_TYPE_HANDLE childModelHande =
                Schema_GetModelByName(schemaHandle, something->what.reportedProperty.type);

            /* if this is a model type use the appropriate APIs for it */
            if (childModelHande != NULL)
            {
                if (Schema_AddModelModel(modelTypeHandle,
                                         something->what.reportedProperty.name,
                                         childModelHande,
                                         something->what.reportedProperty.offset,
                                         NULL) != SCHEMA_OK)
                {
                    /*Codes_SRS_CODEFIRST_99_076:[If any Schema APIs fail, CODEFIRST_SCHEMA_ERROR shall be returned.]*/
                    result = CODEFIRST_SCHEMA_ERROR;
            ...
            {
                if (Schema_AddModelReportedProperty(modelTypeHandle,
                                                    something->what.reportedProperty.name,
                                                    something->what.reportedProperty.type) != SCHEMA_OK)
                {
                    /*Codes_SRS_CODEFIRST_99_076:[If any Schema APIs fail, CODEFIRST_SCHEMA_ERROR shall be returned.]*/
                    result = CODEFIRST_SCHEMA_ERROR;
        ...
        {
            SCHEMA_MODEL_TYPE_HANDLE childModelHande =
                Schema_GetModelByName(schemaHandle, something->what.desiredProperty.type);
            ...
            (strcmp(something->what.action.modelName, modelReflectedData->what.model.name) == 0))
        {
            ...
            {
                if (Schema_AddModelActionArgument(actionHandle,
                                                  something->what.action.arguments[i].name,
                                                  something->what.action.arguments[i].type) != SCHEMA_OK)
                {
                    /*Codes_SRS_CODEFIRST_99_076:[If any Schema APIs fail, CODEFIRST_SCHEMA_ERROR shall be returned.]*/
                    result = CODEFIRST_SCHEMA_ERROR;
            ...
            (strcmp(something->what.method.modelName, modelReflectedData->what.model.name) == 0))
        {
            ...
            {
                if (Schema_AddModelMethodArgument(methodHandle,
                                                  something->what.method.arguments[i].name,
                                                  something->what.method.arguments[i].type) != SCHEMA_OK)
                {
                    /*Codes_SRS_CODEFIRST_99_076: [ If any Schema APIs fail, CodeFirst_RegisterSchema shall return NULL.
                     * ]*/
                    result = CODEFIRST_SCHEMA_ERROR;
...
static CODEFIRST_RESULT buildModelTypes(SCHEMA_HANDLE                           schemaHandle,
                                        const REFLECTED_DATA_FROM_DATAPROVIDER *reflectedData)
{
    CODEFIRST_RESULT           result = CODEFIRST_OK;
    const REFLECTED_SOMETHING *something;
...
static CODEFIRST_RESULT CodeFirst_Init_impl(const char *overrideSchemaNamespace, bool calledFromCodeFirst_Init)
{
    ...
    {
        /*Codes_SRS_CODEFIRST_99_003:[ If the module is already initialized, the initialization shall fail and the
         * return value shall be CODEFIRST_ALREADY_INIT.]*/
    ...
    else
    {
        ...
        /*Codes_SRS_CODEFIRST_99_002:[ CodeFirst_Init shall initialize the CodeFirst module. If initialization is
         * successful, it shall return CODEFIRST_OK.]*/
...
/*Codes_SRS_CODEFIRST_99_002:[ CodeFirst_Init shall initialize the CodeFirst module. If initialization is successful, it
 * shall return CODEFIRST_OK.]*/
...
void CodeFirst_Deinit(void)
{
    ...
    else
    {
        ...
        /*Codes_SRS_CODEFIRST_99_005:[ CodeFirst_Deinit shall deinitialize the module, freeing all the resources and
         * placing the module in an uninitialized state.]*/
...
static const REFLECTED_SOMETHING *FindModelInCodeFirstMetadata(const REFLECTED_SOMETHING *reflectedData,
                                                               const char                *modelName)
{
    ...
    {
        if ((result->type == REFLECTION_MODEL_TYPE) && (strcmp(result->what.model.name, modelName) == 0))
        {
            /* found model type */
            break;
        }
...
static const REFLECTED_SOMETHING *FindChildModelInCodeFirstMetadata(const REFLECTED_SOMETHING *reflectedData,
                                                                    const REFLECTED_SOMETHING *startModel,
                                                                    ...
                                                                    size_t                    *offset)
{
    ...
    *offset                           = 0;
    ...
    /* Codes_SRS_CODEFIRST_99_139:[If the relativeActionPath is empty then the action shall be looked up in the device
     * model.] */
    while ((*relativePath != 0) && (result != NULL))
    {
        /* Codes_SRS_CODEFIRST_99_142:[The relativeActionPath argument shall be in the format
        ...
        for (childModelProperty = reflectedData; childModelProperty != NULL;
             childModelProperty = childModelProperty->next)
        {
                ...
                (strlen(childModelProperty->what.property.name) == propertyNameLength))
            {
                ...
                /* Codes_SRS_CODEFIRST_99_140:[CodeFirst_InvokeAction shall pass to the action wrapper that it calls a
                 * pointer to the model where the action is defined.] */
...
EXECUTE_COMMAND_RESULT CodeFirst_InvokeAction(DEVICE_HANDLE          deviceHandle,
                                              void                  *callbackUserContext,
                                              const char            *relativeActionPath,
                                              const char            *actionName,
                                              size_t                 parameterCount,
                                              const AGENT_DATA_TYPE *parameterValues)
{
    EXECUTE_COMMAND_RESULT result;
    DEVICE_HEADER_DATA *   deviceHeader = (DEVICE_HEADER_DATA *)callbackUserContext;

    /*Codes_SRS_CODEFIRST_99_068:[ If the function is called before CodeFirst is initialized then EXECUTE_COMMAND_ERROR
     * shall be returned.] */
    if (g_state == CODEFIRST_STATE_NOT_INIT)
    {
    ...
    /*Codes_SRS_CODEFIRST_99_066:[ If actionName, relativeActionPath or deviceHandle is NULL then EXECUTE_COMMAND_ERROR
     * shall be returned*/
    else if ((actionName == NULL) || (deviceHandle == NULL) || (relativeActionPath == NULL))
    {
        result = EXECUTE_COMMAND_ERROR;
        LogError("action Name is NULL %s ", ENUM_TO_STRING(EXECUTE_COMMAND_RESULT, result));
    }
    /*Codes_SRS_CODEFIRST_99_067:[ If parameterCount is greater than zero and parameterValues is NULL then
    ...
    else
    {
        ...
        if (((childModel = FindModelInCodeFirstMetadata(deviceHeader->ReflectedData->reflectedData, modelName)) ==
             NULL) ||
            /* Codes_SRS_CODEFIRST_99_138:[The relativeActionPath argument shall be used by CodeFirst_InvokeAction to
               find the child model where the action is declared.] */
            ((childModel = FindChildModelInCodeFirstMetadata(deviceHeader->ReflectedData->reflectedData,
                                                             childModel,
                                                             relativeActionPath,
                                                             &offset)) == NULL))
        {
            /*Codes_SRS_CODEFIRST_99_141:[If a child model specified in the relativeActionPath argument cannot be found
             * by CodeFirst_InvokeAction, it shall return EXECUTE_COMMAND_ERROR.] */
            result = EXECUTE_COMMAND_ERROR;
            LogError("action %s was not found %s ", actionName, ENUM_TO_STRING(EXECUTE_COMMAND_RESULT, result));
        }
        else
        {
            /* Codes_SRS_CODEFIRST_99_062:[ When CodeFirst_InvokeAction is called it shall look through the codefirst
             * metadata associated with a specific device for a previously declared action (function) named
             * actionName.]*/
            /* Codes_SRS_CODEFIRST_99_078:[If such a function is not found then the function shall return
            ...
            for (something = deviceHeader->ReflectedData->reflectedData; something != NULL; something = something->next)
            {
                ...
                {
                    /*Codes_SRS_CODEFIRST_99_063:[ If the function is found, then CodeFirst shall call the wrapper of
                     * the found function inside the data provider. The wrapper is linked in the reflected data to the
                     * function name. The wrapper shall be called with the same arguments as CodeFirst_InvokeAction has
                     * been called.]*/
                    /*Codes_SRS_CODEFIRST_99_064:[ If the wrapper call succeeds then CODEFIRST_OK shall be returned. ]*/
                    /*Codes_SRS_CODEFIRST_99_065:[ For all the other return values CODEFIRST_ACTION_EXECUTION_ERROR
                     * shall be returned.]*/
                    /* Codes_SRS_CODEFIRST_99_140:[CodeFirst_InvokeAction shall pass to the action wrapper that it calls
                     * a pointer to the model where the action is defined.] */
                    /*Codes_SRS_CODEFIRST_02_013: [The wrapper's return value shall be returned.]*/
                    result =
                        something->what.action.wrapper(deviceHeader->data + offset, parameterCount, parameterValues);
                    break;
                }
            }
        }
...
METHODRETURN_HANDLE CodeFirst_InvokeMethod(DEVICE_HANDLE          deviceHandle,
                                           void                  *callbackUserContext,
                                           const char            *relativeMethodPath,
                                           ...
                                           const AGENT_DATA_TYPE *parameterValues)
{
    ...
    else if ((methodName == NULL) || (deviceHandle == NULL) || (relativeMethodPath == NULL))
    {
        result = NULL;
        LogError("invalid args: DEVICE_HANDLE deviceHandle=%p, void* callbackUserContext=%p, const char* "
                 "relativeMethodPath=%p, const char* methodName=%p, size_t parameterCount=%zu, const AGENT_DATA_TYPE* "
                 "parameterValues=%p",
                 deviceHandle,
                 callbackUserContext,
                 relativeMethodPath,
...
/* Codes_SRS_CODEFIRST_99_002:[ CodeFirst_RegisterSchema shall create the schema information and give it to the Schema
 * module for one schema, identified by the metadata argument. On success, it shall return a handle to the schema.] */
SCHEMA_HANDLE CodeFirst_RegisterSchema(const char *schemaNamespace, const REFLECTED_DATA_FROM_DATAPROVIDER *metadata)
{
    ...
    {
        LogError("invalid arg const char* schemaNamespace=%p, const REFLECTED_DATA_FROM_DATAPROVIDER* metadata=%p",
                 schemaNamespace,
                 metadata);
        result = NULL;
    }
    else
    {
        ...
        /* Codes_SRS_CODEFIRST_99_121:[If the schema has already been registered, CodeFirst_RegisterSchema shall return
         * its handle.] */
...
AGENT_DATA_TYPE_TYPE CodeFirst_GetPrimitiveType(const char *typeName)
{
    ...
    else if ((strcmp(typeName, "_Bool") == 0) || (strcmp(typeName, "bool") == 0))
    {
    ...
    else
    {
        ...
        for (size_t i = 0; i < nDesiredProperties; i++)
        {
            ...
            else
            {
                ...
                {
                    /*Codes_SRS_CODEFIRST_02_036: [ CodeFirst_CreateDevice shall initialize all the desired properties
...
/* Codes_SRS_CODEFIRST_99_079:[CodeFirst_CreateDevice shall create a device and allocate a memory block that should hold
 * the device data.] */
void *CodeFirst_CreateDevice(SCHEMA_MODEL_TYPE_HANDLE                model,
                             const REFLECTED_DATA_FROM_DATAPROVIDER *metadata,
                             size_t                                  dataSize,
                             bool                                    includePropertyPath)
{
    ...
    {
        /*Codes_SRS_CODEFIRST_02_037: [ CodeFirst_CreateDevice shall call CodeFirst_Init, passing NULL for
        ...
        /* Codes_SRS_CODEFIRST_99_082: [ CodeFirst_CreateDevice shall pass to Device_Create the function
         * CodeFirst_InvokeAction, action callback argument and the CodeFirst_InvokeMethod ] */
        else
        {
            ...
            else
            {
                ...
                if (Device_Create(model,
                                  CodeFirst_InvokeAction,
                                  deviceHeader,
                                  CodeFirst_InvokeMethod,
                                  deviceHeader,
                                  includePropertyPath,
                                  &deviceHeader->DeviceHandle) != DEVICE_OK)
                {
                    free(deviceHeader->data);
                    free(deviceHeader);
                ...
                else if ((newDevices = (DEVICE_HEADER_DATA **)
                              realloc(g_Devices, sizeof(DEVICE_HEADER_DATA *) * (g_DeviceCount + 1))) == NULL)
                {
                    Device_Destroy(deviceHeader->DeviceHandle);
                    free(deviceHeader->data);
                    free(deviceHeader);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        /* Codes_SRS_CODEFIRST_99_101:[On success, CodeFirst_CreateDevice shall return a non NULL
...
void CodeFirst_DestroyDevice(void *device)
{
    ...
    if (device != NULL)
    {
        ...
        /*Codes_SRS_CODEFIRST_02_039: [ If the current device count is zero then CodeFirst_DestroyDevice shall
         * deallocate all other used resources. ]*/
...
static const REFLECTED_SOMETHING *FindValue(DEVICE_HEADER_DATA *deviceHeader,
                                            void               *value,
                                            const char         *modelName,
                                            size_t              startOffset,
                                            STRING_HANDLE       valuePath)
{
    ...
    {
        if (result->type == REFLECTION_PROPERTY_TYPE && (strcmp(result->what.property.modelName, modelName) == 0) &&
    ...
    if (result != NULL)
    {
        /* Codes_SRS_CODEFIRST_99_133:[CodeFirst_SendAsync shall allow sending of properties that are part of a child
         * model.] */
        if (result->what.property.offset < valueOffset)
        {
            /* find recursively the property in the inner model, if there is one */
            result = FindValue(deviceHeader,
                               value,
                               result->what.property.type,
                               startOffset + result->what.property.offset,
                               valuePath);
        }
    }
...
static const REFLECTED_SOMETHING *FindReportedProperty(DEVICE_HEADER_DATA *deviceHeader,
                                                       void               *value,
                                                       const char         *modelName,
                                                       size_t              startOffset,
                                                       STRING_HANDLE       valuePath)
{
    ...
    if (result != NULL)
    {
        ...
        {
            /* find recursively the property in the inner model, if there is one */
            result = FindReportedProperty(deviceHeader,
                                          value,
                                          result->what.reportedProperty.type,
                                          startOffset + result->what.reportedProperty.offset,
                                          valuePath);
        }
    }
...
/* Codes_SRS_CODEFIRST_99_130:[If a pointer to the beginning of a device block is passed to CodeFirst_SendAsync instead
 * of a pointer to a property, CodeFirst_SendAsync shall send all the properties that belong to that device.] */
/* Codes_SRS_CODEFIRST_99_131:[The properties shall be given to Device as one transaction, as if they were all passed as
...
static CODEFIRST_RESULT SendAllDeviceProperties(DEVICE_HEADER_DATA *deviceHeader, TRANSACTION_HANDLE transaction)
{
    ...
    for (something = deviceHeader->ReflectedData->reflectedData; something != NULL; something = something->next)
    {
            ...
            (strcmp(something->what.property.modelName, modelName) == 0))
        {
            ...
            /* Codes_SRS_CODEFIRST_99_098:[The marshalling shall be done by calling the Create_AGENT_DATA_TYPE_from_Ptr
             * function associated with the property.] */
            if (something->what.property.Create_AGENT_DATA_TYPE_from_Ptr(deviceAddress +
                                                                             something->what.property.offset,
                                                                         &agentDataType) != AGENT_DATA_TYPES_OK)
            {
                /* Codes_SRS_CODEFIRST_99_099:[If Create_AGENT_DATA_TYPE_from_Ptr fails, CodeFirst_SendAsync shall
                 * return CODEFIRST_AGENT_DATA_TYPE_ERROR.] */
            ...
            else
            {
                ...
                if (Device_PublishTransacted(transaction, something->what.property.name, &agentDataType) != DEVICE_OK)
                {
                    ...
                    /* Codes_SRS_CODEFIRST_99_094:[If any Device API fail, CodeFirst_SendAsync shall return
...
static CODEFIRST_RESULT SendAllDeviceReportedProperties(DEVICE_HEADER_DATA                    *deviceHeader,
                                                        REPORTED_PROPERTIES_TRANSACTION_HANDLE transaction)
{
    const char *               modelName = Schema_GetModelName(deviceHeader->ModelHandle);
    const REFLECTED_SOMETHING *something;
    unsigned char *            deviceAddress = (unsigned char *)deviceHeader->data;
    ...
    for (something = deviceHeader->ReflectedData->reflectedData; something != NULL; something = something->next)
    {
            ...
            (strcmp(something->what.reportedProperty.modelName, modelName) == 0))
        {
            ...
            if (something->what.reportedProperty.Create_AGENT_DATA_TYPE_from_Ptr(
                    deviceAddress + something->what.reportedProperty.offset,
                    &agentDataType) != AGENT_DATA_TYPES_OK)
            {
                result = CODEFIRST_AGENT_DATA_TYPE_ERROR;
                LOG_CODEFIRST_ERROR;
            ...
            {
                if (Device_PublishTransacted_ReportedProperty(transaction,
                                                              something->what.reportedProperty.name,
                                                              &agentDataType) != DEVICE_OK)
                {
                    Destroy_AGENT_DATA_TYPE(&agentDataType);
                    result = CODEFIRST_DEVICE_PUBLISH_FAILED;
                    LOG_CODEFIRST_ERROR;
...
/* Codes_SRS_CODEFIRST_99_088:[CodeFirst_SendAsync shall send to the Device module a set of properties, a destination
 * and a destinationSize.]*/
CODEFIRST_RESULT CodeFirst_SendAsync(unsigned char **destination, size_t *destinationSize, size_t numProperties, ...)
{
    CODEFIRST_RESULT result;
    va_list          ap;

    if ((numProperties == 0) || (destination == NULL) || (destinationSize == NULL))
    {
        /* Codes_SRS_CODEFIRST_04_002: [If CodeFirst_SendAsync receives destination or destinationSize NULL,
         * CodeFirst_SendAsync shall return Invalid Argument.]*/
        /* Codes_SRS_CODEFIRST_99_103:[If CodeFirst_SendAsync is called with numProperties being zero,
         * CODEFIRST_INVALID_ARG shall be returned.] */
        result = CODEFIRST_INVALID_ARG;
        LOG_CODEFIRST_ERROR;
    }
    else
    {
        /*Codes_SRS_CODEFIRST_02_040: [ CodeFirst_SendAsync shall call CodeFirst_Init, passing NULL for
         * overrideSchemaNamespace. ]*/
        (void)CodeFirst_Init_impl(NULL, false); /*lazy init*/
        ...
        /* Codes_SRS_CODEFIRST_99_105:[The properties are passed as pointers to the memory locations where the data
         * exists in the device block allocated by CodeFirst_CreateDevice.] */
        ...
        for (i = 0; i < numProperties; i++)
        {
            ...
            /* Codes_SRS_CODEFIRST_99_095:[For each value passed to it, CodeFirst_SendAsync shall look up to which
             * device the value belongs.] */
            DEVICE_HEADER_DATA *currentValueDeviceHeader = FindDevice(value);
            if (currentValueDeviceHeader == NULL)
            {
                /* Codes_SRS_CODEFIRST_99_104:[If a property cannot be associated with a device, CodeFirst_SendAsync
                 * shall return CODEFIRST_INVALID_ARG.] */
                result = CODEFIRST_INVALID_ARG;
                LOG_CODEFIRST_ERROR;
                break;
            }
            else if ((deviceHeader != NULL) && (currentValueDeviceHeader != deviceHeader))
            {
                /* Codes_SRS_CODEFIRST_99_096:[All values have to belong to the same device, otherwise
                 * CodeFirst_SendAsync shall return CODEFIRST_VALUES_FROM_DIFFERENT_DEVICES_ERROR.] */
                result = CODEFIRST_VALUES_FROM_DIFFERENT_DEVICES_ERROR;
                LOG_CODEFIRST_ERROR;
                break;
            }
            /* Codes_SRS_CODEFIRST_99_090:[All the properties shall be sent together by using the transacted APIs of the
             * device.] */
            /* Codes_SRS_CODEFIRST_99_091:[CodeFirst_SendAsync shall start a transaction by calling
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        /* Codes_SRS_CODEFIRST_99_134:[If CodeFirst_Notify fails for any other reason it shall return
                         * CODEFIRST_ERROR.] */
                    ...
                    else
                    {
                        ...
                        {
                            /* Codes_SRS_CODEFIRST_99_134:[If CodeFirst_Notify fails for any other reason it shall
                             * return CODEFIRST_ERROR.] */
                            result = CODEFIRST_ERROR;
                            LOG_CODEFIRST_ERROR;
                            STRING_delete(valuePath);
                            break;
                        }
                        else if ((propertyReflectedData = FindValue(deviceHeader, value, modelName, 0, valuePath)) ==
                                 NULL)
                        {
                            /* Codes_SRS_CODEFIRST_99_104:[If a property cannot be associated with a device,
                             * CodeFirst_SendAsync shall return CODEFIRST_INVALID_ARG.] */
                            result = CODEFIRST_INVALID_ARG;
                            LOG_CODEFIRST_ERROR;
                            STRING_delete(valuePath);
                            break;
                        ...
                        else
                        {
                            ...
                            /* Codes_SRS_CODEFIRST_99_097:[For each value marshalling to AGENT_DATA_TYPE shall be
                             * performed.] */
                            /* Codes_SRS_CODEFIRST_99_098:[The marshalling shall be done by calling the
                             * Create_AGENT_DATA_TYPE_from_Ptr function associated with the property.] */
                            if (propertyReflectedData->what.property.Create_AGENT_DATA_TYPE_from_Ptr(value,
                                                                                                     &agentDataType) !=
                                AGENT_DATA_TYPES_OK)
                            {
                                /* Codes_SRS_CODEFIRST_99_099:[If Create_AGENT_DATA_TYPE_from_Ptr fails,
                                 * CodeFirst_SendAsync shall return CODEFIRST_AGENT_DATA_TYPE_ERROR.] */
                                result = CODEFIRST_AGENT_DATA_TYPE_ERROR;
                                LOG_CODEFIRST_ERROR;
                                STRING_delete(valuePath);
                                break;
                            ...
                            {
                                /* Codes_SRS_CODEFIRST_99_092:[CodeFirst shall publish each value by using
                                 * Device_PublishTransacted.] */
                                /* Codes_SRS_CODEFIRST_99_136:[CodeFirst_SendAsync shall build the full path for each
                                 * property and then pass it to Device_PublishTransacted.] */
                                if (Device_PublishTransacted(transaction, STRING_c_str(valuePath), &agentDataType) !=
                                    DEVICE_OK)
                                {
                                    Destroy_AGENT_DATA_TYPE(&agentDataType);

                                    /* Codes_SRS_CODEFIRST_99_094:[If any Device API fail, CodeFirst_SendAsync shall
                                     * return CODEFIRST_DEVICE_PUBLISH_FAILED.] */
                                    result = CODEFIRST_DEVICE_PUBLISH_FAILED;
                                    LOG_CODEFIRST_ERROR;
                                    STRING_delete(valuePath);
                                    break;
...
CODEFIRST_RESULT
CodeFirst_SendAsyncReported(unsigned char **destination, size_t *destinationSize, size_t numReportedProperties, ...)
{
    CODEFIRST_RESULT result;
    if ((destination == NULL) || (destinationSize == NULL) || numReportedProperties == 0)
    {
        /*Codes_SRS_CODEFIRST_02_018: [ If parameter destination, destinationSize or any of the values passed through
         * va_args is NULL then CodeFirst_SendAsyncReported shall fail and return CODEFIRST_INVALID_ARG. ]*/
        LogError("invalid argument unsigned char** destination=%p, size_t* destinationSize=%p, size_t "
                 "numReportedProperties=%zu",
                 destination,
                 destinationSize,
                 numReportedProperties);
        result = CODEFIRST_INVALID_ARG;
    }
    else
    {
        /*Codes_SRS_CODEFIRST_02_046: [ CodeFirst_SendAsyncReported shall call CodeFirst_Init, passing NULL for
         * overrideSchemaNamespace. ]*/
        (void)CodeFirst_Init_impl(NULL, false); /*lazy init*/
        ...
        va_list                                ap;
        result = CODEFIRST_ACTION_EXECUTION_ERROR; /*this initialization squelches a false warning about result not
                                                      being initialized*/
        ...
        {
            void *value = (void *)va_arg(ap, void *);
            /*Codes_SRS_CODEFIRST_02_018: [ If parameter destination, destinationSize or any of the values passed
             * through va_args is NULL then CodeFirst_SendAsyncReported shall fail and return CODEFIRST_INVALID_ARG. ]*/
            if (value == NULL)
            {
            ...
            else
            {
                ...
                /*Codes_SRS_CODEFIRST_02_019: [ If values passed through va_args do not belong to the same device then
                ...
                /*Codes_SRS_CODEFIRST_02_022: [ CodeFirst_SendAsyncReported shall start a transaction by calling
                 * Device_CreateTransaction_ReportedProperties. ]*/
                else if ((deviceHeader == NULL) && ((transaction = Device_CreateTransaction_ReportedProperties(
                                                         currentValueDeviceHeader->DeviceHandle)) == NULL))
                {
                    result = CODEFIRST_DEVICE_PUBLISH_FAILED;
                    LOG_CODEFIRST_ERROR;
                    break;
                ...
                else
                {
                    ...
                    if (value == ((unsigned char *)deviceHeader->data))
                    {
                        /*Codes_SRS_CODEFIRST_02_021: [ If the value passed through va_args is a complete model
                         * instance, then CodeFirst_SendAsyncReported shall send all the reported properties of that
                         * device. ]*/
                    ...
                    {
                        /*Codes_SRS_CODEFIRST_02_020: [ If values passed through va_args are not all of type
                         * REFLECTED_REPORTED_PROPERTY then CodeFirst_SendAsyncReported shall fail and return
                        ...
                        else
                        {
                            ...
                            /*Codes_SRS_CODEFIRST_02_025: [ CodeFirst_SendAsyncReported shall compute for every
                             * AGENT_DATA_TYPE the valuePath. ]*/
                            if ((propertyReflectedData =
                                     FindReportedProperty(deviceHeader, value, modelName, 0, valuePath)) == NULL)
                            {
                                result = CODEFIRST_INVALID_ARG;
                                LOG_CODEFIRST_ERROR;
                                STRING_delete(valuePath);
                            ...
                            {
                                AGENT_DATA_TYPE agentDataType;
                                /*Codes_SRS_CODEFIRST_02_023: [ CodeFirst_SendAsyncReported shall convert all
                                 * REPORTED_PROPERTY model components to AGENT_DATA_TYPE. ]*/
                                if (propertyReflectedData->what.reportedProperty.Create_AGENT_DATA_TYPE_from_Ptr(
                                        value,
                                        &agentDataType) != AGENT_DATA_TYPES_OK)
                                {
                                    result = CODEFIRST_AGENT_DATA_TYPE_ERROR;
                                ...
                                {
                                    /*Codes_SRS_CODEFIRST_02_024: [ CodeFirst_SendAsyncReported shall call
                                     * Device_PublishTransacted_ReportedProperty for every AGENT_DATA_TYPE converted
                                     * from REPORTED_PROPERTY. ]*/
                                    if (Device_PublishTransacted_ReportedProperty(transaction,
                                                                                  STRING_c_str(valuePath),
                                                                                  &agentDataType) != DEVICE_OK)
                                    {
                                        Destroy_AGENT_DATA_TYPE(&agentDataType);
                                        result = CODEFIRST_DEVICE_PUBLISH_FAILED;
        ...
        /*Codes_SRS_CODEFIRST_02_027: [ If any error occurs, CodeFirst_SendAsyncReported shall fail and return
         * CODEFIRST_ERROR. ]*/
        ...
        /*Codes_SRS_CODEFIRST_02_026: [ CodeFirst_SendAsyncReported shall call
         * Device_CommitTransaction_ReportedProperties to commit the transaction. ]*/
        else
        {
            ...
            /*Codes_SRS_CODEFIRST_02_029: [ CodeFirst_SendAsyncReported shall call
             * Device_DestroyTransaction_ReportedProperties to destroy the transaction. ]*/
            Device_DestroyTransaction_ReportedProperties(transaction);
        }
...
{
    EXECUTE_COMMAND_RESULT result;
    /*Codes_SRS_CODEFIRST_02_014: [If parameter device or command is NULL then CodeFirst_ExecuteCommand shall return
     * EXECUTE_COMMAND_ERROR.] */
    if ((device == NULL) || (command == NULL))
    {
        result = EXECUTE_COMMAND_ERROR;
        LogError(
            "invalid argument (NULL) passed to CodeFirst_ExecuteCommand void* device = %p, const char* command = %p",
            device,
            command);
    }
    else
    {
        ...
        {
            /*Codes_SRS_CODEFIRST_02_016: [If finding the device fails, then CodeFirst_ExecuteCommand shall return
             * EXECUTE_COMMAND_ERROR.]*/
            result = EXECUTE_COMMAND_ERROR;
        ...
        {
            /*Codes_SRS_CODEFIRST_02_017: [Otherwise CodeFirst_ExecuteCommand shall call Device_ExecuteCommand and
             * return what Device_ExecuteCommand is returning.] */
...
{
    METHODRETURN_HANDLE result;
    if ((device == NULL) || (methodName == NULL) /*methodPayload can be NULL*/
    )
    {
        result = NULL;
        LogError(
            "invalid argument (NULL) passed to CodeFirst_ExecuteMethod void* device = %p, const char* methodName = %p",
            device,
            methodName);
    }
    else
...
{
    CODEFIRST_RESULT result;
    /*Codes_SRS_CODEFIRST_02_030: [ If argument device is NULL then CodeFirst_IngestDesiredProperties shall fail and
     * return CODEFIRST_INVALID_ARG. ]*/
    /*Codes_SRS_CODEFIRST_02_031: [ If argument jsonPayload is NULL then CodeFirst_IngestDesiredProperties shall fail
     * and return CODEFIRST_INVALID_ARG. ]*/
    if ((device == NULL) || (jsonPayload == NULL))
    {
    ...
    {
        /*Codes_SRS_CODEFIRST_02_032: [ CodeFirst_IngestDesiredProperties shall locate the device associated with
         * device. ]*/
        DEVICE_HEADER_DATA *deviceHeader = FindDevice(device);
        if (deviceHeader == NULL)
        {
            /*Codes_SRS_CODEFIRST_02_034: [ If there is any failure, then CodeFirst_IngestDesiredProperties shall fail
             * and return CODEFIRST_ERROR. ]*/
        ...
        else
        {
             ...
             * ]*/
            if (Device_IngestDesiredProperties(device, deviceHeader->DeviceHandle, jsonPayload, parseDesiredNode) !=
                DEVICE_OK)
            {
