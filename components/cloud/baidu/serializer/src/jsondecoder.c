...
/* Codes_SRS_JSON_DECODER_99_049:[ JSONDecoder shall not allocate new string values for the leafs, but rather point to
 * strings in the original JSON.] */
...
static JSON_DECODER_RESULT ParseString(PARSER_STATE *parserState, char **stringBegin)
{
    ...
    *stringBegin               = parserState->json;
    ...
    if (*(parserState->json) != '"')
    {
        /* Codes_SRS_JSON_DECODER_99_007:[ If parsing the JSON fails due to the JSON string being malformed,
         * JSONDecoder_JSON_To_MultiTree shall return JSON_DECODER_PARSE_ERROR.] */
    ...
    else
    {
        ...
        while ((*(parserState->json) != '"') && (*(parserState->json) != '\0'))
        {
            ...
            /* Codes_SRS_JSON_DECODER_99_033:[ Alternatively, there are two-character sequence escape  representations
             * of some popular characters.  So, for example, a string containing only a single reverse solidus character
             * may be represented more compactly as "\\".] */
            if (*(parserState->json) == '\\')
            {
                parserState->json++;
                if (
...
static JSON_DECODER_RESULT ParseNumber(PARSER_STATE *parserState)
{
    ...
    /* Codes_SRS_JSON_DECODER_99_043:[ A number contains an integer component that may be prefixed with an optional
     * minus sign, which may be followed by a fraction part and/or an exponent part.] */
    ...
    else
    {
        ...
        /* Codes_SRS_JSON_DECODER_99_047:[ An exponent part begins with the letter E in upper or lowercase, which may be
...
static JSON_DECODER_RESULT ParseValue(PARSER_STATE *parserState, MULTITREE_HANDLE currentNode, char **stringBegin)
{
    ...
    /* Codes_SRS_JSON_DECODER_99_018:[ A JSON value MUST be an object, array, number, or string, or one of the following
    ...
    /* Tests_SRS_JSON_DECODER_99_018:[ A JSON value MUST be an object, array, number, or string, or one of the following
     * three literal names: false null true] */
    ...
    else if ((ISDIGIT(*(parserState->json))) || (*(parserState->json) == '-'))
    {
        *stringBegin = parserState->json;
...
static JSON_DECODER_RESULT ParseOpenCurly(PARSER_STATE *parserState)
{
    ...
    /* Codes_SRS_JSON_DECODER_99_021:[    An object structure is represented as a pair of curly brackets surrounding
     * zero or more name/value pairs (or members).] */
    if (*(parserState->json) != '{')
    {
        /* Codes_SRS_JSON_DECODER_99_007:[ If parsing the JSON fails due to the JSON string being malformed,
         * JSONDecoder_JSON_To_MultiTree shall return JSON_DECODER_PARSE_ERROR.] */
    ...
    if (result == JSON_DECODER_OK)
    {
        ...
        *(parserState->json - 1) = 0;
        ...
        if (result == JSON_DECODER_OK)
        {
            ...
            /* Codes_SRS_JSON_DECODER_99_002:[ JSONDecoder_JSON_To_MultiTree shall use the MultiTree APIs to create the
             * multi tree and add leafs to the multi tree.] */
            /* Codes_SRS_JSON_DECODER_99_003:[ When a JSON element is decoded from the JSON object then a leaf shall be
             * added to the MultiTree.] */
            ...
            {
                /* Codes_SRS_JSON_DECODER_99_038:[ If any MultiTree API fails, JSONDecoder_JSON_To_MultiTree shall
                 * return JSON_DECODER_MULTITREE_FAILED.] */
            ...
            else
            {
                ...
                {
                    /* Codes_SRS_JSON_DECODER_99_005:[ The leaf node added in the multi tree shall have the value the
                     * string value of the JSON element as parsed from the JSON object.] */
                    if (MultiTree_SetValue(childNode, valueBegin) != MULTITREE_OK)
                    {
                        /* Codes_SRS_JSON_DECODER_99_038:[ If any MultiTree API fails, JSONDecoder_JSON_To_MultiTree
                         * shall return JSON_DECODER_MULTITREE_FAILED.] */
                        result = JSON_DECODER_MULTITREE_FAILED;
                    }
                }
            }
...
static JSON_DECODER_RESULT ParseArray(PARSER_STATE *parserState, MULTITREE_HANDLE currentNode)
{
    ...
    /* Codes_SRS_JSON_DECODER_99_026:[ An array structure is represented as square brackets surrounding zero or more
     * values (or elements).] */
    if (*(parserState->json) != '[')
    {
        /* Codes_SRS_JSON_DECODER_99_007:[ If parsing the JSON fails due to the JSON string being malformed,
    ...
    else
    {
        ...
        while ((jsonChar != ']') && (jsonChar != '\0'))
        {
            ...
            /* Codes_SRS_JSON_DECODER_99_039:[ For array elements the multi tree node name shall be the string
             * representation of the array index.] */
...
static JSON_DECODER_RESULT ParseObjectOrArray(PARSER_STATE *parserState, MULTITREE_HANDLE currentNode)
{
    ...
    if ((result == JSON_DECODER_OK) && (*(parserState->json) != '\0'))
    {
        /* Codes_SRS_JSON_DECODER_99_007:[ If parsing the JSON fails due to the JSON string being malformed,
         * JSONDecoder_JSON_To_MultiTree shall return JSON_DECODER_PARSE_ERROR.] */
        result = JSON_DECODER_PARSE_ERROR;
...
{
    /* Codes_SRS_JSON_DECODER_99_009:[ On success, JSONDecoder_JSON_To_MultiTree shall return a handle to the multi tree
     * it created in the multiTreeHandle argument and it shall return JSON_DECODER_OK.] */
...
JSON_DECODER_RESULT JSONDecoder_JSON_To_MultiTree(char *json, MULTITREE_HANDLE *multiTreeHandle)
{
    ...
    if ((json == NULL) || (multiTreeHandle == NULL))
    {
        /* Codes_SRS_JSON_DECODER_99_001:[ If any of the parameters passed to the JSONDecoder_JSON_To_MultiTree function
         * is NULL then the function call shall return JSON_DECODER_INVALID_ARG.] */
    ...
    {
        /* Codes_SRS_JSON_DECODER_99_008:[ JSONDecoder_JSON_To_MultiTree shall create a multi tree based on the json
         * string argument.] */
        /* Codes_SRS_JSON_DECODER_99_002:[ JSONDecoder_JSON_To_MultiTree shall use the MultiTree APIs to create the
         * multi tree and add leafs to the multi tree.] */
        /* Codes_SRS_JSON_DECODER_99_009:[ On success, JSONDecoder_JSON_To_MultiTree shall return a handle to the multi
         * tree it created in the multiTreeHandle argument and it shall return JSON_DECODER_OK.] */
        *multiTreeHandle = MultiTree_Create(NOPCloneFunction, NoFreeFunction);
        if (*multiTreeHandle == NULL)
        {
            /* Codes_SRS_JSON_DECODER_99_038:[ If any MultiTree API fails, JSONDecoder_JSON_To_MultiTree shall return
