...
#ifdef _MSC_VER
#pragma warning(                                                                                                       \
    disable : 4701) /* potentially uninitialized local variable 'result' used */ /* the scanner cannot track variable  \
                                                                                    "i" and link it to childCount*/
#endif
...
JSON_ENCODER_RESULT
JSONEncoder_EncodeTree(MULTITREE_HANDLE treeHandle, STRING_HANDLE destination, JSON_ENCODER_TOSTRING_FUNC toStringFunc)
{
    JSON_ENCODER_RESULT result;

    size_t childCount;

    /* Codes_SRS_JSON_ENCODER_99_032:[If any of the arguments passed to JSONEncoder_EncodeTree is NULL then
     * JSON_ENCODER_INVALID_ARG shall be returned.] */
    if ((treeHandle == NULL) || (destination == NULL) || (toStringFunc == NULL))
    {
        result = JSON_ENCODER_INVALID_ARG;
        LogError("(result = %s)", ENUM_TO_STRING(JSON_ENCODER_RESULT, result));
    }
    /*Codes_SRS_JSON_ENCODER_99_035:[ JSON encoder shall inquire the number of child nodes (further called childCount)
     * of the current node (given by parameter treeHandle.]*/
    ...
    else
    {
        ...
        else
        {
            ...
            for (i = 0; (i < childCount) && (result == JSON_ENCODER_OK); i++)
            {
                ...
                if ((i > 0) && (STRING_concat(destination, ", ") != 0))
                {
                    result = JSON_ENCODER_ERROR;
                    LogError("(result = %s)", ENUM_TO_STRING(JSON_ENCODER_RESULT, result));
                }
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (innerChildCount > 0)
                                {
                                    ...
                                    {
                                        if ((result = JSONEncoder_EncodeTree(childTreeHandle, child, toStringFunc)) !=
                                            JSON_ENCODER_OK)
                                        {
                                            LogError("(result = %s)", ENUM_TO_STRING(JSON_ENCODER_RESULT, result));
                                        }
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                /*do nothing, result = JSON_ENCODER_OK is set above at the beginning of
                                                 * the FOR loop*/
                                            }
...
JSON_ENCODER_TOSTRING_RESULT JSONEncoder_CharPtr_ToString(STRING_HANDLE destination, const void *value)
{
    ...
    /*Coes_SRS_JSON_ENCODER_99_047:[ JSONEncoder_CharPtr_ToString shall return JSON_ENCODER_TOSTRING_INVALID_ARG if
     * destination or value parameters passed to it are NULL.]*/
    ...
    /*Codes_SRS_JSON_ENCODER_99_048:[ JSONEncoder_CharPtr_ToString shall use strcpy_s to copy from value to
     * destination.]*/
    else if (STRING_concat(destination, (const char *)value) != 0)
    {
        /*Codes_SRS_JSON_ENCODER_99_049:[ If strcpy_s fails then JSONEncoder_CharPtr_ToString shall return
    ...
    {
        /*Codes_SRS_JSON_ENCODER_99_050:[ If strcpy_s doesn't fail, then JSONEncoder_CharPtr_ToString shall return
