...
METHODRETURN_HANDLE MethodReturn_Create(int statusCode, const char *jsonValue)
{
     ...
     * NULL. ]*/
    if ((jsonValue != NULL) &&
        ((temp = json_parse_string(jsonValue), ((temp != NULL) ? json_value_free(temp) : (void)NULL), temp) == NULL))
    {
    ...
    else
    {
        ...
        else
        {
            if (jsonValue == NULL)
            {
                /*Codes_SRS_METHODRETURN_02_001: [ MethodReturn_Create shall create a non-NULL handle containing
                 * statusCode and a clone of jsonValue. ]*/
...
const METHODRETURN_DATA *MethodReturn_GetReturn(METHODRETURN_HANDLE handle)
{
    ...
    {
        /*Codes_SRS_METHODRETURN_02_011: [ Otherwise, MethodReturn_GetReturn shall return a non-NULL const pointer to a
         * METHODRETURN_DATA. ]*/
        result = &(handle->data);
    }
    return result;
