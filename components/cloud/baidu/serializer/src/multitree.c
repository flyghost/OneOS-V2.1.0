...
MULTITREE_HANDLE MultiTree_Create(MULTITREE_CLONE_FUNCTION cloneFunction, MULTITREE_FREE_FUNCTION freeFunction)
{
    ...
    /* Codes_SRS_MULTITREE_99_052:[If any of the arguments passed to MultiTree_Create is NULL, the call shall return
     * NULL.]*/
    ...
    else
    {
        ...
        /*Codes_SRS_MULTITREE_99_006:[MultiTree_Create returns a non - NULL pointer if the tree has been successfully
         * created.]*/
...
static const char *CreateLeaf_ResultAsString[CREATELEAF_RESULT_COUNT] = {TOSTRING(CREATELEAF_OK),
...
#ifdef _MSC_VER
#pragma warning(                                                                                                       \
    disable : 4701) /* potentially uninitialized local variable 'result' used */ /* the scanner cannot track linked    \
                                                                                    "newNode" and "result" therefore   \
                                                                                    the warning*/
#endif
static CREATELEAF_RESULT
createLeaf(MULTITREE_HANDLE_DATA *node, const char *name, const char *value, MULTITREE_HANDLE_DATA **childNode)
{
    CREATELEAF_RESULT result;
    /*can only create it if it doesn't exist*/
    if (strlen(name) == 0)
    {
        /*Codes_SRS_MULTITREE_99_024:[ if a child name is empty (such as in  "/child1//child12"),
         * MULTITREE_EMPTY_CHILD_NAME shall be returned.]*/
        ...
        else
        {
            ...
            if (newNode != NULL)
            {
                ...
                MULTITREE_HANDLE_DATA **newChildren =
                    (MULTITREE_HANDLE_DATA **)realloc(node->children,
                                                      (node->nChildren + 1) * sizeof(MULTITREE_HANDLE_DATA *));
                if (newChildren == NULL)
                {
...
#ifdef _MSC_VER
#pragma warning(                                                                                                       \
    default : 4701) /* potentially uninitialized local variable 'result' used */ /* the scanner cannot track linked    \
                                                                                    "newNode" and "result" therefore   \
                                                                                    the warning*/
#endif
}
...
MULTITREE_RESULT MultiTree_AddLeaf(MULTITREE_HANDLE treeHandle, const char *destinationPath, const void *value)
{
    ...
    /*Codes_SRS_MULTITREE_99_050:[ If destinationPath a string with zero characters, MULTITREE_INVALID_ARG shall be
     * returned.]*/
    ...
    else
    {
        ...
        /*Codes_SRS_MULTITREE_99_014:[DestinationPath is a string in the following format: /child1/child12 or
         * child1/child12] */
        ...
        {
            /*Codes_SRS_MULTITREE_99_017:[ Subsequent names designate hierarchical children in the tree. The last child
             * designates the child that will receive the value.]*/
            CREATELEAF_RESULT res = createLeaf(node, destinationPath, (const char *)value, NULL);
            switch (res)
            {
            default:
            {
                /*Codes_SRS_MULTITREE_99_025:[The function shall return MULTITREE_ERROR to indicate any other error not
                 * specified here.]*/
            ...
            {
                /*Codes_SRS_MULTITREE_99_021:[ If the node already has a value assigned to it,
                 * MULTITREE_ALREADY_HAS_A_VALUE shall be returned and the existing value shall not be changed.]*/
            ...
            case CREATELEAF_OK:
            {
                /*Codes_SRS_MULTITREE_99_034:[ The function returns MULTITREE_OK when data has been stored in the
                 * tree.]*/
                result = MULTITREE_OK;
                break;
            }
        ...
        else
        {
            ...
            if (strncpy_s(firstInnerNodeName,
                          INNER_NODE_NAME_SIZE,
                          destinationPath,
                          whereIsDelimiter - destinationPath) != 0)
            {
                /*Codes_SRS_MULTITREE_99_025:[ The function shall return MULTITREE_ERROR to indicate any other error not
                 * specified here.]*/
                result = MULTITREE_ERROR;
            ...
            else
            {
                ...
                if (child == NULL)
                {
                    ...
                    /*Codes_SRS_MULTITREE_99_023:[ The newly created children along the path shall have a NULL value by
                     * default.]*/
...
/* Codes_SRS_MULTITREE_99_053:[ MultiTree_AddChild shall add a new node with the name childName to the multi tree node
 * identified by treeHandle] */
MULTITREE_RESULT MultiTree_AddChild(MULTITREE_HANDLE treeHandle, const char *childName, MULTITREE_HANDLE *childHandle)
{
    ...
    if ((treeHandle == NULL) || (childName == NULL) || (childHandle == NULL))
    {
        result = MULTITREE_INVALID_ARG;
        LogError("(result = %s)", ENUM_TO_STRING(MULTITREE_RESULT, result));
    }
    else
    {
        ...
        case CREATELEAF_ALREADY_EXISTS:
        {
            /* Codes_SRS_MULTITREE_99_061:[ If a child node with the same name already exists, MultiTree_AddChild shall
             * return MULTITREE_ALREADY_HAS_A_VALUE.] */
            result = MULTITREE_ALREADY_HAS_A_VALUE;
            LogError("(result = %s)", ENUM_TO_STRING(MULTITREE_RESULT, result));
            break;
        }
        ...
        case CREATELEAF_EMPTY_NAME:
        {
            /* Tests_SRS_MULTITREE_99_066:[ If the childName argument is an empty string, MultiTree_AddChild shall
             * return MULTITREE_EMPTY_CHILD_NAME.] */
            result = MULTITREE_EMPTY_CHILD_NAME;
            LogError("(result = %s)", ENUM_TO_STRING(MULTITREE_RESULT, result));
            break;
        }
...
MULTITREE_RESULT MultiTree_GetChildCount(MULTITREE_HANDLE treeHandle, size_t *count)
{
    ...
    {
        /*Codes_SRS_MULTITREE_99_029:[ This function writes in *count the number of direct children for a tree node
         * specified by the parameter treeHandle]*/
        *count = ((MULTITREE_HANDLE_DATA *)treeHandle)->nChildren;
        /*Codes_SRS_MULTITREE_99_035:[ The function shall return MULTITREE_OK when *count contains the number of
         * children of the node pointed to be parameter treeHandle.]*/
        result = MULTITREE_OK;
    }
    return result;
}
...
MULTITREE_RESULT MultiTree_GetChild(MULTITREE_HANDLE treeHandle, size_t index, MULTITREE_HANDLE *childHandle)
{
    ...
    {
        MULTITREE_HANDLE_DATA *node = (MULTITREE_HANDLE_DATA *)treeHandle;
        /*Codes_SRS_MULTITREE_99_032:[If parameter index is out of range, the function shall return
        ...
        {
            /*Codes_SRS_MULTITREE_99_030:[ This function writes in *childHandle parameter the "index"th child of the
             * node pointed to by parameter treeHandle]*/
            /*Codes_SRS_MULTITREE_99_035:[ The function returns MULTITREE_OK when *childHandle contains a handle to the
             * "index"th child of the tree designated by parameter treeHandle.]*/
...
MULTITREE_RESULT MultiTree_GetName(MULTITREE_HANDLE treeHandle, STRING_HANDLE destination)
{
    ...
    {
        MULTITREE_HANDLE_DATA *node = (MULTITREE_HANDLE_DATA *)treeHandle;
        /*Codes_SRS_MULTITREE_99_051:[ The function returns MULTITREE_EMPTY_CHILD_NAME when used with the root of the
         * tree.]*/
        if (node->name == NULL)
        {
            result = MULTITREE_EMPTY_CHILD_NAME;
        ...
        /*Codes_SRS_MULTITREE_99_036:[ This function fills the buffer pointed to by parameter destination with the name
         * of the root node of the tree designated by parameter treeHandle.]*/
        ...
        {
            /*Codes_SRS_MULTITREE_99_039:[ The function returns MULTITREE_OK when destination contains the name of the
             * root node of the tree designated by treeHandle parameter.]*/
            result = MULTITREE_OK;
        }
    }
...
/* Codes_SRS_MULTITREE_99_063:[ MultiTree_GetChildByName shall retrieve the handle of the child node childName from the
 * treeNode node.] */
MULTITREE_RESULT
MultiTree_GetChildByName(MULTITREE_HANDLE treeHandle, const char *childName, MULTITREE_HANDLE *childHandle)
{
    MULTITREE_RESULT result;

    /* Codes_SRS_MULTITREE_99_065:[ If any argument is NULL, MultiTree_GetChildByName shall return
     * MULTITREE_INVALID_ARG.] */
    if ((treeHandle == NULL) || (childHandle == NULL) || (childName == NULL))
    {
        result = MULTITREE_INVALID_ARG;
        LogError("(result = %s)", ENUM_TO_STRING(MULTITREE_RESULT, result));
    }
    else
    {
        ...
        if (i == node->nChildren)
        {
            /* Codes_SRS_MULTITREE_99_068:[ If the specified child is not found, MultiTree_GetChildByName shall return
...
MULTITREE_RESULT MultiTree_GetValue(MULTITREE_HANDLE treeHandle, const void **destination)
{
    ...
    else
    {
        ...
        {
            /*Codes_SRS_MULTITREE_99_041:[This function updates the *destination parameter to the internally stored
             * value.]*/
...
MULTITREE_RESULT MultiTree_SetValue(MULTITREE_HANDLE treeHandle, void *value)
{
    ...
    else
    {
        ...
        if (node->value != NULL)
        {
            /* Codes_SRS_MULTITREE_99_076:[ If the node already has a value then MultiTree_SetValue shall return
        ...
        {
            /* Codes_SRS_MULTITREE_99_072:[ MultiTree_SetValue shall set the value of the node indicated by the
             * treeHandle argument to the value of the argument value.] */
            if (node->cloneFunction(&node->value, value) != 0)
            {
                /* Codes_SRS_MULTITREE_99_075:[ MultiTree_SetValue shall return MULTITREE_ERROR to indicate any other
                 * error.] */
                result = MULTITREE_ERROR;
                LogError("(result = %s)", ENUM_TO_STRING(MULTITREE_RESULT, result));
            }
...
void MultiTree_Destroy(MULTITREE_HANDLE treeHandle)
{
    if (treeHandle != NULL)
    {
        ...
        for (i = 0; i < node->nChildren; i++)
        {
            /*Codes_SRS_MULTITREE_99_047:[ This function frees any system resource used by the tree designated by
             * parameter treeHandle]*/
            MultiTree_Destroy(node->children[i]);
        }
        /*Codes_SRS_MULTITREE_99_047:[ This function frees any system resource used by the tree designated by parameter
         * treeHandle]*/
...
MULTITREE_RESULT MultiTree_GetLeafValue(MULTITREE_HANDLE treeHandle, const char *leafPath, const void **destination)
{
     ...
     */
    if ((treeHandle == NULL) || (leafPath == NULL) || (destination == NULL))
    {
        result = MULTITREE_INVALID_ARG;
        LogError("(result = %s)", ENUM_TO_STRING(MULTITREE_RESULT, result));
    }
    /* Codes_SRS_MULTITREE_99_058:[ The last child designates the child that will receive the value. If a child name is
     * empty (such as in  "/child1//child12"), MULTITREE_EMPTY_CHILD_NAME shall be returned.] */
    ...
    else
    {
        ...
        if (*pos == '\0')
        {
            /* Codes_SRS_MULTITREE_99_069:[ If a child name is empty (such as in  "/child1//child12"),
             * MULTITREE_EMPTY_CHILD_NAME shall be returned.] */
            result = MULTITREE_EMPTY_CHILD_NAME;
            LogError("(result = %s)", ENUM_TO_STRING(MULTITREE_RESULT, result));
        }
        else
        {
            ...
            /* Codes_SRS_MULTITREE_99_056:[ The leafPath argument is a string in the following format: /child1/child12
             * or child1/child12.] */
            ...
            while (*pos != '\0')
            {
                ...
                {
                    /* Codes_SRS_MULTITREE_99_071:[ When the child node is not found, MultiTree_GetLeafValue shall
                     * return MULTITREE_CHILD_NOT_FOUND.] */
                    result = MULTITREE_CHILD_NOT_FOUND;
                    LogError("(result = %s)", ENUM_TO_STRING(MULTITREE_RESULT, result));
                    break;
                }
                else
                {
                    for (i = 0; i < childCount; i++)
                    {
                        ...
                        {
                            /* Codes_SRS_MULTITREE_99_057:[ Subsequent names designate hierarchical children in the
                             * tree.] */
            ...
            if (*pos == 0)
            {
                if (node->value == NULL)
                {
                    /* Codes_SRS_MULTITREE_99_070:[ If an attempt is made to get the value for a node that does not have
                     * a value set, then MultiTree_GetLeafValue shall return MULTITREE_EMPTY_VALUE.] */
                    result = MULTITREE_EMPTY_VALUE;
                    LogError("(result = %s)", ENUM_TO_STRING(MULTITREE_RESULT, result));
                }
                /*Codes_SRS_MULTITREE_99_053:[ MultiTree_GetLeafValue shall copy into the *destination argument the
                 * value of the node identified by the leafPath argument.]*/
                else
                {
                    *destination = node->value;
...
/* Codes_SRS_MULTITREE_99_077:[ MultiTree_DeleteChild shall remove the direct children node (no recursive search) set by
 * childName.] */
MULTITREE_RESULT MultiTree_DeleteChild(MULTITREE_HANDLE treeHandle, const char *childName)
{
     ...
     */
    if ((treeHandle == NULL) || (childName == NULL))
    {
        result = MULTITREE_INVALID_ARG;
        LogError("(result = %s)", ENUM_TO_STRING(MULTITREE_RESULT, result));
    }
    else
    {
        ...
        {
            /* Codes_SRS_MULTITREE_99_079:[If childName is not found, MultiTree_DeleteChild shall return
             * MULTITREE_CHILD_NOT_FOUND.] */
            result = MULTITREE_CHILD_NOT_FOUND;
        ...
        else
        {
            ...
            /* Codes_SRS_MULTITREE_99_077:[ MultiTree_DeleteChild shall remove the direct children node (no recursive
             * search) set by childName  */
