...
SERIALIZER_RESULT serializer_init(const char *overrideSchemaNamespace)
{
    ...
    else
    {
        ...
        /* Codes_SRS_SCHEMALIB_99_076:[serializer_init shall pass the value of overrideSchemaNamespace argument to
         * CodeFirst_Init.] */
        ...
        {
            /* Codes_SRS_SCHEMALIB_99_075:[When an serializer_init call fails for any reason the previous initialization
             * state shall be preserved. The initialized state shall only be changed on a succesfull Init.] */
...
SERIALIZER_RESULT serializer_setconfig(SERIALIZER_CONFIG which, void *value)
{
    ...
    /* Codes_SRS_SCHEMALIB_99_137:[ If the value argument is NULL, serializer_setconfig shall return
     * SERIALIZER_INVALID_ARG.] */
    if (value == NULL)
    {
        result = SERIALIZER_INVALID_ARG;
    }
    /* Codes_SRS_SCHEMALIB_99_142:[ When the which argument is SerializeDelayedBufferMaxSize, serializer_setconfig shall
     * invoke DataPublisher_SetMaxBufferSize with the dereferenced value argument, and shall return SERIALIZER_OK.] */
    else if (which == SerializeDelayedBufferMaxSize)
    {
        DataPublisher_SetMaxBufferSize(*(size_t *)value);
        result = SERIALIZER_OK;
    }
    /* Codes_SRS_SCHEMALIB_99_138:[ If the which argument is not one of the declared members of the SERIALIZER_CONFIG
     * enum, serializer_setconfig shall return SERIALIZER_INVALID_ARG.] */
    else
    {
        result = SERIALIZER_INVALID_ARG;
    }
