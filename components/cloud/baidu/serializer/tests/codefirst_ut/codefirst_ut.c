...
EXECUTE_COMMAND_RESULT test1(TruckType               *device,
                             double                   P1,
                             int                      P2,
                             float                    P3,
                             long                     P4,
                             ...
                             bool                     P10,
                             ascii_char_ptr           P11,
                             EDM_DATE_TIME_OFFSET     P12,
                             EDM_GUID                 P13,
                             EDM_BINARY               P14,
                             ascii_char_ptr_no_quotes P15)
{
    (void)device;
...
METHODRETURN_HANDLE test1Method(TruckType               *device,
                                double                   P1,
                                int                      P2,
                                float                    P3,
                                long                     P4,
...
static char *umockvalue_stringify_EDM_DATE_TIME_OFFSET(const EDM_DATE_TIME_OFFSET *dateTimeOffset)
{
    size_t needed = snprintf(NULL,
                             0,
                             "struct tm = ( %d-%d-%d %d:%d:%d ) %" PRIu8 " %" PRIu64 " %" PRIu8 " %" PRIu8 " %" PRIu8,
...
static int umockvalue_are_equal_EDM_DATE_TIME_OFFSET(const EDM_DATE_TIME_OFFSET *left,
                                                     const EDM_DATE_TIME_OFFSET *right)
{
...
my_Device_PublishTransacted(TRANSACTION_HANDLE transactionHandle, const char *propertyName, const AGENT_DATA_TYPE *data)
{
    (void)transactionHandle;
    if (((strcmp(propertyName, "this_is_double") == 0) && (data != Create_AGENT_DATA_TYPE_from_DOUBLE_agentData))
        /*the below cannot be checked easily because there are 2 types (int, int32_t) setting the same
           CReate_AGET_DATA_TYPE_from_SINT32_agentData*/
        /*same goes for long and sint64_t*/
        /*(
        (strcmp(propertyName, "this_is_int") == 0) &&
        (data != Create_AGENT_DATA_TYPE_from_DOUBLE_agentData)
        )*/
        || ((strcmp(propertyName, "this_is_float") == 0) && (data != Create_AGENT_DATA_TYPE_from_FLOAT_agentData)) ||
        ((strcmp(propertyName, "this_is_sint8_t") == 0) && (data != Create_AGENT_DATA_TYPE_from_SINT8_agentData)) ||
        ((strcmp(propertyName, "this_is_uint8_t") == 0) && (data != Create_AGENT_DATA_TYPE_from_UINT8_agentData)) ||
        ((strcmp(propertyName, "this_is_int16_t") == 0) && (data != Create_AGENT_DATA_TYPE_from_SINT16_agentData)) ||
        ((strcmp(propertyName, "this_is_bool") == 0) && (data != Create_EDM_BOOLEAN_from_int_agentData)) ||
...
static DEVICE_RESULT
my_Device_EndTransaction(TRANSACTION_HANDLE transactionHandle, unsigned char **destination, size_t *destinationSize)
{
    (void)destination;
...
DECLARE_STRUCT(whereIsMyDevice_Struct, double, Lat_Field, double, Long_Field);

DECLARE_STRUCT(theCarIsBehindTheVan_Struct, int, Alt_Field, whereIsMyDevice_Struct, whereIsMyCar_Field);

DECLARE_MODEL(truckType_Model, WITH_ACTION(reset_Action), WITH_ACTION(setSpeed_Action, double, theSpeed));
...
DECLARE_MODEL(OuterType, WITH_DATA(InnerType, Inner), WITH_ACTION(OuterType_reset_Action));
...
DECLARE_MODEL(InnerType_onDesiredProperty,
              ...
              WITH_DESIRED_PROPERTY(int,
                                    this_is_desired_int_Property_2_onDesiredProperty,
                                    onthis_is_desired_int_Property_2_onDesiredProperty));
...
static DEVICE_RESULT my_Device_Create(SCHEMA_MODEL_TYPE_HANDLE modelHandle,
                                      pfDeviceActionCallback   deviceActionCallback,
                                      void                    *actionCallbackUserContext,
...
static SCHEMA_RESULT my_Schema_GetModelDesiredPropertyCount(SCHEMA_MODEL_TYPE_HANDLE modelTypeHandle,
...
TEST_SUITE_INITIALIZE(TestClassInitialize)
{
    ...
    REGISTER_GLOBAL_MOCK_HOOK(Device_CreateTransaction_ReportedProperties,
                              my_Device_CreateTransaction_ReportedProperties);
    ...
    REGISTER_GLOBAL_MOCK_HOOK(Device_DestroyTransaction_ReportedProperties,
                              my_Device_DestroyTransaction_ReportedProperties);
...
/*Tests_CODEFIRST_002:[ CodeFirst_RegisterSchema shall create the schema information and give it to the Schema module
 * for one schema, identified by the metadata argument. On success, it shall return a handle to the model.]*/
...
/*Tests_SRS_CODEFIRST_99_003:[ If the module is already initialized, the initialization shall fail and the return value
 * shall be CODEFIRST_ALREADY_INIT.]*/
...
/*Tests_SRS_CODEFIRST_99_066:[ If actionName, relativeActionPath or deviceHandle is NULL then EXECUTE_COMMAND_ERROR
 * shall be returned*/
...
TEST_FUNCTION(CodeFirst_InvokeAction_With_NULL_Relative_Path_Fails)
{
    ...
    EXECUTE_COMMAND_RESULT result =
        CodeFirst_InvokeAction(TEST_DEVICE_HANDLE, g_InvokeActionCallbackArgument, NULL, "reset", 0, NULL);
...
/*Tests_SRS_CODEFIRST_99_068:[ If the function is called before CodeFirst is initialized then EXECUTE_COMMAND_ERROR
 * shall be returned.] */
...
/*Tests_SRS_CODEFIRST_99_067:[ If parameterCount is greater than zero and parameterValues is NULL then
 * EXECUTE_COMMAND_ERROR shall be returned.]*/
/*Tests_SRS_SERIALIZER_99_039:[ WITH_ACTION shall declare an action of the current data provider called as the first
 * macro parameter (name) and having the first parameter called parameter1Name of type parameter1Type, the second
 * parameter named parameter2Name having the type parameter2Type and so on. ]*/
...
TEST_FUNCTION(CodeFirst_InvokeAction_with_unknown_actions_fails)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CodeFirst_InvokeAction(TEST_DEVICE_HANDLE, g_InvokeActionCallbackArgument, "", "asjnhfaslk", 1, &someDouble);
...
/*Tests_SRS_CODEFIRST_99_063:[ If the function is found, then CodeFirst shall call the wrapper of the found function
 * inside the data provider. The wrapper is linked in the reflected data to the function name. The wrapper shall be
 * called with the same arguments as CodeFirst_InvokeAction has been called.]*/
/*Tests_SRS_SERIALIZER_99_039:[ WITH_ACTION shall declare an action of the current data provider called as the first
 * macro parameter (name) and having the first parameter called parameter1Name of type parameter1Type, the second
 * parameter named parameter2Name having the type parameter2Type and so on. ]*/
/*Tests_SRS_SERIALIZER_99_040:[ In addition to declaring the function, DECLARE_IOT_METHOD shall provide a definition for
 * a wrapper that takes as parameters a size_t parameterCount and const AGENT_DATA_TYPE*.] */
/*Tests_SRS_SERIALIZER_99_041:[ This wrapper shall convert all the arguments to predefined types and then call the
 * function written by the data provider developer.]*/
...
TEST_FUNCTION(CodeFirst_InvokeAction_calls_reset_action_succeeds)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CodeFirst_InvokeAction(TEST_DEVICE_HANDLE, g_InvokeActionCallbackArgument, "", "reset", 0, NULL);
 ...
 * inside the data provider. The wrapper is linked in the reflected data to the function name. The wrapper shall be
 * called with the same arguments as CodeFirst_InvokeAction has been called.]*/
/*Tests_SRS_SERIALIZER_99_045:[ If the number of passed parameters doesn't match the number of declared parameters,
 * wrapper execution shall fail and return DATA_PROVIDER_INVALID_ARG;]*/
...
TEST_FUNCTION(CodeFirst_InvokeAction_calling_setSpeed_action_with_insuficcient_parameters_fails)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CodeFirst_InvokeAction(TEST_DEVICE_HANDLE, g_InvokeActionCallbackArgument, "", "setSpeed", 0, NULL);
...
TEST_FUNCTION(CodeFirst_InvokeAction_calling_setSpeed_action_with_NULL_ParameterValues_fails)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CodeFirst_InvokeAction(TEST_DEVICE_HANDLE, g_InvokeActionCallbackArgument, "", "setSpeed", 1, NULL);
...
TEST_FUNCTION(CodeFirst_InvokeAction_calling_setSpeed_action_with_too_many_parameters_fails)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result = CodeFirst_InvokeAction(TEST_DEVICE_HANDLE,
                                                           g_InvokeActionCallbackArgument,
                                                           "",
                                                           "setSpeed",
                                                           2,
                                                           arrayOfAgentDataType);
 ...
 * wrapper execution shall fail and return DATA_PROVIDER_INVALID_ARG;]*/
/*Tests_SRS_CODEFIRST_99_065:[ For all the other return values CODEFIRST_ACTION_EXECUTION_ERROR shall be returned.]*/
/*Tests_SRS_SERIALIZER_99_046:[ If the types of the parameters do not match the declared types, DATAPROVIDER_INVALID_ARG
 * shall be returned.]*/
/*Tests_SRS_SERIALIZER_99_039:[ WITH_ACTION shall declare an action of the current data provider called as the first
 * macro parameter (name) and having the first parameter called parameter1Name of type parameter1Type, the second
 * parameter named parameter2Name having the type parameter2Type and so on. ]*/
TEST_FUNCTION(CodeFirst_InvokeAction_calling_setSpeed_with_invalid_type_for_Parameter1_fails)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CodeFirst_InvokeAction(TEST_DEVICE_HANDLE, g_InvokeActionCallbackArgument, "", "setSpeed", 1, &someInt32);
...
TEST_FUNCTION(CodeFirst_InvokeAction_calling_setSpeed_succeeds)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CodeFirst_InvokeAction(TEST_DEVICE_HANDLE, g_InvokeActionCallbackArgument, "", "setSpeed", 1, &someDouble);
...
/*Tests_SRS_SERIALIZER_99_042:[The parameter types are either predefined parameter types(specs SRS_SERIALIZER_99_004 -
 * SRS_SERIALIZER_99_014)*/
/*Tests_SRS_SERIALIZER_99_039:[ WITH_ACTION shall declare an action of the current data provider called as the first
 * macro parameter (name) and having the first parameter called parameter1Name of type parameter1Type, the second
 * parameter named parameter2Name having the type parameter2Type and so on. ]*/
...
TEST_FUNCTION(CodeFirst_InvokeAction_calling_test1_with_NULL_parameterValues_fails)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CodeFirst_InvokeAction(TEST_DEVICE_HANDLE, g_InvokeActionCallbackArgument, "", "test1", 11, NULL);
...
TEST_FUNCTION(CodeFirst_InvokeAction_calling_test1_succeeds)
{
    ...
    ASSERT_IS_TRUE(umockvalue_are_equal_EDM_DATE_TIME_OFFSET(&someEdmDateTimeOffset, &DummyDataProvider_test1_P12) ==
                   0);
...
/* Tests_SRS_CODEFIRST_99_140:[CodeFirst_InvokeAction shall pass to the action wrapper that it calls a pointer to the
 * model where the action is defined.] */
/* Tests_SRS_CODEFIRST_99_139:[If the relativeActionPath is empty then the action shall be looked up in the device
 * model.] */
/* Tests_SRS_CODEFIRST_99_138:[The relativeActionPath argument shall be used by CodeFirst_InvokeAction to find the child
 * model where the action is declared.] */
/* Tests_SRS_CODEFIRST_99_142:[The relativeActionPath argument shall be in the format
 * "childModel1/childModel2/.../childModelN".] */
TEST_FUNCTION(CodeFirst_InvokeAction_For_A_Child_Model_Passes_The_OuterType_Instance_To_The_Callback)
{
    /// arrange
    (void)CodeFirst_Init(NULL);
    OuterType *device = (OuterType *)
        CodeFirst_CreateDevice(TEST_MODEL_HANDLE, &ALL_REFLECTED(testModelInModelReflected), sizeof(OuterType), false);
    umock_c_reset_all_calls();
...
/* Tests_SRS_CODEFIRST_99_141:[If a child model specified in the relativeActionPath argument cannot be found by
 * CodeFirst_InvokeAction, it shall return EXECUTE_COMMAND_ERROR.] */
TEST_FUNCTION(CodeFirst_InvokeAction_For_A_Child_Model_And_The_Model_Is_Not_Found_Fails)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CodeFirst_InvokeAction(TEST_DEVICE_HANDLE, g_InvokeActionCallbackArgument, "Inne", "reset", 0, NULL);
...
TEST_FUNCTION(CodeFirst_InvokeAction_For_A_Child_Model_And_The_Model_Is_Not_Found_Because_Path_Too_Deep_Fails)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CodeFirst_InvokeAction(TEST_DEVICE_HANDLE, g_InvokeActionCallbackArgument, "Inner/FakeInner", "reset", 0, NULL);
...
TEST_FUNCTION(CodeFirst_InvokeAction_For_A_Child_Model_And_The_Action_Is_Not_Found_Fails)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CodeFirst_InvokeAction(TEST_DEVICE_HANDLE, g_InvokeActionCallbackArgument, "Inner", "rst", 0, NULL);
...
/* Tests_SRS_CODEFIRST_99_080:[If CodeFirst_CreateDevice is invoked with a NULL iotHubClientHandle or model, it shall
 * return NULL.] */
...
/* Tests_SRS_CODEFIRST_99_079:[CodeFirst_CreateDevice shall create a device and allocate a memory block that should hold
 * the device data.] */
/* Tests_SRS_CODEFIRST_99_081:[CodeFirst_CreateDevice shall use Device_Create to create a device handle.] */
/* Tests_SRS_CODEFIRST_99_082: [ CodeFirst_CreateDevice shall pass to Device_Create the function CodeFirst_InvokeAction,
 * action callback argument and the CodeFirst_InvokeMethod ]*/
...
TEST_FUNCTION(CodeFirst_CreateDevice_With_Valid_Arguments_and_includePropertyPath_false_Succeeds_1)
{
    ...
    STRICT_EXPECTED_CALL(Device_Create(TEST_MODEL_HANDLE,
                                       CodeFirst_InvokeAction,
    ...
    STRICT_EXPECTED_CALL(Schema_AddDeviceRef(IGNORED_PTR_ARG)).IgnoreArgument(1);
...
/* Tests_SRS_CODEFIRST_99_106:[If CodeFirst_CreateDevice is called when the modules is not initialized is shall return
 * NULL.] */
...
/* Tests_SRS_CODEFIRST_99_087:[In order to release the device handle, CodeFirst_DestroyDevice shall call
 * Device_Destroy.] */
TEST_FUNCTION(CodeFirst_DestroyDevice_With_Valid_Argument_Destroys_The_Device)
{
    ...
    STRICT_EXPECTED_CALL(Schema_ReleaseDeviceRef(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(Schema_DestroyIfUnused(IGNORED_PTR_ARG)).IgnoreArgument(1);
...
/* Tests_SRS_CODEFIRST_99_103:[If CodeFirst_SendAsync is called with numProperties being zero, CODEFIRST_INVALID_ARG
 * shall be returned.] */
...
/* Tests_SRS_CODEFIRST_99_105:[The properties are passed as pointers to the memory locations where the data exists in
 * the device block allocated by CodeFirst_CreateDevice.] */
...
/* Tests_SRS_CODEFIRST_99_095:[For each value passed to it, CodeFirst_SendAsync shall look up to which device the value
 * belongs.] */
/* Tests_SRS_CODEFIRST_99_097:[For each value marshalling to AGENT_DATA_TYPE shall be performed.] */
/* Tests_SRS_CODEFIRST_99_098:[The marshalling shall be done by calling the Create_AGENT_DATA_TYPE_from_Ptr function
...
TEST_FUNCTION(CodeFirst_SendAsync_With_One_Property_Succeeds)
{
    // arrange
    (void)CodeFirst_Init(NULL);
    SimpleDevice_Model *device = (SimpleDevice_Model *)
        CodeFirst_CreateDevice(TEST_MODEL_HANDLE, &ALL_REFLECTED(testReflectedData), sizeof(SimpleDevice_Model), false);
    unsigned char *destination;
    size_t         destinationSize;
    umock_c_reset_all_calls();
    ...
    STRICT_EXPECTED_CALL(STRING_concat(IGNORED_PTR_ARG, IGNORED_PTR_ARG)).IgnoreArgument_handle().IgnoreArgument_s2();
...
TEST_FUNCTION(CodeFirst_SendAsync_2_Properties_Succeeds)
{
    ...
    // act
    CODEFIRST_RESULT result = CodeFirst_SendAsync(&destination,
                                                  &destinationSize,
                                                  2,
                                                  &device->this_is_double_Property,
                                                  &device->this_is_int_Property);
...
/* Tests_SRS_CODEFIRST_99_094:[If any Device API fail, CodeFirst_SendAsync shall return
...
TEST_FUNCTION(When_StartTransaction_Fails_CodeFirst_SendAsync_Fails)
{
    ...
    STRICT_EXPECTED_CALL(Device_StartTransaction(TEST_DEVICE_HANDLE)).SetReturn((TRANSACTION_HANDLE)NULL);
    device->this_is_double_Property = 42.0;
    unsigned char *destination;
    size_t         destinationSize;
...
TEST_FUNCTION(When_PublishTransacted_Fails_CodeFirst_SendAsync_Fails)
{
    ...
    EXPECTED_CALL(Destroy_AGENT_DATA_TYPE(IGNORED_PTR_ARG));
    STRICT_EXPECTED_CALL(STRING_delete(IGNORED_PTR_ARG)).IgnoreArgument_handle();
    STRICT_EXPECTED_CALL(Device_CancelTransaction(IGNORED_PTR_ARG)).IgnoreArgument_transactionHandle();
    device->this_is_double_Property = 42.0;
    unsigned char *destination;
    size_t         destinationSize;
...
/* Tests_SRS_CODEFIRST_99_104:[If a property cannot be associated with a device, CodeFirst_SendAsync shall return
...
/* Tests_SRS_CODEFIRST_99_099:[If Create_AGENT_DATA_TYPE_from_Ptr fails, CodeFirst_SendAsync shall return
...
TEST_FUNCTION(When_Creating_The_Agent_Data_Type_Fails_Then_CodeFirst_SendAsync_Fails)
{
    ...
    STRICT_EXPECTED_CALL(Schema_GetModelName(TEST_MODEL_HANDLE));
    STRICT_EXPECTED_CALL(STRING_concat(IGNORED_PTR_ARG, IGNORED_PTR_ARG)).IgnoreArgument_handle().IgnoreArgument_s2();
    EXPECTED_CALL(Create_AGENT_DATA_TYPE_from_DOUBLE(IGNORED_PTR_ARG, 0.0)).SetReturn(AGENT_DATA_TYPES_ERROR);
    STRICT_EXPECTED_CALL(STRING_delete(IGNORED_PTR_ARG)).IgnoreArgument_handle();
    STRICT_EXPECTED_CALL(Device_CancelTransaction(IGNORED_PTR_ARG)).IgnoreArgument_transactionHandle();
    device->this_is_double_Property = 42.0;
    unsigned char *destination;
...
TEST_FUNCTION(When_Creating_The_Agent_Data_Type_Fails_For_The_2nd_Property_Then_CodeFirst_SendAsync_Fails)
{
    ...
    STRICT_EXPECTED_CALL(Schema_GetModelName(TEST_MODEL_HANDLE));
    STRICT_EXPECTED_CALL(STRING_concat(IGNORED_PTR_ARG, IGNORED_PTR_ARG)).IgnoreArgument_handle().IgnoreArgument_s2();
    EXPECTED_CALL(Create_AGENT_DATA_TYPE_from_SINT32(IGNORED_PTR_ARG, 0)).SetReturn(AGENT_DATA_TYPES_ERROR);
    STRICT_EXPECTED_CALL(STRING_delete(IGNORED_PTR_ARG)).IgnoreArgument_handle();
    STRICT_EXPECTED_CALL(Device_CancelTransaction(IGNORED_PTR_ARG)).IgnoreArgument_transactionHandle();
    device->this_is_double_Property = 42.0;
    unsigned char *destination;
...
/* Tests_SRS_CODEFIRST_99_096:[All values have to belong to the same device, otherwise CodeFirst_SendAsync shall return
...
TEST_FUNCTION(Properties_From_2_Different_Devices_Make_CodeFirst_SendAsync_Fail)
{
    // arrange
    (void)CodeFirst_Init(NULL);
    SimpleDevice_Model *device1 = (SimpleDevice_Model *)
        CodeFirst_CreateDevice(TEST_MODEL_HANDLE, &ALL_REFLECTED(testReflectedData), sizeof(SimpleDevice_Model), false);
    SimpleDevice_Model *device2 = (SimpleDevice_Model *)
        CodeFirst_CreateDevice(TEST_MODEL_HANDLE, &ALL_REFLECTED(testReflectedData), sizeof(SimpleDevice_Model), false);
    umock_c_reset_all_calls();
    ...
    CODEFIRST_RESULT result = CodeFirst_SendAsync(&destination,
                                                  &destinationSize,
                                                  2,
                                                  &device1->this_is_double_Property,
                                                  &device2->this_is_double_Property);
...
/* Tests_SRS_CODEFIRST_99_130:[If a pointer to the beginning of a device block is passed to CodeFirst_SendAsync instead
 * of a pointer to a property, CodeFirst_SendAsync shall send all the properties that belong to that device.] */
/* Tests_SRS_CODEFIRST_99_131:[The properties shall be given to Device as one transaction, as if they were all passed as
 * individual arguments to Code_First.] */
...
/* Tests_SRS_CODEFIRST_99_136:[CodeFirst_SendAsync shall build the full path for each property and then pass it to
 * Device_PublishTransacted.] */
TEST_FUNCTION(CodeFirst_SendAsync_Can_Send_A_Property_From_A_Child_Model)
{
    // arrange
    (void)CodeFirst_Init(NULL);
    OuterType *device = (OuterType *)CodeFirst_CreateDevice(TEST_OUTERTYPE_MODEL_HANDLE,
                                                            &ALL_REFLECTED(testModelInModelReflected),
                                                            sizeof(OuterType),
                                                            false);
    umock_c_reset_all_calls();
...
/* Tests_SRS_CODEFIRST_04_002: [If CodeFirst_SendAsync receives destination or destinationSize NULL, CodeFirst_SendAsync
 * shall return Invalid Argument.]*/
...
/*Tests_SRS_CODEFIRST_99_088:[CodeFirst_SendAsync shall send to the Device module a set of properties, a destination and
 * a destinationSize.] */
...
/* Tests_SRS_CODEFIRST_99_002:[ CodeFirst_RegisterSchema shall create the schema information and give it to the Schema
 * module for one schema, identified by the metadata argument. On success, it shall return a handle to the model.] */
TEST_FUNCTION(CodeFirst_RegisterSchema_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(Schema_Create("TestSchema", IGNORED_PTR_ARG)).IgnoreArgument_metadata();
    ...
    STRICT_EXPECTED_CALL(
        Schema_AddStructTypeProperty(TEST_CAR_BEHIND_VAN_HANDLE, "whereIsMyCar_Field", "whereIsMyDevice_Struct"));
    ...
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "SimpleDevice_Model"))
        .SetReturn(TEST_TRUCKTYPE_MODEL_HANDLE);
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "double")).SetReturn(NULL);
    STRICT_EXPECTED_CALL(
        Schema_AddModelReportedProperty(TEST_TRUCKTYPE_MODEL_HANDLE, "new_reported_this_is_double", "double"));
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "int")).SetReturn(NULL);
    STRICT_EXPECTED_CALL(
        Schema_AddModelReportedProperty(TEST_TRUCKTYPE_MODEL_HANDLE, "new_reported_this_is_int", "int"));
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "int")).SetReturn(NULL);
    ...
    STRICT_EXPECTED_CALL(Schema_CreateModelAction(TEST_MODEL_HANDLE, "reset_Action")).SetReturn(RESET_ACTION_HANDLE);
...
TEST_FUNCTION(CodeFirst_RegisterSchema_With_Model_In_Model_succeeds)
{
    ...
    umock_c_reset_all_calls();
    STRICT_EXPECTED_CALL(Schema_GetSchemaByNamespace("TestSchema"));
    STRICT_EXPECTED_CALL(Schema_Create("TestSchema", TEST_SCHEMA_METADATA)).IgnoreArgument_metadata();
    ...
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "OuterType")).SetReturn(TEST_OUTERTYPE_MODEL_HANDLE);
    STRICT_EXPECTED_CALL(Schema_CreateModelAction(TEST_OUTERTYPE_MODEL_HANDLE, "OuterType_reset_Action"));
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "InnerType")).SetReturn(TEST_INNERTYPE_MODEL_HANDLE);
    STRICT_EXPECTED_CALL(
        Schema_AddModelModel(TEST_OUTERTYPE_MODEL_HANDLE, "Inner", TEST_INNERTYPE_MODEL_HANDLE, 8, NULL));
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "InnerType")).SetReturn(TEST_INNERTYPE_MODEL_HANDLE);
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "int"));
    STRICT_EXPECTED_CALL(Schema_AddModelDesiredProperty(TEST_INNERTYPE_MODEL_HANDLE,
                                                        "this_is_desired_int_Property_2",
                                                        "int",
                                                        IGNORED_PTR_ARG,
                                                        IGNORED_PTR_ARG,
                                                        IGNORED_PTR_ARG,
    ...
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "int"));
    STRICT_EXPECTED_CALL(
        Schema_AddModelReportedProperty(TEST_INNERTYPE_MODEL_HANDLE, "this_is_reported_int_Property_2", "int"));
...
TEST_FUNCTION(When_Schema_Create_Fails_Then_CodeFirst_RegisterSchema_Fails)
{
    ...
    umock_c_reset_all_calls();
    STRICT_EXPECTED_CALL(Schema_GetSchemaByNamespace("TestSchema"));
    STRICT_EXPECTED_CALL(Schema_Create("TestSchema", TEST_SCHEMA_METADATA)).SetReturn((SCHEMA_HANDLE)NULL);
...
TEST_FUNCTION(When_Schema_AddModelProperty_Fails_Then_CodeFirst_RegisterSchema_Fails)
{
    ...
    STRICT_EXPECTED_CALL(Schema_AddModelProperty(TEST_MODEL_HANDLE, "this_is_int", "int")).SetReturn(SCHEMA_ERROR);
    STRICT_EXPECTED_CALL(Schema_Destroy(TEST_SCHEMA_HANDLE));
...
TEST_FUNCTION(When_Schema_CreateModelAction_Fails_Then_CodeFirst_RegisterSchema_Fails)
{
    ...
    umock_c_reset_all_calls();
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "TruckType")).SetReturn(TEST_MODEL_HANDLE);
    STRICT_EXPECTED_CALL(Schema_CreateModelAction(TEST_MODEL_HANDLE, "setSpeed")).SetReturn((SCHEMA_ACTION_HANDLE)NULL);
    STRICT_EXPECTED_CALL(Schema_Destroy(TEST_SCHEMA_HANDLE));
...
TEST_FUNCTION(When_Schema_AddModelActionArgument_Fails_Then_CodeFirst_RegisterSchema_Fails)
{
    ...
    umock_c_reset_all_calls();
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "TruckType")).SetReturn(TEST_MODEL_HANDLE);
    STRICT_EXPECTED_CALL(Schema_CreateModelAction(TEST_MODEL_HANDLE, "setSpeed")).SetReturn(SETSPEED_ACTION_HANDLE);
    STRICT_EXPECTED_CALL(Schema_AddModelActionArgument(SETSPEED_ACTION_HANDLE, "theSpeed", "double"))
...
/* Tests_SRS_CODEFIRST_99_121:[If the schema has already been registered, CodeFirst_RegisterSchema shall return its
 * handle.] */
TEST_FUNCTION(When_Schema_Was_Already_Registered_CodeFirst_Returns_Its_Handle)
{
    ...
    (void)CodeFirst_Init(NULL);
    umock_c_reset_all_calls();
    STRICT_EXPECTED_CALL(Schema_GetSchemaByNamespace("TestSchema")).SetReturn((SCHEMA_HANDLE)TEST_SCHEMA_HANDLE);
...
/*Tests_SRS_CODEFIRST_02_014: [If parameter device or command is NULL then CodeFirst_ExecuteCommand shall return
...
/*Tests_SRS_CODEFIRST_02_016: [If finding the device fails, then CodeFirst_ExecuteCommand shall return
 * EXECUTE_COMMAND_ERROR.] */
...
/*Tests_SRS_CODEFIRST_02_017: [Otherwise CodeFirst_ExecuteCommand shall call Device_ExecuteCommand and return what
 * Device_ExecuteCommand is returning.] */
TEST_FUNCTION(CodeFirst_ExecuteCommand_calls_Device_ExecuteCommand)
{
    ...
    STRICT_EXPECTED_CALL(Device_ExecuteCommand(IGNORED_PTR_ARG, TEST_COMMAND)).IgnoreArgument(1);
...
/*Tests_SRS_CODEFIRST_02_018: [ If parameter destination, destinationSize or any of the values passed through va_args is
 * NULL then CodeFirst_SendAsyncReported shall fail and return CODEFIRST_INVALID_ARG. ]*/
...
/*Tests_SRS_CODEFIRST_02_019: [ If values passed through va_args do not belong to the same device then
 * CodeFirst_SendAsyncReported shall fail and return CODEFIRST_VALUES_FROM_DIFFERENT_DEVICES_ERROR. ]*/
TEST_FUNCTION(CodeFirst_SendAsyncReported_with_reportedProperties_from_different_devices_fails)
{
    ...
    TruckType *device1 = (TruckType *)
        CodeFirst_CreateDevice(TEST_MODEL_HANDLE, &DummyDataProvider_allReflected, sizeof(TruckType), false);
    TruckType *device2 = (TruckType *)
        CodeFirst_CreateDevice(TEST_MODEL_HANDLE, &DummyDataProvider_allReflected, sizeof(TruckType), false);
    device1->reported_this_is_int = 1;
    device2->reported_this_is_int = 2;
    umock_c_reset_all_calls();

    STRICT_EXPECTED_CALL(Device_CreateTransaction_ReportedProperties(IGNORED_PTR_ARG)).IgnoreArgument_deviceHandle();
    STRICT_EXPECTED_CALL(STRING_new());
    STRICT_EXPECTED_CALL(Schema_GetModelName(IGNORED_PTR_ARG)).IgnoreArgument_modelTypeHandle().SetReturn("TruckType");
    STRICT_EXPECTED_CALL(STRING_concat(IGNORED_PTR_ARG, "reported_this_is_int")).IgnoreArgument_handle();
    STRICT_EXPECTED_CALL(Create_AGENT_DATA_TYPE_from_SINT32(IGNORED_PTR_ARG, IGNORED_NUM_ARG))
        .IgnoreArgument_agentData()
        .IgnoreArgument_v();
    STRICT_EXPECTED_CALL(STRING_c_str(IGNORED_PTR_ARG)).IgnoreArgument_handle();
    STRICT_EXPECTED_CALL(
        Device_PublishTransacted_ReportedProperty(IGNORED_PTR_ARG, "reported_this_is_int", IGNORED_PTR_ARG))
        .IgnoreArgument_transactionHandle()
        .IgnoreArgument_data();
    STRICT_EXPECTED_CALL(STRING_delete(IGNORED_PTR_ARG)).IgnoreArgument_handle();
    STRICT_EXPECTED_CALL(Destroy_AGENT_DATA_TYPE(IGNORED_PTR_ARG)).IgnoreArgument_agentData();
    STRICT_EXPECTED_CALL(Device_DestroyTransaction_ReportedProperties(IGNORED_PTR_ARG))
        .IgnoreArgument_transactionHandle();

    /// act
    CODEFIRST_RESULT result = CodeFirst_SendAsyncReported(&destination,
                                                          &destinationSize,
                                                          2,
                                                          &device1->reported_this_is_int,
                                                          &device2->reported_this_is_int);
...
/*Tests_SRS_CODEFIRST_02_020: [ If values passed through va_args are not all of type REFLECTED_REPORTED_PROPERTY then
 * CodeFirst_SendAsyncReported shall fail and return CODEFIRST_INVALID_ARG. ]*/
TEST_FUNCTION(CodeFirst_SendAsyncReported_WITH_DATA_fails)
{
    ...
    STRICT_EXPECTED_CALL(Device_CreateTransaction_ReportedProperties(IGNORED_PTR_ARG)).IgnoreArgument_deviceHandle();
    STRICT_EXPECTED_CALL(STRING_new());
    STRICT_EXPECTED_CALL(Schema_GetModelName(IGNORED_PTR_ARG)).IgnoreArgument_modelTypeHandle();
    STRICT_EXPECTED_CALL(STRING_delete(IGNORED_PTR_ARG)).IgnoreArgument_handle();
    STRICT_EXPECTED_CALL(Device_DestroyTransaction_ReportedProperties(IGNORED_PTR_ARG))
        .IgnoreArgument_transactionHandle();
    ...
    /// act
    CODEFIRST_RESULT result =
        CodeFirst_SendAsyncReported(&destination,
                                    &destinationSize,
                                    1,
                                    &device->this_is_int); /*note: this_is_int is defined with WITH_DATA*/
...
TEST_FUNCTION(CodeFirst_SendAsyncReported_with_reportedProperty_outside_any_device_fails)
{
    ...
    /// act
    CODEFIRST_RESULT result = CodeFirst_SendAsyncReported(
        &destination,
        &destinationSize,
        1,
        &(device + 1)->reported_this_is_int); /*note: this is out of the memory of the device*/
...
static void CodeFirst_SendReportedAsync_all_inert_path(void)
{
    ...
    STRICT_EXPECTED_CALL(Create_AGENT_DATA_TYPE_from_DOUBLE(IGNORED_PTR_ARG, 5.5)).IgnoreArgument_agentData();
    STRICT_EXPECTED_CALL(
        Device_PublishTransacted_ReportedProperty(IGNORED_PTR_ARG, "new_reported_this_is_double", IGNORED_PTR_ARG))
        .IgnoreArgument_transactionHandle()
        .IgnoreArgument_data();
    EXPECTED_CALL(Destroy_AGENT_DATA_TYPE(IGNORED_PTR_ARG));

    STRICT_EXPECTED_CALL(Create_AGENT_DATA_TYPE_from_SINT32(IGNORED_PTR_ARG, -5)).IgnoreArgument_agentData();
    STRICT_EXPECTED_CALL(
        Device_PublishTransacted_ReportedProperty(IGNORED_PTR_ARG, "new_reported_this_is_int", IGNORED_PTR_ARG))
        .IgnoreArgument_transactionHandle()
        .IgnoreArgument_data();
    EXPECTED_CALL(Destroy_AGENT_DATA_TYPE(IGNORED_PTR_ARG));
...
/*Tests_SRS_CODEFIRST_02_021: [ If the value passed through va_args is a complete model instance, then
 * CodeFirst_SendAsyncReported shall send all the reported properties of that device. ]*/
/*Tests_SRS_CODEFIRST_02_022: [ CodeFirst_SendAsyncReported shall start a transaction by calling
 * Device_CreateTransaction_ReportedProperties. ]*/
/*Tests_SRS_CODEFIRST_02_023: [ CodeFirst_SendAsyncReported shall convert all REPORTED_PROPERTY model components to
 * AGENT_DATA_TYPE. ]*/
/*Tests_SRS_CODEFIRST_02_024: [ CodeFirst_SendAsyncReported shall call Device_PublishTransacted_ReportedProperty for
 * every AGENT_DATA_TYPE converted from REPORTED_PROPERTY. ]*/
/*Tests_SRS_CODEFIRST_02_026: [ CodeFirst_SendAsyncReported shall call Device_CommitTransaction_ReportedProperties to
 * commit the transaction. ]*/
/*Tests_SRS_CODEFIRST_02_028: [ CodeFirst_SendAsyncReported shall return CODEFIRST_OK when it succeeds. ]*/
/*Tests_SRS_CODEFIRST_02_029: [ CodeFirst_SendAsyncReported shall call Device_DestroyTransaction_ReportedProperties to
...
static void CodeFirst_SendReportedAsync_one_inert_path(void)
{
    ...
    STRICT_EXPECTED_CALL(STRING_new());
    STRICT_EXPECTED_CALL(Schema_GetModelName(TEST_MODEL_HANDLE));
    STRICT_EXPECTED_CALL(STRING_concat(IGNORED_PTR_ARG, "new_reported_this_is_double")).IgnoreArgument_handle();
    STRICT_EXPECTED_CALL(Create_AGENT_DATA_TYPE_from_DOUBLE(IGNORED_PTR_ARG, 5.5)).IgnoreArgument_agentData();
    STRICT_EXPECTED_CALL(STRING_c_str(IGNORED_PTR_ARG)).IgnoreArgument_handle();
    STRICT_EXPECTED_CALL(
        Device_PublishTransacted_ReportedProperty(IGNORED_PTR_ARG, "new_reported_this_is_double", IGNORED_PTR_ARG))
...
TEST_FUNCTION(CodeFirst_SendReportedAsync_one_reportedProperty_happy_path)
{
    ...
    /// act
    CODEFIRST_RESULT result =
        CodeFirst_SendAsyncReported(&destination, &destinationSize, 1, &(device->new_reported_this_is_double));
...
/*Tests_SRS_CODEFIRST_02_030: [ If argument device is NULL then CodeFirst_IngestDesiredProperties shall fail and return
 * CODEFIRST_INVALID_ARG. ]*/
...
/*Tests_SRS_CODEFIRST_02_031: [ If argument jsonProperties is NULL then CodeFirst_IngestDesiredProperties shall fail and
 * return CODEFIRST_INVALID_ARG. ]*/
...
/*Tests_SRS_CODEFIRST_02_034: [ If there is any failure, then CodeFirst_IngestDesiredProperties shall fail and return
 * CODEFIRST_ERROR. ]*/
TEST_FUNCTION(CodeFirst_IngestDesiredProperties_fails)
{
    ...
    /// act
    CODEFIRST_RESULT result = CodeFirst_IngestDesiredProperties(
        device - 1,
        "{\"a\":3}",
        false); /*notice how "device-1" is a non-valid memory address as far as FindDevice is concerned*/
...
TEST_FUNCTION(CodeFirst_CreateDevice_passes_onDesiredProperty_callbacks)
{
    ...
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "OuterType_onDesiredProperty"))
        .SetReturn(TEST_OUTERTYPE_MODEL_HANDLE);
    STRICT_EXPECTED_CALL(
        Schema_CreateModelAction(TEST_OUTERTYPE_MODEL_HANDLE, "OuterType_reset_Action_onDesiredProperty"));
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "InnerType_onDesiredProperty"))
        .SetReturn(TEST_INNERTYPE_MODEL_HANDLE);
    STRICT_EXPECTED_CALL(Schema_AddModelModel(TEST_OUTERTYPE_MODEL_HANDLE,
                                              "Inner_onDesiredProperty",
                                              TEST_INNERTYPE_MODEL_HANDLE,
                                              8,
                                              onInner_onDesiredProperty));
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "InnerType_onDesiredProperty"))
        .SetReturn(TEST_INNERTYPE_MODEL_HANDLE);
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "int"));
    STRICT_EXPECTED_CALL(Schema_AddModelDesiredProperty(TEST_INNERTYPE_MODEL_HANDLE,
        ...
        .IgnoreArgument_offset();
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "int"));
    STRICT_EXPECTED_CALL(Schema_AddModelReportedProperty(TEST_INNERTYPE_MODEL_HANDLE,
                                                         "this_is_reported_int_Property_2_onDesiredProperty",
                                                         "int"));
    STRICT_EXPECTED_CALL(
        Schema_CreateModelAction(TEST_INNERTYPE_MODEL_HANDLE, "InnerType_reset_Action_onDesiredProperty"));
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "double"));
    STRICT_EXPECTED_CALL(
        Schema_AddModelProperty(TEST_INNERTYPE_MODEL_HANDLE, "this_is_double2_onDesiredProperty", "double"));
    STRICT_EXPECTED_CALL(Schema_GetModelByName(TEST_SCHEMA_HANDLE, "int"));
    ...
    SCHEMA_HANDLE result =
        CodeFirst_RegisterSchema("TestSchema", &ALL_REFLECTED(testModelInModelReflected_with_onDesiredProperty));
...
/*Tests_SRS_CODEFIRST_02_037: [ CodeFirst_CreateDevice shall call CodeFirst_Init, passing NULL for
 * overrideSchemaNamespace. ]*/
...
/*Tests_SRS_CODEFIRST_02_039: [ If the current device count is zero then CodeFirst_DestroyDevice shall deallocate all
...
/*Tests_SRS_CODEFIRST_02_046: [ CodeFirst_SendAsyncReported shall call CodeFirst_Init, passing NULL for
 * overrideSchemaNamespace. ]*/
...
/*Tests_SRS_CODEFIRST_02_054: [ If parameterCount is greater than 0 and parameterValues is NULL then then
 * CodeFirst_InvokeMethod shall fail and return NULL. ]*/
...
TEST_FUNCTION(CodeFirst_InvokeMethod_happy_path)
{
    ...
    METHODRETURN_HANDLE result =
        CodeFirst_InvokeMethod(TEST_DEVICE_HANDLE, g_InvokeActionCallbackArgument, "", "resetMethod", 0, NULL);
...
TEST_FUNCTION(CodeFirst_InvokeMethod_fails_when_Schema_GetModelName_fails)
{
    ...
    STRICT_EXPECTED_CALL(Schema_GetModelName(TEST_MODEL_HANDLE)).SetReturn("NULL");
...
TEST_FUNCTION(CodeFirst_InvokeMethod_fails_when_relativePath_does_not_exist_fails)
{
    ...
    /// act
    METHODRETURN_HANDLE result = CodeFirst_InvokeMethod(TEST_DEVICE_HANDLE,
                                                        g_InvokeActionCallbackArgument,
                                                        "someInexistingRelativePath",
                                                        "resetMethod",
                                                        0,
                                                        NULL);
...
TEST_FUNCTION(CodeFirst_InvokeMethod_fails_when_method_does_not_exist_fails)
{
    ...
    METHODRETURN_HANDLE result = CodeFirst_InvokeMethod(TEST_DEVICE_HANDLE,
                                                        g_InvokeActionCallbackArgument,
                                                        "",
                                                        "resetMethod_which_does_not_exist",
                                                        0,
                                                        NULL);
...
TEST_FUNCTION(CodeFirst_InvokeMethod_fails_when_model_does_not_exist_fails)
{
    ...
    STRICT_EXPECTED_CALL(Schema_GetModelName(TEST_MODEL_HANDLE)).SetReturn("TruckType_which_does_not_exist");
...
/*Tests_SRS_CODEFIRST_02_063: [ CodeFirst_ExecuteMethod shall call Device_ExecuteMethod and return what
 * Device_ExecuteMethod returns. ]*/
...
TEST_FUNCTION(CodeFirst_ExecuteMethod_unhappy_path_1)
{
    ...
    STRICT_EXPECTED_CALL(Device_ExecuteMethod(TEST_DEVICE_HANDLE, "reset", NULL)).SetReturn(NULL);
...
TEST_FUNCTION(CodeFirst_ExecuteMethod_unhappy_path_2)
{
    ...
    /// act
    METHODRETURN_HANDLE result =
        CodeFirst_ExecuteMethod((char *)device - 1, "reset", NULL); /*device-1 is not a valid Device*/
