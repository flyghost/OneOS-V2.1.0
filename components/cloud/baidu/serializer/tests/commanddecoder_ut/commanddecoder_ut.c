...
static void SetupCommand(const char *quotedActionName, const char *actionName)
{
    ...
    STRICT_EXPECTED_CALL(Schema_GetModelActionByName(TEST_MODEL_HANDLE, actionName)).SetReturn(SetACStateActionHandle);
}

void SetupArgumentCalls(SCHEMA_ACTION_HANDLE          actionHandle,
                        size_t                        index,
                        SCHEMA_ACTION_ARGUMENT_HANDLE argHandle,
                        const char                   *argName,
                        const char                   *argType)
{

    STRICT_EXPECTED_CALL(Schema_GetModelActionArgumentByIndex(actionHandle, index)).SetReturn(argHandle);
    STRICT_EXPECTED_CALL(Schema_GetActionArgumentName(argHandle)).SetReturn(argName);
    STRICT_EXPECTED_CALL(Schema_GetActionArgumentType(argHandle)).SetReturn(argType);
}
...
static AGENT_DATA_TYPES_RESULT my_Create_AGENT_DATA_TYPE_from_Members(AGENT_DATA_TYPE       *agentData,
                                                                      const char            *typeName,
                                                                      size_t                 nMembers,
                                                                      const char *const     *memberNames,
                                                                      const AGENT_DATA_TYPE *memberValues)
{
    (void)memberValues;
    (void)typeName;
    (void)agentData;
...
char *umockvalue_stringify_SCHEMA_MODEL_ELEMENT(const SCHEMA_MODEL_ELEMENT *value)
{
    ...
    size_t needed = snprintf(NULL,
                             0,
                             "{.elementType=%s, .elementHandle=%p}",
                             ENUM_TO_STRING(SCHEMA_ELEMENT_TYPE, value->elementType),
                             value->elementHandle.modelHandle);
...
TEST_SUITE_INITIALIZE(TestClassInitialize)
{
    ...
    Schema_GetModelElementByName_desiredProperty_int_field.elementHandle.desiredPropertyHandle =
        TEST_DESIRED_PROPERTY_HANDLE_INT_FIELD;
...
TEST_FUNCTION(CommandDecoder_Create_With_NULL_Model_Handle_Fails)
{
    ...
    // act
    COMMAND_DECODER_HANDLE result = CommandDecoder_Create(NULL,
                                                          ActionCallbackMock,
                                                          TEST_CALLBACK_CONTEXT_VALUE,
                                                          methodCallbackMock,
                                                          TEST_CALLBACK_CONTEXT_VALUE);
...
/* Tests_CommandDecoder_Create shall give its receive callback to the transport by calling
 * IoTHubClient_SetMessageCallback when transportType is TRANSPORT_THREADED.*/
/* Tests_SRS_COMMAND_DECODER_02_017: [If the receive callback is different than NULL, then CommandDecoder_Create shall
 * call Transport_StartReceive.] */
TEST_FUNCTION(CommandDecoder_Create_With_Non_NULL_Callback_Argument_Succeeds)
{
    ...
    // act
    COMMAND_DECODER_HANDLE result = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                          ActionCallbackMock,
                                                          TEST_CALLBACK_CONTEXT_VALUE,
                                                          methodCallbackMock,
                                                          TEST_CALLBACK_CONTEXT_VALUE);
...
/*Tests_SRS_COMMAND_DECODER_02_018: [CommandDecoder_Create shall give its receive callback to the transport by calling
 * IoTHubClient_LL_SetMessageCallback when transportType is TRANSPORT_LL.]*/
...
/* Tests_SRS_COMMAND_DECODER_01_004: [If any error is encountered during CommandDecoder_Create CommandDecoder_Create
 * shall return NULL.] */
TEST_FUNCTION(When_SetReceiveCallback_Fails_Then_gballoc_Fails)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)).IgnoreArgument(1).SetReturn(NULL);
...
/* Tests_SRS_COMMAND_DECODER_01_007: [If CommandDecoder_Destroy is called with a NULL handle, CommandDecoder_Destroy
 * shall do nothing.] */
...
/* Tests_SRS_COMMAND_DECODER_07_015: [CommandDecoder_Destroy shall remove the running thread by calling
 * Transport_StopReceive.] */
/* Tests_SRS_COMMAND_DECODER_01_005: [CommandDecoder_Destroy shall free all resources associated with the
 * commandDecoderHandle instance.] */
TEST_FUNCTION(CommandDecoder_Destroy_succeeds)
{
    /// arrange

    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
                                                                        TEST_CALLBACK_CONTEXT_VALUE,
                                                                        methodCallbackMock,
                                                                        TEST_CALLBACK_CONTEXT_VALUE);
...
/* Tests_SRS_COMMAND_DECODER_01_010: [If either parameter is NULL, the processing shall stop and the command shall not
 * be dispatched and it shall return EXECUTE_COMMAND_ERROR.] */
...
/*Tests_SRS_COMMAND_DECODER_01_011: [If the size of the command is 0 then the processing shall stop and the command
 * shall not be dispatched and it shall return EXECUTE_COMMAND_ERROR.]*/
...
/*Tests_SRS_COMMAND_DECODER_01_013: [If parsing the JSON to a multi tree fails, the processing shall stop and the
 * command shall not be dispatched and it shall return EXECUTE_COMMAND_ERROR.]*/
TEST_FUNCTION(When_copying_the_command_content_fails_returns_EXECUTE_COMMAND_ERROR)
{
    ...
    STRICT_EXPECTED_CALL(
        gballoc_malloc(strlen(TEST_COMMAND) + 1)) /*this creates a copy of the command that is given to JSON decoder*/
        .SetReturn(NULL);
...
TEST_FUNCTION(When_Parsing_The_JSON_To_MultiTree_Fails_Then_No_Command_Is_Dispatched)
{
    ...
    STRICT_EXPECTED_CALL(JSONDecoder_JSON_To_MultiTree(TestCommand, IGNORED_PTR_ARG))
        .IgnoreArgument(2)
...
/* Tests_SRS_COMMAND_DECODER_01_015: [If any MultiTree API call fails then the processing shall stop and the command
 * shall not be dispatched and it shall return EXECUTE_COMMAND_ERROR.] */
TEST_FUNCTION(When_Getting_The_Schema_For_The_Model_Fails_Then_No_Command_Is_Dispatched)
{
    ...
    STRICT_EXPECTED_CALL(JSONDecoder_JSON_To_MultiTree(TestCommand, IGNORED_PTR_ARG)).IgnoreArgument(2);
    STRICT_EXPECTED_CALL(Schema_GetSchemaForModelType(TEST_MODEL_HANDLE)).SetReturn((SCHEMA_HANDLE)NULL);
...
/* Tests_SRS_COMMAND_DECODER_99_010:[ If any Schema API fails then the command shall not be dispatched and it shall
...
/*Tests_SRS_COMMAND_DECODER_99_021:[ If the parsing of the command fails for any other reason the command shall not be
 * dispatched.]*/
...
/* Tests_SRS_COMMAND_DECODER_99_011:[ CommandDecoder shall attempt to extract the command arguments from the command
 * JSON by looking them up under the node "Parameters".] */
/* Tests_SRS_COMMAND_DECODER_99_027:[ The value for an argument of primitive type shall be decoded by using the
 * CreateAgentDataType_From_String API.] */
/* Tests_SRS_COMMAND_DECODER_01_014: [CommandDecoder shall use the MultiTree APIs to extract a specific element from the
 * command JSON.] */
/* Tests_SRS_COMMAND_DECODER_99_005:[ If an action is decoded successfully then the callback actionCallback shall be
 * called, passing to it the callback action context, decoded name and arguments.] */
/* Tests_SRS_COMMAND_DECODER_01_008: [Each argument shall be looked up as a field, member of the "Parameters" node.]  */
TEST_FUNCTION(
    CommandDecoder_ExecuteCommand_With_Valid_Command_With_1_Arg_Decodes_The_Argument_And_Calls_The_ActionCallback)
{
    // arrange
    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
    ...
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG)) /*this is allocating memory for the argument array*/
        .IgnoreArgument(1);
    SetupArgumentCalls(SetACStateActionHandle,
                       0,
                       StateActionArgument,
                       StateActionArgument_Name,
                       StateActionArgument_Type);
    ...
    STRICT_EXPECTED_CALL(CodeFirst_GetPrimitiveType("bool")).SetReturn(EDM_BOOLEAN_TYPE);
...
TEST_FUNCTION(CommandDecoder_When_GetActionArgumentName_Fails_ExecuteCommand_Fails)
{
    ...
    STRICT_EXPECTED_CALL(Schema_GetActionArgumentName(StateActionArgument)).SetReturn((const char *)NULL);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(MultiTree_Destroy(IGNORED_PTR_ARG)).IgnoreArgument_treeHandle();
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)) /*free-ing the copy of the buffer*/
...
TEST_FUNCTION(CommandDecoder_When_GetActionArgumentType_Fails_ExecuteCommand_Fails)
{
    ...
    STRICT_EXPECTED_CALL(Schema_GetModelActionArgumentByIndex(SetACStateActionHandle, 0))
        .SetReturn(StateActionArgument);
    STRICT_EXPECTED_CALL(Schema_GetActionArgumentName(StateActionArgument)).SetReturn(StateActionArgument_Name);
    STRICT_EXPECTED_CALL(Schema_GetActionArgumentType(StateActionArgument)).SetReturn((const char *)NULL);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(MultiTree_Destroy(IGNORED_PTR_ARG)).IgnoreArgument_treeHandle();
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)) /*free-ing the copy of the buffer*/
...
/* Tests_SRS_COMMAND_DECODER_99_012:[ If any argument is missing in the command text then the command shall not be
 * dispatched and it shall return EXECUTE_COMMAND_ERROR.] */
...
/* Tests_SRS_COMMAND_DECODER_99_011:[ CommandDecoder shall attempt to extract from the command text the value for each
 * action argument.] */
/* Tests_SRS_COMMAND_DECODER_99_027:[ The value for an argument of primitive type shall be decoded by using the
 * CreateAgentDataType_From_String API.] */
/* Tests_SRS_COMMAND_DECODER_01_014: [CommandDecoder shall use the MultiTree APIs to extract a specific element from the
 ...
 * called, passing to it the callback action context, decoded name and arguments.] */
TEST_FUNCTION(
    CommandDecoder_ExecuteCommand_With_Valid_Command_With_2_Args_Decodes_The_Arguments_And_Calls_The_ActionCallback)
{
    // arrange
    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
    ...
    SetupArgumentCalls(SetACStateActionHandle,
                       1,
                       OtherArgActionArgument,
                       OtherArgActionArgument_Name,
                       OtherArgActionArgument_Type);
    ...
    STRICT_EXPECTED_CALL(CodeFirst_GetPrimitiveType("ascii_char_ptr")).SetReturn(EDM_STRING_TYPE);
...
TEST_FUNCTION(CommandDecoder_When_GetArgument_Name_For_The_2nd_Argument_Fails_Then_ExecuteCommand_Fails)
{
    ...
    STRICT_EXPECTED_CALL(Schema_GetActionArgumentName(OtherArgActionArgument)).SetReturn((const char *)NULL);
...
TEST_FUNCTION(CommandDecoder_When_GetArgument_Type_For_The_2nd_Argument_Fails_Then_ExecuteCommand_Fails)
{
    ...
    STRICT_EXPECTED_CALL(Schema_GetModelActionArgumentByIndex(SetACStateActionHandle, 1))
        .SetReturn(OtherArgActionArgument);
    STRICT_EXPECTED_CALL(Schema_GetActionArgumentName(OtherArgActionArgument)).SetReturn(OtherArgActionArgument_Name);
    STRICT_EXPECTED_CALL(Schema_GetActionArgumentType(OtherArgActionArgument)).SetReturn((const char *)NULL);
...
/* Tests_SRS_COMMAND_DECODER_99_029:[ If the argument type is complex then a complex type value shall be built from the
 * child nodes.] */
/* Tests_SRS_COMMAND_DECODER_99_030:[ For each child node a value shall be built by using AgentTypeSystem APIs.] */
/* Tests_SRS_COMMAND_DECODER_99_031:[ The complex type value that aggregates the children shall be built by using the
 * Create_AGENT_DATA_TYPE_from_Members.] */
/* Tests_SRS_COMMAND_DECODER_99_033:[ In order to determine which are the members of a complex types, Schema APIs for
 * structure types shall be used.] */
/* Tests_SRS_COMMAND_DECODER_01_014: [CommandDecoder shall use the MultiTree APIs to extract a specific element from the
 * command JSON.] */
/* Tests_SRS_COMMAND_DECODER_99_005:[ If an action is decoded successfully then the callback actionCallback shall be
 * called, passing to it the callback action context, decoded name and arguments.] */
TEST_FUNCTION(CommandDecoder_When_The_Argument_Is_Complex_The_Nodes_Are_Scanned_To_Get_The_Members)
{
    ...
    SetupArgumentCalls(SetACStateActionHandle,
                       ...
                       LocationActionArgument_Name,
                       LocationActionArgument_Type);
    STRICT_EXPECTED_CALL(MultiTree_GetChildByName(TEST_COMMAND_ARGS_NODE, "Location", IGNORED_PTR_ARG))
        .CopyOutArgumentBuffer(3, &TEST_ARG1_NODE, sizeof(TEST_ARG1_NODE));
    STRICT_EXPECTED_CALL(CodeFirst_GetPrimitiveType("GeoLocation")).SetReturn(EDM_NO_TYPE);
    STRICT_EXPECTED_CALL(Schema_GetStructTypeByName(TEST_SCHEMA_HANDLE, "GeoLocation")).SetReturn(TEST_STRUCT_1_HANDLE);
    ...
    /* member 1 */
    STRICT_EXPECTED_CALL(Schema_GetStructTypePropertyByIndex(TEST_STRUCT_1_HANDLE, 0)).SetReturn(memberProperty1);
    STRICT_EXPECTED_CALL(Schema_GetPropertyName(memberProperty1)).SetReturn("Lat");
    STRICT_EXPECTED_CALL(Schema_GetPropertyType(memberProperty1)).SetReturn("double");
    ...
    STRICT_EXPECTED_CALL(CodeFirst_GetPrimitiveType("double")).SetReturn(EDM_DOUBLE_TYPE);
    ...
    /* member 2 */
    STRICT_EXPECTED_CALL(Schema_GetStructTypePropertyByIndex(TEST_STRUCT_1_HANDLE, 1)).SetReturn(memberProperty2);
    STRICT_EXPECTED_CALL(Schema_GetPropertyName(memberProperty2)).SetReturn("Long");
    STRICT_EXPECTED_CALL(Schema_GetPropertyType(memberProperty2)).SetReturn("double");
    ...
    EXPECTED_CALL(
        Create_AGENT_DATA_TYPE_from_Members(IGNORED_PTR_ARG, "GeoLocation", 2, IGNORED_PTR_ARG, IGNORED_PTR_ARG))
        .ValidateArgument(2)
        .ValidateArgument(3);
 ...
 * dispatched.]*/
TEST_FUNCTION(
    CommandDecoder_When_The_Argument_Is_Complex_The_Nodes_Are_Scanned_To_Get_The_Members_and_fail_when_gbaloc_fails_1)
{
    // arrange
    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
 ...
 * dispatched.]*/
TEST_FUNCTION(
    CommandDecoder_When_The_Argument_Is_Complex_The_Nodes_Are_Scanned_To_Get_The_Members_and_fail_when_gbaloc_fails_2)
{
    // arrange
    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
 ...
 * return EXECUTE_COMMAND_ERROR.]*/
TEST_FUNCTION(
    CommandDecoder_When_Getting_The_Structure_PropertyCount_For_A_Complex_Type_Fails_Then_ExecuteCommand_Fails)
{
    // arrange
    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
...
/* Tests_SRS_COMMAND_DECODER_99_034:[ If Schema APIs indicate that a complex type has 0 members then the command shall
 * not be dispatched and it shall return EXECUTE_COMMAND_ERROR.] */
...
TEST_FUNCTION(CommandDecoder_When_Getting_The_Struct_Member_Name_Fails_Then_ExecuteCommand_Fails)
{
    ...
    /* member 1 */
    STRICT_EXPECTED_CALL(Schema_GetStructTypePropertyByIndex(TEST_STRUCT_1_HANDLE, 0)).SetReturn(memberProperty1);
    STRICT_EXPECTED_CALL(Schema_GetPropertyName(memberProperty1)).SetReturn((const char *)NULL);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
...
TEST_FUNCTION(CommandDecoder_When_Getting_The_Struct_Member_Type_Fails_Then_ExecuteCommand_Fails)
{
    ...
    STRICT_EXPECTED_CALL(Schema_GetStructTypePropertyByIndex(TEST_STRUCT_1_HANDLE, 0)).SetReturn(memberProperty1);
    STRICT_EXPECTED_CALL(Schema_GetPropertyName(memberProperty1)).SetReturn("Lat");
    STRICT_EXPECTED_CALL(Schema_GetPropertyType(memberProperty1)).SetReturn((const char *)NULL);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(gballoc_free(IGNORED_PTR_ARG)).IgnoreArgument(1);
 ...
 * shall not be dispatched and it shall return EXECUTE_COMMAND_ERROR.] */
TEST_FUNCTION(
    CommandDecoder_When_Getting_The_Child_Node_For_A_Member_Property_For_A_Complex_Type_Fails_Then_ExecuteCommand_Fails)
{
    // arrange
    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
 ...
 * shall not be dispatched and it shall return EXECUTE_COMMAND_ERROR.] */
TEST_FUNCTION(
    CommandDecoder_When_Getting_The_Child_Value_For_A_Member_Property_For_A_Complex_Type_Fails_Then_ExecuteCommand_Fails)
{
    // arrange
    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
...
/* Tests_SRS_COMMAND_DECODER_99_028:[ If decoding the argument fails then the command shall not be dispatched and it
 * shall return EXECUTE_COMMAND_ERROR.] */
TEST_FUNCTION(
    CommandDecoder_When_Creating_The_Agent_Data_Type_Value_For_A_Member_Property_For_A_Complex_Type_Fails_Then_ExecuteCommand_Fails)
{
    // arrange
    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
 ...
 * shall return EXECUTE_COMMAND_ERROR.] */
TEST_FUNCTION(
    CommandDecoder_When_Creating_The_Complex_Type_Agent_Data_Type_Value_For_A_Complex_Type_Fails_Then_ExecuteCommand_Fails)
{
    // arrange
    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
    ...
    EXPECTED_CALL(
        Create_AGENT_DATA_TYPE_from_Members(IGNORED_PTR_ARG, "GeoLocation", 1, IGNORED_PTR_ARG, IGNORED_PTR_ARG))
        .ValidateArgument(2)
        .ValidateArgument(3)
        .SetReturn(AGENT_DATA_TYPES_INVALID_ARG);
 ...
 * shall return EXECUTE_COMMAND_ERROR.] */
TEST_FUNCTION(
    CommandDecoder_When_Decoding_The_2nd_Member_Of_A_Complex_Property_Fails_Execute_Command_Frees_The_Previously_Allocated_Member)
{
    // arrange
    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
...
TEST_FUNCTION(CommandDecoder_Decoding_An_Action_With_Nested_Complex_Types_Succeeds)
{
    ...
    STRICT_EXPECTED_CALL(Schema_GetPropertyName(memberNestedComplexTypeProperty)).SetReturn("NestedLocation");
    STRICT_EXPECTED_CALL(Schema_GetPropertyType(memberNestedComplexTypeProperty)).SetReturn("NestedGeoLocation");
    ...
    STRICT_EXPECTED_CALL(CodeFirst_GetPrimitiveType("NestedGeoLocation")).SetReturn(EDM_NO_TYPE);
    ...
    /* member 1, nested */
    STRICT_EXPECTED_CALL(Schema_GetStructTypePropertyByIndex(TEST_STRUCT_2_HANDLE, 0)).SetReturn(memberProperty1);
    STRICT_EXPECTED_CALL(Schema_GetPropertyName(memberProperty1)).SetReturn("Lat");
    STRICT_EXPECTED_CALL(Schema_GetPropertyType(memberProperty1)).SetReturn("double");
    ...
    /* member 2, nested */
    STRICT_EXPECTED_CALL(Schema_GetStructTypePropertyByIndex(TEST_STRUCT_2_HANDLE, 1)).SetReturn(memberProperty2);
    STRICT_EXPECTED_CALL(Schema_GetPropertyName(memberProperty2)).SetReturn("Long");
    STRICT_EXPECTED_CALL(Schema_GetPropertyType(memberProperty2)).SetReturn("double");
    ...
    EXPECTED_CALL(
        Create_AGENT_DATA_TYPE_from_Members(IGNORED_PTR_ARG, "NestedGeoLocation", 2, IGNORED_PTR_ARG, IGNORED_PTR_ARG))
        .ValidateArgument(2)
        .ValidateArgument(3);
...
/* Tests_SRS_COMMAND_DECODER_99_035:[ CommandDecoder_ExecuteCommand shall support paths to actions that are in child
 * models (i.e. ChildModel/SomeAction.] */
TEST_FUNCTION(CommandDecoder_ExecuteCommand_With_A_Command_In_A_Child_Model_Calls_The_ActionCallback)
{
    ...
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG))
        /*this is relativeActionPath*/ /*well - first part of it ="ChildModel"*/
        .IgnoreArgument(1);
 ...
 * models (i.e. ChildModel/SomeAction.] */
TEST_FUNCTION(
    CommandDecoder_ExecuteCommand_With_A_Command_In_A_Child_Model_Calls_The_ActionCallback_and_returns_REJECTED)
{
    // arrange
    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
    ...
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG))
        /*this is relativeActionPath*/ /*well - last part of it ="SetACState"*/
        .IgnoreArgument(1);
 ...
 * models (i.e. ChildModel/SomeAction.] */
TEST_FUNCTION(
    CommandDecoder_ExecuteCommand_With_A_Command_In_A_Child_Model_Calls_The_ActionCallback_and_returns_ABANDONED)
{
    // arrange
    COMMAND_DECODER_HANDLE commandDecoderHandle = CommandDecoder_Create(TEST_MODEL_HANDLE,
                                                                        ActionCallbackMock,
...
/* Tests_SRS_COMMAND_DECODER_99_036:[ If a child model cannot be found by using Schema APIs then the command shall not
 * be dispatched and it shall return EXECUTE_COMMAND_ERROR.] */
/* Tests_SRS_COMMAND_DECODER_99_037:[ The relative path passed to the actionCallback shall be in the format
...
/*Tests_SRS_COMMAND_DECODER_02_001: [ If startAddress is NULL then CommandDecoder_IngestDesiredProperties shall fail and
 * return EXECUTE_COMMAND_ERROR. ]*/
TEST_FUNCTION(CommandDecoder_IngestDesiredProperties_with_NULL_startAddress_fails)
{
    ...
    EXECUTE_COMMAND_RESULT result =
        CommandDecoder_IngestDesiredProperties(NULL, commandDecoderHandle, "some properties", false);
...
/*Tests_SRS_COMMAND_DECODER_02_002: [ If handle is NULL then CommandDecoder_IngestDesiredProperties shall fail and
 * return EXECUTE_COMMAND_ERROR. ]*/
TEST_FUNCTION(CommandDecoder_IngestDesiredProperties_with_NULL_handle_fails)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CommandDecoder_IngestDesiredProperties(deviceMemoryArea, NULL, "some properties", false);
...
/*Tests_SRS_COMMAND_DECODER_02_003: [ If desiredProperties is NULL then CommandDecoder_IngestDesiredProperties shall
 * fail and return EXECUTE_COMMAND_ERROR. ]*/
TEST_FUNCTION(CommandDecoder_IngestDesiredProperties_with_NULL_desiredProperties_fails)
{
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CommandDecoder_IngestDesiredProperties(deviceMemoryArea, commandDecoderHandle, NULL, false);
...
void CommandDecoder_IngestDesiredProperties_with_1_simple_desired_property_succeeds_inert_path(
    unsigned char   *deviceMemoryArea,
    const char      *desiredPropertiesJSON,
    const char      *three,
    size_t           one,
    MULTITREE_HANDLE childHandle,
    bool             desiredPropertyHasCallback)
{
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, desiredPropertiesJSON)).IgnoreArgument_destination();
    ...
    STRICT_EXPECTED_CALL(STRING_new()).SetReturn(TEST_STRING_HANDLE_CHILD_NAME);

    STRICT_EXPECTED_CALL(MultiTree_GetName(
        childHandle,
        TEST_STRING_HANDLE_CHILD_NAME)); /*this fills in TEST_STRING_HANDLE_CHILD_NAME with "int_field"*/
    ...
    STRICT_EXPECTED_CALL(Schema_GetModelDesiredPropertyType(TEST_DESIRED_PROPERTY_HANDLE_INT_FIELD)).SetReturn("int");

    STRICT_EXPECTED_CALL(Schema_GetSchemaForModelType(TEST_MODEL_HANDLE)).SetReturn(TEST_SCHEMA);

    /*this is DecodeValueFromNode expected calls*/

    STRICT_EXPECTED_CALL(CodeFirst_GetPrimitiveType("int")).SetReturn(EDM_INT32_TYPE);
    ...
    STRICT_EXPECTED_CALL(
        Schema_GetModelDesiredProperty_pfDesiredPropertyFromAGENT_DATA_TYPE(TEST_DESIRED_PROPERTY_HANDLE_INT_FIELD))
        .SetReturn(int_pfDesiredPropertyFromAGENT_DATA_TYPE);

    STRICT_EXPECTED_CALL(Schema_GetModelDesiredProperty_offset(TEST_DESIRED_PROPERTY_HANDLE_INT_FIELD)).SetReturn(2);

    STRICT_EXPECTED_CALL(
        int_pfDesiredPropertyFromAGENT_DATA_TYPE(IGNORED_PTR_ARG, (unsigned char *)deviceMemoryArea + 2))
        .IgnoreArgument_source();
    ...
    {
        STRICT_EXPECTED_CALL(onDesiredPropertySimpleProperty(IGNORED_PTR_ARG)).IgnoreArgument_v();
    }
...
/*Tests_SRS_COMMAND_DECODER_02_005: [ CommandDecoder_IngestDesiredProperties shall create a MULTITREE_HANDLE ouf of the
 * clone of desiredProperties. ]*/
/*Tests_SRS_COMMAND_DECODER_02_007: [ If the child name corresponds to a desired property then an AGENT_DATA_TYPE shall
 * be constructed from the MULTITREE node. ]*/
/*Tests_SRS_COMMAND_DECODER_02_008: [ The desired property shall be constructed in memory by calling
 * pfDesiredPropertyFromAGENT_DATA_TYPE. ]*/
/*Tests_SRS_COMMAND_DECODER_02_010: [ If the complete MULTITREE has been parsed then
 * CommandDecoder_IngestDesiredProperties shall succeed and return EXECUTE_COMMAND_SUCCESS. ]*/
TEST_FUNCTION(CommandDecoder_IngestDesiredProperties_with_1_simple_desired_property_happy_path)
{
    ...
    CommandDecoder_IngestDesiredProperties_with_1_simple_desired_property_succeeds_inert_path(deviceMemoryArea,
                                                                                              desiredPropertiesJSON,
                                                                                              three,
                                                                                              one,
                                                                                              childHandle,
                                                                                              false);
    ...
    /// act
    EXECUTE_COMMAND_RESULT result =
        CommandDecoder_IngestDesiredProperties(deviceMemoryArea, commandDecoderHandle, desiredPropertiesJSON, false);
...
/*Tests_SRS_COMMAND_DECODER_02_011: [ Otherwise CommandDecoder_IngestDesiredProperties shall fail and return
...
TEST_FUNCTION(CommandDecoder_IngestDesiredProperties_with_1_simple_desired_property_succeeds_unhappy_paths)
{
    ...
    for (size_t i = 0; i < umock_c_negative_tests_call_count(); i++)
    {
        ...
        if (j == sizeof(calls_that_cannot_fail) / sizeof(calls_that_cannot_fail[0]))
        {
            ...
            /// act
            EXECUTE_COMMAND_RESULT result = CommandDecoder_IngestDesiredProperties(deviceMemoryArea,
                                                                                   commandDecoderHandle,
                                                                                   desiredPropertiesJSON,
                                                                                   false);
...
void CommandDecoder_IngestDesiredProperties_with_1_simple_model_in_model_desired_property_inert_path(
    unsigned char   *deviceMemoryArea,
    const char      *desiredPropertiesJSON,
    const char      *three,
    size_t           one,
    MULTITREE_HANDLE childHandle,
    bool             desiredPropertiesHaveCallbacks)
{
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, desiredPropertiesJSON)).IgnoreArgument_destination();
    ...
    STRICT_EXPECTED_CALL(MultiTree_GetName(
        childHandle,
        TEST_STRING_HANDLE_CHILD_NAME)); /*this fills in TEST_STRING_HANDLE_CHILD_NAME with "modelInModel"*/
...

    {
        ...
        STRICT_EXPECTED_CALL(MultiTree_GetName(childHandle, TEST_STRING_HANDLE_CHILD_NAME));
            /*13*/ /*this fills in TEST_STRING_HANDLE_CHILD_NAME with "int_field"*/
        ...
        STRICT_EXPECTED_CALL(Schema_GetModelDesiredProperty_pfDesiredPropertyFromAGENT_DATA_TYPE(
                                 TEST_DESIRED_PROPERTY_HANDLE_INT_FIELD)) /*22*/
            .SetReturn(int_pfDesiredPropertyFromAGENT_DATA_TYPE);

        STRICT_EXPECTED_CALL(Schema_GetModelDesiredProperty_offset(TEST_DESIRED_PROPERTY_HANDLE_INT_FIELD)) /*23*/
            .SetReturn(2);

        STRICT_EXPECTED_CALL(int_pfDesiredPropertyFromAGENT_DATA_TYPE(IGNORED_PTR_ARG,
                                                                      (unsigned char *)deviceMemoryArea +
                                                                          12)) /*notice here the new offset (2+10)*/
            .IgnoreArgument_source();
    ...
    if (desiredPropertiesHaveCallbacks)
    {
        STRICT_EXPECTED_CALL(onDesiredPropertyModelInModel(IGNORED_PTR_ARG)).IgnoreArgument_v();
    }
...
/*Tests_SRS_COMMAND_DECODER_02_009: [ If the child name corresponds to a model in model then the function shall call
 * itself recursively. ]*/
TEST_FUNCTION(CommandDecoder_IngestDesiredProperties_with_1_simple_model_in_model_desired_property_happy_path)
{
    ...
    CommandDecoder_IngestDesiredProperties_with_1_simple_model_in_model_desired_property_inert_path(
        deviceMemoryArea,
        desiredPropertiesJSON,
        three,
        one,
...
/*Tests_SRS_COMMAND_DECODER_02_013: [ If the desired property has a non-NULL pfOnDesiredProperty then it shall be
 * called. ]*/
...
/*Tests_SRS_COMMAND_DECODER_02_012: [ If the child model in model has a non-NULL pfOnDesiredProperty then
 * pfOnDesiredProperty shall be called. ]*/
TEST_FUNCTION(
    CommandDecoder_IngestDesiredProperties_with_1_model_in_model_desired_property_calls_onDesiredProperty_happy_path)
{
...
/*Tests_SRS_COMMAND_DECODER_02_015: [ If fulMethodName is NULL then CommandDecoder_ExecuteMethod shall fail and return
 * NULL. ]*/
...
/*Tests_SRS_COMMAND_DECODER_02_025: [ If methodCallback is NULL then CommandDecoder_ExecuteMethod shall fail and return
 * NULL. ]*/
...
/*Tests_SRS_COMMAND_DECODER_02_015: [ If fullMethodName is NULL then CommandDecoder_ExecuteMethod shall fail and return
 * NULL. ]*/
...
/*Tests_SRS_COMMAND_DECODER_02_016: [ If methodPayload is not NULL then CommandDecoder_ExecuteMethod shall build a
 * MULTITREE_HANDLE out of methodPayload. ]*/
/*Tests_SRS_COMMAND_DECODER_02_017: [ CommandDecoder_ExecuteMethod shall get the SCHEMA_HANDLE associated with the
 * modelHandle passed at CommandDecoder_Create. ]*/
/*Tests_SRS_COMMAND_DECODER_02_018: [ CommandDecoder_ExecuteMethod shall validate that consecutive segments of the
 * fullMethodName exist in the model. ]*/
/*Tests_SRS_COMMAND_DECODER_02_019: [ CommandDecoder_ExecuteMethod shall locate the final model to which the methodName
 * applies. ]*/
/*Tests_SRS_COMMAND_DECODER_02_020: [ CommandDecoder_ExecuteMethod shall verify that the model has a method called
 * methodName. ]*/
/*Tests_SRS_COMMAND_DECODER_02_021: [ For every argument of methodName, CommandDecoder_ExecuteMethod shall build an
 * AGENT_DATA_TYPE from the node with the same name from the MULTITREE_HANDLE. ]*/
/*Tests_SRS_COMMAND_DECODER_02_022: [ CommandDecoder_ExecuteMethod shall call methodCallback passing the context, the
 * methodName, number of arguments and the AGENT_DATA_TYPE. ]*/
...
/*Tests_SRS_COMMAND_DECODER_02_023: [ If any of the previous operations fail, then CommandDecoder_ExecuteMethod shall
 * return NULL. ]*/
...
static void
CommandDecoder_ExecuteMethod_with_1_arg_payload_inert_path(size_t *one, const char *methodPayload, const char **aValue)
{
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, methodPayload)).IgnoreArgument_destination();
    STRICT_EXPECTED_CALL(JSONDecoder_JSON_To_MultiTree(IGNORED_PTR_ARG, IGNORED_PTR_ARG))
        .IgnoreArgument_json()
        .IgnoreArgument_multiTreeHandle();
    STRICT_EXPECTED_CALL(Schema_GetSchemaForModelType(TEST_MODEL_HANDLE));
    ...
    STRICT_EXPECTED_CALL(Schema_GetMethodArgumentName(TEST_METHOD_ARGUMENT_HANDLE_0)).SetReturn("a");
    STRICT_EXPECTED_CALL(Schema_GetMethodArgumentType(TEST_METHOD_ARGUMENT_HANDLE_0)).SetReturn("int");
...
TEST_FUNCTION(CommandDecoder_ExecuteMethod_with_1_arg_payload_unhapy_paths)
{
    ...
    for (size_t i = 0; i < umock_c_negative_tests_call_count(); i++)
    {
        ...
        )
        {
            ...
            /// act
            METHODRETURN_HANDLE methodReturn =
                CommandDecoder_ExecuteMethod(commandDecoderHandle, "methodA", methodPayload);
...
static void CommandDecoder_ExecuteMethod_with_2_arg_payload_inert_path(size_t      *two,
                                                                       ...
                                                                       const char **bValue)
{
    ...
    { /*scope for processing every individual argument*/
        ...
        STRICT_EXPECTED_CALL(Schema_GetMethodArgumentName(TEST_METHOD_ARGUMENT_HANDLE_1)).SetReturn("b");
        STRICT_EXPECTED_CALL(Schema_GetMethodArgumentType(TEST_METHOD_ARGUMENT_HANDLE_1)).SetReturn("int");
...
TEST_FUNCTION(CommandDecoder_ExecuteMethod_with_2_arg_payload_unhapy_paths)
{
    ...
    for (size_t i = 0; i < umock_c_negative_tests_call_count(); i++)
    {
        if ((i != 11) && /*CodeFirst_GetPrimitiveType*/
...
static void CommandDecoder_ExecuteMethod_model_in_model_with_2_arg_payload_inert_path(size_t      *two,
                                                                                      const char  *methodPayload,
                                                                                      const char **aValue,
                                                                                      const char **bValue)
{
    ...
    STRICT_EXPECTED_CALL(
        gballoc_free(IGNORED_PTR_ARG)) /*this is the string "innermodel" for relative relativeMethodPath*/
        .IgnoreArgument_ptr();
...
TEST_FUNCTION(CommandDecoder_ExecuteMethod_model_in_model_with_2_arg_payload_hapy_path)
{
    ...
    /// act
    METHODRETURN_HANDLE methodReturn =
        CommandDecoder_ExecuteMethod(commandDecoderHandle, "innermodel/methodA", methodPayload);
