...
static void sendComplete(void *context, IO_SEND_RESULT send_result)
{
    ...
    if (mqtt_client != NULL)
    {
        if (send_result == IO_SEND_OK)
        {
            ...
            {
                /*Codes_SRS_MQTT_CLIENT_07_032: [If the actionResult parameter is of type MQTT_CLIENT_ON_DISCONNECT the
                 * the msgInfo value shall be NULL.]*/
...
static void logIncomingRawTrace(MQTT_CLIENT        *mqtt_client,
                                CONTROL_PACKET_TYPE packet,
                                uint8_t             flags,
                                const uint8_t      *data,
                                size_t              length)
{
    ...
    if (mqtt_client != NULL && mqtt_client->rawBytesTrace)
    {
        if (data != NULL && length > 0)
        {
            ...
            LOG(AZ_LOG_TRACE,
                ...
                "<- %s %s: 0x%02x 0x%02x ",
                tmBuffer,
                retrievePacketType((CONTROL_PACKET_TYPE)packet),
    ...
    if (mqtt_client != NULL)
    {
        if (open_result == IO_OPEN_OK && !mqtt_client->socketConnected)
        {
            ...
            else
            {
                ...
                /*Codes_SRS_MQTT_CLIENT_07_009: [On success mqtt_client_connect shall send the MQTT CONNECT to the
                 * endpoint.]*/
...
static void onIoError(void *context)
{
    ...
    {
        /*Codes_SRS_MQTT_CLIENT_07_032: [If the actionResult parameter is of type MQTT_CLIENT_ON_DISCONNECT the the
         * msgInfo value shall be NULL.]*/
        /* Codes_SRS_MQTT_CLIENT_07_036: [ If an error is encountered by the ioHandle the mqtt_client shall call
         * xio_close. ] */
        ...
        if ((iterator != NULL && len > 0) || packet == PINGRESP_TYPE)
        {
            ...
            case CONNACK_TYPE:
            {
                if (mqtt_client->fnOperationCallback != NULL)
                {
                    ...
                    /*Codes_SRS_MQTT_CLIENT_07_028: [If the actionResult parameter is of type CONNECT_ACK then the
                     * msgInfo value shall be a CONNECT_ACK structure.]*/
                    ...
                    connack.returnCode =
                        (rc < ((uint8_t)CONN_REFUSED_UNKNOWN)) ? (CONNECT_RETURN_CODE)rc : CONN_REFUSED_UNKNOWN;

                    if (mqtt_client->logTrace)
                    {
                        trace_log = STRING_construct_sprintf("CONNACK | SESSION_PRESENT: %s | RETURN_CODE: 0x%x",
                                                             connack.isSessionPresent ? TRUE_CONST : FALSE_CONST,
                                                             connack.returnCode);
                        log_incoming_trace(mqtt_client, trace_log);
                        STRING_delete(trace_log);
                    }
                    mqtt_client->fnOperationCallback(mqtt_client,
            ...
            case PUBLISH_TYPE:
            {
                if (mqtt_client->fnMessageRecv != NULL)
                {
                    ...
                    QOS_VALUE qosValue       = (flags == 0) ? DELIVER_AT_MOST_ONCE
                                                      : (flags & QOS_LEAST_ONCE_FLAG_MASK) ? DELIVER_AT_LEAST_ONCE
                                                                                           : DELIVER_EXACTLY_ONCE;

                    if (mqtt_client->logTrace)
                    {
                        trace_log = STRING_construct_sprintf("PUBLISH | IS_DUP: %s | RETAIN: %d | QOS: %s",
                                                             isDuplicateMsg ? TRUE_CONST : FALSE_CONST,
                                                             isRetainMsg ? 1 : 0,
                                                             ENUM_TO_STRING(QOS_VALUE, qosValue));
                    }
                    ...
                    else
                    {
                        ...
                        MQTT_MESSAGE_HANDLE msgHandle =
                            mqttmessage_create(packetId, topicName, qosValue, iterator, length);
                        if (msgHandle == NULL)
                        {
            ...
            case PUBCOMP_TYPE:
            {
                if (mqtt_client->fnOperationCallback)
                {
                    ...
                    /*Codes_SRS_MQTT_CLIENT_07_029: [If the actionResult parameter are of types PUBACK_TYPE,
                     * PUBREC_TYPE, PUBREL_TYPE or PUBCOMP_TYPE then the msgInfo value shall be a PUBLISH_ACK
                     * structure.]*/
                    ...
                    if (mqtt_client->logTrace)
                    {
                        trace_log = STRING_construct_sprintf(
                            "%s | PACKET_ID: %" PRIu16,
                            packet == PUBACK_TYPE
                                ? "PUBACK"
                                : (packet == PUBREC_TYPE) ? "PUBREC" : (packet == PUBREL_TYPE) ? "PUBREL" : "PUBCOMP",
            ...
            case SUBACK_TYPE:
            {
                if (mqtt_client->fnOperationCallback)
                {
                    ...
                    /*Codes_SRS_MQTT_CLIENT_07_030: [If the actionResult parameter is of type SUBACK_TYPE then the
                     * msgInfo value shall be a SUBSCRIBE_ACK structure.]*/
                    ...
                    if (suback.qosReturn != NULL)
                    {
                        while (remainLen > 0)
                        {
                            ...
                            suback.qosReturn[suback.qosCount++] =
                                (qosRet <= ((uint8_t)DELIVER_EXACTLY_ONCE)) ? (QOS_VALUE)qosRet : DELIVER_FAILURE;
                            remainLen--;
                            if (mqtt_client->logTrace)
                            {
                                STRING_sprintf(trace_log,
                                               " | RETURN_CODE: %" PRIu16,
                                               suback.qosReturn[suback.qosCount - 1]);
                            }
                        }
            ...
            case UNSUBACK_TYPE:
            {
                if (mqtt_client->fnOperationCallback)
                {
                    ...
                    /*Codes_SRS_MQTT_CLIENT_07_031: [If the actionResult parameter is of type UNSUBACK_TYPE then the
                     * msgInfo value shall be a UNSUBSCRIBE_ACK structure.]*/
...
MQTT_CLIENT_HANDLE mqtt_client_init(ON_MQTT_MESSAGE_RECV_CALLBACK msgRecv,
                                    ON_MQTT_OPERATION_CALLBACK    opCallback,
                                    void                         *opCallbackCtx,
                                    ON_MQTT_ERROR_CALLBACK        onErrorCallBack,
                                    void                         *errorCBCtx)
{
    MQTT_CLIENT *result;
    /*Codes_SRS_MQTT_CLIENT_07_001: [If the parameters ON_MQTT_MESSAGE_RECV_CALLBACK is NULL then mqttclient_init shall
     * return NULL.]*/
    if (msgRecv == NULL)
    {
        result = NULL;
    ...
    else
    {
        ...
        else
        {
            ...
            /*Codes_SRS_MQTT_CLIENT_07_003: [mqttclient_init shall allocate MQTTCLIENT_DATA_INSTANCE and return the
             * MQTTCLIENT_HANDLE on success.]*/
            ...
            {
                /*Codes_SRS_MQTT_CLIENT_07_002: [If any failure is encountered then mqttclient_init shall return
                 * NULL.]*/
...
{
    /*Codes_SRS_MQTT_CLIENT_07_004: [If the parameter handle is NULL then function mqtt_client_deinit shall do
     * nothing.]*/
    if (handle != NULL)
    {
...
{
    int result;
    /*SRS_MQTT_CLIENT_07_006: [If any of the parameters handle, ioHandle, or mqttOptions are NULL then
     * mqtt_client_connect shall return a non-zero value.]*/
    if (handle == NULL || mqttOptions == NULL)
    {
        LOG(AZ_LOG_ERROR,
            LOG_LINE,
            "mqtt_client_connect: NULL argument (handle = %p, mqttOptions = %p)",
            handle,
            mqttOptions);
        result = __FAILURE__;
    }
    else
    {
        ...
        if (xioHandle == NULL)
        {
            /*Codes_SRS_MQTT_CLIENT_07_007: [If any failure is encountered then mqtt_client_connect shall return a
             * non-zero value.]*/
        ...
        else
        {
            ...
            mqtt_client->maxPingRespTime   = (DEFAULT_MAX_PING_RESPONSE_TIME < mqttOptions->keepAliveInterval / 2)
                                               ? DEFAULT_MAX_PING_RESPONSE_TIME
                                               : mqttOptions->keepAliveInterval / 2;
            if (cloneMqttOptions(mqtt_client, mqttOptions) != 0)
            {
                LOG(AZ_LOG_ERROR, LOG_LINE, "Error: Clone Mqtt Options failed");
                result = __FAILURE__;
            }
            /*Codes_SRS_MQTT_CLIENT_07_008: [mqtt_client_connect shall open the XIO_HANDLE by calling into the xio_open
             * interface.]*/
            else if (xio_open(xioHandle,
                              onOpenComplete,
                              mqtt_client,
                              onBytesReceived,
                              mqtt_client,
...
int mqtt_client_publish(MQTT_CLIENT_HANDLE handle, MQTT_MESSAGE_HANDLE msgHandle)
{
    ...
    {
        /*Codes_SRS_MQTT_CLIENT_07_019: [If one of the parameters handle or msgHandle is NULL then mqtt_client_publish
    ...
    {
        /*Codes_SRS_MQTT_CLIENT_07_021: [mqtt_client_publish shall get the message information from the
         * MQTT_MESSAGE_HANDLE.]*/
        const APP_PAYLOAD *payload = mqttmessage_getApplicationMsg(msgHandle);
        if (payload == NULL)
        {
            /*Codes_SRS_MQTT_CLIENT_07_020: [If any failure is encountered then mqtt_client_unsubscribe shall return a
             * non-zero value.]*/
        ...
        else
        {
            ...
            BUFFER_HANDLE publishPacket = mqtt_codec_publish(qos,
                                                             isDuplicate,
                                                             isRetained,
                                                             packetId,
                                                             topicName,
            ...
            {
                /*Codes_SRS_MQTT_CLIENT_07_020: [If any failure is encountered then mqtt_client_unsubscribe shall return
                 * a non-zero value.]*/
            ...
            else
            {
                ...
                /*Codes_SRS_MQTT_CLIENT_07_022: [On success mqtt_client_publish shall send the MQTT SUBCRIBE packet to
                 * the endpoint.]*/
                size_t size = BUFFER_length(publishPacket);
                if (sendPacketItem(mqtt_client, BUFFER_u_char(publishPacket), size) != 0)
                {
                    /*Codes_SRS_MQTT_CLIENT_07_020: [If any failure is encountered then mqtt_client_unsubscribe shall
...
int mqtt_client_subscribe(MQTT_CLIENT_HANDLE handle, uint16_t packetId, SUBSCRIBE_PAYLOAD *subscribeList, size_t count)
{
    ...
    {
        /*Codes_SRS_MQTT_CLIENT_07_013: [If any of the parameters handle, subscribeList is NULL or count is 0 then
         * mqtt_client_subscribe shall return a non-zero value.]*/
        LogError("Invalid parameter specified mqtt_client: %p, subscribeList: %p, count: %d, packetId: %d",
                 mqtt_client,
                 subscribeList,
                 count,
                 packetId);
    ...
    else
    {
        ...
        if (subPacket == NULL)
        {
            /*Codes_SRS_MQTT_CLIENT_07_014: [If any failure is encountered then mqtt_client_subscribe shall return a
             * non-zero value.]*/
        ...
        else
        {
            ...
            /*Codes_SRS_MQTT_CLIENT_07_015: [On success mqtt_client_subscribe shall send the MQTT SUBCRIBE packet to the
             * endpoint.]*/
...
int mqtt_client_unsubscribe(MQTT_CLIENT_HANDLE handle, uint16_t packetId, const char **unsubscribeList, size_t count)
{
    ...
    {
        /*Codes_SRS_MQTT_CLIENT_07_016: [If any of the parameters handle, unsubscribeList is NULL or count is 0 then
         * mqtt_client_unsubscribe shall return a non-zero value.]*/
        LogError("Invalid parameter specified mqtt_client: %p, unsubscribeList: %p, count: %d, packetId: %d",
                 mqtt_client,
                 unsubscribeList,
                 count,
                 packetId);
    ...
    else
    {
        ...
        if (unsubPacket == NULL)
        {
            /*Codes_SRS_MQTT_CLIENT_07_017: [If any failure is encountered then mqtt_client_unsubscribe shall return a
             * non-zero value.]*/
        ...
        else
        {
            ...
            /*Codes_SRS_MQTT_CLIENT_07_018: [On success mqtt_client_unsubscribe shall send the MQTT SUBCRIBE packet to
             * the endpoint.]*/
            if (sendPacketItem(mqtt_client, BUFFER_u_char(unsubPacket), size) != 0)
            {
                /*Codes_SRS_MQTT_CLIENT_07_017: [If any failure is encountered then mqtt_client_unsubscribe shall return
                 * a non-zero value.].]*/
...
int mqtt_client_disconnect(MQTT_CLIENT_HANDLE handle, ON_MQTT_DISCONNECTED_CALLBACK callback, void *ctx)
{
    ...
    if (mqtt_client == NULL)
    {
        /*Codes_SRS_MQTT_CLIENT_07_010: [If the parameters handle is NULL then mqtt_client_disconnect shall return a
         * non-zero value.]*/
        result = __FAILURE__;
    }
    else
    {
        if (mqtt_client->clientConnected)
        {
            ...
            {
                /*Codes_SRS_MQTT_CLIENT_07_011: [If any failure is encountered then mqtt_client_disconnect shall return
                 * a non-zero value.]*/
            ...
            {
                /* Codes_SRS_MQTT_CLIENT_07_037: [ if callback is not NULL callback shall be called once the mqtt
                ...
                /*Codes_SRS_MQTT_CLIENT_07_012: [On success mqtt_client_disconnect shall send the MQTT DISCONNECT packet
                 * to the endpoint.]*/
                if (sendPacketItem(mqtt_client, BUFFER_u_char(disconnectPacket), size) != 0)
                {
                    /*Codes_SRS_MQTT_CLIENT_07_011: [If any failure is encountered then mqtt_client_disconnect shall
                     * return a non-zero value.]*/
...
void mqtt_client_dowork(MQTT_CLIENT_HANDLE handle)
{
    ...
    {
        /*Codes_SRS_MQTT_CLIENT_07_024: [mqtt_client_dowork shall call the xio_dowork function to complete
         * operations.]*/
        xio_dowork(mqtt_client->xioHandle);
        ...
        if (mqtt_client->socketConnected && mqtt_client->clientConnected && mqtt_client->keepAliveInterval > 0)
        {
            ...
            {
                /* Codes_SRS_MQTT_CLIENT_07_035: [If the timeSincePing has expired past the maxPingRespTime then
                 * mqtt_client_dowork shall call the Error Callback function with the message
                 * MQTT_CLIENT_NO_PING_RESPONSE] */
                if (mqtt_client->timeSincePing > 0 &&
                    ((current_ms - mqtt_client->timeSincePing) / 1000) > mqtt_client->maxPingRespTime)
                {
                ...
                {
                    /*Codes_SRS_MQTT_CLIENT_07_026: [if keepAliveInternal is > 0 and the send time is greater than the
                     * MQTT KeepAliveInterval then it shall construct an MQTT PINGREQ packet.]*/
