...
static int addListItemsToUnsubscribePacket(BUFFER_HANDLE ctrlPacket,
                                           const char  **payloadList,
                                           size_t        payloadCount,
...
static int addListItemsToSubscribePacket(BUFFER_HANDLE      ctrlPacket,
                                         SUBSCRIBE_PAYLOAD *payloadList,
                                         size_t             payloadCount,
                                         STRING_HANDLE      trace_log)
{
    int result = 0;
                ...
                *iterator = payloadList[index].qosReturn;
                ...
                if (trace_log != NULL)
                {
                    STRING_sprintf(trace_log,
                                   " | TOPIC_NAME: %s | QOS: %d",
                                   payloadList[index].subscribeTopic,
                                   (int)payloadList[index].qosReturn);
                }
            }
        }
    }
...
static int constructConnectVariableHeader(BUFFER_HANDLE              ctrlPacket,
                                          const MQTT_CLIENT_OPTIONS *mqttOptions,
                                          STRING_HANDLE              trace_log)
{
    int result = 0;
...
static int constructPublishVariableHeader(BUFFER_HANDLE              ctrlPacket,
                                          const PUBLISH_HEADER_INFO *publishHeader,
                                          STRING_HANDLE              trace_log)
{
    int    result   = 0;
    size_t topicLen = 0;
    ...
    else
    {
        ...
        {
            iterator += currLen;
            /* The Topic Name MUST be present as the first field in the PUBLISH Packet Variable header.It MUST be 792 a
...
static int
constructConnPayload(BUFFER_HANDLE ctrlPacket, const MQTT_CLIENT_OPTIONS *mqttOptions, STRING_HANDLE trace_log)
{
    int result = 0;
    ...
    else
    {
        ...
        else
        {
            ...
            if (willMessageLen > USHRT_MAX || willTopicLen > USHRT_MAX || usernameLen > USHRT_MAX ||
                passwordLen > USHRT_MAX)
            {
                result = __FAILURE__;
            }
            else
            {
                ...
                if (trace_log != NULL)
                {
                    (void)STRING_sprintf(connect_payload_trace,
                                         " | CLEAN: %s",
                                         mqttOptions->useCleanSession ? "1" : "0");
                }
    ...
    else
    {
        ...
        if (remainLen <= 0x7f)
        {
            ...
            if (totalLen > 0)
            {
                ...
                {
                    /* Codes_SRS_MQTT_CODEC_07_035: [ If any error is encountered then the packet state will be marked
                     * as error and mqtt_codec_bytesReceived shall return a non-zero value. ] */
                ...
                else
                {
                    ...
                    {
                        /* Codes_SRS_MQTT_CODEC_07_035: [ If any error is encountered then the packet state will be
                         * marked as error and mqtt_codec_bytesReceived shall return a non-zero value. ] */
...
static void completePacketData(MQTTCODEC_INSTANCE *codecData)
{
    if (codecData)
    {
        ...
        {
            codecData->packetComplete(codecData->callContext,
                                      codecData->currPacket,
                                      codecData->headerFlags,
...
void mqtt_codec_destroy(MQTTCODEC_HANDLE handle)
{
    ...
    if (handle != NULL)
    {
        ...
        /* Codes_SRS_MQTT_CODEC_07_004: [mqtt_codec_destroy shall deallocate all memory that has been allocated by this
         * object.] */
        BUFFER_delete(codecData->headerData);
        free(codecData);
    }
...
BUFFER_HANDLE mqtt_codec_connect(const MQTT_CLIENT_OPTIONS *mqttOptions, STRING_HANDLE trace_log)
{
    ...
    /* Codes_SRS_MQTT_CODEC_07_008: [If the parameters mqttOptions is NULL then mqtt_codec_connect shall return a null
     * value.] */
    if (mqttOptions == NULL)
    {
        result = NULL;
    ...
    {
        /* Codes_SRS_MQTT_CODEC_07_009: [mqtt_codec_connect shall construct a BUFFER_HANDLE that represents a MQTT
         * CONNECT packet.] */
        result = BUFFER_new();
        if (result != NULL)
        {
            ...
            else
            {
                ...
                {
                    /* Codes_SRS_MQTT_CODEC_07_010: [If any error is encountered then mqtt_codec_connect shall return
                     * NULL.] */
                    BUFFER_delete(result);
                    result = NULL;
                }
                else
                {
                    ...
                    {
                        /* Codes_SRS_MQTT_CODEC_07_010: [If any error is encountered then mqtt_codec_connect shall
                         * return NULL.] */
                        BUFFER_delete(result);
                        result = NULL;
                    }
...
{
    /* Codes_SRS_MQTT_CODEC_07_011: [On success mqtt_codec_disconnect shall construct a BUFFER_HANDLE that represents a
...
BUFFER_HANDLE mqtt_codec_publish(QOS_VALUE      qosValue,
                                 bool           duplicateMsg,
                                 bool           serverRetain,
                                 uint16_t       packetId,
                                 const char    *topicName,
                                 ...
                                 STRING_HANDLE  trace_log)
{
    ...
    else
    {
        ...
        if (duplicateMsg)
            headerFlags |= PUBLISH_DUP_FLAG;
        if (serverRetain)
            headerFlags |= PUBLISH_QOS_RETAIN;
        ...
        /* Codes_SRS_MQTT_CODEC_07_007: [mqtt_codec_publish shall return a BUFFER_HANDLE that represents a MQTT PUBLISH
         * message.] */
        result = BUFFER_new();
        if (result != NULL)
        {
            ...
            if (trace_log != NULL)
            {
                varible_header_log = STRING_construct_sprintf(" | IS_DUP: %s | RETAIN: %d | QOS: %s",
                                                              duplicateMsg ? TRUE_CONST : FALSE_CONST,
                                                              serverRetain ? 1 : 0,
            ...
            else
            {
                ...
                if (buffLen > 0)
                {
                    ...
                    {
                        /* Codes_SRS_MQTT_CODEC_07_006: [If any error is encountered then mqtt_codec_publish shall
                         * return NULL.] */
                        BUFFER_delete(result);
                        result = NULL;
                    }
...
{
    /* Codes_SRS_MQTT_CODEC_07_013: [On success mqtt_codec_publishAck shall return a BUFFER_HANDLE representation of a
     * MQTT PUBACK packet.] */
...
{
    /* Codes_SRS_MQTT_CODEC_07_015: [On success mqtt_codec_publishRecieved shall return a BUFFER_HANDLE representation
     * of a MQTT PUBREC packet.] */
...
{
    /* Codes_SRS_MQTT_CODEC_07_017: [On success mqtt_codec_publishRelease shall return a BUFFER_HANDLE representation of
     * a MQTT PUBREL packet.] */
...
{
    /* Codes_SRS_MQTT_CODEC_07_019: [On success mqtt_codec_publishComplete shall return a BUFFER_HANDLE representation
     * of a MQTT PUBCOMP packet.] */
...
{
    /* Codes_SRS_MQTT_CODEC_07_021: [On success mqtt_codec_ping shall construct a BUFFER_HANDLE that represents a MQTT
     * PINGREQ packet.] */
    BUFFER_HANDLE result = BUFFER_new();
    if (result != NULL)
    {
...
BUFFER_HANDLE
mqtt_codec_subscribe(uint16_t packetId, SUBSCRIBE_PAYLOAD *subscribeList, size_t count, STRING_HANDLE trace_log)
{
    BUFFER_HANDLE result;
    ...
    {
        /* Codes_SRS_MQTT_CODEC_07_026: [mqtt_codec_subscribe shall return a BUFFER_HANDLE that represents a MQTT
         * SUBSCRIBE message.]*/
        result = BUFFER_new();
        if (result != NULL)
        {
            ...
            {
                /* Codes_SRS_MQTT_CODEC_07_025: [If any error is encountered then mqtt_codec_subscribe shall return
                 * NULL.] */
                BUFFER_delete(result);
                result = NULL;
            }
            else
            {
                ...
                /* Codes_SRS_MQTT_CODEC_07_024: [mqtt_codec_subscribe shall iterate through count items in the
                 * subscribeList.] */
                ...
                else
                {
                    ...
                    {
                        /* Codes_SRS_MQTT_CODEC_07_025: [If any error is encountered then mqtt_codec_subscribe shall
                         * return NULL.] */
                        BUFFER_delete(result);
                        result = NULL;
                    }
...
BUFFER_HANDLE
mqtt_codec_unsubscribe(uint16_t packetId, const char **unsubscribeList, size_t count, STRING_HANDLE trace_log)
{
    BUFFER_HANDLE result;
    /* Codes_SRS_MQTT_CODEC_07_027: [If the parameters unsubscribeList is NULL or if count is 0 then
     * mqtt_codec_unsubscribe shall return NULL.] */
    ...
    {
        /* Codes_SRS_MQTT_CODEC_07_030: [mqtt_codec_unsubscribe shall return a BUFFER_HANDLE that represents a MQTT
         * SUBSCRIBE message.] */
        result = BUFFER_new();
        if (result != NULL)
        {
            if (constructSubscibeTypeVariableHeader(result, packetId) != 0)
            {
                /* Codes_SRS_MQTT_CODEC_07_029: [If any error is encountered then mqtt_codec_unsubscribe shall return
                 * NULL.] */
                BUFFER_delete(result);
                result = NULL;
            }
            else
            {
                ...
                /* Codes_SRS_MQTT_CODEC_07_028: [mqtt_codec_unsubscribe shall iterate through count items in the
                 * unsubscribeList.] */
                if (addListItemsToUnsubscribePacket(result, unsubscribeList, count, unsub_trace) != 0)
                {
                    /* Codes_SRS_MQTT_CODEC_07_029: [If any error is encountered then mqtt_codec_unsubscribe shall
                     * return NULL.] */
                    BUFFER_delete(result);
                    result = NULL;
                }
...
int mqtt_codec_bytesReceived(MQTTCODEC_HANDLE handle, const unsigned char *buffer, size_t size)
{
    ...
    /* Codes_SRS_MQTT_CODEC_07_031: [If the parameters handle or buffer is NULL then mqtt_codec_bytesReceived shall
    ...
    /* Codes_SRS_MQTT_CODEC_07_031: [If the parameters handle or buffer is NULL then mqtt_codec_bytesReceived shall
     * return a non-zero value.] */
    /* Codes_SRS_MQTT_CODEC_07_032: [If the parameters size is zero then mqtt_codec_bytesReceived shall return a
     * non-zero value.] */
    ...
    {
        /* Codes_SRS_MQTT_CODEC_07_033: [mqtt_codec_bytesReceived constructs a sequence of bytes into the corresponding
         * MQTT packets and on success returns zero.] */
        result       = 0;
        size_t index = 0;
        for (index = 0; index < size && result == 0; index++)
        {
            ...
            if (codec_Data->codecState == CODEC_STATE_FIXED_HEADER)
            {
                ...
                else
                {
                    ...
                    {
                        /* Codes_SRS_MQTT_CODEC_07_034: [Upon a constructing a complete MQTT packet
                         * mqtt_codec_bytesReceived shall call the ON_PACKET_COMPLETE_CALLBACK function.] */
            ...
            {
                /* Codes_SRS_MQTT_CODEC_07_035: [If any error is encountered then the packet state will be marked as
                 * error and mqtt_codec_bytesReceived shall return a non-zero value.] */
                codec_Data->currPacket = PACKET_TYPE_ERROR;
                result                 = __FAILURE__;
            }
        }
