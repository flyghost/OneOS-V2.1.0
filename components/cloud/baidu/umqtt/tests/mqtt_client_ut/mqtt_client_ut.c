...
static int my_xio_open(XIO_HANDLE          handle,
...
static void setup_publish_callback_mocks(unsigned char *PUBLISH_RESP, size_t length, QOS_VALUE qos_value)
{
    ...
    STRICT_EXPECTED_CALL(
        mqttmessage_create(TEST_PACKET_ID, IGNORED_PTR_ARG, qos_value, IGNORED_PTR_ARG, TEST_APP_PAYLOAD.length))
        .IgnoreArgument(2)
        .IgnoreArgument(4);
...
static void setup_mqtt_client_subscribe_mocks()
{
    ...
    STRICT_EXPECTED_CALL(BUFFER_length(TEST_BUFFER_HANDLE));
    STRICT_EXPECTED_CALL(BUFFER_u_char(TEST_BUFFER_HANDLE));
    // STRICT_EXPECTED_CALL(tickcounter_get_current_ms(TEST_COUNTER_HANDLE,
    // IGNORED_PTR_ARG)).IgnoreArgument_current_ms();
    EXPECTED_CALL(xio_send(IGNORED_PTR_ARG, IGNORED_PTR_ARG, IGNORED_NUM_ARG, IGNORED_PTR_ARG, IGNORED_PTR_ARG));
    STRICT_EXPECTED_CALL(BUFFER_delete(TEST_BUFFER_HANDLE));
}
...
{
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, TEST_CLIENT_ID)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, TEST_WILL_MSG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, TEST_USERNAME)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, TEST_PASSWORD)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(xio_open(TEST_IO_HANDLE,
                                  IGNORED_PTR_ARG,
                                  IGNORED_PTR_ARG,
                                  IGNORED_PTR_ARG,
...
static void
TestOpCallback(MQTT_CLIENT_HANDLE handle, MQTT_CLIENT_EVENT_RESULT actionResult, const void *msgInfo, void *context)
{
    (void)handle;
    switch (actionResult)
    {
    case MQTT_CLIENT_ON_CONNACK:
    {
        if (context != NULL && msgInfo != NULL)
        {
            ...
            if (connack->isSessionPresent == validate->isSessionPresent && connack->returnCode == validate->returnCode)
            {
    ...
    case MQTT_CLIENT_ON_SUBSCRIBE_ACK:
    {
        if (context != NULL && msgInfo != NULL)
        {
            ...
            if (testData->actionResult == actionResult && validate->packetId == suback->packetId &&
                validate->qosCount == suback->qosCount)
            {
...
/*Tests_SRS_MQTT_CLIENT_07_003: [mqttclient_init shall allocate MQTTCLIENT_DATA_INSTANCE and return the
...
/*Tests_SRS_MQTT_CLIENT_07_001: [If the parameters ON_MQTT_MESSAGE_RECV_CALLBACK is NULL then mqttclient_init shall
 * return NULL.]*/
...
/*Tests_SRS_MQTT_CLIENT_07_006: [If any of the parameters handle, ioHandle, or mqttOptions are NULL then
...
/*Tests_SRS_MQTT_CLIENT_07_007: [If any failure is encountered then mqtt_client_connect shall return a non-zero
 * value.]*/
TEST_FUNCTION(mqtt_client_connect_fails)
{
    ...
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, TEST_CLIENT_ID)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, TEST_WILL_TOPIC)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, TEST_WILL_MSG)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, TEST_USERNAME)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_PTR_ARG, TEST_PASSWORD)).IgnoreArgument(1);
    STRICT_EXPECTED_CALL(xio_open(TEST_IO_HANDLE,
...
TEST_FUNCTION(mqtt_client_on_bytes_received_bytesReceived_fail_succeeds)
{
    ...
    EXPECTED_CALL(mqtt_codec_bytesReceived(IGNORED_PTR_ARG, IGNORED_PTR_ARG, IGNORED_NUM_ARG)).SetReturn(__FAILURE__);
...
/* Tests_SRS_MQTT_CLIENT_07_013: [If any of the parameters handle, subscribeList is NULL or count is 0 then
...
/*Tests_SRS_MQTT_CLIENT_07_015: [On success mqtt_client_subscribe shall send the MQTT SUBCRIBE packet to the
 * endpoint.]*/
...
/*Tests_SRS_MQTT_CLIENT_07_016: [If any of the parameters handle, unsubscribeList is NULL or count is 0 then
...
/*Tests_SRS_MQTT_CLIENT_07_018: [On success mqtt_client_unsubscribe shall send the MQTT SUBCRIBE packet to the
 * endpoint.]*/
...
TEST_FUNCTION(mqtt_client_publish_mqtt_codec_publish_fail)
{
    ...
    EXPECTED_CALL(mqtt_codec_publish(DELIVER_AT_MOST_ONCE,
                                     true,
                                     true,
                                     IGNORED_NUM_ARG,
                                     IGNORED_PTR_ARG,
...
TEST_FUNCTION(mqtt_client_disconnect_fail)
{
    ...
    size_t calls_cannot_fail[] = {1, 2, 4, 5, 6, 7, 8, 9};
...
/* Tests_SRS_MQTT_CLIENT_07_037: [ if callback is not NULL callback shall be called once the mqtt connection has been
 * disconnected ] */
...
/*Tests_SRS_MQTT_CLIENT_07_026: [if keepAliveInternal is > 0 and the send time is greater than the MQTT
 * KeepAliveInterval then it shall construct an MQTT PINGREQ packet.]*/
...
/* Tests_SRS_MQTT_CLIENT_07_035: [If the timeSincePing has expired past the maxPingRespTime then mqtt_client_dowork
 * shall call the Operation Callback function with the message MQTT_CLIENT_NO_PING_RESPONSE] */
...
/*Test_SRS_MQTT_CLIENT_07_027: [The callbackCtx parameter shall be an unmodified pointer that was passed to the
 * mqtt_client_init function.]*/
TEST_FUNCTION(mqtt_client_recvCompleteCallback_context_NULL_fails)
{
    ...
    MQTT_CLIENT_HANDLE mqttHandle =
        mqtt_client_init(TestRecvCallback, TestOpCallback, (void *)&testData, TestErrorCallback, NULL);
    umock_c_reset_all_calls();
...
/*Test_SRS_MQTT_CLIENT_07_028: [If the actionResult parameter is of type CONNECT_ACK then the msgInfo value shall be a
 * CONNECT_ACK structure.]*/
...
/*Test_SRS_MQTT_CLIENT_07_029: [If the actionResult parameter are of types PUBACK_TYPE, PUBREC_TYPE, PUBREL_TYPE or
 * PUBCOMP_TYPE then the msgInfo value shall be a PUBLISH_ACK structure.]*/
TEST_FUNCTION(mqtt_client_recvCompleteCallback_PUBLISH_EXACTLY_ONCE_succeeds)
{
    // arrange
    unsigned char PUBLISH_RESP[] = {0x00, 0x0a, 0x74, 0x6f, 0x70, 0x69, 0x63, 0x20, 0x4e, 0x61,
                                    0x6d, 0x65, 0x12, 0x34, 0x4d, 0x65, 0x73, 0x73, 0x61, 0x67,
    ...
    MQTT_CLIENT_HANDLE mqttHandle =
        mqtt_client_init(TestRecvCallback, TestOpCallback, (void *)&PUBLISH_RESP, TestErrorCallback, NULL);
    umock_c_reset_all_calls();
    ...
    //    STRICT_EXPECTED_CALL(tickcounter_get_current_ms(TEST_COUNTER_HANDLE, IGNORED_PTR_ARG)).IgnoreArgument(2);
    EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(mqttmessage_create(TEST_PACKET_ID,
                                            IGNORED_PTR_ARG,
                                            DELIVER_EXACTLY_ONCE,
                                            IGNORED_PTR_ARG,
                                            TEST_APP_PAYLOAD.length))
        .IgnoreArgument(2)
        .IgnoreArgument(4);
    STRICT_EXPECTED_CALL(mqttmessage_setIsDuplicateMsg(TEST_MESSAGE_HANDLE, true));
    STRICT_EXPECTED_CALL(mqttmessage_setIsRetained(TEST_MESSAGE_HANDLE, true));
...
TEST_FUNCTION(mqtt_client_recvCompleteCallback_PUBLISH_EXACTLY_ONCE_fail)
{
    ...
    // size_t calls_cannot_fail[] = { 0, 1, 7, 8, 10, 11, 12 };
    size_t count = umock_c_negative_tests_call_count();
    for (size_t index = 0; index < count; index++)
    {
...
{
    // arrange
    unsigned char PUBLISH_VALUE[] = {0x00, 0x04, 0x6d, 0x73, 0x67, 0x41, 0x54, 0x68, 0x69, 0x73,
                                     0x20, 0x69, 0x73, 0x20, 0x74, 0x68, 0x65, 0x20, 0x61, 0x70,
                                     0x70, 0x20, 0x6d, 0x73, 0x67, 0x20, 0x41, 0x2e};
    size_t        length          = sizeof(PUBLISH_VALUE) / sizeof(PUBLISH_VALUE[0]);

    uint8_t flag = 0x00;

    MQTT_CLIENT_HANDLE mqttHandle =
        mqtt_client_init(TestRecvCallback, TestOpCallback, (void *)&PUBLISH_VALUE, TestErrorCallback, NULL);
    umock_c_reset_all_calls();
...
/*Test_SRS_MQTT_CLIENT_07_031: [If the actionResult parameter is of type UNSUBACK_TYPE then the msgInfo value shall be a
 * UNSUBSCRIBE_ACK structure.]*/
