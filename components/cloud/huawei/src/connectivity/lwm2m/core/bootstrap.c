...
void bootstrap_step(lwm2m_context_t *contextP, uint32_t currentTime, time_t *timeoutP)
{
    ...
    while (targetP != NULL)
    {
        ...
        case STATE_DEREGISTERED:
            // pay attention: targetP->lifetime come from securityobj instance, it's init value is 10. too small, too
            // short for STATE_BS_HOLD_OFF. of course, the value could be wrote by the bs server. encode decode used for
            // securityobj/serverobj instance
...
static void prv_resultCallback(lwm2m_transaction_t *transacP, void *message)
{
    ...
    if (message == NULL)
    {
        dataP->callback(transacP->peerH, COAP_503_SERVICE_UNAVAILABLE, uriP, NULL, dataP->userData);
    }
    else
    {
        coap_packet_t *packet = (coap_packet_t *)message;

        dataP->callback(transacP->peerH, packet->code, uriP, NULL, dataP->userData);
    }
    lwm2m_free(dataP);
}
