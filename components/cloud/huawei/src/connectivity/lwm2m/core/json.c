...
static int prv_convertRecord(lwm2m_uri_t *uriP, _record_t *recordArray, int count, lwm2m_data_t **dataP)
{
    ...
    for (index = 0; index < count; index++)
    {
        ...
        if (recordArray[index].ids[1] != LWM2M_MAX_ID)
        {
            ...
            {
                targetP = prv_findDataItem(parentP->value.asChildren.array,
...
int json_parse(lwm2m_uri_t *uriP, uint8_t *buffer, size_t bufferLen, lwm2m_data_t **dataP)
{
    ...
    *dataP      = NULL;
    ...
    do
    {
        ...
        case 'b':
            ...
            case 'n':
            {
                ...
                index -= 3;
                itemLen = 0;
                while (buffer[index + itemLen] != '}' && buffer[index + itemLen] != ',' && index + itemLen < bufferLen)
                {
                    itemLen++;
                }
    ...
    if (eFound == true)
    {
        ...
        {
            if (parsedP->type != LWM2M_TYPE_OBJECT || parsedP->id != uriP->objectId)
                goto error;
            if (!LWM2M_URI_IS_SET_INSTANCE(uriP))
            {
...
static int prv_serializeValue(lwm2m_data_t *tlvP, uint8_t *buffer, size_t bufferLen)
{
    ...
    case LWM2M_TYPE_STRING:
        ...
        if (bufferLen - head < tlvP->value.asBuffer.length)
            return -1;
    ...
    case LWM2M_TYPE_INTEGER:
    {
        ...
        if (0 == lwm2m_data_decode_int(tlvP, &value))
            return -1;
    ...
    case LWM2M_TYPE_FLOAT:
    {
        ...
        if (0 == lwm2m_data_decode_float(tlvP, &value))
            return -1;
    ...
    case LWM2M_TYPE_BOOLEAN:
    {
        ...
        if (0 == lwm2m_data_decode_bool(tlvP, &value))
            return -1;
