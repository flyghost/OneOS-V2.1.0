...
int MQTTClientInit(MQTTClient    *c,
                   Network       *network,
...
static int readPacket(MQTTClient *c, Timer *timer)
{
    ...
    if (rem_len > 0 &&
        ((rc = c->ipstack->mqttread(c->ipstack, c->readbuf + len, rem_len, TimerLeftMS(timer))) != rem_len))
    {
        rc = 0;
        goto exit;
    }
    ...
    if (c->keepAliveInterval > 0)
        TimerCountdown(&c->last_received,
                       c->keepAliveInterval);    // record the fact that we have successfully received a packet
exit:
    return rc;
}
    ...
    while (spos < sublen && tpos <= topiclen)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (spos > 0 && spos + 2 == sublen && tpos == topiclen && sub[spos - 1] == '+' && sub[spos] == '/' &&
                    sub[spos + 1] == '#')
                {
                    *result             = 1;
                    multilevel_wildcard = 1;
...
int cycle(MQTTClient *c, Timer *timer)
{
    ...
    case PUBLISH:
    {
        ...
        if (MQTTDeserialize_publish(&msg.dup,
                                    &intQoS,
                                    &msg.retained,
                                    &msg.id,
                                    &topicName,
    ...
    case PUBREL:
    {
        ...
        if (MQTTDeserialize_ack(&type, &dup, &mypacketid, c->readbuf, c->readbuf_size) != 1)
            rc = FAILURE;
                                          ...
                                          c->buf_size,
                                          (packet_type == PUBREC) ? PUBREL : PUBCOMP,
                                          0,
                                          mypacketid)) <= 0)
            rc = FAILURE;
        else
...
int MQTTSubscribeWithResults(MQTTClient     *c,
