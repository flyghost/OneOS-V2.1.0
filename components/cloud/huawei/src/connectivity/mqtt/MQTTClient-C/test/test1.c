...
void messageArrived(MessageData *md)
{
    ...
    assert("Good message lengths", pubmsg.payloadlen == m->payloadlen, "payloadlen was %d", m->payloadlen);

    if (pubmsg.payloadlen == m->payloadlen)
        assert("Good message contents",
...
int test2(struct Options options)
{
    ...
    assert("Session present is 0", connack.sessionPresent == 0, "sessionPresent was %d", connack.sessionPresent);

    /* set up some state */
    rc = MQTTSubscribeWithResults(&c, test_topic, subsqos, messageArrived, &suback);
    assert("Good rc from subscribe", rc == MQTT_SUCCESS, "rc was %d", rc);
    assert("Good granted QoS", suback.grantedQoS == subsqos, "granted QoS was %d", suback.grantedQoS);
    ...
    assert("Good rc in connack", connack.rc == 0, "rc was %d", connack.rc);
    assert("Session present is 1", connack.sessionPresent == 1, "sessionPresent was %d", connack.sessionPresent);
...
int check_subs_exist(MQTTClient *c, const char *test_topic, int which)
{
    ...
    rc = (((which == 1 || which == 3) && test1_message_data) || (which == 2 && test1_message_data == NULL))
             ? MQTT_SUCCESS
             : FAILURE;
    assert("test1 subscription", rc == MQTT_SUCCESS, "test1_message_data %p\n", test1_message_data);
    rc = (((which == 2 || which == 3) && test2_message_data) || (which == 1 && test2_message_data == NULL))
             ? MQTT_SUCCESS
             : FAILURE;
...
int test3(struct Options options)
{
    ...
    assert("Not connected", MQTTIsConnected(&c) == 0, "isconnected was %d", MQTTIsConnected(&c));
    ...
    assert("Good rc in connack", MQTTIsConnected(&c) == 1, "isconnected was %d", MQTTIsConnected(&c));
    ...
    rc = MQTTSubscribeWithResults(&c, test_topic, subsqos, messageArrived, &suback);
    assert("Good rc from subscribe", rc == MQTT_SUCCESS, "rc was %d", rc);
    assert("Granted QoS rc from subscribe", suback.grantedQoS == QOS2, "rc was %d", suback.grantedQoS);
    ...
    assert("Disconnected", !MQTTIsConnected(&c), "isConnected was %d", MQTTIsConnected(&c));
    NetworkDisconnect(&n);
