...
static int jffs2_read_super(struct super_block *sb)
{
	...
	c->sector_size = c->fal_part->flash->block_size;
	c->flash_size  = c->fal_part->info->size;
...
//	}
	...
	if (jffs2_sb == NULL) {
		jffs2_sb = os_malloc(sizeof (struct super_block));
		...
		jffs2_sb->s_dev = t;
        c->fal_part = fal_part_find((const char *)jffs2_sb->s_dev);
        if (c->fal_part == OS_NULL)
        {
            os_kprintf("invalide fal part!\n");
            return OS_ERROR;
        }

		c->inocache_list = os_malloc(sizeof(struct jffs2_inode_cache *) * INOCACHE_HASHSIZE);
		if (!c->inocache_list) {
			os_free(jffs2_sb);
			return ENOMEM;
		}
...
static struct _inode *new_inode(struct super_block *sb)
{
	...
	inode = os_malloc(sizeof (struct _inode));
	if (inode == NULL)
		return 0;
