...
{
    /* set zero to avoid information leak */
                       ...
                       lfs_size_t         size)
{
    ...
    else
    {
        while (size > 0)
        {
            ...
            rcache->size  = lfs_min(lfs_min(lfs_alignup(off+hint, lfs->cfg->read_size), lfs->cfg->block_size) - rcache->off,
                                    lfs->cfg->cache_size);

            err = lfs->cfg->read(lfs->cfg, rcache->block, rcache->off, rcache->buffer, rcache->size);
                      ...
                      lfs_size_t         size)
{
    ...
    for (i = 0; i < size; i += diff)
    {
        ...
        err = lfs_bd_read(lfs, pcache, rcache, hint-i, block, off+i, &dat, diff);
        if (err)
        {
            ret = err;
            break;
        ...
        err = memcmp(dat, data + i, diff);
        if (err)
        {
            ret = (err < 0) ? LFS_CMP_LT : LFS_CMP_GT;
            break;
        }
    }
...
static int lfs_bd_flush(lfs_t *lfs, lfs_cache_t *pcache, lfs_cache_t *rcache, bool validate)
{
    ...
    if (pcache->block != LFS_BLOCK_NULL && pcache->block != LFS_BLOCK_INLINE)
    {
        ...
        diff = lfs_alignup(pcache->size, lfs->cfg->prog_size);

        err = lfs->cfg->prog(lfs->cfg, pcache->block, pcache->off, pcache->buffer, diff);
        LFS_ASSERT(err <= 0);
        ...
        else
        {
            if (validate)
            {
                ...
                err = lfs_bd_cmp(lfs, NULL, rcache, diff, pcache->block, pcache->off, pcache->buffer, diff);

                if (err < 0)
                {
                    ret = err;
                }
                else if (err != LFS_CMP_EQ)
                {
                    ret = LFS_ERR_CORRUPT;
                }
            }
...
static int lfs_bd_sync(lfs_t *lfs, lfs_cache_t *pcache, lfs_cache_t *rcache, bool validate)
{
    ...
    err = lfs_bd_flush(lfs, pcache, rcache, validate);
                       ...
                       lfs_size_t   size)
{
    ...
    while (size > 0)
    {
        ...
        /* pcache must have been flushed, either by programming and entire block or manually flushing the pcache */
...
static int lfs_bd_erase(lfs_t *lfs, lfs_block_t block)
{
    ...
    err = lfs->cfg->erase(lfs->cfg, block);
...
static int lfs_alloc(lfs_t *lfs, lfs_block_t *block)
{
    ...
    while (true)
    {
        while (lfs->free.i != lfs->free.size)
        {
            ...
            if (!(lfs->free.buffer[off / 32] & (1U << (off % 32))))
            {
                *block = (lfs->free.off + off) % lfs->cfg->block_count;
                ...
                /* eagerly find next off so an alloc ack can discredit old lookahead blocks */
                while (lfs->free.i != lfs->free.size &&
        ...
        err = lfs_fs_rawtraverse(lfs, lfs_alloc_lookahead, lfs, true);
        if (err)
        {
            lfs_alloc_drop(lfs);
            ret = err;
                                   ...
                                   lfs_size_t        gsize)
{
    ...
    while (off >= sizeof(lfs_tag_t) + lfs_tag_dsize(ntag))
    {
        ...
        err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(ntag), dir->pair[0], off, &ntag, sizeof(ntag));
        if (err)
        {
            ret = err;
            break;
                           ...
                           lfs_size_t         size)
{
    ...
    else
    {
        ...
        while (size > 0)
        {
            ...
            err           = lfs_dir_getslice(lfs, dir, gmask, gtag, rcache->off, rcache->buffer, rcache->size);
                            ...
                            void                   *data)
{
    ...
    while (true)
    {
        if (off + lfs_tag_dsize(ptag) < dir->off)
        {
            ...
            err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(tag), dir->pair[0], off, &tag, sizeof(tag));
        ...
        /* do we need to filter? inlining the filtering logic here allows for some minor optimizations */
        if (lfs_tag_id(tmask) != 0)
        {
            /* scan for duplicates and update tag based on creates/deletes */
            filter = lfs_dir_traverse(lfs,
                                      dir,
                                      off,
                                      ptag,
                                      attrs,
        ...
        {
            err = cb(data, tag + LFS_MKTAG(0, diff, 0), buffer);
...
static lfs_stag_t lfs_dir_fetchmatch(lfs_t            *lfs,
                                     ...
                                     void             *data)
{
    /* we can find tag very efficiently during a fetch, since we're already scanning the entire directory */
    ...
    else
    {
        ...
        for (i = 0; i < 2; i++)
        {
            err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(revs[i]), pair[i], 0, &revs[i], sizeof(revs[i]));
        ...
        if (!need_ret)
        {
            ...
            for (i = 0; i < 2; i++)
            {
                ...
                ptag        = 0xffffffff;
                tempcount   = 0;
                temptail[0] = LFS_BLOCK_NULL;
                temptail[1] = LFS_BLOCK_NULL;
                tempsplit   = false;
                tempbesttag = besttag;
                ...
                while (true)
                {
                    ...
                    err = lfs_bd_read(lfs, NULL, &lfs->rcache, lfs->cfg->block_size, dir->pair[0], off, &tag, sizeof(tag));
                    ...
                    if (lfs_tag_type1(tag) == LFS_TYPE_CRC)
                    {
                        ...
                        /* 
                         * toss our crc into the filesystem seed for pseudorandom numbers, note we use another crc here
                         * as a collection function because it is sufficiently random and convenient
                         */
                    ...
                    {
                        res = cb(data, tag, &(struct lfs_diskoff){dir->pair[0], off+sizeof(tag)});
                        if (res < 0)
                        {
...
static int lfs_dir_fetch(lfs_t            *lfs,
                         ...
                         const lfs_block_t pair[2])
{
    /* note, mask=-1, tag=-1 can never match a tag since this pattern has the invalid bit set */
                             ...
                             lfs_gstate_t     *gstate)
{
    ...
    lfs_stag_t   res = lfs_dir_get(lfs,
                                 dir,
                                 LFS_MKTAG(0x7ff, 0, 0),
                                 LFS_MKTAG(LFS_TYPE_MOVESTATE, 0, sizeof(temp)),
                                 &temp);
...
static int lfs_dir_getinfo(lfs_t *lfs, lfs_mdir_t *dir, uint16_t id, struct lfs_info *info)
{
    ...
    else
    {
        ...
        tag = lfs_dir_get(lfs,
                          ...
                          LFS_MKTAG(LFS_TYPE_NAME, id, lfs->name_max + 1),
                          info->name);
        if (tag < 0)
        {
            ret = (int)tag;
...
static int lfs_dir_find_match(void *data, lfs_tag_t tag, const void *buffer)
{
    ...
    int err = lfs_bd_cmp(lfs, NULL, &lfs->rcache, diff, disk->block, disk->off, name->name, diff);
    ...
    else
    {
        ...
        {
            ret = (name->size < lfs_tag_size(tag)) ? LFS_CMP_LT : LFS_CMP_GT;
        }
        else
        {
            /* found a match! */
        ...
        *path = name;
        ...
        while (true) 
        {
            tag = lfs_dir_fetchmatch(lfs,
                                     ...
                                     (strchr(name, '/') == NULL) ? id : NULL,  /* are we last name? */
                                     lfs_dir_find_match,
...
static int lfs_dir_commitattr(lfs_t *lfs, struct lfs_commit *commit, lfs_tag_t tag, const void *buffer)
{
    ...
    {
        /* write out tag */
        ntag = lfs_tobe32((tag & 0x7fffffff) ^ commit->ptag);

        err  = lfs_dir_commitprog(lfs, commit, &ntag, sizeof(ntag));
        if (err)
        {
            ret = err;
        }
...
static int lfs_dir_commitcrc(lfs_t *lfs, struct lfs_commit *commit)
{
    ...
    while (commit->off < end)
    {
        ...
        noff = lfs_min(end - off, 0x3fe) + off;
        ...
        err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(tag), commit->block, noff, &tag, sizeof(tag));
        if (err && err != LFS_ERR_CORRUPT)
        {
            ret  = err;
            flag = true;
    ...
    if (!flag)
    {
        ...
        else
        {
            ...
            while (off < end)
            {
                ...
                {
                    /* check against written crc, may catch blocks that become readonly and match our commit size exactly */
                    if (i == off1 && crc != crc1)
                    {
                        ret = LFS_ERR_CORRUPT;
                        flag = true;
...
static int lfs_dir_alloc(lfs_t *lfs, lfs_mdir_t *dir)
{
    ...
    if (LFS_ERR_OK == ret)
    {
        ...
        /* rather than clobbering one of the blocks we just pretend the revision may be valid */
        err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(dir->rev), dir->pair[0], 0, &dir->rev, sizeof(dir->rev));
...
#ifndef LFS_READONLY
static int lfs_dir_drop(lfs_t *lfs, lfs_mdir_t *dir, lfs_mdir_t *tail) /*从目录链表中删除tail表示的目录,参数dir作为目录链表的previous目录,tail为需要删除的目录即参数dir的下一个目录*/
{
    /* steal state */
...
static int lfs_dir_compact_relocate(lfs_t *lfs, lfs_mdir_t *dir, bool *tired)
{
    int err;
    int ret = LFS_ERR_OK;
                           ...
                           uint16_t                end)
{
    ...
    while (LFS_ERR_OK == ret)
    {
        ...
        if (lfs->cfg->block_cycles > 0 && (dir->rev % ((lfs->cfg->block_cycles+1)|1) == 0))
        {
            if (lfs_pair_cmp(dir->pair, (const lfs_block_t[2]){0, 1}) == 0)
            {
                /* oh no! we're writing too much to the superblock, should we expand? */
                res = lfs_fs_rawsize(lfs);
                ...
                /* do we have extra space? littlefs can't reclaim this space by itself, so expand cautiously */
                if ((lfs_size_t)res < lfs->cfg->block_count/2)
                {
                    LFS_DEBUG("Expanding superblock at rev %"PRIu32, dir->rev);

                    err = lfs_dir_split(lfs, dir, attrs, attrcount, source, begin, end);
            ...
            else
            {
                ...
                err = lfs_dir_compact_relocate(lfs, dir, &tired);
        ...
        while (true)
        {
            ...
            commit.crc   = 0xffffffff;
            commit.begin = 0;
            commit.end   = (lfs->cfg->metadata_max ? lfs->cfg->metadata_max : lfs->cfg->block_size) - 8;
                          ...
                          int                     attrcount)
{
    ...
    while (LFS_ERR_OK == ret)
    {
        ...
        {
            err = lfs_fs_pred(lfs, dir->pair, &pdir);
    ...
    if ((LFS_ERR_OK == ret) && need_compact)
    {
        /* fall back to compaction */
        lfs_cache_drop(lfs, &lfs->pcache);

        err = lfs_dir_compact(lfs, dir, attrs, attrcount, dir, 0, dir->count);
        ...
        for (d = lfs->mlist; d; d = d->next)
        {
            if (lfs_pair_cmp(d->m.pair, olddir.pair) == 0)
            {
                while (d->id >= d->m.count && d->m.split)
                {
                    ...
                    err = lfs_dir_fetch(lfs, &d->m, d->m.tail);
...
static int lfs_rawmkdir(lfs_t *lfs, const char *path)
{
    ...
    int ret = lfs_fs_forceconsistency(lfs);
...
static int lfs_dir_rawopen(lfs_t *lfs, lfs_dir_t *dir, const char *path)
{
    ...
    else if (lfs_tag_type3(tag) != LFS_TYPE_DIR)
    {
        ret = LFS_ERR_NOTDIR;
    }
    else
...
static int lfs_dir_rawseek(lfs_t *lfs, lfs_dir_t *dir, lfs_off_t off)
{
    ...
    else
    {
        ...
        while (off > 0)
        {
            diff      = lfs_min(dir->m.count - dir->id, off);
            dir->id  += diff;
            dir->pos += diff;
            off      -= diff;
                        ...
                        lfs_off_t         *off)
{
    ...
    {
        current = lfs_ctz_index(lfs, &(lfs_off_t){size-1});
        target  = lfs_ctz_index(lfs, &pos);

        while (current > target)
        {
            skip = lfs_min(lfs_npw2(current-target+1) - 1, lfs_ctz(current));

            err = lfs_bd_read(lfs, pcache, rcache, sizeof(head), head, 4*skip, &head, sizeof(head));
...
static int lfs_file_flush(lfs_t *lfs, lfs_file_t *file)
{
    ...
    if (file->flags & LFS_F_WRITING)
    {
        ...
        if (!(file->flags & LFS_F_INLINE))
        {
            ...
            {
                /* copy over a byte at a time, leave it up to caching to make this efficient */
                res = lfs_file_rawread(lfs, &orig, &data, 1);
            ...
            if (LFS_ERR_OK == ret)
            {
                ...
                while (true)
                {
                    err = lfs_bd_flush(lfs, &file->cache, &lfs->rcache, true);
    ...
    else
    {
        ...
        else
        {
            ...
            while (nsize > 0)
            {
                ...
                {
                    err = lfs_bd_read(lfs, NULL, &file->cache, lfs->cfg->block_size, file->block, file->off, data, diff);
                                     ...
                                     lfs_size_t  size)
{
    ...
    while (LFS_ERR_OK == ret)
    {
        ...
        if ((file->flags & LFS_F_INLINE) &&
            ...
            > lfs_min(0x3fe, lfs_min(lfs->cfg->cache_size,
        ...
        while (nsize > 0)
        {
            ...
            while (true)
            {
                err = lfs_bd_prog(lfs, &file->cache, &lfs->rcache, true, file->block, file->off, data, diff);
...
static int lfs_file_rawtruncate(lfs_t *lfs, lfs_file_t *file, lfs_off_t size)
{
    ...
    {
        pos     = file->pos;
        oldsize = lfs_file_rawsize(lfs, file);
        ...
        else if (size > oldsize)
        {
            ...
            res = lfs_file_rawseek(lfs, file, 0, LFS_SEEK_END);
        ...
        {
            /* restore pos */
            res = lfs_file_rawseek(lfs, file, pos, LFS_SEEK_SET);
...
static lfs_soff_t lfs_file_rawsize(lfs_t *lfs, lfs_file_t *file)
{
    ...
    if (file->flags & LFS_F_WRITING)
    {
        ret = (lfs_soff_t)lfs_max(file->pos, file->ctz.size);
    }
    else
#endif
    {
        ret = (lfs_soff_t)file->ctz.size;
    }
...
static int lfs_rawstat(lfs_t *lfs, const char *path, struct lfs_info *info)
{
    ...
    {
        ret = lfs_dir_getinfo(lfs, &cwd, lfs_tag_id(tag), info);
    }
...
static int lfs_rawremove(lfs_t *lfs, const char *path)
{
    ...
    while (LFS_ERR_OK == ret)
    {
        tag = lfs_dir_find(lfs, &cwd, &path, NULL);
...
static int lfs_rawrename(lfs_t *lfs, const char *oldpath, const char *newpath)
{
    ...
    while (LFS_ERR_OK == ret)
    {
        ...
        oldtag = lfs_dir_find(lfs, &oldcwd, &oldpath, NULL);
    ...
    if (LFS_ERR_OK == err)
    {
        ...
        while (!lfs_pair_isnull(dir.tail))
        {
            ...
            if (tag && !lfs_tag_isdelete(tag))
            {
                ...
                /* check version */
                major_version = (0xffff & (superblock.version >> 16));
                minor_version = (0xffff & (superblock.version >>  0));
                       ...
                       bool   includeorphans) 
{
    ...
    if (LFS_ERR_OK == ret)
    {
        ...
        for (f = (lfs_file_t*)lfs->mlist; f; f = f->next)
        {
            ...
            {
                err = lfs_ctz_traverse(lfs, &f->cache, &lfs->rcache, f->ctz.head, f->ctz.size, cb, data);
            ...
            {
                err = lfs_ctz_traverse(lfs, &f->cache, &lfs->rcache, f->block, f->pos, cb, data);
...
static int lfs_fs_parent_match(void *data, lfs_tag_t tag, const void *buffer)
{
    ...
    err = lfs_bd_read(lfs, &lfs->pcache, &lfs->rcache, lfs->cfg->block_size, disk->block, disk->off, &child, sizeof(child));
    ...
    else
    {
        ...
        ret = (lfs_pair_cmp(child, find->pair) == 0) ? LFS_CMP_EQ : LFS_CMP_LT;
    }
...
static int lfs_fs_relocate(lfs_t *lfs, const lfs_block_t oldpair[2], lfs_block_t newpair[2])
{
    ...
    /* find parent */
    tag = lfs_fs_parent(lfs, oldpair, &parent);
    ...
    else if (tag != LFS_ERR_NOENT)
    {
        ...
        else
        {
            ...
            if (lfs_gstate_hasmovehere(&lfs->gstate, parent.pair)) 
            {
                ...
                LFS_DEBUG("Fixing move while relocating " "{0x%"PRIx32", 0x%"PRIx32"} 0x%"PRIx16"\n",
    ...
    {
        /* find pred */
        err = lfs_fs_pred(lfs, oldpair, &parent);

        if (err && err != LFS_ERR_NOENT)
        {
            ret = err;
        }
        else if (err != LFS_ERR_NOENT)  // if we can't find dir, it must be new
        {
            /*
             * fix pending move in this pair? this looks like an optimization but
             * is in fact _required_ since relocating may outdate the move.
...
static int lfs_fs_demove(lfs_t *lfs) 
{
    ...
    if (lfs_gstate_hasmove(&lfs->gdisk))
    {
        ...
        err = lfs_dir_fetch(lfs, &movedir, lfs->gdisk.pair);

        if (LFS_ERR_OK == err)
        {
            /* prep gstate and delete move id */
            moveid = lfs_tag_id(lfs->gdisk.tag);
...
static int lfs_fs_deorphan(lfs_t *lfs)
{
    ...
    if (lfs_gstate_hasorphans(&lfs->gstate))
    {
        ...
        while (!lfs_pair_isnull(pdir.tail))
        {
            err = lfs_dir_fetch(lfs, &dir, pdir.tail);
            ...
            if (!pdir.split)
            {
                ...
                tag = lfs_fs_parent(lfs, pdir.tail, &parent);
                ...
                res = lfs_dir_get(lfs, &parent, LFS_MKTAG(0x7ff, 0x3ff, 0), tag, pair);
                ...
                {
                    /* we have desynced */
                    LFS_DEBUG("Fixing half-orphan {0x%"PRIx32", 0x%"PRIx32"} " "-> {0x%"PRIx32", 0x%"PRIx32"}",
...
lfs_ssize_t lfs_getattr(lfs_t *lfs, const char *path, uint8_t type, void *buffer, lfs_size_t size)
{
    ...
    else
    {
        ...
        res = lfs_rawgetattr(lfs, path, type, buffer, size);
...
int lfs_file_opencfg(lfs_t *lfs, lfs_file_t *file, const char *path, int flags, const struct lfs_file_config *cfg)
{
    ...
    if (LFS_ERR_OK == err)
    {
        LFS_TRACE("lfs_file_opencfg(%p, %p, \"%s\", %x, %p {" ".buffer=%p})",
...
lfs_ssize_t lfs_file_read(lfs_t *lfs, lfs_file_t *file, void *buffer, lfs_size_t size)
{
    ...
    else
    {
        ...
        res = lfs_file_rawread(lfs, file, buffer, size);
...
lfs_soff_t lfs_file_seek(lfs_t *lfs, lfs_file_t *file, lfs_soff_t off, int whence)
{
    ...
    else
    {
        ...
        res = lfs_file_rawseek(lfs, file, off, whence);
