...
struct lfs_config
{
    ...
    /* Lock the underlying block device. Negative error codes are propogated to the user. */
    int (*lock)(const struct lfs_config *c);

    /* Unlock the underlying block device. Negative error codes are propogated to the user. */
    int (*unlock)(const struct lfs_config *c);
#endif

    /* Minimum size of a block read. All read operations will be a multiple of this value. */
    lfs_size_t read_size;

    /* Minimum size of a block program. All program operations will be a multiple of this value. */
...
#ifdef LFS_ATTR
/* Custom attribute structure, used to describe custom attributes committed atomically during file writes. */
struct lfs_attr
{
    /* 8-bit type of attribute, provided by user and used to identify the attribute */
    uint8_t type;
