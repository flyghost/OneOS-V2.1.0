...
static struct authnone_private *g_authnone_private = OS_NULL;

AUTH *authnone_create(void)
{
    register struct authnone_private *ap = g_authnone_private;
    XDR xdr_stream;
    register XDR *xdrs;
    extern bool_t xdr_opaque_auth(XDR * xdrs, struct opaque_auth * ap);
    struct opaque_auth auth;
    os_uint32_t *auth_buf, *auth_base;
    ...
    buf_len = ((str_len) + (sizeof(os_uint32_t)) - 1) & ~((sizeof(os_uint32_t)) - 1);
    buf_len += sizeof(struct nfs_credentia);
    if (_credentia.auxi && _credentia.auxi_count)
    {
        buf_len += sizeof(os_uint32_t) * _credentia.auxi_count;
    }
    auth_buf = auth_base = os_malloc(buf_len);
    if (auth_buf == NULL)
    {
        return NULL;
    }
    memset(auth_buf, 0, buf_len);
    *auth_buf++ = htonl(os_tick_get());
    *auth_buf++ = htonl(str_len);
    memcpy(auth_buf, _credentia.name, str_len);
    auth_buf += (str_len + sizeof(os_uint32_t) - 1) >> 2;
    *auth_buf++ = htonl(_credentia.uid);
    *auth_buf++ = htonl(_credentia.gid);
    if (_credentia.auxi && _credentia.auxi_count)
    {
        os_uint32_t tmp_cnt = 0;
    ...
    if (ap == 0)
    {
        ap = (struct authnone_private *) os_malloc(sizeof(*ap));
        if (ap == 0)
        {
            os_free(auth_base);
            return NULL;
    ...
    if (!ap->mcnt)
    {
        ...
        auth.oa_length = (auth_buf - auth_base) * sizeof(os_uint32_t);
...
{
    if (g_authnone_private)
    {
        os_free(g_authnone_private);
        g_authnone_private  = OS_NULL;
    }
}
