	...
	unsigned int recvsz)
{
	...
	cl = (CLIENT *) os_malloc (sizeof(CLIENT));
	if (cl == NULL)
	{
		LOG_W(NFS_TAG, "clntudp_create: out of memory\n");
		goto fooy;
	}
	sendsz = ((sendsz + 3) / 4) * 4;
	recvsz = ((recvsz + 3) / 4) * 4;
	cu = (struct cu_data *) os_malloc (sizeof(*cu) + sendsz + recvsz);
	if (cu == NULL)
	{
		LOG_W(NFS_TAG, "clntudp_create: out of memory\n");
		goto fooy;
	...
	cu->cu_sendsz = sendsz;
	cu->cu_recvsz = recvsz;
	call_msg.rm_xid = ((unsigned long)os_task_self()) ^ ((unsigned long)os_tick_get()) ^ (xid_count++);
	...
	if (*sockp < 0)
	{
		*sockp = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
		if (*sockp < 0)
		{
			LOG_W(NFS_TAG, "create socket error\n");
			goto fooy;
		}
		cu->cu_closeit = TRUE;
	}
...
send_again:
	...
	if (inlen < 4)
	{
		LOG_W(NFS_TAG, "recv error, len %d\n", inlen);
		cu->cu_error.re_errno = errno;
...
static bool_t clntudp_control(CLIENT *cl, int request, char *info)
{
	...
	case CLSET_TIMEOUT:
		...
		/* set socket option, note: lwip support timeval timeout */
		setsockopt(cu->cu_sock, SOL_SOCKET, SO_RCVTIMEO, 
			info, sizeof(struct timeval));
		}
		break;
	case CLGET_TIMEOUT:
