...
bool_t xdr_u_longlong_t (XDR * xdrs, unsigned long long* ullp)
{
  ...
  switch (xdrs->x_op)
    ...
    case XDR_DECODE:
      if (!XDR_GETLONG (xdrs, (long *)&t1) ||
          !XDR_GETLONG (xdrs, (long *)&t2))
        return FALSE;
      *ullp = ((uint64_t) t1) << 32;
      *ullp |= t2;
      return TRUE;
...
bool_t xdr_bytes(XDR *xdrs, char** cpp, unsigned int *sizep, unsigned int maxsize)
{
	...
	case XDR_DECODE:
		...
		if (sp == NULL) {
			*cpp = sp = (char *) os_malloc(nodesize);
		}
		if (sp == NULL) {
			LOG_W(NFS_TAG, "xdr_bytes: out of memory\n");
			return (FALSE);
		}
		/* fall into ... */
...
bool_t xdr_string(XDR *xdrs, char **cpp, unsigned int maxsize)
{
	...
	case XDR_DECODE:
		...
		if (sp == NULL)
			*cpp = sp = (char *) os_malloc(nodesize);
		if (sp == NULL) {
			LOG_W(NFS_TAG, "xdr_string: out of memory\n");
			return (FALSE);
		}
		sp[size] = 0;
		/* fall into ... */
...
bool_t xdr_array(XDR *xdrs, char **addrp, unsigned int *sizep, unsigned int maxsize, unsigned int elsize, xdrproc_t elproc)
{
	...
	if (target == NULL)
		...
		case XDR_DECODE:
			if (c == 0)
				return (TRUE);
			*addrp = target = os_malloc(nodesize);
			if (target == NULL) {
				LOG_W(NFS_TAG, "xdr_array: out of memory\n");
				return (FALSE);
			}
...
bool_t xdr_reference(XDR *xdrs, char **pp, unsigned int size, xdrproc_t proc)
{
	...
	if (loc == NULL)
		...
		case XDR_DECODE:
			*pp = loc = (char*) os_malloc(size);
			if (loc == NULL) {
				LOG_W(NFS_TAG, "xdr_reference: out of memory\n");
				return (FALSE);
			}
