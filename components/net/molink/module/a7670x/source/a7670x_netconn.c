...
#define MO_LOG_TAG "a7670x.netconn"
#define MO_LOG_LVL MO_LOG_INFO
#include <mo_log.h>

#define SEND_DATA_MAX_SIZE        (1360)

#define A7670X_NETCONN_MQ_NAME "a7670x_nc_mq"

#ifndef A7670X_NETCONN_MQ_MSG_SIZE
#define A7670X_NETCONN_MQ_MSG_SIZE (sizeof(mo_notconn_msg_t))
#endif

#ifndef A7670X_NETCONN_MQ_MSG_MAX
#define A7670X_NETCONN_MQ_MSG_MAX  (16)
#endif
...
#define A7670X_EVENT_SEND_OK      (1L << 2)
#define A7670X_EVENT_SEND_FAIL    (1L << 3)
...
mo_netconn_t *a7670x_netconn_create(mo_object_t *module, mo_netconn_type_t type)
{
    ...
    netconn->mq = os_mq_create(A7670X_NETCONN_MQ_NAME, A7670X_NETCONN_MQ_MSG_SIZE, A7670X_NETCONN_MQ_MSG_MAX);
...
static os_err_t a7670x_tcp_connect(mo_object_t *module, mo_netconn_t *netconn, char *ip_addr, os_uint16_t port)
{
    ...
    os_event_recv(&a7670x->netconn_evt, event, OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR, OS_NO_WAIT, OS_NULL);
...
os_size_t a7670x_netconn_send(mo_object_t *module, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    ...
    while (sent_size < size)
    {
        ...
        {
            result = OS_ERROR;
            ERROR("Drv hardware send data fail, there is data loss.\r\n");
            goto __exit;
        }
        ...
        result = os_event_recv(&a7670x->netconn_evt,
                               A7670X_EVENT_SEND_OK | A7670X_EVENT_SEND_FAIL,
                               OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
        ...
        if (event & A7670X_EVENT_SEND_FAIL)
        {
...
static void urc_send_msg_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    if (result != OS_EOK)
    {
        os_event_send(&a7670x->netconn_evt, SET_EVENT(connect_id, A7670X_EVENT_SEND_FAIL));
    }
    else
    {
        os_event_send(&a7670x->netconn_evt, SET_EVENT(connect_id, A7670X_EVENT_SEND_OK));
    }
