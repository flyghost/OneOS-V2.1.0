 ...
 * @file        a7670x_ping.c
 *
 * @brief       a7670x module link kit ping api implement
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "a7670x.ping"
#define MO_LOG_LVL MO_LOG_INFO
#include <mo_log.h>

#define A7670X_MIN_PING_PKG_LEN (4)
#define A7670X_MAX_PING_PKG_LEN (188)
#define A7670X_MIN_PING_TIMEOUT (10000)
#define A7670X_MAX_PING_TIMEOUT (100000)

#ifdef A7670X_USING_PING_OPS

typedef enum a7670x_ping_ret_tpye
{
    PING_OK = 1,
    PING_TIMEOUT,
    PING_SUMMARY,
    PING_UNKNOWN_RET
}a7670x_ping_ret_tpye_t;

os_err_t a7670x_ping(mo_object_t *self, const char *host, os_uint16_t len, os_uint32_t timeout, struct ping_resp *resp)
{
    ...
    if ((len > A7670X_MAX_PING_PKG_LEN) || (len < A7670X_MIN_PING_PKG_LEN))
    {
        ERROR("%s ping: ping package len[%d] is out of range[%d, %d].",
                  self->name,
                  len,
                  A7670X_MIN_PING_PKG_LEN,
                  A7670X_MAX_PING_PKG_LEN);
        return OS_ERROR;
    }

    if (timeout > A7670X_MAX_PING_TIMEOUT)
    {
        ERROR("%s ping: user set ping timeout value %ums is out of range[%dms, %dms].",
                  self->name,
                  timeout,
                  A7670X_MIN_PING_TIMEOUT,
                  A7670X_MAX_PING_TIMEOUT);
        return OS_ERROR;
    }

    /* Def timeout value is 5000ms */
    if (timeout < A7670X_MIN_PING_TIMEOUT)
    {
        ...
        timeout = A7670X_MIN_PING_TIMEOUT;
    }
