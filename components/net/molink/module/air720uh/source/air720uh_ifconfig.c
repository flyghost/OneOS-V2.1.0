 ...
 *
 * @file        air720uh_ifconfig.c
 *
 * @brief       air720uh module link kit ifconfig api implement
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "air720uh_ifconfig"
...
os_err_t air720uh_ifconfig(mo_object_t *self)
{
    ...
    if (air720uh_get_ipaddr(self, ipaddr) != OS_EOK)
    {
        memset(ipaddr, 0, sizeof(ipaddr));
    }

    char imei[MO_IMEI_LEN + 1] = {0};
    if (air720uh_get_imei(self, imei, sizeof(imei)) != OS_EOK)
    {
        memset(imei, 0, sizeof(imei));
    }

    char iccid[MO_ICCID_LEN + 1] = {0};
    if (air720uh_get_iccid(self, iccid, sizeof(iccid)) != OS_EOK)
    {
    ...
    if (air720uh_get_csq(self, &rssi, &ber) != OS_EOK)
    {
        rssi = 0;
        ber  = 0;
    }
...
os_err_t air720uh_get_ipaddr(mo_object_t *self, char ip[])
{
    ...
    os_err_t result = at_parser_exec_cmd(parser, &resp, "AT+CGPADDR=5");
    if (result != OS_EOK)
    {
