 ...
 *
 * @file        air723ug_ifconfig.c
 *
 * @brief       air723ug module link kit ifconfig api implement
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "air723ug_ifconfig"
...
os_err_t air723ug_ifconfig(mo_object_t *self)
{
    ...
    if (air723ug_get_ipaddr(self, ipaddr) != OS_EOK)
    {
        memset(ipaddr, 0, sizeof(ipaddr));
    }

    char imei[MO_IMEI_LEN + 1] = {0};
    if (air723ug_get_imei(self, imei, sizeof(imei)) != OS_EOK)
    {
        memset(imei, 0, sizeof(imei));
    }

    char iccid[MO_ICCID_LEN + 1] = {0};
    if (air723ug_get_iccid(self, iccid, sizeof(iccid)) != OS_EOK)
    {
    ...
    if (air723ug_get_csq(self, &rssi, &ber) != OS_EOK)
    {
        rssi = 0;
        ber  = 0;
    }
