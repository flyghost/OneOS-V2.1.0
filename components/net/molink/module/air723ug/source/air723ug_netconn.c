 ...
 * @file        air723ug.c
 *
 * @brief       air723ug module link kit netconn api implement
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "air723ug_netconn"
...
#define AIR723_NETCONN_MQ_NAME "air723_nc_mq"

#ifndef AIR723_NETCONN_MQ_MSG_SIZE
#define AIR723_NETCONN_MQ_MSG_SIZE (sizeof(mo_notconn_msg_t))
#endif

#ifndef AIR723_NETCONN_MQ_MSG_MAX
#define AIR723_NETCONN_MQ_MSG_MAX  (5)
#endif

#define SET_EVENT(socket, event) (((socket + 1) << 16) | (event))

#define AIR723UG_EVENT_SEND_OK (1L << 0)
#define AIR723UG_EVENT_SEND_FAIL (1L << 1)
#define AIR723UG_EVENT_RECV_OK (1L << 2)
#define AIR723UG_EVENT_CONN_OK (1L << 3)
#define AIR723UG_EVENT_CONN_FAIL (1L << 4)
#define AIR723UG_EVENT_CLOSE_OK (1L << 5)
#define AIR723UG_EVENT_CLOSE_FAIL (1L << 6)
#define AIR723UG_EVENT_DOMAIN_OK (1L << 7)
#define AIR723UG_EVENT_LINK_OK (1L << 8)
#define AIR723UG_EVENT_LINK_FAIL (1L << 9)

#define AIR723UG_MULTI_CONN_ENABLE (1)
#define AIR723UG_MULTI_CONN_DISABLE (0)

#ifdef AIR723UG_USING_NETCONN_OPS

static os_err_t air723ug_lock(os_mutex_t *mutex)
{
    return os_mutex_recursive_lock(mutex, OS_WAIT_FOREVER);
}

static os_err_t air723ug_unlock(os_mutex_t *mutex)
{
    return os_mutex_recursive_unlock(mutex);
}

static mo_netconn_t *air723ug_netconn_alloc(mo_object_t *module)
{
    mo_air723ug_t *air723ug = os_container_of(module, mo_air723ug_t, parent);

    for (int i = 0; i < AIR723UG_NETCONN_NUM; i++)
    {
        if (NETCONN_STAT_NULL == air723ug->netconn[i].stat)
        {
            air723ug->netconn[i].connect_id = i;
            return &air723ug->netconn[i];
        }
    }
...
static mo_netconn_t *air723ug_get_netconn_by_id(mo_object_t *module, os_int32_t connect_id)
{
    OS_ASSERT(OS_NULL != module);

    if (0 > connect_id || AIR723UG_NETCONN_NUM <= connect_id)
    {
    ...
    return &air723ug->netconn[connect_id];
}

os_err_t air723ug_netconn_get_info(mo_object_t *module, mo_netconn_info_t *info)
{
    mo_air723ug_t *air723ug = os_container_of(module, mo_air723ug_t, parent);

    info->netconn_array = air723ug->netconn;
    info->netconn_nums = sizeof(air723ug->netconn) / sizeof(air723ug->netconn[0]);

    return OS_EOK;
}

mo_netconn_t *air723ug_netconn_create(mo_object_t *module, mo_netconn_type_t type)
{
    mo_air723ug_t *air723ug = os_container_of(module, mo_air723ug_t, parent);

    air723ug_lock(&air723ug->netconn_lock);

    mo_netconn_t *netconn = air723ug_netconn_alloc(module);

    if (OS_NULL == netconn)
    {
        air723ug_unlock(&air723ug->netconn_lock);
        return OS_NULL;
    }

    netconn->mq = os_mq_create(AIR723_NETCONN_MQ_NAME,
                               AIR723_NETCONN_MQ_MSG_SIZE,
                               AIR723_NETCONN_MQ_MSG_MAX);
...
static os_err_t air723ug_netconn_close(mo_object_t *module, mo_netconn_t *netconn)
{
    os_uint32_t event;
    mo_air723ug_t *air723ug;
    at_parser_t *parser;
    char at_cmd[20] = {0};
    ...
    event = SET_EVENT(netconn->connect_id, AIR723UG_EVENT_CLOSE_OK | AIR723UG_EVENT_CLOSE_FAIL);
    air723ug = os_container_of(module, mo_air723ug_t, parent);
    os_event_recv(&air723ug->netconn_evt, event, OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR, OS_NO_WAIT, OS_NULL);
    ...
    result = os_event_recv(&air723ug->netconn_evt,
    ...
    result = os_event_recv(&air723ug->netconn_evt,
                           AIR723UG_EVENT_CLOSE_OK | AIR723UG_EVENT_CLOSE_FAIL,
                           OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
    ...
    if (event & AIR723UG_EVENT_CONN_FAIL)
    {
...
os_err_t air723ug_netconn_destroy(mo_object_t *module, mo_netconn_t *netconn)
{
    ...
    case NETCONN_STAT_CONNECT:
        air723ug_netconn_close(module, netconn);
        break;
    default:
...
os_err_t air723ug_netconn_gethostbyname(mo_object_t *self, const char *domain_name, ip_addr_t *addr)
{
    ...
    mo_air723ug_t *air723ug = os_container_of(self, mo_air723ug_t, parent);

    os_event_recv(&air723ug->netconn_evt,
                  AIR723UG_EVENT_DOMAIN_OK,
                  OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
                  OS_NO_WAIT,
                  OS_NULL);

    air723ug->netconn_data = addr;
...
__exit:
    air723ug->netconn_data = OS_NULL;
    return result;
}

os_err_t air723ug_check_link_status(mo_object_t *module, mo_netconn_t *netconn)
{
    ...
    os_event_recv(&air723ug->netconn_evt,
                  AIR723UG_EVENT_LINK_OK | AIR723UG_EVENT_LINK_FAIL,
                  OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
                  OS_NO_WAIT,
                  OS_NULL);
    ...
    if (AIR723UG_EVENT_LINK_OK != link_status)
    { /* active link */
        ...
        if (AIR723UG_MULTI_CONN_DISABLE == mode)
        {
...
os_err_t air723ug_netconn_connect(mo_object_t *module, mo_netconn_t *netconn, ip_addr_t addr, os_uint16_t port)
{
    os_uint32_t event;
    os_err_t result;
    at_parser_t *parser = &module->parser;
    ...
    event = SET_EVENT(netconn->connect_id, AIR723UG_EVENT_CONN_OK | AIR723UG_EVENT_CONN_FAIL);
    os_event_recv(&air723ug->netconn_evt, event, OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR, OS_NO_WAIT, OS_NULL);
    ...
    result = os_event_recv(&air723ug->netconn_evt,
                           AIR723UG_EVENT_CONN_OK | AIR723UG_EVENT_CONN_FAIL,
                           OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
                           1 * OS_TICK_PER_SECOND,
                           &event);
    if (result != OS_EOK)
...
os_size_t air723ug_netconn_send(mo_object_t *module, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    at_parser_t *parser = &module->parser;
    ...
    while (sent_size < size)
    {
        ...
        result = os_event_recv(&air723ug->netconn_evt,
                               AIR723UG_EVENT_SEND_OK | AIR723UG_EVENT_SEND_FAIL,
                               OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
                               1 * OS_TICK_PER_SECOND,
                               &event);
        if (result != OS_EOK)
        ...
        if (event & AIR723UG_EVENT_SEND_FAIL)
        {
...
static void urc_send_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    {
        os_event_send(&air723ug->netconn_evt, SET_EVENT(connect_id, AIR723UG_EVENT_SEND_OK));
    }
    else if (strstr(data, "SEND FAIL"))
    {
        os_event_send(&air723ug->netconn_evt, SET_EVENT(connect_id, AIR723UG_EVENT_SEND_FAIL));
    }
}
...
static void urc_close_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    mo_netconn_t *netconn = air723ug_get_netconn_by_id(module, connect_id);
    if (OS_NULL == netconn)
    {
...
static void urc_recv_dns(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    if ((j > 2) && (j % 3 == 2))
    {
        ...
        inet_aton(recvip, (ip_addr_t *)air723ug->netconn_data);
        os_event_send(&air723ug->netconn_evt, AIR723UG_EVENT_DOMAIN_OK);
    }
    else
    {
...
static void urc_connect_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    {
        os_event_send(&air723ug->netconn_evt, SET_EVENT(connect_id, AIR723UG_EVENT_CONN_OK));
    }
    else
    {
        os_event_send(&air723ug->netconn_evt, SET_EVENT(connect_id, AIR723UG_EVENT_CONN_FAIL));
    }
}
...
static void urc_destory_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    {
        os_event_send(&air723ug->netconn_evt, SET_EVENT(connect_id, AIR723UG_EVENT_CLOSE_OK));
    }
    else
    {
        os_event_send(&air723ug->netconn_evt, SET_EVENT(connect_id, AIR723UG_EVENT_CLOSE_FAIL));
    }
}
...
static void urc_link_status_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    {
        os_event_send(&air723ug->netconn_evt, AIR723UG_EVENT_LINK_OK);
    }
    else
    {
        os_event_send(&air723ug->netconn_evt, AIR723UG_EVENT_LINK_FAIL);
    }
}
...
os_err_t air723ug_netconn_init(mo_air723ug_t *module)
{
