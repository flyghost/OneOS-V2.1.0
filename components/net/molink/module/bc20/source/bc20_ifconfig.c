 ...
 *
 * @file        bc20_ifconfig.c
 *
 * @brief       bc20 module link kit ifconfig api implement
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "bc20.ifconfig"
#define MO_LOG_LVL  MO_LOG_INFO
#include "mo_log.h"

#define BC20_IFCONFIG_INVALID_DFT (-1)

os_err_t bc20_ifconfig(mo_object_t *module)
{
    char ipaddr[IPADDR_MAX_STR_LEN + 1] = {0};
    if (bc20_get_ipaddr(module, ipaddr) != OS_EOK)
    {
        memset(ipaddr, 0, sizeof(ipaddr));
    }

    char imei[MO_IMEI_LEN + 1] = {0};
    if (bc20_get_imei(module, imei, sizeof(imei)) != OS_EOK)
    {
        memset(imei, 0, sizeof(imei));
    }

    char iccid[MO_ICCID_LEN + 1] = {0};
    if (bc20_get_iccid(module, iccid, sizeof(iccid)) != OS_EOK)
    {
    ...
    if (bc20_get_csq(module, &rssi, &ber) != OS_EOK)
    {
        rssi = 0;
        ber  = 0;
    }
...
os_err_t bc20_set_dnsserver(mo_object_t *module, dns_server_t dns)
{
    ...
    if (OS_NULL == parser)
    {
        ERROR("BC20 %s: at parser is NULL.", __FUNCTION__);
        return OS_ERROR;
    }
    ...
    {
        ERROR("BC20 %s: with invalid param.", __FUNCTION__);
        return OS_ERROR;
    }
...
os_err_t bc20_get_dnsserver(mo_object_t *module, dns_server_t *dns)
{
    ...
    if (OS_NULL == dns)
    {
        ERROR("BC20 %s: dns is NULL.", __FUNCTION__);
        return OS_ERROR;
    }
    ...
    {
        ERROR("BC20 %s: get dns failed.", __FUNCTION__);
        result = OS_ERROR;
        goto __exit;
    }
    ...
    DEBUG("BC20 %s: primary_dns[%s],secondary_dns[%s]", __FUNCTION__, dns->primary_dns, dns->secondary_dns);
...
{
    at_parser_t *parser = &module->parser;
    os_int8_t    ucid   = BC20_IFCONFIG_INVALID_DFT;
    os_int8_t    len    = BC20_IFCONFIG_INVALID_DFT;
