 ...
 * @file        bc26_netconn.c
 *
 * @brief       bc26 module link kit netconnect api
 *
 * @revision
 * Date         Author          Notes
...
#define BC26_NETCONN_BUFF_DEF (256)

#ifdef BC26_USING_NETCONN_OPS

static os_err_t bc26_lock(os_mutex_t *mutex)
{
    OS_ASSERT(OS_NULL != mutex);
    return os_mutex_recursive_lock(mutex, OS_WAIT_FOREVER);
}

static os_err_t bc26_unlock(os_mutex_t *mutex)
{
    OS_ASSERT(OS_NULL != mutex);
    return os_mutex_recursive_unlock(mutex);
}

static os_bool_t bc26_check_state(mo_object_t *module, os_int32_t connect_id)
{
    OS_ASSERT(OS_NULL != module);

    at_parser_t *parser = &module->parser;

    char resp_buff[BC26_NETCONN_BUFF_DEF] = {0};
...
static mo_netconn_t *bc26_netconn_alloc(mo_object_t *module)
{
    OS_ASSERT(OS_NULL != module);

    mo_bc26_t *bc26 = os_container_of(module, mo_bc26_t, parent);

    for (int i = 0; i < BC26_NETCONN_NUM; i++)
    {
        if (NETCONN_STAT_NULL == bc26->netconn[i].stat)
        {
            if (bc26_check_state(module, i))
            {
                bc26->netconn[i].connect_id = i;

                return &bc26->netconn[i];
            }
        }
    }
...
static mo_netconn_t *bc26_get_netconn_by_id(mo_object_t *module, os_int32_t connect_id)
{
    OS_ASSERT(OS_NULL != module);

    if (0 > connect_id || BC26_NETCONN_NUM <= connect_id)
    {
    ...
    return &bc26->netconn[connect_id];
}
...
mo_netconn_t *bc26_netconn_create(mo_object_t *module, mo_netconn_type_t type)
{
    ...
    mo_netconn_t *netconn = bc26_netconn_alloc(module);
...
os_err_t bc26_netconn_connect(mo_object_t *module, mo_netconn_t *netconn, ip_addr_t addr, os_uint16_t port)
{
    ...
    os_uint32_t event = SET_EVENT(netconn->connect_id, BC26_EVENT_CONN_OK | BC26_EVENT_CONN_FAIL);

    os_event_recv(&bc26->netconn_evt, event, OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR, OS_NO_WAIT, OS_NULL);
    ...
    result = os_event_recv(&bc26->netconn_evt,
                           BC26_EVENT_CONN_OK | BC26_EVENT_CONN_FAIL,
                           OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
...
os_size_t bc26_netconn_send(mo_object_t *module, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    ...
    bc26->curr_connect = netconn->connect_id;
    ...
    while (sent_size < size)
    {
        ...
        result = os_event_recv(&bc26->netconn_evt,
                               BC26_EVENT_SEND_OK | BC26_EVENT_SEND_FAIL,
                               OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
                               1 * OS_TICK_PER_SECOND,
                               &event);
        if (result != OS_EOK)
...
os_err_t bc26_netconn_gethostbyname(mo_object_t *module, const char *domain_name, ip_addr_t *addr)
{
    ...
    os_event_recv(&bc26->netconn_evt,
                  BC26_EVENT_DOMAIN_OK,
                  OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
                  OS_NO_WAIT,
                  OS_NULL);

    bc26->netconn_data = addr;
...
static void urc_connect_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    if (0 == result)
    {
        os_event_send(&bc26->netconn_evt, SET_EVENT(connect_id, BC26_EVENT_CONN_OK));
    }
    else
    {
        os_event_send(&bc26->netconn_evt, SET_EVENT(connect_id, BC26_EVENT_CONN_FAIL));
    }
}
...
static void urc_send_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    os_int32_t curr_connect = bc26->curr_connect;

    if (strstr(data, "SEND OK"))
    {
        os_event_send(&bc26->netconn_evt, SET_EVENT(curr_connect, BC26_EVENT_SEND_OK));
    }
    else if (strstr(data, "SEND FAIL"))
    {
        os_event_send(&bc26->netconn_evt, SET_EVENT(curr_connect, BC26_EVENT_SEND_FAIL));
    }
}
...
static void urc_close_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    mo_netconn_t *netconn = bc26_get_netconn_by_id(module, connect_id);
    if (OS_NULL == netconn)
    {
...
static void urc_close_result_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    os_event_send(&bc26->netconn_evt, BC26_EVNET_CLOSE_OK);
...
static void urc_dnsqip_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    if (3 == j)
    {
        ...
        inet_aton(recvip, (ip_addr_t *)bc26->netconn_data);

        os_event_send(&bc26->netconn_evt, BC26_EVENT_DOMAIN_OK);
    }
    else
    {
...
void bc26_netconn_init(mo_bc26_t *module)
{
    /* Init module netconn array */
    memset(module->netconn, 0, sizeof(module->netconn));
    for (int i = 0; i < BC26_NETCONN_NUM; i++)
    {
        module->netconn[i].connect_id = BC26_CONN_ID_NULL;
    }
