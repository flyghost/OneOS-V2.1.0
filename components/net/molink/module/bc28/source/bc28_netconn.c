...
#define MO_LOG_TAG "bc28.netconn"
#define MO_LOG_LVL  MO_LOG_INFO
#include "mo_log.h"

#define BC28_EVENT_DOMAIN_OK     (1L << 0)
#define BC28_EVENT_DOMAIN_FAIL   (1L << 1)
#ifdef  BC28_USING_PING_OPS
#define BC28_EVENT_PING_OK       (1L << 2)
#define BC28_EVENT_PING_FAIL     (1L << 3)
...
#define BC28_SEND_BLOCK_SIZE     (128)
...
#define BC28_PING_INVALID_DEF    (-1)
#define BC28_PING_BUFFER_SIZE    (128)
#endif /* BC28_USING_PING_OPS */

#define BC28_NETCONN_MQ_NAME     "bc28_nc_mq"
#ifndef BC28_NETCONN_MQ_MSG_SIZE
#define BC28_NETCONN_MQ_MSG_SIZE (sizeof(mo_notconn_msg_t))
#endif /* BC28_NETCONN_MQ_MSG_SIZE */
#ifndef BC28_NETCONN_MQ_MSG_MAX
#define BC28_NETCONN_MQ_MSG_MAX  (5)
#endif /* BC28_NETCONN_MQ_MSG_MAX */
...
mo_netconn_t *bc28_netconn_create(mo_object_t *module, mo_netconn_type_t type)
{
    ...
    netconn->mq = os_mq_create(BC28_NETCONN_MQ_NAME,
                               BC28_NETCONN_MQ_MSG_SIZE,
                               BC28_NETCONN_MQ_MSG_MAX);
...
os_err_t bc28_netconn_destroy(mo_object_t *module, mo_netconn_t *netconn)
{
    ...
    os_int32_t   conn_id = BC28_CONN_ID_NULL;
...
os_err_t bc28_netconn_gethostbyname(mo_object_t *module, const char *domain_name, ip_addr_t *addr)
{
    os_err_t     result = OS_EOK;
    os_uint32_t  event  = 0;
    at_parser_t *parser = &module->parser;
    ...
    bc28->netconn_data = addr_str;

    os_event_clear(&bc28->netconn_evt, BC28_EVENT_DOMAIN_OK | BC28_EVENT_DOMAIN_FAIL);
    ...
    result = os_event_recv(&bc28->netconn_evt,
                           BC28_EVENT_DOMAIN_OK | BC28_EVENT_DOMAIN_FAIL,
    ...
    if (BC28_EVENT_DOMAIN_FAIL & event)
    {
    ...
    {
        ERROR("BC28 domain resolve: recv invalid addr len:%d", strlen(addr_str));
        result = OS_ERROR;
        goto __exit;
    }
    else
    {
        DEBUG("BC28 domain resolve: \"%s\" domain ip is %s, addrlen %d", domain_name, addr_str, strlen(addr_str));
...
static os_size_t bc28_single_packet_send(at_parser_t *parser, const char *data, os_size_t size)
{
    os_size_t  sent_size     = 0; /* raw data sent size */
    os_size_t  curr_pkt_size = 0; /* raw data current packet size */

    char hex_str_buff[BC28_SEND_BLOCK_SIZE] = {0};

    while (size > sent_size)
    {
        curr_pkt_size = (size - sent_size) < (BC28_SEND_BLOCK_SIZE / 2) ? (size - sent_size) : BC28_SEND_BLOCK_SIZE / 2;
...
static os_size_t bc28_hexdata_send(at_parser_t *parser, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    os_err_t   result            = OS_EOK;
    ...
    while (sent_size < size)
    {
        ...
        /* send data */
        if (cur_pkt_size != bc28_single_packet_send(parser, data + sent_size, cur_pkt_size))
        {
            result = OS_ERROR;
...
os_size_t bc28_netconn_send(mo_object_t *module, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    ...
    if (OS_EOK != bc28_lock(&bc28->netconn_lock))
    {
    ...
    {
        sent_size = bc28_hexdata_send(parser, netconn, data, size);
    }
    else
    {
    ...
    if (OS_EOK != bc28_unlock(&bc28->netconn_lock))
    {
...
#ifdef BC28_USING_PING_OPS
os_err_t bc28_ping_handler(mo_object_t *module, const char *host, os_uint16_t len, os_uint32_t timeout, struct ping_resp *resp)
{
    at_parser_t *parser   = &module->parser;
    os_err_t     result   = OS_EOK;
    os_int16_t   ttl      = BC28_PING_INVALID_DEF;
    ip_addr_t    addr     = {0};
    ...
    char ret_buff[BC28_PING_BUFFER_SIZE]  = {0};
    char resp_buff[AT_RESP_BUFF_SIZE_DEF] = {0};
    ...
    if (OS_NULL == parser)
    {
        ERROR("BC28 ping: at parser is NULL.");
        return OS_ERROR;
    }

    if ((len < BC28_MIN_PING_PKG_LEN) || (len > BC28_MAX_PING_PKG_LEN))
    {
        ERROR("BC28 ping: ping package len[%d] is out of range[%d, %d].",
    ...
    {
        ERROR("BC28 ping: user set ping timeout value %ums is out of range[%dms, %dms].",
                  timeout, BC28_MIN_PING_TIMEOUT, BC28_MAX_PING_TIMEOUT);
        return OS_ERROR;
    }

    DEBUG("BC28 ping: %s, len: %u, timeout: %ums", host, len, timeout);

    /* DNS: bc28 dosen't support ping domain name, do dns before ping */
    result = bc28_netconn_gethostbyname(module, host, &addr);
    if (OS_EOK != result)
    {
        ERROR("BC28 ping: dns error, host: %s", host);
        return result;
    }

    strncpy(ip_send, inet_ntoa(addr), IPADDR_MAX_STR_LEN);
    DEBUG("BC28 ping: domain ip is %s, addrlen %d", ip_send, strlen(ip_send));
    /* DNS: finished */
    ...
    os_event_clear(&bc28->netconn_evt, BC28_EVENT_PING_OK | BC28_EVENT_PING_FAIL);

    bc28->netconn_data = ret_buff;
    ...
    result = os_event_recv(&bc28->netconn_evt,
                           BC28_EVENT_PING_OK | BC28_EVENT_PING_FAIL,
                           OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
                           at_resp.timeout,
                           &event);
    if (result != OS_EOK)
    ...
    {
        ERROR("BC28 ping %s fail, invalid ping result.", host);
        result = OS_ERROR;
        goto __exit;
    }
    else
    {
        DEBUG("BC28 ping: resp parse ip[%s], req_time[%u], ttl[%hd]", ip_recv, req_time, ttl);
        if (0 > ttl || 0 == req_time)
        {
            result = OS_ERROR;
...
static void urc_ping_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    memcpy(bc28->netconn_data, data, size);

__exit:

    if (OS_EOK == result)
    {
        os_event_send(&bc28->netconn_evt, BC28_EVENT_PING_OK);
    }
    else
    {
        os_event_send(&bc28->netconn_evt, BC28_EVENT_PING_FAIL);
    }
...
static void urc_dns_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    memcpy(bc28->netconn_data, ret_buff, strlen(ret_buff));

__exit:

    if (OS_EOK == result)
    {
        os_event_send(&bc28->netconn_evt, BC28_EVENT_DOMAIN_OK);
    }
    else
    {
        os_event_send(&bc28->netconn_evt, BC28_EVENT_DOMAIN_FAIL);
    }
