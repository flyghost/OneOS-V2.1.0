...
#define MO_LOG_TAG "bc28.netserv"
...
os_err_t bc28_set_cgact(mo_object_t *module, os_uint8_t cid, os_uint8_t act_stat)
{
    at_parser_t *parser = &module->parser;
...
void bc28_edrx_urc_handler(struct at_parser *parser, const char *data, os_size_t size)
{
...
static at_urc_t bc28_netserv_urc_table[] = {
    {.prefix = "+CEDRXP:",  .suffix = "\r\n", .func = bc28_edrx_urc_handler},
};

void bc28_netserv_init(mo_bc28_t *module)
{
    /* Set netserv urc table */
    at_parser_t *parser = &(module->parent.parser);
    at_parser_set_urc_table(parser, bc28_netserv_urc_table, sizeof(bc28_netserv_urc_table) / sizeof(at_urc_t));
    return;
}
