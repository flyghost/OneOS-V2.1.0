 ...
 *
 * @file        bc95_ifconfig.c
 *
 * @brief       bc95 module link kit ifconfig api implement
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "bc95.ifconfig"
#define MO_LOG_LVL  MO_LOG_INFO
#include "mo_log.h"

#define BC95_IFCONFIG_INVALID_DEF (-1)

os_err_t bc95_ifconfig(mo_object_t *module)
{
    char ipaddr[IPADDR_MAX_STR_LEN + 1] = {0};
    if (bc95_get_ipaddr(module, ipaddr) != OS_EOK)
    {
        memset(ipaddr, 0, sizeof(ipaddr));
    }

    char imei[MO_IMEI_LEN + 1] = {0};
    if (bc95_get_imei(module, imei, sizeof(imei)) != OS_EOK)
    {
        memset(imei, 0, sizeof(imei));
    }

    char iccid[MO_ICCID_LEN + 1] = {0};
    if (bc95_get_iccid(module, iccid, sizeof(iccid)) != OS_EOK)
    {
    ...
    if (bc95_get_csq(module, &rssi, &ber) != OS_EOK)
    {
        rssi = 0;
        ber  = 0;
    }
...
os_err_t bc95_set_dnsserver(mo_object_t *module, dns_server_t dns)
{
    ...
    if (OS_NULL == parser)
    {
        ERROR("BC95 %s: at parser is NULL.", __FUNCTION__);
        return OS_ERROR;
    }
    ...
    {
        ERROR("BC95 %s: with invalid param.", __FUNCTION__);
        return OS_ERROR;
    }
...
os_err_t bc95_get_dnsserver(mo_object_t *module, dns_server_t *dns)
{
    at_parser_t *parser         = &module->parser;
    os_err_t     result         = OS_EOK;
    ...
    if (OS_NULL == dns)
    {
        ERROR("BC95 %s: dns is NULL.", __FUNCTION__);
        return OS_ERROR;
    }
    ...
    {
        ERROR("BC95 %s: get secondary dns failed.", __FUNCTION__);
        result = OS_ERROR;
        goto __exit;
    }
    ...
    DEBUG("BC95 %s: primary_dns[%s],secondary_dns[%s]", __FUNCTION__, dns->primary_dns, dns->secondary_dns);

__exit:
    return result;
}

os_err_t bc95_get_ipaddr(mo_object_t *module, char ip[])
{
    at_parser_t *parser = &module->parser;
    os_int8_t    ucid   = BC95_IFCONFIG_INVALID_DEF;
    os_int8_t    len    = BC95_IFCONFIG_INVALID_DEF;
