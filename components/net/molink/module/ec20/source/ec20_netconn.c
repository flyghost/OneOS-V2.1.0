 ...
 * @file        ec20_netconn.c
 *
 * @brief       ec20 module link kit netconnect api
 *
 * @revision
 * Date         Author          Notes
...
#define EC20_RESP_DEF_SIZE (256)

#define EC20_NETCONN_MQ_NAME "ec20_nc_mq"

#ifndef EC20_NETCONN_MQ_MSG_SIZE
#define EC20_NETCONN_MQ_MSG_SIZE (sizeof(mo_notconn_msg_t))
#endif
...
static os_err_t ec20_lock(os_mutex_t *mutex)
{
    return os_mutex_recursive_lock(mutex, OS_WAIT_FOREVER);
}

static os_err_t ec20_unlock(os_mutex_t *mutex)
{
    return os_mutex_recursive_unlock(mutex);
}

static os_bool_t ec20_check_state(mo_object_t *module, os_int32_t connect_id)
{
...
static mo_netconn_t *ec20_netconn_alloc(mo_object_t *module)
{
    mo_ec20_t *ec20 = os_container_of(module, mo_ec20_t, parent);

    for (int i = 0; i < EC20_NETCONN_NUM; i++)
    {
        if (NETCONN_STAT_NULL == ec20->netconn[i].stat)
        {
            if (ec20_check_state(module, i))
            {
                ec20->netconn[i].connect_id = i;

                return &ec20->netconn[i];
            }
        }
    }
...
static mo_netconn_t *ec20_get_netconn_by_id(mo_object_t *module, os_int32_t connect_id)
{
    OS_ASSERT(OS_NULL != module);

    if (0 > connect_id || EC20_NETCONN_NUM <= connect_id)
    {
    ...
    return &ec20->netconn[connect_id];
}

os_err_t ec20_netconn_get_info(mo_object_t *module, mo_netconn_info_t *info)
{
    mo_ec20_t *ec20 = os_container_of(module, mo_ec20_t, parent);
...
__exit:
    if (result != OS_EOK)
    {
        ERROR("Set EC20 PDP act failed");
    }

    /* set pdp activity flag */
    ec20->pdp_act = (OS_EOK == result) ? OS_TRUE : OS_FALSE;

    return result;
}

mo_netconn_t *ec20_netconn_create(mo_object_t *module, mo_netconn_type_t type)
{
    mo_ec20_t *ec20 = os_container_of(module, mo_ec20_t, parent);

    ec20_lock(&ec20->netconn_lock);

    mo_netconn_t *netconn = ec20_netconn_alloc(module);
...
os_err_t ec20_netconn_connect(mo_object_t *module, mo_netconn_t *netconn, ip_addr_t addr, os_uint16_t port)
{
    at_parser_t *parser = &module->parser;
    mo_ec20_t *ec20 = os_container_of(module, mo_ec20_t, parent);
    ...
    os_uint32_t event = SET_EVENT(netconn->connect_id, EC20_EVENT_CONN_OK | EC20_EVENT_CONN_FAIL);

    os_event_recv(&ec20->netconn_evt, event, OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR, OS_NO_WAIT, OS_NULL);
    ...
    result = os_event_recv(&ec20->netconn_evt,
    ...
    result = os_event_recv(&ec20->netconn_evt,
                           EC20_EVENT_CONN_OK | EC20_EVENT_CONN_FAIL,
                           OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
...
os_size_t ec20_netconn_send(mo_object_t *module, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    ...
    ec20->curr_connect = netconn->connect_id;
    ...
    while (sent_size < size)
    {
        ...
        result = os_event_recv(&ec20->netconn_evt,
                               EC20_EVENT_SEND_OK | EC20_EVENT_SEND_FAIL,
                               OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
                               1 * OS_TICK_PER_SECOND,
                               &event);
        if (result != OS_EOK)
...
os_err_t ec20_netconn_gethostbyname(mo_object_t *self, const char *domain_name, ip_addr_t *addr)
{
    ...
    if (OS_FALSE == ec20->pdp_act)
    {
        ...
        if (OS_EOK != result)
        {
            ERROR("%s active pdp context failed", __func__);
            goto __exit;
        }
    }
    ...
    os_event_recv(&ec20->netconn_evt,
                  EC20_EVENT_DOMAIN_OK,
                  OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
                  OS_NO_WAIT,
                  OS_NULL);

    ec20->netconn_data = addr;
    ...
    result = os_event_recv(&ec20->netconn_evt,
                           EC20_EVENT_DOMAIN_OK,
                           OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
                           (60 + 5) * OS_TICK_PER_SECOND,
                           OS_NULL);

__exit:
    ec20->netconn_data = OS_NULL;
...
static void urc_connect_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    if (0 == result)
    {
        os_event_send(&ec20->netconn_evt, SET_EVENT(connect_id, EC20_EVENT_CONN_OK));
    }
    else
    {
        os_event_send(&ec20->netconn_evt, SET_EVENT(connect_id, EC20_EVENT_CONN_FAIL));
    }
}
...
static void urc_send_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    os_int32_t curr_connect = ec20->curr_connect;

    if (strstr(data, "SEND OK"))
    {
        os_event_send(&ec20->netconn_evt, SET_EVENT(curr_connect, EC20_EVENT_SEND_OK));
    }
    else if (strstr(data, "SEND FAIL"))
    {
        os_event_send(&ec20->netconn_evt, SET_EVENT(curr_connect, EC20_EVENT_SEND_FAIL));
    }
}
...
static void urc_close_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    mo_netconn_t *netconn = ec20_get_netconn_by_id(module, connect_id);
    if (OS_NULL == netconn)
    {
...
static void urc_dnsqip_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    if (3 == j)
    {
        ...
        inet_aton(recvip, (ip_addr_t *)ec20->netconn_data);

        os_event_send(&ec20->netconn_evt, EC20_EVENT_DOMAIN_OK);
    }
    else
    {
