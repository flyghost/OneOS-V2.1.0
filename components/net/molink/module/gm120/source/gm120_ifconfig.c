 ...
 *
 * @file        gm120_ifconfig.c
 *
 * @brief       gm120_ifconfig.c module api
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "gm120_ifconfig"
...
os_err_t gm120_ifconfig(mo_object_t *self)
{
    ...
    if (gm120_get_ipaddr(self, ipaddr) != OS_EOK)
    {
        memset(ipaddr, 0, sizeof(ipaddr));
    }

    char imei[MO_IMEI_LEN + 1] = {0};
    if (gm120_get_imei(self, imei, sizeof(imei)) != OS_EOK)
    {
        memset(imei, 0, sizeof(imei));
    }

    char iccid[MO_ICCID_LEN + 1] = {0};
    if (gm120_get_iccid(self, iccid, sizeof(iccid)) != OS_EOK)
    {
    ...
    if (gm120_get_csq(self, &rssi, &ber) != OS_EOK)
    {
        rssi = 0;
        ber  = 0;
    }
