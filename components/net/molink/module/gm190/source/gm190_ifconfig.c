 ...
 *
 * @file        gm190_ifconfig.c
 *
 * @brief       gm190_ifconfig.c module api
 *
 * @revision
 * Date         Author          Notes
...
os_err_t gm190_ifconfig(mo_object_t *self)
{
    char ipaddr[IPADDR_MAX_STR_LEN + 1] = {0};
    if (gm190_get_ipaddr(self, ipaddr) != OS_EOK)
    {
        memset(ipaddr, 0, sizeof(ipaddr));
    }

    char imei[MO_IMEI_LEN + 1] = {0};
    if (gm190_get_imei(self, imei, sizeof(imei)) != OS_EOK)
    {
        memset(imei, 0, sizeof(imei));
    }

    char iccid[MO_ICCID_LEN + 1] = {0};
    if (gm190_get_iccid(self, iccid, sizeof(iccid)) != OS_EOK)
    {
...
#define GM190_IPADDR_MAX_STR_LEN  (64)
    ...
    if (at_resp_get_data_by_line(&resp, 1, "+CGPADDR:%d, \"%[^\"]", &ucid, ipaddr) <= 0)
    {
