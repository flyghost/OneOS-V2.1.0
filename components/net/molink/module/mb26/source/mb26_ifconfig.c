...
#define MO_LOG_TAG "mb26.ifconfig"
...
os_err_t mb26_ifconfig(mo_object_t *self)
{
    ...
    if (mb26_get_imsi(self, imsi, sizeof(imsi)) != OS_EOK)
    {
...
os_err_t mb26_get_ipaddr(mo_object_t *self, char ip[])
{
    ...
    if (at_resp_get_data_by_kw(&resp, "+CGPADDR:", "+CGPADDR: %u,\"%[^\"]", &ucid, addr) <= 0)
    {
