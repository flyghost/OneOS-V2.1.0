 ...
 *
 * @file       me3616_general.c
 *
 * @brief      me3616 module link kit general api
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "me3616_general"
...
os_err_t me3616_get_imsi(mo_object_t * self,char * value, os_size_t len)
{
...
os_err_t me3616_get_iccid(mo_object_t * self,char * value, os_size_t len)
{
    ...
    if(OS_EOK != at_parser_exec_cmd(parser, &resp, "AT*MICCID"))
    {
        return OS_ERROR;
    }

    if(at_resp_get_data_by_kw(&resp, "*MICCID:", "*MICCID: %s", value) <= 0)
    {
...
os_err_t me3616_get_cfun(mo_object_t * self, os_uint8_t *fun_lvl)
{
    at_parser_t *parser = &self->parser;
