 ...
 *
 * @file       me3630_w_general.c
 *
 * @brief      me3630_w module link kit general api
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "me3630_w_general"
...
os_err_t me3630_w_get_imei(mo_object_t * self,char * value, os_size_t len)
{
    ...
    if(at_resp_get_data_by_line(&resp, resp.line_counts - 1, "%s", value) <= 0)
    {
...
os_err_t me3630_w_get_iccid(mo_object_t * self,char * value, os_size_t len)
{
    ...
    {
        ERROR("Get %s module ccid failed.",self->name);
        return OS_ERROR;
    }
