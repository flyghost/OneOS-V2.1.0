 ...
 *
 * @file        me3630_w_netconn.c
 *
 * @brief       me3630_w module link kit netconnect api
 *
 * @revision
 * Date         Author          Notes
...
#define ME3630_W_NETCONN_MQ_NAME "me3630_w_nc_mq"

#ifndef ME3630_W_NETCONN_MQ_MSG_SIZE
#define ME3630_W_NETCONN_MQ_MSG_SIZE (sizeof(mo_notconn_msg_t))
#endif
...
static os_err_t me3630_w_lock(os_mutex_t *mutex)
{
    return os_mutex_recursive_lock(mutex, OS_WAIT_FOREVER);
}

static os_err_t me3630_w_unlock(os_mutex_t *mutex)
{
...
static mo_netconn_t *me3630_w_netconn_alloc(mo_object_t *module)
{
    mo_me3630_w_t *me3630_w = os_container_of(module, mo_me3630_w_t, parent);

    for (int i = 0; i < ME3630_W_NETCONN_NUM; i++)
    {
        if (NETCONN_STAT_NULL == me3630_w->netconn[i].stat)
        {
            if (OS_EOK == me3630_w_check_state(module, i + 1))
            {
                /* ME3630_W connect id is 1 to 5 */
                me3630_w->netconn[i].connect_id = i + 1;

                return &me3630_w->netconn[i];
            }
        }
    }
...
static mo_netconn_t *me3630_w_get_netconn_by_id(mo_object_t *module, os_int32_t connect_id)
{
    OS_ASSERT(OS_NULL != module);

    if (1 > connect_id || ME3630_W_NETCONN_NUM < connect_id)
    {
    ...
    for (int i = 0; i < ME3630_W_NETCONN_NUM; i++)
    {
        if (connect_id == me3630_w->netconn[i].connect_id)
        {
            return &me3630_w->netconn[i];
        }
    }
...
mo_netconn_t *me3630_w_netconn_create(mo_object_t *module, mo_netconn_type_t type)
{
    mo_me3630_w_t *me3630_w = os_container_of(module, mo_me3630_w_t, parent);
    me3630_w_lock(&me3630_w->netconn_lock);
    mo_netconn_t *netconn = me3630_w_netconn_alloc(module);
...
#define ZIP_CALL_TIMES (6)
    ...
    os_uint32_t event = SET_EVENT(netconn->connect_id, ME3630_W_EVENT_CONN_OK | ME3630_W_EVENT_CONN_FAIL);
    os_event_recv(&me3630_w->netconn_evt, event, OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR, OS_NO_WAIT, OS_NULL);
    if (OS_TRUE != me3630_w_check_zipcall(module))
    {
        if (OS_FALSE == me3630_w_pdp_set(module))
        {
        ...
        int i = 0;
        while (OS_TRUE != me3630_w_check_zipcall(module))
        {
...
os_size_t me3630_w_netconn_send(mo_object_t *module, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    ...
    me3630_w->curr_connect = netconn->connect_id;
    ...
    while (sent_size < size)
    {
        ...
        result = os_event_recv(&me3630_w->netconn_evt,
        ...
        result = os_event_recv(&me3630_w->netconn_evt,
                               ME3630_W_EVENT_SEND_OK | ME3630_W_EVENT_SEND_FAIL,
                               OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
        ...
        if (event & ME3630_W_EVENT_SEND_FAIL)
        {
...
os_err_t me3630_w_netconn_gethostbyname(mo_object_t *self, const char *domain_name, ip_addr_t *addr)
{
    ...
    mo_me3630_w_t *me3630_w = os_container_of(self, mo_me3630_w_t, parent);
    ...
    inet_aton(recvip, (ip_addr_t *)me3630_w->netconn_data);

__exit:
    if (result != OS_EOK)
    {
        me3630_w->netconn_data = OS_NULL;
    }
...
static void urc_send_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    os_int32_t curr_connect = me3630_w->curr_connect;

    if (data_size > 0)
    {
        os_event_send(&me3630_w->netconn_evt, SET_EVENT(curr_connect, ME3630_W_EVENT_SEND_OK));
    }
    else
    {
        os_event_send(&me3630_w->netconn_evt, SET_EVENT(curr_connect, ME3630_W_EVENT_SEND_FAIL));
    }
}
...
__exit:
    ...
    {
        WARN("ME3630_W network init sucess");
    }
}

void me3630_w_netconn_init(mo_me3630_w_t *module)
{
    /* Init module netconn array */
    memset(module->netconn, 0, sizeof(module->netconn));
    for (int i = 1; i < ME3630_W_NETCONN_NUM; i++)
    {
        module->netconn[i].connect_id = -1;
    }

    me3630_w_network_init(&module->parent);
