 ...
 * @file        n58_general.c
 *
 * @brief       n58 module link kit general api
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "n58.general"
#define MO_LOG_LVL MO_LOG_INFO
#include <mo_log.h>

#define N58_VERSION_INFO_LEN (30)
...
os_err_t n58_get_firmware_version(mo_object_t *self, mo_firmware_version_t *version)
{
    ...
    char source_line[N58_VERSION_INFO_LEN] = {0};

    if (at_resp_get_data_by_kw(&resp, "+CGMR:", "+CGMR: %s", source_line) < 0)
    {
        ERROR("Get %s module version infomation failed.", self->name);
        return OS_ERROR;
    }
