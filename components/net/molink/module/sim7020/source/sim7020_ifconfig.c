 ...
 *
 * @file        sim7020_ifconfig.c
 *
 * @brief       sim7020 module link kit ifconfig api implement
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "sim7020.ifconfig"
...
os_err_t sim7020_ifconfig(mo_object_t *self)
{
    ...
    if (sim7020_get_ipaddr(self, ipaddr) != OS_EOK)
    {
        memset(ipaddr, 0, sizeof(ipaddr));
        ret = OS_ERROR;
    }

    if (sim7020_get_imei(self, imei, sizeof(imei)) != OS_EOK)
    {
        memset(imei, 0, sizeof(imei));
        ret = OS_ERROR;
    }

    if (sim7020_get_iccid(self, iccid, sizeof(iccid)) != OS_EOK)
    {
        memset(iccid, 0, sizeof(iccid));
        ret = OS_ERROR;
    }

    if (sim7020_get_imsi(self, imsi, sizeof(imsi)) != OS_EOK)
    {
        memset(imsi, 0, sizeof(imsi));
        ret = OS_ERROR;
    }

    if (sim7020_get_csq(self, &rssi, &ber) != OS_EOK)
    {
        rssi = 0;
        ber  = 0;
        ret  = OS_ERROR;
