 ...
 *
 * @file        sim7070x_ifconfig.c
 *
 * @brief       sim7070x module link kit ifconfig api implement
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "sim7070x_ifconfid"
...
os_err_t sim7070x_ifconfig(mo_object_t *self)
{
    ...
    if (sim7070x_get_ipaddr(self, ipaddr) != OS_EOK)
    {
        memset(ipaddr, 0, sizeof(ipaddr));
        ret = OS_ERROR;
    }

    if (sim7070x_get_imei(self, imei, sizeof(imei)) != OS_EOK)
    {
        memset(imei, 0, sizeof(imei));
        ret = OS_ERROR;
    }

    if (sim7070x_get_iccid(self, iccid, sizeof(iccid)) != OS_EOK)
    {
        memset(iccid, 0, sizeof(iccid));
        ret = OS_ERROR;
    }

    if (sim7070x_get_csq(self, &rssi, &ber) != OS_EOK)
    {
        rssi = 0;
        ber  = 0;
        ret = OS_ERROR;
...
os_err_t sim7070x_get_ipaddr(mo_object_t *self, char ip[])
{
    ...
    result = at_parser_exec_cmd(parser, &resp, "AT+CGPADDR=1");
    if (result != OS_EOK)
    {
    ...
    /* Response for ex: +CGPADDR: 1,10.36.117.237 */
    if (at_resp_get_data_by_kw(&resp, "+CGPADDR: 1", "+CGPADDR: 1,%s", ipaddr) <= 0)
    {
