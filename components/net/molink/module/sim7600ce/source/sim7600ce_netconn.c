 ...
 *
 * @file        sim7600ce_netconn.c
 *
 * @brief       sim7600ce module link kit netconnect api
 *
 * @revision
 * Date         Author          Notes
...
#define MO_LOG_TAG "sim7600ce.netconn"
...
#ifndef SIM7600CE_NETCONN_MQ_MSG_SIZE
#define SIM7600CE_NETCONN_MQ_MSG_SIZE (sizeof(mo_notconn_msg_t))
#endif
...
#define SIM7600CE_EVENT_SEND_OK      (1L << 2)
#define SIM7600CE_EVENT_SEND_FAIL    (1L << 3)
...
static os_err_t sim7600ce_lock(os_mutex_t *mutex)
{
    return os_mutex_recursive_lock(mutex, OS_WAIT_FOREVER);
}

static os_err_t sim7600ce_unlock(os_mutex_t *mutex)
{
    return os_mutex_recursive_unlock(mutex);
}

static mo_netconn_t *sim7600ce_netconn_alloc(mo_object_t *module)
{
    mo_sim7600ce_t *sim7600ce = os_container_of(module, mo_sim7600ce_t, parent);

    for (int i = 0; i < SIM7600CE_NETCONN_NUM; i++)
    {
        if (NETCONN_STAT_NULL == sim7600ce->netconn[i].stat)
        {
            /* TODO: Remove after adding the API of setting local port  */
            /* NOTE: SIM7600CE module dosen't support auto allocate udp port */
            sim7600ce->netconn[i].local_port = i + 50000;
            sim7600ce->netconn[i].connect_id = i;
            return &sim7600ce->netconn[i];
        }
    }
...
static mo_netconn_t *sim7600ce_get_netconn_by_id(mo_object_t *module, os_int32_t connect_id)
{
    OS_ASSERT(OS_NULL != module);

    if (0 > connect_id || SIM7600CE_NETCONN_NUM <= connect_id)
    {
    ...
    for (int i = 0; i < SIM7600CE_NETCONN_NUM; i++)
    {
        if (connect_id == sim7600ce->netconn[i].connect_id)
        {
            return &sim7600ce->netconn[i];
        }
    }
...
static os_err_t sim7600ce_net_server_config(mo_object_t *module)
{
    ...
    result = sim7600ce_netconn_set_transfer_mode(module);
    if (OS_EOK != result)
    {
    ...
    result = sim7600ce_netconn_server_open(module);
    if (OS_EOK != result)
    {
...
mo_netconn_t *sim7600ce_netconn_create(mo_object_t *module, mo_netconn_type_t type)
{
    ...
    mo_netconn_t *netconn = sim7600ce_netconn_alloc(module);
    ...
    result = sim7600ce_net_server_config(module);
    if (OS_EOK != result)
    {
    ...
    netconn->mq = os_mq_create(SIM7600CE_NETCONN_MQ_NAME, SIM7600CE_NETCONN_MQ_MSG_SIZE, SIM7600CE_NETCONN_MQ_MSG_MAX);
...
static os_err_t sim7600ce_tcp_connect(mo_object_t *module, mo_netconn_t *netconn, char *ip_addr, os_uint16_t port)
{
    ...
    result = os_event_recv(&sim7600ce->netconn_evt,
    ...
    result = os_event_recv(&sim7600ce->netconn_evt,
                           SIM7600CE_EVENT_NETCONN_OK | SIM7600CE_EVENT_NETCONN_FAIL,
                           OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
    ...
    if (event & SIM7600CE_EVENT_NETCONN_FAIL)
    {
        result = OS_ERROR;
...
static os_err_t sim7600ce_udp_connect(mo_object_t *module, mo_netconn_t *netconn)
{
    at_parser_t *parser = &module->parser;
...
os_err_t sim7600ce_netconn_connect(mo_object_t *module, mo_netconn_t *netconn, ip_addr_t addr, os_uint16_t port)
{
    ...
    case NETCONN_TYPE_TCP:
        result = sim7600ce_tcp_connect(module, netconn, remote_ip, port);
        break;

    case NETCONN_TYPE_UDP:
        result = sim7600ce_udp_connect(module, netconn);
        break;
...
os_size_t sim7600ce_netconn_send(mo_object_t *module, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    ...
    while (sent_size < size)
    {
        ...
        {
            result = OS_ERROR;
            ERROR("Drv send data fail, there is data loss.\r\n");
            goto __exit;
        }
        ...
        result = os_event_recv(&sim7600ce->netconn_evt,
                               SIM7600CE_EVENT_SEND_OK | SIM7600CE_EVENT_SEND_FAIL,
                               OS_EVENT_OPTION_OR | OS_EVENT_OPTION_CLEAR,
        ...
        if (event & SIM7600CE_EVENT_SEND_FAIL)
        {
...
static void urc_tcp_connect_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    if (0 == result)
    {
        os_event_send(&sim7600ce->netconn_evt, SET_EVENT(connect_id, SIM7600CE_EVENT_NETCONN_OK));
    }
    else
    {
        os_event_send(&sim7600ce->netconn_evt, SET_EVENT(connect_id, SIM7600CE_EVENT_NETCONN_FAIL));
    }
...
static void urc_send_msg_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    if (result != OS_EOK)
    {
        os_event_send(&sim7600ce->netconn_evt, SET_EVENT(connect_id, SIM7600CE_EVENT_SEND_FAIL));
    }
    else
    {
        os_event_send(&sim7600ce->netconn_evt, SET_EVENT(connect_id, SIM7600CE_EVENT_SEND_OK));
    }
