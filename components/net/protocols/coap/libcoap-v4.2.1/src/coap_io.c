...
#else
  if (ioctl(sock->fd, (unsigned long)FIONBIO, &on) == COAP_SOCKET_ERROR) {
#endif
    coap_log(LOG_WARNING,
...
#else
  if (ioctl(new_client->fd, (unsigned long)FIONBIO, &on) == COAP_SOCKET_ERROR) {
#endif
...
#ifdef COAP_FROM_HY
  int is_bcast = 0;  
  extern coap_address_t g_broadaddr;
  if(0 == strcmp(inet_ntoa(server->addr), inet_ntoa(g_broadaddr.addr)))
  {
      is_bcast = 1;
  }
#endif
