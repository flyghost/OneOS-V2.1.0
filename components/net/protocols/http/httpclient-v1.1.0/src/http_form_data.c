...
int httpclient_formdata_addtext(http_client_data_t* client_data, char* content_disposition, char* content_type, char* name, char* data, int data_len)
{
    int buf_len;
    ...
    if((content_disposition == NULL) || (name == NULL) || (data == NULL) || (data_len == 0)) {
        LOG_E(HTTP_TAG, "%s:%d invalid params", __func__, __LINE__);
        return -1;
    }

    if(strlen(data) > data_len) {
        LOG_E(HTTP_TAG, "%s:%d invalid data_len %d strlen data %d", __func__, __LINE__, data_len, strlen(data));
        return -1;
    }

    if((data_info = found_formdata_info(client_data)) == NULL) {
        if((data_info = found_empty_formdata_info()) == NULL) {
            LOG_E(HTTP_TAG, "%s:%d found no client_data info", __func__, __LINE__);
            return -1;
        }
    }
    ...
    if(data_info->is_used == 0) {
        ...
        if(data_info->form_data == NULL) {
            data_info->is_used = 0;
            LOG_E(HTTP_TAG, "%s:%d form data malloc failed", __func__, __LINE__);
            return -1;
        }
    ...
    if( current->data == NULL) {
        ...
        LOG_E(HTTP_TAG, "%s:%d data malloc failed", __func__, __LINE__);
        return -1;
    }
...
void httpclient_clear_form_data(http_client_data_t * client_data)
{
    ...
    if(data_info == NULL) {
        LOG_D(HTTP_TAG, "No form data info found");
        return;
    }
    ...
    if(current != NULL) {
        ...
        LOG_E(HTTP_TAG, "No form data in form data info");
    }
...
int httpclient_formdata_addfile(http_client_data_t* client_data, char* content_disposition, char* name, char* content_type, char* file_path)
{
    int buf_len;
    formdata_info_t* data_info;
    formdata_node_t* previous;
...
int httpclient_formdata_len(http_client_data_t *client_data)
{
    ...
    do {
        ...
        if(current->is_file == 1) {
            ...
            if(fd == NULL) {
                LOG_E(HTTP_TAG, "%s: open file(%s) failed errno=%d", __func__, current->file_path, errno);
                return -1;
            }
...
int http_client_send_formdata(http_client_t *client, http_client_data_t *client_data)
{
...
#if CONFIG_HTTP_FILE_OPERATE
    {
        ...
        while(!feof(fd)) {
            ...
            if(ret <= 0) {
                LOG_E(HTTP_TAG, "fread failed returned %d errno=%d", ret, errno);
                return -1;
            }

            ret = http_tcp_send_wrapper(client, data, ret);
            if (ret > 0) {
                LOG_E(HTTP_TAG, "Written %d bytes", ret);
            } else if ( ret == 0 ) {
                LOG_E(HTTP_TAG, "ret == 0,Connection was closed by server");
                return HTTP_ECLSD; /* Connection was closed by server */
            } else {
                LOG_E(HTTP_TAG, "Connection error (send returned %d) errno=%d", ret, errno);
                return HTTP_ECONN;
            }
