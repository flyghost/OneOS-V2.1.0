                    ...
                    char * buffer)
{
    ...
    else
    {
        ...
        {
            lwm2m_log(COMP_LOG_ERROR, UNKNOWN_CMD_MSG"\n");
        }
    }
}
                   ...
                   int indent)
{
    ...
    if (length == 0) lwm2m_log(COMP_LOG_DEBUG, "\n");
    ...
    while (i < length)
    {
        ...
        {
            lwm2m_log(COMP_LOG_DEBUG, "%02X ", array[j]);
            if (j%4 == 3) lwm2m_log(COMP_LOG_DEBUG, " ");
        }
        if (length > 16)
        {
            while (j < 16)
        ...
        for (j = 0 ; j < 16 && i+j < length; j++)
        {
            if (isprint(array[j]))
            {
                lwm2m_log(COMP_LOG_DEBUG, "%c", array[j]);
            }
            else
            {
            	lwm2m_log(COMP_LOG_DEBUG, ".");
                ...
                int indent)
{
    ...
    while (0 != (result = lwm2m_decode_TLV((uint8_t*)buffer + length, buffer_len - length, &type, &id, &dataIndex, &dataLen)))
    {
        ...
        lwm2m_log(COMP_LOG_DEBUG, "ID: %d", id);

        lwm2m_log(COMP_LOG_DEBUG, " type: ");
        switch (type)
        {
        case LWM2M_TYPE_OBJECT_INSTANCE:
            lwm2m_log(COMP_LOG_DEBUG, "Object Instance");
            break;
        case LWM2M_TYPE_MULTIPLE_RESOURCE:
            lwm2m_log(COMP_LOG_DEBUG, "Multiple Instances");
            break;
        case LWM2M_TYPE_OPAQUE:
            lwm2m_log(COMP_LOG_DEBUG, "Resource Value");
            break;
        default:
            lwm2m_log(COMP_LOG_ERROR, "unknown (%d)", (int)type);
            break;
        }
        lwm2m_log(COMP_LOG_DEBUG, "\n");
        ...
        else
        {
            ...
            lwm2m_log(COMP_LOG_DEBUG, "data (%d bytes):\n", dataLen);
            ...
            {
                print_indent(stream, indent+2);
                lwm2m_log(COMP_LOG_DEBUG, "data as Integer: %lld\n", intValue);
            }
            if (0 < sscanf((const char*)buffer + length + dataIndex, "%lg", &floatValue))
            {
                print_indent(stream, indent+2);
                lwm2m_log(COMP_LOG_DEBUG, "data as Float: %.16g\n", floatValue);
            }
                 ...
                 int indent)
{
    ...
    print_indent(stream, indent);
    lwm2m_log(COMP_LOG_DEBUG, "%d bytes received of type ", dataLength);

    switch (format)
    {
    case LWM2M_CONTENT_TEXT:
        lwm2m_log(COMP_LOG_DEBUG, "text/plain:\n");
        output_buffer(stream, data, dataLength, indent);
        break;

    case LWM2M_CONTENT_OPAQUE:
        lwm2m_log(COMP_LOG_DEBUG, "application/octet-stream:\n");
        output_buffer(stream, data, dataLength, indent);
        break;

    case LWM2M_CONTENT_TLV:
        lwm2m_log(COMP_LOG_DEBUG, "application/vnd.oma.lwm2m+tlv:\n");
        output_tlv(stream, data, dataLength, indent);
        break;

    case LWM2M_CONTENT_JSON:
        lwm2m_log(COMP_LOG_DEBUG, "application/vnd.oma.lwm2m+json:\n");
        print_indent(stream, indent);
        for (i = 0 ; i < dataLength ; i++)
        {
            lwm2m_log(COMP_LOG_DEBUG, "%c", data[i]);
        }
        lwm2m_log(COMP_LOG_DEBUG, "\n");
        break;

    case LWM2M_CONTENT_LINK:
        lwm2m_log(COMP_LOG_DEBUG, "application/link-format:\n");
        print_indent(stream, indent);
        for (i = 0 ; i < dataLength ; i++)
        {
            lwm2m_log(COMP_LOG_DEBUG, "%c", data[i]);
    ...
    default:
        lwm2m_log(COMP_LOG_ERROR, "Unknown (%d):\n", format);
        output_buffer(stream, data, dataLength, indent);
        break;
    }
}
              ...
              int indent)
{
    ...
    for(i= 0 ; i < size ; i++)
    {
        ...
        case LWM2M_TYPE_OBJECT:
            lwm2m_log(COMP_LOG_DEBUG, "LWM2M_TYPE_OBJECT\n");
            dump_tlv(stream, dataP[i].value.asChildren.count, dataP[i].value.asChildren.array, indent + 1);
            break;
        case LWM2M_TYPE_OBJECT_INSTANCE:
            lwm2m_log(COMP_LOG_DEBUG, "LWM2M_TYPE_OBJECT_INSTANCE\n");
            dump_tlv(stream, dataP[i].value.asChildren.count, dataP[i].value.asChildren.array, indent + 1);
            break;
        case LWM2M_TYPE_MULTIPLE_RESOURCE:
            lwm2m_log(COMP_LOG_DEBUG, "LWM2M_TYPE_MULTIPLE_RESOURCE\n");
            dump_tlv(stream, dataP[i].value.asChildren.count, dataP[i].value.asChildren.array, indent + 1);
            break;
        case LWM2M_TYPE_UNDEFINED:
            lwm2m_log(COMP_LOG_DEBUG, "LWM2M_TYPE_UNDEFINED\n");
            break;
        case LWM2M_TYPE_STRING:
            lwm2m_log(COMP_LOG_DEBUG, "LWM2M_TYPE_STRING\n");
            print_indent(stream, indent + 1);
            lwm2m_log(COMP_LOG_DEBUG, "\"%.*s\"\n", (int)dataP[i].value.asBuffer.length, dataP[i].value.asBuffer.buffer);
            break;
        case LWM2M_TYPE_OPAQUE:
            lwm2m_log(COMP_LOG_DEBUG, "LWM2M_TYPE_OPAQUE\n");
            output_buffer(stream, dataP[i].value.asBuffer.buffer, dataP[i].value.asBuffer.length, indent + 1);
            break;
        case LWM2M_TYPE_INTEGER:
            lwm2m_log(COMP_LOG_DEBUG, "LWM2M_TYPE_INTEGER: ");
            print_indent(stream, indent + 1);
            lwm2m_log(COMP_LOG_DEBUG, "%lld", dataP[i].value.asInteger);
            lwm2m_log(COMP_LOG_DEBUG, "\n");
            break;
        case LWM2M_TYPE_FLOAT:
            lwm2m_log(COMP_LOG_DEBUG, "LWM2M_TYPE_FLOAT: ");
            print_indent(stream, indent + 1);
            lwm2m_log(COMP_LOG_DEBUG, "%lld", dataP[i].value.asInteger);
            lwm2m_log(COMP_LOG_DEBUG, "\n");
            break;
        case LWM2M_TYPE_BOOLEAN:
            lwm2m_log(COMP_LOG_DEBUG, "LWM2M_TYPE_BOOLEAN: ");
            lwm2m_log(COMP_LOG_DEBUG, "%s", dataP[i].value.asBoolean ? "true" : "false");
            lwm2m_log(COMP_LOG_DEBUG, "\n");
            break;
        case LWM2M_TYPE_OBJECT_LINK:
            lwm2m_log(COMP_LOG_DEBUG, "LWM2M_TYPE_OBJECT_LINK\r\n");
            break;
        default:
            lwm2m_log(COMP_LOG_ERROR, "unknown (%d)\n", (int)dataP[i].type);
            break;
...
void print_status(FILE * stream,
                  uint8_t status)
{
    lwm2m_log(COMP_LOG_DEBUG, "%d.%02d (%s)", (status&0xE0)>>5, status&0x1F, prv_status_to_string(status));
}
