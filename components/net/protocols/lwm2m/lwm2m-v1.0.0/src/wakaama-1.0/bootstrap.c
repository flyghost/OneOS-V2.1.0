                               ...
                               lwm2m_coap_packet_t * message)
{
    ...
    {
        lwm2m_log(COMP_LOG_INFO, "Received ACK/2.04, Bootstrap pending, waiting for DEL/PUT from BS server...\n");
                                 ...
                                 lwm2m_server_t * bootstrapServer)
{
    ...
    if (bootstrapServer->sessionH != NULL)
    {
        ...
        lwm2m_log(COMP_LOG_DEBUG, "Bootstrap server connection opened\n");
        ...
        {
            lwm2m_log(COMP_LOG_DEBUG, "CI bootstrap requested to BS server\n");
            bootstrapServer->status = STATE_BS_INITIATED;
        }
    }
    else
    {
        lwm2m_log(COMP_LOG_DEBUG, "Connecting bootstrap server failed\n");
                    ...
                    time_t * timeoutP)
{
    ...
    while (targetP != NULL)
    {
        lwm2m_log(COMP_LOG_DEBUG, "Initial status: %s", STR_STATUS(targetP->status));
        ...
        lwm2m_log(COMP_LOG_DEBUG, "Finalal status: %s", STR_STATUS(targetP->status));
                               ...
                               void * fromSessionH)
{
     ...
     && bootstrapServer->status == STATE_BS_PENDING)
    {
        ...
        {
            lwm2m_log(COMP_LOG_INFO, "Bootstrap server status changed to STATE_BS_FINISHING\n");
...
lwm2m_status_t bootstrap_getStatus(lwm2m_context_t * contextP)
{
    ...
    lwm2m_log(COMP_LOG_DEBUG, "Returned status: %s\n", STR_STATUS(bs_status));
...
{
    lwm2m_log(COMP_LOG_DEBUG, "Initial status: %s\n", STR_STATUS(serverP->status));

    switch (serverP->status)
    {
    case STATE_BS_HOLD_OFF:
        serverP->status = STATE_BS_PENDING;
        lwm2m_log(COMP_LOG_DEBUG, "Status changed to: %s\n", STR_STATUS(serverP->status));
        break;
    ...
    default:
        lwm2m_log(COMP_LOG_WARN, "Returning COAP_IGNORE\n");
        return COAP_IGNORE;
    }
                                ...
                                lwm2m_coap_packet_t * response)
{
    ...
    lwm2m_log(COMP_LOG_INFO, "Code: %02X\n", message->code);
    LOG_URI(uriP);
    ...
    lwm2m_log(COMP_LOG_DEBUG, "Server status: %s\n", STR_STATUS(serverP->status));
