                         ...
                         size_t bufferLen)
{
    ...
    {
		lwm2m_log(COMP_LOG_DEBUG, "buffer is NULL! ");
        return 0;
    }
...
lwm2m_data_t * lwm2m_data_new(int size)
{
    ...
    lwm2m_log(COMP_LOG_DEBUG, "size: %d", size);
                     ...
                     lwm2m_data_t * dataP)
{
    ...
    lwm2m_log(COMP_LOG_DEBUG, "size: %d\n", size);
                              ...
                              lwm2m_data_t * dataP)
{
    ...
    lwm2m_log(COMP_LOG_DEBUG, "length: %d\n", length);
    if (length == 0)
    {
        dataP->value.asBuffer.length = 0;
        dataP->value.asBuffer.buffer = NULL;
...
void lwm2m_data_encode_nstring(const char * string,
                               ...
                               lwm2m_data_t * dataP)
{
    lwm2m_log(COMP_LOG_DEBUG, "length: %d, string: \"%s\"\n", length, string);
...
void lwm2m_data_encode_int(int64_t value,
                           lwm2m_data_t * dataP)
{
    lwm2m_log(COMP_LOG_DEBUG, "value: %lld", value);
                          ...
                          int64_t * valueP)
{
    ...
    lwm2m_log(COMP_LOG_DEBUG,"Entering");
    switch (dataP->type)
    {
    case LWM2M_TYPE_INTEGER:
    ...
    lwm2m_log(COMP_LOG_DEBUG, "result: %d, value: %lld", result, *valueP);
...
void lwm2m_data_encode_float(double value,
                             lwm2m_data_t * dataP)
{
    lwm2m_log(COMP_LOG_DEBUG, "value: %f", value);
                            ...
                            double * valueP)
{
    ...
    lwm2m_log(COMP_LOG_DEBUG, "result: %d, value: %f\n", result, *valueP);

    return result;
}

void lwm2m_data_encode_bool(const bool value,
                            lwm2m_data_t * dataP)
{
    lwm2m_log(COMP_LOG_DEBUG, "value: %s\n", value?"true":"false");
                           ...
                           bool * valueP)
{
    ...
    lwm2m_log(COMP_LOG_DEBUG, "result: %d, value: %s\n", result, *valueP ? "true" : "false");

    return result;
}

void lwm2m_data_encode_objlink(const uint16_t objectId,
                           const uint16_t objectInstanceId,
                           lwm2m_data_t * dataP)
{
    lwm2m_log(COMP_LOG_DEBUG, "value: %d/%d\n", objectId, objectInstanceId);
...
void lwm2m_data_include(lwm2m_data_t * subDataP,
                        ...
                        lwm2m_data_t * dataP)
{
    lwm2m_log(COMP_LOG_DEBUG, "count: %d\n", count);
...
void lwm2m_data_encode(lwm2m_data_codec_type_t type, const void* valueP, size_t var, lwm2m_data_t* dataP)
{
    switch(type)
    {
        ...
        case LWM2M_CODEC_TYPE_STRING:
        {
            ...
            lwm2m_data_encode_string(string, dataP);
            break;
        }
        ...
        {
            int64_t* value = (int64_t *)valueP;
            lwm2m_data_encode_int(*value, dataP);
            break;
        }
        case LWM2M_CODEC_TYPE_FLOAT:
        {
            double* value = (double *) valueP;
            lwm2m_data_encode_float(*value, dataP);
            break;
        }
        case LWM2M_CODEC_TYPE_BOOLEAN:
        {
            bool* value = (bool *)valueP;
            lwm2m_data_encode_bool(*value, dataP);
            break;
        }
...
int lwm2m_data_decode(lwm2m_data_codec_type_t type, const lwm2m_data_t* dataP, void* valueP)
{
    int ret = 0;
    switch(type)
    {
        ...
        {
            int64_t *value = (int64_t *)valueP;
            ret = lwm2m_data_decode_int(dataP, value);
            break;
        }
        case LWM2M_CODEC_TYPE_FLOAT:
        {
            double *value = (double*)valueP;
            ret = lwm2m_data_decode_float(dataP, value);
            break;
        }
        case LWM2M_CODEC_TYPE_BOOLEAN:
        {
            bool *value = (bool*)valueP;
            ret = lwm2m_data_decode_bool(dataP, value);
            break;
        }
        default:
            ret = -1;
                     ...
                     lwm2m_data_t ** dataP)
{
    ...
    lwm2m_log(COMP_LOG_DEBUG, "format: %s, bufferLen: %d\n", STR_MEDIA_TYPE(format), bufferLen);
    LOG_URI(uriP);
    switch (format)
    {
...
{
    LOG_URI(uriP);
    lwm2m_log(COMP_LOG_DEBUG,"size: %d, formatP: %s\n", size, STR_MEDIA_TYPE(*formatP));
    ...
    if (*formatP == LWM2M_CONTENT_OPAQUE
     && dataP->type != LWM2M_TYPE_OPAQUE)
    {
        lwm2m_log(COMP_LOG_ERROR, "Opaque format is reserved to opaque resources.\n");
        return -1;
    }

    lwm2m_log(COMP_LOG_DEBUG, "Final format: %s\n", STR_MEDIA_TYPE(*formatP));
