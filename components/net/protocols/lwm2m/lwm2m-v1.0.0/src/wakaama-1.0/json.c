               ...
               lwm2m_data_t ** dataP)
{
    ...
    lwm2m_log(COMP_LOG_DEBUG, "bufferLen: %d, buffer: \"%s\"\n", bufferLen, (char *)buffer);
    LOG_URI(uriP);
    *dataP = NULL;
    recordArray = NULL;
    parsedP = NULL;
    ...
    if (eFound == true)
    {
        ...
        else
        {
            ...
            {
                res = lwm2m_string_to_uri((char *)buffer + bnStart, bnLen, &baseURI);
    ...
    lwm2m_log(COMP_LOG_DEBUG, "Parsing successful. count: %d\n", count);
    return count;

error:
    lwm2m_log(COMP_LOG_ERROR, "Parsing failed\n");
    if (parsedP != NULL)
    {
        lwm2m_data_free(count, parsedP);
        parsedP = NULL;
