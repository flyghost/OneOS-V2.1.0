...
static void tls_default_dbg_func(void *fd, int level, const char *file, int line, const char *str)
{
    //avoid warning
    ((void) fd);
    ((void) level);
    ...
    mbedtls_printf("%s:%04d: %s", file + MBEDTLS_SRC_PATH_LENTH, line, str); //not show the file path
}
...
void mbedtls_ssl_config_init( mbedtls_ssl_config *conf )
{
    ...
    #if defined(MBEDTLS_DEBUG_C) && defined(MBDTLS_DEBUG_LOG_LEVEL)
    mbedtls_debug_set_threshold( MBDTLS_DEBUG_LOG_LEVEL );
    #endif

    conf->f_dbg = tls_default_dbg_func;
}
