...
{
    (void)ctx;
    if (sizeof(test_ticket) < len)
    {
        os_kprintf("Recv new session ticket len overlength =%d, please add\r\n", len);
    }
 ...
 ***********************************************************************************************************************
 */
int onetls_tls_client_test(void)
{
    int ret = 0;
    int sock = 0;
    ...
    ret = onetls_set_outband_psk_hint(ctx, test_psk_hint, sizeof(test_psk_hint));
    if (ret != 0) {
    ...
    onetls_socket_timeout(ctx, 10000);
    ...
    os_kprintf("Please type data in server during 10s:\r\n");
    do {
...
#include <shell.h>
SH_CMD_EXPORT(tls_test, onetls_tls_client_test, "start onetls tls1.3 client test");
#endif
