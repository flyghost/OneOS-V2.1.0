 ...
 *
 * @file        sensor_temp_test.c
 *
 * @brief       The test file for temp sensor.
 *
 * @revision
 * Date         Author          Notes
...
static int sensor_temp_test(int argc, char *argv[])
{
    int  i;
    char sensor_name[24];
    ...
    if (argc != 2)
    {
        os_kprintf("usage:sensor_temp_test <sensor_name>\r\n");
        return -1;
    }

    snprintf(sensor_name, sizeof(sensor_name) - 1, "temp_%s", argv[1]);
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        {
            os_kprintf("sensor temp (%d.%03d)\r\n", sensor_data.data.temp / 1000, sensor_data.data.temp % 1000);
        }
...
SH_CMD_EXPORT(sensor_temp_test, sensor_temp_test, "sensor_temp_test");
