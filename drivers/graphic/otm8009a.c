...
/*
 * Constant tables of register settings used to transmit DSI command packets as power up initialization sequence of the KoD LCD
 */
...
void DSI_IO_WriteCmd(uint32_t NbrParams, uint8_t *pParams)
{
    os_device_dsi_t *dsi = (os_device_dsi_t *)os_device_find("dsi");

    OS_ASSERT(dsi);

    os_dsi_cmd(dsi, BSP_OTM8009A_DSI_CHANNEL, pParams, NbrParams);
}
...
{
    os_kprintf("otm8009a init start.\r\n");

    OTM8009A_Init(OTM8009A_FORMAT_RGB888, 0);

    os_kprintf("otm8009a init end.\r\n");

    return 0;
}

/* depends on os_hw_lcd_init */
OS_INIT_EXPORT(otm8009a_init, "4", OS_INIT_SUBLEVEL_LOW);
