...
extern void beken_uart_isr_rx(unsigned char uport);
...
{
#ifdef BEKEN_DRV_DEBUG //open driver logs
     ...
     * length. */
    length = os_vsnprintf(rt_log_buf, sizeof(rt_log_buf) - 1, fmt, args);
    if (length > RT_CONSOLEBUF_SIZE - 1)
        length = RT_CONSOLEBUF_SIZE - 1;
    os_kprintf("%s", rt_log_buf);
    va_end(args);
#else
    va_list ap;
    char string[128];
    ...
    va_start(ap, fmt);
    os_vsnprintf(string, sizeof(string) - 1, fmt, ap);
    string[127] = 0;
    bk_send_string(uart_print_port, string);
    va_end(ap);
#endif

#endif /* BEKEN_DRV_DEBUG */
}
...
INT32 uart_printf(const char *fmt, ...)
{
    ...
    va_list args;
    va_start(args, fmt);
    rc = os_vsnprintf(buf, sizeof(buf), fmt, args);
    va_end(args);
...
void uart_hw_set_change(UINT8 uport, bk_uart_config_t *uart_config)
{
    ...
    if(parity_en)
    {
        if(parity_en == BK_PARITY_ODD)
            parity_mode = 0; // fix beken odd/even bug by oneos (1 to 0)
        else
            parity_mode = 1;
        parity_en = 1;
    }
...
#else /* irq receive data */

	beken_uart_isr_rx(UART1_PORT);
#endif /* BOARD_BK7231N */
    }
