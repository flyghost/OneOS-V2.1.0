...
static void sae_pick_next_group(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	wpa_printf(MSG_INFO, "SAE: Previously selected group: %d", sae->group);
	...
	wpa_printf(MSG_INFO, "SAE: Selected new group: %d", groups[i]);
}
			...
			const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	u32 session_timeout = 0, acct_interim_interval;
			...
			u16 reason_code)
{
	...
	if (hostapd_drv_send_mlme(hapd, &reply, send_len, 0) < 0)
		wpa_printf(MSG_INFO, "Failed to send deauth error");
}
			   ...
			   size_t ies_len)
{
	...
	if (hostapd_drv_send_mlme(hapd, reply, send_len, 0) < 0) {
		wpa_printf(MSG_INFO, "Failed to send assoc resp error");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
			    ...
			    size_t len, int reassoc, int ok)
{
	...
	hapd->new_assoc_sta_cb(hapd, sta, !new_assoc);  // hostapd_new_assoc_sta
