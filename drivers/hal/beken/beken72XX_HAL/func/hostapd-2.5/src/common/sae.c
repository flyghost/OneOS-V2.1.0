...
int sae_set_group(struct sae_data *sae, int group)
{
	...
	bk_printf("%s: before %d %d\n", __func__, xPortGetFreeHeapSize(), xPortGetMinimumEverFreeHeapSize());
	sae_clear_data(sae);
...
static struct crypto_bignum * sae_get_rand(struct sae_data *sae)
{
	...
	if (order_len > SAE_MAX_PRIME_LEN)
		return NULL;

#ifdef CONFIG_SAE_SMALL_STACK
	val = os_malloc(SAE_MAX_PRIME_LEN);
	if (!val)
		return NULL;
#endif
			      ...
			      size_t password_len)
{
	...
	for (counter = 1; counter <= k || !x; counter++) {
		...
		wpa_printf(MSG_INFO, "SAE: counter = %u", counter);
