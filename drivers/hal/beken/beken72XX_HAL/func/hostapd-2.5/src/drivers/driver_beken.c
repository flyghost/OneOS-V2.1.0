...
int wpa_driver_associate(void *priv, struct wpa_driver_associate_params *params)
{
	...
	if (params->bcn_len)
	{
        os_memcpy((u8 *)param->u.assoc_req.bcn_buf, params->bcn_ie, params->bcn_len);
	}
...
int wpa_driver_authenticate(void *priv, struct wpa_driver_auth_params *params)
{
	...
	param->u.authen_req.mfp = params->mgmt_frame_protection;
	param->u.authen_req.proto = params->proto;   // FIXME:.......
	...
	// SAE Data
	param->u.authen_req.sae_data_len = params->sae_data_len;
	//param->u.authen_req.sae_data = param->u.authen_req.ie + params->ie_len;
	if (params->sae_data_len)
		os_memcpy(param->u.authen_req.sae_data, params->sae_data, params->sae_data_len);
	if (hostapd_ioctl(drv, param, blen))
		ret = -1;
