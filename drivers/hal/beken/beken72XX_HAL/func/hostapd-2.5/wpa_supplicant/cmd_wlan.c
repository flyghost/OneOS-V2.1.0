...
{
	os_printf("%02x:%02x:%02x:%02x:%02x:%02x  ssid=%-32.32s  "
	...
	*value++ = '\0';
	...
	if (os_strcmp(cmd, "ssid") == 0) {
		...
		if (psk_len >= 8 && psk_len <= psk_max_len) {
			...
			os_strlcpy(g_sta_param_ptr->key, value, sizeof(config.u.psk));
...
int cmd_wlan_sta_exec(char *cmd)
{
	...
	if (os_strncmp(cmd, "config ", 7) == 0) {
		...
		results.ap = os_malloc(size * sizeof(wlan_sta_ap_t));
		if (results.ap == NULL) {
			os_printf("no mem\n");
			ret = -1;
			goto out;
		...
		wlan_sta_ap_t *ap = os_malloc(sizeof(wlan_sta_ap_t));
		if (ap == NULL) {
			os_printf("no mem\n");
			ret = -1;
			goto out;
