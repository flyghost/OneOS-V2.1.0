...
static int sme_set_sae_group(struct wpa_supplicant *wpa_s)
{
	...
	if (!index_within_array(groups, wpa_s->sme.sae_group_index)) {
		bk_printf("%s %d: gindx %d\n", __func__, __LINE__, wpa_s->sme.sae_group_index);
		return -1;
	}
						 ...
						 const u8 *bssid)
{
	...
	if (sme_set_sae_group(wpa_s) < 0) {
		wpa_printf(MSG_DEBUG, "SAE: Failed to select group 2");
		return NULL;
	}
				    ...
				    int start)
{
	...
	params.proto = ssid->proto;		// TODO: Added by tiancj
