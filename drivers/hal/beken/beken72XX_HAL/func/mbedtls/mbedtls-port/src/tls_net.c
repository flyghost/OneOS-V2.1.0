...
//#include <sys/types.h>
#include "sockets.h"  ////#include <sys/socket.h>
...
int mbedtls_net_connect( mbedtls_net_context *ctx, const char *host, const char *port, int proto )
{
    ...
    for( cur = addr_list; cur != NULL; cur = cur->ai_next )
    {
		...
		#if (0)
		{
			...
			setsockopt(ctx->fd, SOL_SOCKET, SO_SNDTIMEO, (void *)&tv, sizeof(tv));
		}
		#endif
        if( connect( ctx->fd, cur->ai_addr, MSVC_INT_CAST cur->ai_addrlen ) == 0 )
        {
        #if (1)
			{
				...
				stv.tv_sec = 10;	///新大陆要求18S内，返回发送结果
				stv.tv_usec = 0;

				setsockopt(ctx->fd, SOL_SOCKET, SO_SNDTIMEO, (void *)&stv, sizeof(stv));
				bk_printf("ssl socket timeout:%ds-%dsec\r\n",stv.tv_sec,stv.tv_usec);
			}
		#endif
            ret = 0;
            break;
