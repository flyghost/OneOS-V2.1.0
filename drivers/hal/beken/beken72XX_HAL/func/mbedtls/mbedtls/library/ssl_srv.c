...
read_record_header:
    ...
    ret = mbedtls_ssl_confirm_content_len( ssl, 0, MBEDTLS_SSL_BUFFER_MIN);
    if( ret )
        return( ret );
#endif
    ...
    {
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
        if( msg_len > ssl->in_max_content_len )
#else
...
static int ssl_write_hello_verify_request( mbedtls_ssl_context *ssl )
{
    ...
    if( ( ret = ssl->conf->f_cookie_write( ssl->conf->p_cookie,
                                     &p, ssl->out_buf + ssl->out_max_content_len +
...
static int ssl_write_new_session_ticket( mbedtls_ssl_context *ssl )
{
	 ...
	 */
        ret = mbedtls_ssl_confirm_content_len( ssl, 0,
	        ...
	        (ssl->out_msg - ssl->out_buf) + sizeof( mbedtls_ssl_session ) );
        if( ret )
            return( ret );
    }
#endif
    ...
    if( ( ret = ssl->conf->f_ticket_write( ssl->conf->p_ticket,
                                ...
                                ssl->out_msg + 10,
                                ssl->out_msg + ssl->out_max_content_len,
                                &tlen, &lifetime ) ) != 0 )
#else
    if( ( ret = ssl->conf->f_ticket_write( ssl->conf->p_ticket,
                                ssl->session_negotiate,
