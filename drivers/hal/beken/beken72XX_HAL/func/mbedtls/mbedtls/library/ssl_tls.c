...
static int ssl_compress_buf( mbedtls_ssl_context *ssl )
{
     ...
     */
    do {
        ssl->transform_out->ctx_deflate.next_out = ssl->out_msg + ssl->out_msglen;
        ssl->transform_out->ctx_deflate.avail_out = ssl->out_max_content_len + MBEDTLS_SSL_BUFFER_OVERHEAD -
                                                  (ssl->out_msg - ssl->out_buf) - ssl->out_msglen;
	...
	ssl->out_msglen += avail_out - ssl->transform_out->ctx_deflate.avail_out;
	...
	deflatePending(&ssl->transform_out->ctx_deflate, &pending, &bits);
	    ...
	    ret = mbedtls_ssl_confirm_content_len( ssl, 0, ssl->out_max_content_len + 1024);
            if( ret )
	         return( ret );
	    continue;
        }
...
static int ssl_decompress_buf( mbedtls_ssl_context *ssl )
{
    ...
    do {
        ssl->transform_in->ctx_inflate.next_out = ssl->in_msg + ssl->in_msglen;
        ssl->transform_in->ctx_inflate.avail_out = ssl->in_max_content_len +
                                                   MBEDTLS_SSL_BUFFER_OVERHEAD -
                                                   (ssl->in_msg - ssl->in_buf) - ssl->in_msglen;
	...
	if( avail_out == ssl->transform_in->ctx_inflate.avail_out )
        ...
        ssl->in_msglen += avail_out - ssl->transform_in->ctx_inflate.avail_out;
	/* check if we still have more decompressed to write */
		...
		/* we have to increase the out buf and continue */
	    ret = mbedtls_ssl_confirm_content_len( ssl, 1, ssl->in_max_content_len + 1024);
            if( ret )
	         return( ret );
        }
...
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
    ret = mbedtls_ssl_confirm_content_len( ssl, 1, nb_want +
		              (size_t)( ssl->in_hdr - ssl->in_buf ) + 1024 );
    if( ret )
	    return( ret );

    if( nb_want > ssl->in_max_content_len + MBEDTLS_SSL_BUFFER_OVERHEAD -
...
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
            ret = mbedtls_ssl_confirm_content_len( ssl, 1, ssl->in_left + len );
            if( ret )
	            return( ret );
#endif
...
int mbedtls_ssl_write_record( mbedtls_ssl_context *ssl )
{
    ...
    {
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
        ret = mbedtls_ssl_confirm_content_len( ssl, 0, ssl->out_msglen +
...
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
        ret = mbedtls_ssl_confirm_content_len( ssl, 1, ssl->in_hslen);
        if( ret )
	        return( ret );

        if( ssl->in_hslen > ssl->in_max_content_len )
#else
...
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
        required = ssl->in_left + (size_t)( ssl->in_hdr - ssl->in_buf );

        ret = mbedtls_ssl_confirm_content_len( ssl, 1, required );
        if( ret )
	        return( ret );

        if( required > (size_t)(ssl->in_max_content_len + MBEDTLS_SSL_BUFFER_OVERHEAD) )
#else
...
static int ssl_parse_record_header( mbedtls_ssl_context *ssl )
{
    ...
    MBEDTLS_SSL_DEBUG_MSG( 3, ( "input record: msgtype = %d, "
                        "version = [%d:%d], msglen = %d, leftlen = %d",
                        ssl->in_msgtype,
                        major_ver, minor_ver, ssl->in_msglen, MBEDTLS_SSL_BUFFER_LEN
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "unknown record type %d", ssl->in_msgtype ) );
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "major version mismatch remote [%d:%d] vs local [%d:%d]", major_ver, minor_ver, ssl->major_ver, ssl->minor_ver ) );
        return( MBEDTLS_ERR_SSL_INVALID_RECORD );
    }

    if( minor_ver > ssl->conf->max_minor_ver )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "minor version mismatch remote [%d:%d] vs local [%d:%d]", major_ver, minor_ver, ssl->major_ver, ssl->minor_ver ) );
        return( MBEDTLS_ERR_SSL_INVALID_RECORD );
    }
    ...
    if( ssl->in_msglen > MBEDTLS_SSL_BUFFER_LEN
                         - (size_t)( ssl->in_msg - ssl->in_buf ) )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad message length, msglen = %d, leftlen = %d", ssl->in_msglen,
...
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
        if( ssl->in_msglen < 1 ||
            ssl->in_msglen > (size_t)(ssl->in_max_content_len + MBEDTLS_SSL_BUFFER_OVERHEAD) )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad message length %d vs %d ", ssl->in_msglen, ssl->in_max_content_len ) );
            return( MBEDTLS_ERR_SSL_INVALID_RECORD );
        }
#else
        if( ssl->in_msglen < 1 ||
...
int mbedtls_ssl_write_certificate( mbedtls_ssl_context *ssl )
{
    ...
    while( crt != NULL )
    {
        ...
        if( n > ssl->out_max_content_len + MBEDTLS_SSL_BUFFER_OVERHEAD - 3 - i )
        {
 ...
 *
 * If buf is already allocated, the content is copied into the
                                  ...
                                  size_t new_content_len )
{
    ...
    if (in_record)
    {
        ...
        msg = ssl->in_msg;
        offt = ssl->in_offt;
        max_content_len = ssl->in_max_content_len;
    }
    else
    {
        buf = ssl->out_buf;
        ...
        msg = ssl->out_msg;
        offt = NULL;
        max_content_len = ssl->out_max_content_len;
    }

    if( new_content_len >= MBEDTLS_SSL_MAX_CONTENT_LEN )
        new_content_len = MBEDTLS_SSL_MAX_CONTENT_LEN;
    else if (new_content_len <= MBEDTLS_SSL_BUFFER_MIN)
        new_content_len = MBEDTLS_SSL_BUFFER_MIN;
    else if (new_content_len <= 4096)
        new_content_len = 4096;
    else if (new_content_len <= 8192)
        new_content_len = 8192;
    else if (new_content_len <= 12288)
        new_content_len = 12288;
    else if( new_content_len <= MBEDTLS_SSL_MAX_CONTENT_LEN )
        new_content_len = MBEDTLS_SSL_MAX_CONTENT_LEN;

    min = new_len = new_content_len + MBEDTLS_SSL_BUFFER_OVERHEAD;

    if ( max_content_len &&
         max_content_len + (size_t)MBEDTLS_SSL_BUFFER_OVERHEAD < new_len )
            min = max_content_len + MBEDTLS_SSL_BUFFER_OVERHEAD;

    new_buf = mbedtls_calloc( 1, new_len );
    if( !new_buf ) {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "alloc of %lu failed", (long)new_len ) );
        return( MBEDTLS_ERR_SSL_ALLOC_FAILED );
    }

    if( buf ) {
	    memcpy(new_buf, buf, min);
	    /* if offt is in use, also fix it up */
	    if (offt)
		    offt = (offt - buf) + new_buf;
        mbedtls_zeroize(buf, max_content_len + MBEDTLS_SSL_BUFFER_OVERHEAD);
	    mbedtls_free(buf);
    }
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "realloc %s: %u -> (%u + %u)\n",
			   in_record ? "in" : "out",
			   (unsigned int)max_content_len,
			   (unsigned int)new_content_len, MBEDTLS_SSL_BUFFER_OVERHEAD ) );
    ...
    buf = new_buf;
    max_content_len = new_content_len;
 ...
 * MBEDTLS_SSL_BUFFER_OVERHEAD.  max_content_len is
                                     ...
                                     size_t content_len )
{
    ...
    if( buf && content_len <= (size_t)max_content_len )
    {
	    return( 0 );
	}
    ...
    return( mbedtls_ssl_alloc_record_buf(ssl, in_record, content_len ) );
}
...
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
    if( ssl->out_buf )
        memset( ssl->out_buf, 0, ssl->out_max_content_len +
                             MBEDTLS_SSL_BUFFER_OVERHEAD );
    if( partial == 0  && ssl->in_buf )
        memset( ssl->in_buf, 0, ssl->in_max_content_len
                                + MBEDTLS_SSL_BUFFER_OVERHEAD );
#else
...
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
    ret = mbedtls_ssl_confirm_content_len( ssl, 1, ssl->in_msglen );
    if( ret )
	    return( ret );
#endif
...
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
    ret = mbedtls_ssl_confirm_content_len( ssl, 0, len );
    if( ret )
        return( ret );
#endif
...
void mbedtls_ssl_free( mbedtls_ssl_context *ssl )
{
    ...
    {
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
        mbedtls_zeroize( ssl->out_buf, ssl->out_max_content_len
                                       + MBEDTLS_SSL_BUFFER_OVERHEAD );
#else
        mbedtls_zeroize( ssl->out_buf, MBEDTLS_SSL_BUFFER_LEN );
#endif
#ifdef MEM_LEAK_CHECK
        list_memleak_delete(ssl->out_buf);
#endif
    ...
    {
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
        mbedtls_zeroize( ssl->in_buf, ssl->in_max_content_len
                                      + MBEDTLS_SSL_BUFFER_OVERHEAD );
#else
        mbedtls_zeroize( ssl->in_buf, MBEDTLS_SSL_BUFFER_LEN );
#endif
#ifdef MEM_LEAK_CHECK
        list_memleak_delete(ssl->in_buf);
#endif
        mbedtls_free( ssl->in_buf );
    }
...
#ifdef MEM_LEAK_CHECK
        list_memleak_delete(ssl->compress_buf);
#endif
    ...
    if( ssl->transform )
    {
        mbedtls_ssl_transform_free( ssl->transform );
#ifdef MEM_LEAK_CHECK
        list_memleak_delete(ssl->transform);
#endif
        mbedtls_free( ssl->transform );
    }
    ...
    if( ssl->handshake )
    {
        ...
        mbedtls_ssl_session_free( ssl->session_negotiate );
#ifdef MEM_LEAK_CHECK
        list_memleak_delete(ssl->handshake);
        list_memleak_delete(ssl->transform_negotiate);
        list_memleak_delete(ssl->session_negotiate);
#endif
    ...
    if( ssl->session )
    {
        mbedtls_ssl_session_free( ssl->session );
#ifdef MEM_LEAK_CHECK
        list_memleak_delete(ssl->session);
#endif
        mbedtls_free( ssl->session );
    }
...
#ifdef MEM_LEAK_CHECK
        list_memleak_delete(ssl->hostname);
#endif
...
#if defined(MBEDTLS_SSL_DTLS_HELLO_VERIFY) && defined(MBEDTLS_SSL_SRV_C)
#ifdef MEM_LEAK_CHECK
    list_memleak_delete(ssl->cli_id);
#endif
    mbedtls_free( ssl->cli_id );
#endif
...
#ifdef MEM_LEAK_CHECK
        list_memleak_delete(conf->psk);
        list_memleak_delete(conf->psk_identity);
#endif
