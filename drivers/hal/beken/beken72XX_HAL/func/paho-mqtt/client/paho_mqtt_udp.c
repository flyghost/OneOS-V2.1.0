...
#error "must enable LWIP_NETIF_LOOPBACK for publish!"
#endif /* LWIP_NETIF_LOOPBACK */

#ifndef PKG_MQTT_THREAD_STACK_SIZE
#ifdef MQTT_USING_TLS
#define PKG_MQTT_THREAD_STACK_SIZE 6144
#else
#define PKG_MQTT_THREAD_STACK_SIZE 4096
#endif
#endif

#ifdef MQTT_USING_TLS
#if (PKG_MQTT_THREAD_STACK_SIZE < 6144)
 ...
 */
static int mqtt_resolve_uri(MQTT_CLIENT_T *c, struct addrinfo **res)
{
    int rc = 0;
    ...
    /* get host addr ok. */
    {
        ...
        host_addr_new = os_malloc(host_addr_len + 1);
...
_exit:
    ...
    {
        os_free(host_addr_new);
        host_addr_new = NULL;
    }
    return rc;    
}

static int net_connect(MQTT_CLIENT_T *c)
{
    int rc = -1;
...
static int net_disconnect(MQTT_CLIENT_T *c)
{
    if (c->sock >= 0)
    {
        closesocket(c->sock);
...
static int sendPacket(MQTT_CLIENT_T *c, int length)
{
    int rc;
    struct timeval tv;
...
static int net_read(MQTT_CLIENT_T *c, unsigned char *buf,  int len, int timeout)
{
    int bytes = 0;
    int rc;
...
static int decodePacket(MQTT_CLIENT_T *c, int *value, int timeout)
{
    unsigned char i;
    int multiplier = 1;
    int len = 0;
...
static int MQTTPacket_readPacket(MQTT_CLIENT_T *c)
{
    int rc = PAHO_FAILURE;
    MQTTHeader header = {0};
    int len = 0;
...
static int getNextPacketId(MQTT_CLIENT_T *c)
{
    return c->next_packetid = (c->next_packetid == MAX_PACKET_ID) ? 1 : c->next_packetid + 1;
}

static int mqtt_connect(MQTT_CLIENT_T *c)
{
    int rc = -1, len;
    MQTTPacket_connectData *options = &c->condata;

    if (c->is_connected) /* don't send connect packet again if we are already connected */
        goto _exit;
...
static int mqtt_disconnect(MQTT_CLIENT_T *c)
{
    int rc = PAHO_FAILURE;
    int len = 0;
 ...
 */
static int mqtt_subscribe(MQTT_CLIENT_T *c, const char *topicFilter, enum QoS qos)
{
    int rc = PAHO_FAILURE;
    int len = 0;
    int qos_sub = qos;
    ...
    if (rc < 0)
    {
        debug_printf("MQTTPacket_readPacket mqtt_connect fail\n");
        goto _exit;
    }
...
static int deliverMessage(MQTT_CLIENT_T *c, MQTTString *topicName, MQTTMessage *message)
{
    int i;
    int rc = PAHO_FAILURE;
...
static int mqtt_cycle(MQTT_CLIENT_T *c)
{
...
_mqtt_start:
    ...
    for (i = 0; i < MAX_MESSAGE_HANDLERS; i++)
    {
        ...
        rc = mqtt_subscribe(c, topic, QOS2);
    ...
    while (1)
    {
        ...
        tick_now = fclk_get_tick();
        if (((tick_now - c->tick_ping) / TICK_PER_SECOND) > (c->keepAliveInterval - 5))
        {
            timeout.tv_sec = 1;
        ...
        {
            timeout.tv_sec = c->keepAliveInterval - 10 - (tick_now - c->tick_ping) / TICK_PER_SECOND;
...
int paho_mqtt_start(MQTT_CLIENT_T *client)
{
    ...
    int priority = configMAX_PRIORITIES / 3;	
...
int mqtt_publish_with_topic(MQTT_CLIENT_T *c, const char *topicName, MQTTMessage *message)
{
    ...
    data = os_malloc(msg_len);
    if (!data)
        goto exit;

    os_memcpy(data, message, sizeof(MQTTMessage));
    os_memcpy(data + sizeof(MQTTMessage), message->payload, message->payloadlen);
    os_strcpy(data + sizeof(MQTTMessage) + message->payloadlen, topicName);
