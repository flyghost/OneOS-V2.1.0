			...
			const u8 *req_ies, size_t req_ies_len, int reassoc)
{
	...
	const u8 *ie;
	size_t ielen;
#if defined(CONFIG_IEEE80211R_AP) || defined(CONFIG_IEEE80211W_AP) || defined(CONFIG_FILS) || defined(CONFIG_OWE)
	u8 buf[sizeof(struct ieee80211_mgmt) + 1024];
	u8 *p = buf;
#endif /* CONFIG_IEEE80211R_AP || CONFIG_IEEE80211W_AP || CONFIG_FILS || CONFIG_OWE */
...
{
	struct hostapd_data *hapd = ctx;
#if 0//ndef CONFIG_NO_STDOUT_DEBUG
	int level = MSG_DEBUG;
