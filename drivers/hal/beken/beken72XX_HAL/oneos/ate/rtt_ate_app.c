...
void ate_gpio_init(void)
{
    ...
    else if (os_strcmp(name, "uart2") == 0)
    {
        uart_index = 2;
    ...
    {
        uart_index = 0;
        os_kprintf("[ate] err!! not find uart\n");
        return;
    }

    os_task_delay(os_tick_from_ms(2));
...
uint32_t get_ate_mode_state(void)
{
    //ATE_PRT("ateflag:%d\r\n", ate_mode_state);
#if 0
    os_kprintf("test... the state is 1\n");
    return 1;
#else
