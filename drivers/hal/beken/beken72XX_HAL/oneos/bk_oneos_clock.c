 ...
 *
 * @file        bk_oneos_clock.c
 *
 * @brief       Adpt oneos system tick clock by beken driver  functions.
 *
 *
 * @revision
 * Date         Author          Notes
...
static void fclk_hdl(UINT8 param)
{
    ...
    os_tick_increase(); /* oneos tick count */
	GLOBAL_INT_RESTORE();
	...
	if (--second_countdown == 0) 
	{
        ...
        #if defined(OS_USING_ALARM)
        os_alarm_update(NULL, 0);
        #endif
	}
...
{
    UINT64 fclk;
    fclk = (UINT64)os_tick_get();
    return fclk;
}

UINT32 fclk_get_second(void)
{
    return (os_tick_get()/FCLK_SECOND);
}
...
UINT32 bk_cal_init(UINT32 setting)
{
    ...
    if(1 == setting)
    {
        ...
        mcu_ps_machw_init();
#endif
        os_printf("decset:%d %d %d %d\r\n",use_cal_net,current_clock,fclk_get_second(),os_tick_get());
    }
    else
    {
#if CFG_USE_MCU_PS
...
UINT32 fclk_cal_endvalue(UINT32 mode)
{
    ...
    {
        /*26m clock*/
    	value = CFG_XTAL_FREQUENCE / OS_TICK_PER_SECOND;
    }
