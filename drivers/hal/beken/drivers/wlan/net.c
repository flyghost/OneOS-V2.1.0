...
#define NET_DEBUG   0 
#if NET_DEBUG
#define NET_DBG(...)     os_kprintf("[NET]"),os_kprintf(__VA_ARGS__)
#else
#define NET_DBG(...)
#endif
    ...
    if (n->flags & NETIF_FLAG_UP)
    {
        ...
        if (dhcp != NULL)
        {
            if (dhcp->state == DHCP_STATE_BOUND)
            {
                ...
                ip_up_tick = os_tick_get();
                os_kprintf("[ip_up]:start tick =  %d, ip_up tick = %d, total = %d \n", start_connect_tick, ip_up_tick, ip_up_tick - start_connect_tick);
...
void uap_ip_down(void)
{
    ...
    if (uap_ip_start_flag)
    {
        ...
        os_kprintf("please use oneos dhcp stop server\n");
        // dhcp_server_stop();
    }
}
...
int net_configure_address(struct ipv4_config *addr, void *intrfc_handle)
{
    ...
    case ADDR_TYPE_STATIC:
        ...
        netifapi_netif_set_up(if_handle->netif);
        if (!strncmp(if_handle->netif->name, OS_WLAN_DEVICE_STA_NAME, sizeof(if_handle->netif->name)))
        {
    ...
    else
    {
        ...
        os_kprintf("please use dhcp start server\n");
        //dhcpd_start(OS_WLAN_DEVICE_AP_NAME); 
        // dhcp_server_start(net_get_uap_handle());
        dhcp_server_start(if_handle);
        up_iface = 0;
...
void net_wlan_remove_netif(void *mac)
{
    ...
    if (!strncmp(netif->name, OS_WLAN_DEVICE_STA_NAME, sizeof(netif->name)))
    {
