...
输出: 0：BootSwap禁止
      1：BootSwap使能
********************************/
FlagStatus FLASH_OPTBR_BTSEN_Chk(void)
{
	if((FLASH->OPTBR & FLASH_OPTBR_BTSEN_Msk)==(2U<<FLASH_OPTBR_BTSEN_Pos))
  {
    return SET;
  }
  else
...
{
	if((FLASH->OPTBR&FLASH_OPTBR_ACLOCKEN_Msk)==(2U<<FLASH_OPTBR_ACLOCKEN_Pos))
  {
    return SET;
  }
  else
...
功能: 获取FLASH擦写KEY输入状态
输入：无
输出: FLASH_FLSIF_KEYSTA_WP：FLASH写保护状态，未输入KEY
      FLASH_FLSIF_KEYSTA_ALLE_UNLOCK：全擦解锁状态
      FLASH_FLSIF_KEYSTA_SE_UNLOCK：扇区擦除解锁状态
      FLASH_FLSIF_KEYSTA_P_UNLOCK：编程解锁状态
     FLASH_FLSIF_KEYSTA_ERR_LOCK：KEY错误锁定状态，需要复位才能解锁   
********************************/
uint32_t FLASH_FLSIF_KEYSTA_Get(void)
{
	return  (FLASH->FLSIF & (FLASH_FLSIF_KEYSTA_Msk));	
}
