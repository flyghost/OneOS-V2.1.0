...
uint8_t HSPI_Recv_Byte(void)//HSPI接收一字节
{
	return HSPI_SPIRXBUF_Read();
}
void HSPI_Send_Byte(uint8_t data)//HSPI发送一字节
{
	HSPI_SPITXBUF_Write(data);
}

uint8_t HSPI_RW_Byte(uint8_t data)//SPI发送并接收一字节
{
  uint8_t rx_data;
	uint32_t Temp32;
	uint32_t i;

	Temp32 = HSPI_SPICR1_BAUD_Get();
	Temp32 = Temp32>>HSPI_SPICR1_BAUD_Pos;
	Temp32 = 0x1U<<Temp32;
	Temp32 = Temp32*8;
