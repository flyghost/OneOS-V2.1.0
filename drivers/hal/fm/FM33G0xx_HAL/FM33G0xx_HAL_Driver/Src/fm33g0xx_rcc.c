...
/********************************
无晶体模式下LSCLK时钟源选择
功能:无晶体模式下LSCLK时钟源选择，（仅在无晶体配置下有效)
输入: RCC_SYSCLKSEL_LSCKSEL_RCLP： RCLP
      RCC_SYSCLKSEL_LSCKSEL_RCLF： RCLF
输出：无
********************************/
void RCC_SYSCLKSEL_LSCKSEL_Set(uint32_t SetValue)
{
	uint32_t tmpreg;
	tmpreg = RCC->SYSCLKSEL;
	tmpreg &= ~(RCC_SYSCLKSEL_LSCKSEL_Msk);
	tmpreg |= (SetValue & RCC_SYSCLKSEL_LSCKSEL_Msk);
	RCC->SYSCLKSEL = tmpreg;
}

/********************************
读取无晶体模式下LSCLK时钟源的选择
功能:读取无晶体模式下LSCLK时钟源的选择
输入: 无
输出：RCC_SYSCLKSEL_LSCKSEL_RCLP： RCLP
      RCC_SYSCLKSEL_LSCKSEL_RCLF： RCLF
********************************/
uint32_t RCC_SYSCLKSEL_LSCKSEL_Get(void)
{
	return (RCC->SYSCLKSEL & RCC_SYSCLKSEL_LSCKSEL_Msk);
}
...
RCC_SYSCLKSEL_EXTICKSEL_LSCLK：外部引脚中断使用LSCLK采样
RCC_SYSCLKSEL_EXTICKSEL_AHBCLK：外部引脚中断使用AHBCLK采样
********************************/
...
输出：无
********************************/
void RCC_RCLFCON_RCLFENB_Setable(FunState NewState)
{
	if (NewState == ENABLE)
	{
		RCC->RCLFCON &= ~(RCC_RCLFCON_RCLFENB_Msk);
	}
	else
	{
		RCC->RCLFCON |= (RCC_RCLFCON_RCLFENB_Msk);
...
输出：0: 使能
      1：禁止
********************************/
FunState RCC_RCLFCON_RCLFENB_Getable(void)
{
	if (RCC->RCLFCON & (RCC_RCLFCON_RCLFENB_Msk))
	{
		return DISABLE;
	}
	else
...
输出：无
********************************/
void RCC_RCLFTRIM_Write(uint32_t SetValue)
{
	RCC->RCLFTRIM = (SetValue & RCC_RCLFTRIM_RCLFTRIM_Msk);
}
...
输出：RCLF TRIM值
********************************/
uint32_t RCC_RCLFTRIM_Read(void)
{
	return (RCC->RCLFTRIM & RCC_RCLFTRIM_RCLFTRIM_Msk);
}
