...
时基计数器数值调整方向函数
功能:时基计数器数值调整的方向调整
输入:	RTC_ADSIGN_ADSIGN_PLUS：表示增加计数初值
	RTC_ADSIGN_ADSIGN_MINUS：表示减少计数初值
...
void RTC_Deinit(void)
{
  ...
  RTC->AUTOCAL = 0x00000000;
//  RTC->ADOFFSET = 0x00000000;
...
********************************/
void RTC_AUTOCAL_Setable(FunState NewState)
{
	if (NewState == ENABLE)
	{
		RTC->AUTOCAL |= (RTC_AUTOCAL_ACALEN_Msk);
	}
	else
	{
		RTC->AUTOCAL &= ~(RTC_AUTOCAL_ACALEN_Msk);
...
********************************/
FunState RTC_AUTOCAL_Getable(void)
{
	if (RTC->AUTOCAL & (RTC_AUTOCAL_ACALEN_Msk))
	{
		return ENABLE;
	}
	else
...
********************************/
void RTC_ADOFFSET_Write(uint32_t SetValue)
{
	RTC->ADOFFSET = (SetValue&RTC_ADOFFSET_ADOFFSET_Msk);
}
...
********************************/
uint32_t RTC_ADOFFSET_Read(void)
{
	return ((RTC->ADOFFSET)&RTC_ADOFFSET_ADOFFSET_Msk);
}
...
********************************/
void RTC_CALSTEP_SET(uint32_t SetValue)
{
  if(SetValue==RTC_CALSTEP_CALSTEP0P119)
  {
    RTC->CALSTEP=RTC_CALSTEP_CALSTEP0P119;
  }
  else
  {
    RTC->CALSTEP=RTC_CALSTEP_CALSTEP0P238;
  }
}
...
输出:0： 0.238
     1： 0.119
********************************/
FunState RTC_CALSTEP_Get(void)
{
	if (RTC->CALSTEP & (RTC_CALSTEP_CALSTEP_Msk))
	{
		return ENABLE;
	}
	else
...
读取RTC自动温补状态函数
功能:读取RTC自动温补BUSY位
输出:0： RTC未占用ADC进行自动温补
     1： RTC占用ADC进行自动温补
********************************/
FunState RTC_CALBUSY_Get(void)
{
	if((RTC->CALBUSY) & RTC_CALBUSY_AUCALBUSY_Msk)
	{
		return ENABLE;
	}
	else
...
********************************/
uint32_t RTC_ADJCNT_Read(void)
{
	return (RTC->ADJCNT & RTC_ADJCNT_ADJCNT_Msk);
}

/********************************
读RTC自动温度补偿调教值函数
功能:读取RTC当前起作用的自动温补补偿值
输入：无
输出：当前的自动温度补偿调校值		
********************************/
uint32_t RTC_ACALADJ_Read(void)
{
	return (RTC->ACALADJ & RTC_ACALADJ_AUADJUST_Msk);
}
...
输出：无		
********************************/
void RTC_BKREG_Write(uint32_t* prog_data)
{
  uint32_t i;
  for(i=0;i<128;i++)
  {
    RTC->RTCBKREG[i]=prog_data[i];
  }
...
void RTC_Trim_Proc(float err_ppm)
{
  ...
  if(RTC_CALSTEP_Get()) 
  {
    Rtcadj = err_ppm*8.388608;//0.119ppm
  }
  else
  {
    Rtcadj = err_ppm*4.194304;//0.238ppm
  }
