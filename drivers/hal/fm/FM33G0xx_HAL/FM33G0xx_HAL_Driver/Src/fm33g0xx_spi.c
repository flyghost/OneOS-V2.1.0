...
输出：SPI接收字节
********************************/
uint8_t SPI_Recv_Byte(SPIx_Type* SPIx)//SPI接收一字节
{
...
输入：SPI要发送的字节
********************************/
void SPI_Send_Byte(SPIx_Type* SPIx, uint8_t data)//SPI发送一字节
{
...
********************************/
uint8_t SPI_RW_Byte(SPIx_Type* SPIx, uint8_t data)//SPI发送并接收一字节
{
  unsigned char rx_data;
	uint32_t Temp32;
	uint32_t i;
	
	Temp32 = SPIx_SPICR1_BAUD_Get(SPIx);
	Temp32 = Temp32>>SPIx_SPICR1_BAUD_Pos;
