...
int fm33_flash_erase(os_uint32_t addr, os_size_t size)
{
    ...
    os_uint16_t pages_in_block = FM_FLASH_BLOCK_SIZE / FM_FLASH_PAGE_SIZE;
    os_uint16_t remain_in_block = FM_FLASH_BLOCK_SIZE % FM_FLASH_PAGE_SIZE;
    ...
    if (pages_in_block == 0 || remain_in_block > 0)
    {
        os_kprintf("ERROR: blocksize must be times of pagesize(%d/%d)\n", FM_FLASH_BLOCK_SIZE, FM_FLASH_PAGE_SIZE);
        return OS_EINVAL;
    }
