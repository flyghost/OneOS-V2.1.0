...
{
    if (SET == dma_flag_get(DMA0, DMA_CH4, DMA_FLAG_HTF))
    {   
        dma_interrupt_flag_clear(DMA0, DMA_CH4, DMA_FLAG_HTF);   
    }
...
static os_err_t gd32_sdma_dma_start(soft_dma_t *dma, void *buff, os_uint32_t size)
{
    ...
    /* DMA init */
    dma_init((uint32_t)usart->info->dma_periph, usart->info->dma_channel, &usart->dma_init_struct);
 
    /* configure DMA mode */
    dma_circulation_enable((uint32_t)usart->info->dma_periph, usart->info->dma_channel);
   
    dma_memory_to_memory_disable((uint32_t)usart->info->dma_periph, usart->info->dma_channel);
    ...
    dma_interrupt_enable((uint32_t)usart->info->dma_periph, usart->info->dma_channel, DMA_INT_HTF);

    /* enable DMA transfer complete interrupt */
    dma_interrupt_enable((uint32_t)usart->info->dma_periph, usart->info->dma_channel,DMA_INT_FTF);
