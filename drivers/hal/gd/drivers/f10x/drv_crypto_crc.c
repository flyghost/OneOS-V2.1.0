...
static os_uint32_t stm32_crc_update(struct hwcrypto_crc *ctx, const os_uint8_t *in, os_size_t length)
{
    ...
    os_mutex_lock(&stm32_hw_dev->mutex, OS_IPC_WAITING_FOREVER);
...
static int stm32_crc_probe(const os_driver_info_t *drv, const os_device_info_t *dev)
{
    ...
    os_mutex_init(&st_hwcrypto->mutex, OS_HWCRYPTO_DEFAULT_NAME, OS_IPC_FLAG_FIFO, OS_FALSE);
    return 0;
}
...
OS_DRIVER_DEFINE(stm32_crc_driver, "3");
