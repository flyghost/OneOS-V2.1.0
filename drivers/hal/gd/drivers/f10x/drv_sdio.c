...
#define OSHW_SDIO_LOCK(_sdio)   os_mutex_recursive_lock(&_sdio->mutex, OS_IPC_WAITING_FOREVER)
...
static void stm32_sdio_wait_completed(struct stm32_sdio *st_sdio)
{
    ...
    if (os_event_recv(&st_sdio->event,
                      ...
                      &status) != OS_EOK)
    {
        LOG_EXT_E("wait completed timeout");
        cmd->err = OS_ETIMEOUT;
        return;
    }
    ...
    if (status & HW_SDIO_ERRORS)
    {
        ...
        if (cmd->err == OS_EOK)
        {
            LOG_EXT_D("sta:0x%08X [%08X %08X %08X %08X]",
                      status,
                      cmd->resp[0],
                      cmd->resp[1],
                      cmd->resp[2],
        ...
        {
            LOG_EXT_D("err:0x%08x, %s%s%s%s%s%s%s cmd:%d arg:0x%08x rw:%c len:%d blksize:%d",
                      status,
    ...
    {
        cmd->err = OS_EOK;
        LOG_EXT_D("sta:0x%08X [%08X %08X %08X %08X]", status, cmd->resp[0], cmd->resp[1], cmd->resp[2], cmd->resp[3]);
    }
}
...
static os_uint32_t stm32_sdio_clock(SD_HandleTypeDef *hsd)
{
    ...
    LOG_EXT_E("invalid hsd instance.");
    return 0;
}
...
static void stm32_sdio_transfer_by_dma(struct stm32_sdio *st_sdio, struct sdio_pkg *pkg)
{
    ...
    {
        LOG_EXT_E("stm32_sdio_transfer_by_dma invalid args");
        return;
    }
...
static void stm32_sdio_send_command(struct stm32_sdio *st_sdio, struct sdio_pkg *pkg)
{
    ...
    LOG_EXT_D("CMD:%d ARG:0x%08x RES:%s%s%s%s%s%s%s%s%s rw:%c len:%d blksize:%d",
              cmd->cmd_code,
              cmd->arg,
...
static void stm32_sdio_iocfg(struct os_mmcsd_host *host, struct os_mmcsd_io_cfg *io_cfg)
{
    ...
    if (clk_src < 400 * 1000)
    {
        LOG_EXT_E("The clock rate is too low! rata:%d", clk_src);
        return;
    }
    ...
    if (clk > clk_src)
    {
        LOG_EXT_W("Setting rate is greater than clock source rate.");
        clk = clk_src;
    }

    LOG_EXT_D("clk:%d width:%s%s%s power:%s%s%s",
              clk,
    ...
    case MMCSD_POWER_ON:
        ...
        break;
    default:
        LOG_EXT_W("unknown power_mode %d", io_cfg->power_mode);
        break;
    }
...
void stm32_sdio_enable_irq(struct os_mmcsd_host *host, os_int32_t enable)
{
    ...
    if (enable)
    {
        LOG_EXT_D("enable sdio irq");
        st_sdio->hsd->Instance->MASK |= HW_SDIO_IT_SDIOIT;
    }
    else
    {
        LOG_EXT_D("disable sdio irq");
        st_sdio->hsd->Instance->MASK &= ~HW_SDIO_IT_SDIOIT;
    }
}
...
{
    LOG_EXT_D("try to detect device");
    return 0x01;
}
...
static struct os_mmcsd_host *stm32_sdio_host_create(struct stm32_sdio *st_sdio)
{
    ...
    if (host == OS_NULL)
    {
        LOG_EXT_E("L:%d F:%s mmcsd alloc host fail");
        return OS_NULL;
    }

    os_event_init(&st_sdio->event, "sdio", OS_IPC_FLAG_FIFO);
    os_mutex_init(&st_sdio->mutex, "sdio", OS_IPC_FLAG_FIFO, OS_TRUE);
...
static int stm32_sdio_probe(const os_driver_info_t *drv, const os_device_info_t *dev)
{
    ...
    else if (hsd->Instance == SDMMC2)
    {
        ...
        LOG_EXT_I("sdmmc host2.");
        return OS_EOK;
    }
#endif

    LOG_EXT_E("invalid sdmmc host.");
    return OS_ERROR;
}
...
OS_DRIVER_DEFINE(stm32_sdio_driver, "3");
