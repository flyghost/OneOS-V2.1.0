...
{
    if (SET == dma_flag_get(DMA_CH2, DMA_FLAG_HTF))
    {   
        dma_interrupt_flag_clear(DMA_CH2, DMA_FLAG_HTF);   
    }
    
    if (SET == dma_flag_get(DMA_CH2, DMA_FLAG_FTF))
    {
        dma_interrupt_flag_clear(DMA_CH2, DMA_FLAG_FTF);   
    }
...
static os_uint32_t gd32_sdma_dma_get_index(soft_dma_t *dma)
{
    ...
    return usart->sdma_hard_size - dma_transfer_number_get(usart->info->dma_channel);
}
...
static os_err_t gd32_sdma_dma_start(soft_dma_t *dma, void *buff, os_uint32_t size)
{
    ...
    /* DMA deinit */
    dma_deinit(usart->info->dma_channel);
    dma_struct_para_init(&usart->dma_init_struct);
    
    /* set dma init parameters */
    usart->dma_init_struct.direction = DMA_PERIPHERAL_TO_MEMORY;
    usart->dma_init_struct.periph_addr = (uint32_t)&USART_RDATA(usart->info->husart);
    usart->dma_init_struct.periph_width = DMA_PERIPHERAL_WIDTH_8BIT;
    usart->dma_init_struct.periph_inc = DMA_PERIPH_INCREASE_DISABLE;
    usart->dma_init_struct.memory_addr = (uint32_t)buff;
    usart->dma_init_struct.memory_width = DMA_MEMORY_WIDTH_8BIT;
    ...
    /* DMA init */
    dma_init(usart->info->dma_channel, &usart->dma_init_struct);
 
    /* configure DMA mode */  
    dma_circulation_enable(usart->info->dma_channel);
  
    dma_memory_to_memory_disable(usart->info->dma_channel);
    ...
    /* enable DMA channel4 */
    dma_channel_enable(usart->info->dma_channel); 
    ...
    dma_interrupt_enable(usart->info->dma_channel, DMA_INT_HTF);

    /* enable DMA transfer complete interrupt */
    dma_interrupt_enable(usart->info->dma_channel,DMA_INT_FTF);
...
static os_uint32_t gd32_sdma_dma_stop(soft_dma_t *dma)
{
    ...
    /* disable DMA */
    dma_channel_disable(usart->info->dma_channel);;
