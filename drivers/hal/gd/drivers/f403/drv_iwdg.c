...
static void stm32_iwdg_refresh(struct stm32_iwdg *iwdg)
{
    ...
    {
        LOG_EXT_E("watch dog keepalive fail.");
    }
}
...
static int stm32_iwdt_probe(const os_driver_info_t *drv, const os_device_info_t *dev)
{
    ...
    if (os_hw_watchdog_register(&iwdg->wdg, dev->name, OS_DEVICE_FLAG_DEACTIVATE, OS_NULL) != OS_EOK)
    {
        LOG_EXT_E("iwdt device register failed.");
        return OS_ERROR;
    }
    LOG_EXT_D("iwdt device register success.");
    return OS_EOK;
}
...
OS_DRIVER_DEFINE(stm32_iwdt_driver, "1");
