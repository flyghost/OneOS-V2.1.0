...
os_uint16_t gpio_port_base[GPIO_PORT_MAX];
...
{                                                                                                                         \
    ...
    gpio_port_base[GPIO_INDEX_##gpio] = GPIO_PORT_##gpio;                                                                 \
}
...
os_uint16_t  get_pin_base(os_base_t pin)
{
...
static void hc32_pin_write(struct os_device *dev, os_base_t pin, os_base_t value)
{
    ...
    if(value != 0)
    {
        GPIO_SetPins(index->gpio_port, (1 << PIN_OFFSET(pin)));
    }
    else
    {
        GPIO_ResetPins(index->gpio_port, (1 << PIN_OFFSET(pin)));
    }
}
...
static int hc32_pin_read(struct os_device *dev, os_base_t pin)
{
    ...
    value = GPIO_ReadInputPins(index->gpio_port, (1 << PIN_OFFSET(pin)));
...
static void hc32_pin_mode(struct os_device *dev, os_base_t pin, os_base_t mode)
{
    ...
    switch(mode) {
        case PIN_MODE_OUTPUT:
            /* output setting */
            GPIO_Cfg.u16PinDir = PIN_DIR_OUT;
            GPIO_Cfg.u16PullUp = PIN_PU_ON;
            break;
        case PIN_MODE_INPUT:
            /* input setting: not pull. */
            GPIO_Cfg.u16PinDir = PIN_DIR_IN;
            break;
        ...
        case PIN_MODE_INPUT_PULLDOWN:
            ...
            GPIO_Cfg.u16PinDir = PIN_DIR_IN;
            GPIO_Cfg.u16PullUp = PIN_PU_OFF;
            break;
        case PIN_MODE_OUTPUT_OD:
            /* output setting: od. */
            GPIO_Cfg.u16PinDir = PIN_DIR_OUT;
            GPIO_Cfg.u16Latch = PIN_LATCH_ON;
            GPIO_Cfg.u16PinOType = PIN_OTYPE_NMOS;
            GPIO_Cfg.u16PinDrv = PIN_DRV_HIGH;
    ...
    level = os_irq_lock();;
    index->pin_pulls[__PIN_INDEX(pin)].pull_down = GPIO_Cfg.u16PullUp;
    os_irq_unlock(level);
#endif

    GPIO_Init(index->gpio_port, (1 << PIN_OFFSET(pin)), &GPIO_Cfg);
}

static os_err_t hc32_pin_attach_irq(struct os_device *device, os_int32_t pin,
    ...
    if (enabled == PIN_IRQ_ENABLE)
    {
        ...
        os_list_for_each_entry(irq, &hc_gpio_irq_list, struct pin_irq_param, list)
        {
            if(irq->pin == pin)
            {
                ...
                stcExintInit.u32ExIntCh = EXTI_CH(pin);
                EXINT_Init(&stcExintInit);

                stcGpioInit.u16ExInt = PIN_EXINT_ON;
                stcGpioInit.u16PullUp = index->pin_pulls[__PIN_INDEX(pin)].pull_up;
                GPIO_Init(index->gpio_port, 1 << PIN_OFFSET(pin), &stcGpioInit);

                stcIrqSignConfig.enIntSrc = INT_SRC(pin);
                stcIrqSignConfig.enIRQn   = hc_pin_irq_map[__PIN_INDEX(pin)].irqno;
                stcIrqSignConfig.pfnCallback = hc_pin_irq_map[__PIN_INDEX(pin)].pfnCallback;
    ...
    else if (enabled == PIN_IRQ_DISABLE)
    {
        ...
        os_list_for_each_entry(irq, &hc_gpio_irq_list, struct pin_irq_param, list)
        {
            if(irq->pin == pin)
            {
                ...
                stcGpioInit.u16ExInt = PIN_EXINT_OFF;
                stcGpioInit.u16PullUp = index->pin_pulls[__PIN_INDEX(pin)].pull_up;
                GPIO_Init(index->gpio_port, 1 << PIN_OFFSET(pin), &stcGpioInit);

                INTC_IrqSignOut(hc_pin_irq_map[__PIN_INDEX(pin)].irqno);
...
{
    if (Set == EXINT_GetExIntSrc(EXTI_CH(index)))
    {
        ...
        EXINT_ClrExIntSrc(EXTI_CH(index));
    }
