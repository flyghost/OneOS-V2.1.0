...
/* ADC TSPD mask */
#define CR3_TSE_Set             ((uint32_t)0x00400000)
#define CR3_TSE_Reset           ((uint32_t)0xFFBFFFFF)

#define CR3_VREFE_Set             ((uint32_t)0x00800000)
#define CR3_VREFE_Reset           ((uint32_t)0xFF7FFFFF)

#define CR3_VBATE_Set             ((uint32_t)0x01000000)
#define CR3_VBATE_Reset           ((uint32_t)0xFEFFFFFF)

#define CR3_RESO_Reset             ((uint32_t)0xFFFFFFF0)
/**
  * @}
  */
...
void ADC_VBATCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    ADC1->CR3 |= CR2_TSVREFE_Set;
  }
  else
  {
    /* Disable the VBAT channel*/
    ADC1->CR3 &= CR2_TSVREFE_Reset;
  }
}

/**
  * @brief  Set RESO[1:0] bits according to ADC_Resolution value.
	* @param  ADCx: where x can be 1, 2 or 3 to select the ADC peripheral.
	* @param  ADC_Resolution: ADC Conversion Resolution (12bit..6bit)
  *   This parameter can be: ADC_Resolution_12b,ADC_Resolution_10b,ADC_Resolution_8b,ADC_Resolution_6b.
  * @retval None
  */
void ADC_Resolution_Set(ADC_TypeDef* ADCx,uint32_t ADC_Resolution)
{
	uint32_t tmpreg = 0;
  /* Check the parameters */
  assert_param(IS_ADC_ALL_PERIPH(ADCx));
	assert_param(IS_ADC_RESOLUTION(ADC_InitStruct->ADC_Resolution));
	/* Get the ADCx CR3 value */
  tmpreg = ADCx->CR3;
	tmpreg &= CR3_RESO_Reset;
  ADCx->CR3 |= ADC_Resolution;
}
/**
...
/**
  ...
  * @retval None	
  *note：
      1. this function only used on regular ADC mode
