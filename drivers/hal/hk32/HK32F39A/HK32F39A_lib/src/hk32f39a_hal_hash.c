...
/** @defgroup HASH_Number_Of_CSR_Registers HASH Number of Context Swap Registers
  * @{
  */
#define HASH_NUMBER_OF_CSR_REGISTERS              37U     /*!< Number of Context Swap Registers */
...
/**
  ...
  *         is saved.
  * @note   The IMR, STR, CR  registers are saved
...
static void HASH_GetDigest(uint8_t *pMsgDigest, uint8_t Size)
{
  ...
  if(Size == 32)
  {
  	    *(uint32_t*)(msgdigest) = __REV(HASH->HR0);
	    msgdigest+=4U;
	    *(uint32_t*)(msgdigest) = __REV(HASH->HR1);
	    msgdigest+=4U;
	    *(uint32_t*)(msgdigest) = __REV(HASH->HR2);
	    msgdigest+=4U;
	    *(uint32_t*)(msgdigest) = __REV(HASH->HR3);
	    msgdigest+=4U;
	    *(uint32_t*)(msgdigest) = __REV(HASH->HR4);
		msgdigest+=4U;
	    *(uint32_t*)(msgdigest) = __REV(HASH->HR5);
		msgdigest+=4U;
	    *(uint32_t*)(msgdigest) = __REV(HASH->HR6);
		msgdigest+=4U;
	    *(uint32_t*)(msgdigest) = __REV(HASH->HR7);
  }
}
...
if((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
  {  
    ...
    if(hhash->Phase == HAL_HASH_PHASE_READY)
    {
      ...
      /* Select the HASH algorithm,, reset the HASH processor core */
      MODIFY_REG(HASH->CR, HASH_CR_ALGO|HASH_CR_MODE|HASH_CR_INIT, Algorithm | HASH_CR_INIT);
...
if((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
  { 
    ...
    if (hhash->State == HAL_HASH_STATE_READY)
    {
      ...
      if(hhash->Phase == HAL_HASH_PHASE_READY)
      {
        /* Select the HASH algorithm, clear  mode  reset the HASH processor core */
        MODIFY_REG(HASH->CR, HASH_CR_ALGO|HASH_CR_MODE|HASH_CR_INIT, Algorithm | HASH_CR_INIT);
