...
HAL_StatusTypeDef  HAL_NAND_Init(NAND_HandleTypeDef *hnand, FSMC_NAND_PCC_TimingTypeDef *ComSpace_Timing, FSMC_NAND_PCC_TimingTypeDef *AttSpace_Timing)
{
  ...
  if(hnand->Init.NandBank == FSMC_NAND_BANK3)
	  __FSMC_NAND_ENABLE(hnand->Instance, FSMC_NAND_BANK2);
...
#if NAND_USE_INT2_ENABLE
  __FSMC_NAND_ENABLE_IT(hnand->Instance, FSMC_NAND_BANK2,FSMC_IT_IR);
#endif
  /* Page(s) read loop */  
...
//	while(GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_6) == 0);
    ...
    {
      *(uint8_t *)pBuffer++ = *(__IO uint8_t *)(deviceaddress);
    }
...
#if NAND_USE_INT2_ENABLE
  __FSMC_NAND_DISABLE_IT(hnand->Instance,FSMC_NAND_BANK2,FSMC_IT_IR);
#endif
  /* Update the NAND controller state */
  hnand->State = HAL_NAND_STATE_READY;
