...
void TIM_PWMIConfig(TIM_TypeDef* TIMx, TIM_ICInitTypeDef* TIM_ICInitStruct)
{
  ...
  else if (TIM_ICInitStruct->TIM_ICPolarity == TIM_ICPolarity_Falling)   //jane changed
  {
    icoppositepolarity = TIM_ICPolarity_Rising;
  }
  else		//jane add
  {
    icoppositepolarity = TIM_ICPolarity_BothEdge;
  }
...
/**
  ...
  * @param  TIMx: where x can be  1, 2, 3, 4, 5 8 to select the TIM peripheral.//jane add 8
                                ...
                                uint16_t TIM_IC1Polarity, uint16_t TIM_IC2Polarity)
{
  ...
  tmpccer &= (uint16_t)(((uint16_t)~(((uint16_t)TIM_CCER_CC1P)|((uint16_t)TIM_CCER_CC1NP))) 
  				& ((uint16_t)~(((uint16_t)TIM_CCER_CC2P)|((uint16_t)TIM_CCER_CC2NP)))); //Jane changed
                       ...
                       uint16_t TIM_ICFilter)
{
 ...
 // }
  ...
  /**
	* @brief  select the pwm output clear source.
	* @param  TIMx: where x can be 1 or 8 to select the TIM peripheral.
	* @param  NewState: new state of the TIMx Hall sensor interface.
	*	This parameter can be: 
		ENABLE ----- selce V4 to clear PWM 
	 	DISABLE ---- select external pin to clear the PWM.
	* @retval None
	*/
void TIM_PWMClear(TIM_TypeDef* TIMx,FunctionalState NewState)
{
	  /* Check the parameters */
	  assert_param(IS_TIM_LIST6_PERIPH(TIMx));
	  assert_param(IS_FUNCTIONAL_STATE(NewState));
	  if (NewState != DISABLE)
	  {
		/* Set the VC4 clear PWM output Bit */
		TIMx->CR1 |= TIM_PWMClear_VC4;
	  }
	  else
	  {
	    /* Set the external pin clear PWM output Bit */
		TIMx->CR1 &= (uint16_t)~((uint16_t)TIM_PWMClear_VC4);
	  }

}

 /**
	* @brief  select the pwm brake source.
	* @param  TIMx: where x can be 1 or 8 to select the TIM peripheral.
	* @param  NewState: new state of the TIMx Hall sensor interface.
	*	This parameter can be: 
		ENABLE ----- selce V1 VC2 or VC3 to brake PWM 
	 	DISABLE ---- select external pin to brake PWM.
	* @retval None
	*/
void TIM_PWMBrake(TIM_TypeDef* TIMx,FunctionalState NewState)
{
	  /* Check the parameters */
	  assert_param(IS_TIM_LIST6_PERIPH(TIMx));
	  assert_param(IS_FUNCTIONAL_STATE(NewState));
	  ...
	  {
		/* Set the VC4 clear PWM output Bit */
		TIMx->CR1 |= TIM_PWMBrake_VC;
	  }
	  else
	  {
	    /* Set the external pin clear PWM output Bit */
		TIMx->CR1 &= (uint16_t)~((uint16_t)TIM_PWMBrake_VC);
	  }
