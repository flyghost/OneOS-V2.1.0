...
int hk32_flash_write(os_uint32_t addr, const os_uint8_t *buf, size_t size)
{
    ...
    if (addr % 2 != 0)
    {
        tmpData = *(__IO uint16_t*)(addr - 1);
        tmpData &= 0x00ff;
        tmpData += (((uint16_t)(*buf))<<8);
        FLASH_ProgramHalfWord(addr-1, tmpData);
        len -= 1;
        addr += 1;
        offset = 1;
    }
    ...
    for (i = 0; i < hw_cnt; i++)
    {
        FLASH_ProgramHalfWord(addr+i*2,*((uint16_t *)(buf+i*2+ offset)));
    }
    ...
    if (len % 2 != 0)
    {
        tmpData = buf[i*2];
        tmpData <<= 8;
        tmpData += (*(__IO uint16_t*)(addr+i*2))&0x00ff;
        FLASH_ProgramHalfWord(addr + i * 2, tmpData);
    }
...
int hk32_flash_erase(os_uint32_t addr, size_t size)
{
    ...
    FLASH_Unlock();
    status = FLASH_WaitForLastBank1Operation(0xffffff);
