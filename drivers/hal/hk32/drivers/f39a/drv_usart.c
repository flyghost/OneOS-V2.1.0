...
void hk32_usart_dma_callback(DMA_HandleTypeDef *hdma)
{
    hk32_usart_t *uart = os_container_of(hdma, hk32_usart_t, hdma);
...
static os_uint32_t hk32_sdma_dma_get_index(soft_dma_t *dma)
{
    ...
    return uart->sdma_hard_size - __HAL_DMA_GET_COUNTER(&uart->hdma);
}
...
static os_err_t hk32_sdma_dma_init(soft_dma_t *dma)
{
    ...
    uart->hdma.Instance = uart->info->dma_channel;
    uart->hdma.Init.Direction = DMA_PERIPH_TO_MEMORY;
    uart->hdma.Init.PeriphInc = DMA_PINC_DISABLE;
    uart->hdma.Init.MemInc = DMA_MINC_ENABLE;
    uart->hdma.Init.PeriphDataAlignment = DMA_PDATAALIGN_BYTE;
    uart->hdma.Init.MemDataAlignment = DMA_MDATAALIGN_BYTE;
    uart->hdma.Init.Mode = DMA_CIRCULAR;
    uart->hdma.Init.Priority = DMA_PRIORITY_HIGH;

    HAL_DMA_RegisterCallback(&uart->hdma, HAL_DMA_XFER_HALFCPLT_CB_ID, hk32_usart_dma_callback);
    HAL_DMA_RegisterCallback(&uart->hdma, HAL_DMA_XFER_CPLT_CB_ID,     hk32_usart_dma_callback);

    HAL_NVIC_SetPriority(uart->info->dma_irq, 0, 1);
    HAL_NVIC_EnableIRQ(uart->info->dma_irq);
...
static os_err_t hk32_sdma_dma_start(soft_dma_t *dma, void *buff, os_uint32_t size)
{
    ...
    HAL_DMA_Init(&uart->hdma);

    HAL_DMA_Start_IT(&uart->hdma, (uint32_t)&uart->info->huart->DR, (uint32_t)buff, size);
...
static os_err_t hk32_usart_init(struct os_serial_device *serial, struct serial_configure *cfg)
{
    ...
    HAL_NVIC_SetPriority(uart->info->irq, 3, 3);
    HAL_NVIC_EnableIRQ(uart->info->irq);
