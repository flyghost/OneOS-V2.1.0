...
/* interrupt rx mode */
static os_uint32_t lpc_sdma_int_get_index(soft_dma_t *dma)
{
...
static os_err_t lpc_sdma_int_start(soft_dma_t *dma, void *buff, os_uint32_t size)
{
    status_t status = kStatus_Success;
...
static os_uint32_t lpc_sdma_int_stop(soft_dma_t *dma)
{
    status_t status = kStatus_Success;
    lpc_usart_t *usart = os_container_of(dma, lpc_usart_t, sdma);    
...
/* dma rx mode */
static os_uint32_t lpc_sdma_dma_get_index(soft_dma_t *dma)
{
    lpc_usart_t *usart = os_container_of(dma, lpc_usart_t, sdma);
...
static os_err_t lpc_sdma_dma_init(soft_dma_t *dma)
{
    lpc_usart_t *usart = os_container_of(dma, lpc_usart_t, sdma);
...
static os_err_t lpc_sdma_dma_start(soft_dma_t *dma, void *buff, os_uint32_t size)
{
    status_t status = kStatus_Success;
    lpc_usart_t *usart = os_container_of(dma, lpc_usart_t, sdma);
    ...
    if (usart->usart_DmaHandle->rxDmaHandle != OS_NULL && usart->usart_DmaHandle != OS_NULL)
    {
        ...
        {
            LOG_E(DRV_EXT_TAG, "usart need buff size need 128 bytes or more!");
            return OS_ERROR;
        }
...
static os_uint32_t lpc_sdma_dma_stop(soft_dma_t *dma)
{
    lpc_usart_t *usart = os_container_of(dma, lpc_usart_t, sdma);
...
static void lpc_usart_sdma_init(struct usart *usart, dma_ring_t *ring)
{
    ...
    if (usart->usart_handle != OS_NULL)
    {
        ...
        dma->ops.get_index                  = lpc_sdma_int_get_index;
        dma->ops.dma_init                   = OS_NULL;
        dma->ops.dma_start                  = lpc_sdma_int_start;
        dma->ops.dma_stop                   = lpc_sdma_int_stop;
    }
    else if (usart->usart_DmaHandle->rxDmaHandle != OS_NULL && usart->usart_DmaHandle != OS_NULL)
    {
        ...
        dma->ops.get_index                  = lpc_sdma_dma_get_index;
        dma->ops.dma_init                   = lpc_sdma_dma_init;
        dma->ops.dma_start                  = lpc_sdma_dma_start;
        dma->ops.dma_stop                   = lpc_sdma_dma_stop;
    }
    else
    {
