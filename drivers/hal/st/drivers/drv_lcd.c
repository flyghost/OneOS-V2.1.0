...
static os_err_t stm32_lcd_init(struct stm32_lcd *lcd)
{
    ...
    lcd->graphic.info.framebuffer      = (void *)os_aligned_malloc(0x200, LCD_WIDTH * LCD_HEIGHT * (LCD_BITS_PER_PIXEL / 8));
    lcd->graphic.info.framebuffer_avail= 0;
    lcd->graphic.info.framebuffer_num  = 1;
...
OS_DRIVER_DEFINE(stm32_lcd_driver,DEVICE,OS_INIT_SUBLEVEL_MIDDLE);
