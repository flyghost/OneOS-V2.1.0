...
static void lptim_callback(LPTIM_HandleTypeDef *hlptim)
{
...
static void stm32_lptimer_start(os_clockevent_t *ce, os_uint32_t prescaler, os_uint64_t count)
{
    ...
    HAL_LPTIM_TimeOut_Start_IT(lptim, 0xFFFF, count);
}
...
OS_DRIVER_DEFINE(stm32_lptim_driver, PREV, OS_INIT_SUBLEVEL_HIGH);
