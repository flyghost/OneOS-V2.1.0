...
static int stm32_qspi_init(struct os_qspi_device *device, struct os_qspi_configuration *qspi_cfg)
{
    ...
    while (cfg->max_hz < HAL_RCC_GetHCLKFreq() / (i + 1))
    {
        ...
        if (i == 255)
        {
            LOG_E(DBG_TAG,"QSPI init failed, QSPI frequency(%d) is too low.", cfg->max_hz);
            return OS_ERROR;
        }
    }
    /* 80/(1+i) */
    ...
    if (result == HAL_OK)
    {
        LOG_D(DBG_TAG, "qspi init success!");
    }
    else
    {
        LOG_E(DBG_TAG,"qspi init failed (%d)!", result);
    }
...
static os_uint32_t qspixfer(struct os_spi_device *device, struct os_spi_message *message)
{
    ...
    if (sndb)
    {
        ...
        if (qspi_message->parent.length != 0)
        {
            ...
            {
                LOG_E(DBG_TAG,"QSPI send data failed(%d)!", QSPI_Handler->ErrorCode);
    ...
    else if (rcvb) /* recv data */
    {
        ...
        {
            LOG_E(DBG_TAG,"QSPI recv data failed(%d)!", QSPI_Handler->ErrorCode);
            QSPI_Handler->State = HAL_QSPI_STATE_READY;
            goto __exit;
        }
    }
                                      ...
                                      void (*exit_qspi_mode)())
{
    ...
    {
        LOG_E(DBG_TAG,"no memory, qspi bus attach device failed!");
        result = OS_ENOMEM;
        goto __exit;
    }
