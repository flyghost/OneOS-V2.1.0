 ...
 * @file        drv_flash_f3.c
 *
 * @brief       This file provides flash read/write/erase functions for f3.
 *
 * @revision
 * Date         Author          Notes
...
int stm32_flash_write(os_uint32_t addr, const os_uint8_t *buf, size_t size)
{
    ...
    if ((addr + size) > STM32_FLASH_END_ADDRESS)
    {
        LOG_E(LOG_TAG,"ERROR: write outrange flash size! addr is (0x%p)\r\n", (void *)(addr + size));
        return OS_EINVAL;
    }

    if (addr % 4 != 0)
    {
        LOG_E(LOG_TAG,"write addr must be 4-byte alignment");
        return OS_EINVAL;
    }
...
int stm32_flash_erase(os_uint32_t addr, size_t size)
{
    ...
    if ((addr + size) > STM32_FLASH_END_ADDRESS)
    {
        LOG_E(LOG_TAG,"ERROR: erase outrange flash size! addr is (0x%p)\r\n", (void *)(addr + size));
        return OS_EINVAL;
    }
...
__exit:
    ...
    LOG_D(LOG_TAG,"erase done: addr (0x%p), size %d", (void *)addr, size);
    return size;
}
