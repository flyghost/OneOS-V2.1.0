...
int stm32_flash_erase(os_uint32_t addr, size_t size)
{
    ...
    uint32_t FLASH_BANK1_END = FLASH_BASE + FLASH_BANK_SIZE - 1;
#endif
    ...
    {
     /* the blcok only located on bank1 */ 
        if((addr + size - 1) <= FLASH_BANK1_END)
        {
            EraseInitStruct.Banks = FLASH_BANK_1;
            EraseInitStruct.NbPages   = (size + FLASH_PAGE_SIZE - 1) / FLASH_PAGE_SIZE;           
        }
        ...
        {   
        /* the blcok located on bank1 and bank2 */          
            /* case1: erase the bank1 */
            EraseInitStruct.Banks = FLASH_BANK_1;
            EraseInitStruct.NbPages   = ((FLASH_BANK1_END - addr) + FLASH_PAGE_SIZE) / FLASH_PAGE_SIZE;
            if (HAL_FLASHEx_Erase(&EraseInitStruct, &PAGEError) != HAL_OK)
            {
                result = OS_ERROR;
                goto __exit;
            }                        
            /* case2: erase the bank2 */
            EraseInitStruct.Page      = GetPage(FLASH_BANK1_END + 1);
            EraseInitStruct.NbPages   = ((size + addr - (FLASH_BANK1_END + 1)) + FLASH_PAGE_SIZE - 1) / FLASH_PAGE_SIZE;
            EraseInitStruct.Banks = FLASH_BANK_2;                      
        }
    }
    else
    {
     /* the blcok only located on bank2 */ 
        EraseInitStruct.Banks = FLASH_BANK_2;
        EraseInitStruct.NbPages   = (size + FLASH_PAGE_SIZE - 1) / FLASH_PAGE_SIZE;      
    }    
#endif
