...
static uint32_t FLASH_ADDR_BASE;
static uint32_t FLASH_ADDR_END;
static uint32_t FLASH_ERASE_TYPE;
static uint32_t FLASH_TYPEPROGRAM;

static void is_secure(os_uint32_t page_addr)
{
    if(page_addr * STM32_FLASH_PAGE_SIZE < SECURE_ZONE)
    {
        FLASH_ADDR_BASE     = FLASH_BASE_S;
        FLASH_ADDR_END      = FLASH_BASE_S + STM32_FLASH_SIZE;
        FLASH_ERASE_TYPE    = FLASH_TYPEERASE_PAGES;
        FLASH_TYPEPROGRAM = FLASH_TYPEPROGRAM_DOUBLEWORD;
    }
    else
    {
        FLASH_ADDR_BASE     = FLASH_BASE_NS;
        FLASH_ADDR_END      = FLASH_BASE_NS + STM32_FLASH_SIZE;
        FLASH_ERASE_TYPE    = FLASH_TYPEERASE_PAGES;//FLASH_TYPEERASE_PAGES_NS;
        FLASH_TYPEPROGRAM = FLASH_TYPEPROGRAM_DOUBLEWORD;//FLASH_TYPEPROGRAM_DOUBLEWORD_NS;
    }
}
...
static uint32_t GetPage(uint32_t Addr)
{
    ...
    page = (Addr - FLASH_ADDR_BASE) / FLASH_PAGE_SIZE;
...
#else
    /* No Bank swap */
    if (Addr < (FLASH_ADDR_BASE + FLASH_BANK_SIZE))
    {
        bank = FLASH_BANK_1;
    }
    else
...
int stm32_flash_write(os_uint32_t addr, const uint8_t *buf, size_t size)
{
    ...
    for (i = 0; i < size;)
    {
        ...
        /* write data */
        status = HAL_FLASH_Program(FLASH_TYPEPROGRAM, addr, write_data);
...
int stm32_flash_erase(os_uint32_t addr, size_t size)
{
    ...
    EraseInitStruct.TypeErase = FLASH_ERASE_TYPE; /* FLASH_TYPEERASE_MASSERASE or FLASH_TYPEERASE_PAGES */
