...
static int stm32_flash_read_page(fal_flash_t *flash, os_uint32_t page_addr, os_uint8_t *buff, os_uint32_t page_nr)
{
    ...
    int count = stm32_flash_read(flash_info->start_addr + page_addr * flash->page_size, buff, page_nr * flash->page_size);
...
static int stm32_flash_write_page(fal_flash_t *flash, os_uint32_t page_addr, const os_uint8_t *buff, os_uint32_t page_nr)
{
    ...
    int count = stm32_flash_write(flash_info->start_addr + page_addr * flash->page_size, buff, page_nr * flash->page_size);
...
static int stm32_flash_erase_block(fal_flash_t *flash, os_uint32_t page_addr, os_uint32_t page_nr)
{
    ...
    int count =  stm32_flash_erase(flash_info->start_addr + page_addr * flash->page_size, page_nr * flash->page_size);

    return (count == page_nr * flash->page_size) ? 0 : -1;
}

static int stm32_flash_probe(const os_driver_info_t *drv, const os_device_info_t *dev)
{
