...
void mic_entry(void *parameter)
{
    ...
    mic.buffer = os_calloc(1, RECORD_BUFFER_SZ);
    ...
    if (mic.dev == OS_NULL)
    {
        LOG_E(DBG_TAG,"can't find device:%s", MIC_DEVICE_NAME);
        goto __exit;
    }
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            if (e & EVENT_RECORD_DATA)
            {
                ...
                os_device_read_nonblock(mic.dev, 0, mic.buffer + index, RECORD_BUFFER_SZ / 2);
            }
