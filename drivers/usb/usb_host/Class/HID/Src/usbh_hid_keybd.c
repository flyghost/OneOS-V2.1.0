...
#define OS_DEVICE_CTRL_KEYBD_SET_CALLBACK   IOC_USBDEVICE(0)        /* Set Callback. */
static USBH_StatusTypeDef(* keybd_data_in_callback)(HID_KEYBD_Info_TypeDef *keybd_info);

static os_err_t os_keybd_control(os_device_t *dev, int cmd, void *args)
{
...
//    struct os_device *mouse = (struct os_device *)dev;
    ...
    switch (cmd)
    {
    case OS_DEVICE_CTRL_KEYBD_SET_CALLBACK:
        // set callback
        keybd_data_in_callback = (USBH_StatusTypeDef(* )(HID_KEYBD_Info_TypeDef *keybd_info))args;
        break;
    default:
        break;
    }
...
static struct os_device *keybd_dev = NULL;
const static struct os_device_ops keybd_ops = {
    .control = os_keybd_control,
};

os_err_t usbh_keybd_dev_register(USBH_HandleTypeDef *phost, const char *name, void *data)
{
    os_err_t          ret = OS_EOK;
    
    keybd_dev = os_calloc(1, sizeof(struct os_device));
    
    keybd_dev->type = OS_DEVICE_TYPE_USBDEVICE;
    keybd_dev->ops = &keybd_ops;
    
    ret = os_device_register(keybd_dev, name);

    return ret;
}

os_err_t usbh_keybd_dev_unregister()
{
    os_err_t          ret = osOK;
    
    ret = os_device_unregister(keybd_dev);
    keybd_dev = NULL;
...
USBH_StatusTypeDef USBH_HID_KeybdInit(USBH_HandleTypeDef *phost)
{
    ...
    usbh_keybd_dev_register(phost, "keybd", NULL);

    return USBH_OK;
}

USBH_StatusTypeDef USBH_HID_KeybdDeInit(USBH_HandleTypeDef *phost)
{
  

  usbh_keybd_dev_unregister();
  
  return USBH_OK;
}

USBH_StatusTypeDef USBH_HID_Keybd_Callback(USBH_HandleTypeDef *phost)
{
    if (keybd_data_in_callback != NULL)
    {
        keybd_data_in_callback(USBH_HID_GetKeybdInfo(phost));
    }
    return USBH_OK;
}
