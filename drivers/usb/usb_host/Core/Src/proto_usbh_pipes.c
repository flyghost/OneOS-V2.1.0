...
{
  phost->ll_ops->LL_OpenPipe(phost, pipe_num, epnum, dev_address, speed, ep_type, mps);
...
{
  phost->ll_ops->LL_ClosePipe(phost, pipe_num);
...
{
    return phost->ll_ops->LL_AllocPipe(phost, ep_addr);
}
...
{
    return phost->ll_ops->LL_FreePipe(phost, idx);
}
