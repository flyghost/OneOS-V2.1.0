...
def GetGCCRoot(osconfig):
    exec_path = osconfig.COMPILER_PATH
    prefix = osconfig.PREFIX
...
def CheckHeader(osconfig, filename):
    root = GetGCCRoot(osconfig)
...
def GetNewLibVersion(osconfig):
    version = 'unknown'
    root = GetGCCRoot(osconfig)

    if CheckHeader(osconfig, '_newlib_version.h'): # get version from _newlib_version.h file
        ...
        if f:
            ...
            f.close()
    elif CheckHeader(osconfig, 'newlib.h'): # get version from newlib.h
...
def GCCResult(osconfig, str):
    ...
    gcc_cmd = os.path.join(osconfig.COMPILER_PATH, osconfig.CC)
...
def GenerateGCCConfig(osconfig):
    ...
    cc_header += '\n'
    cc_header += GCCResult(osconfig, str)
    cc_header += '\n#endif\n'
