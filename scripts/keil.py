...
def _get_filetype(fn):
    if fn.rfind('.cpp') != -1 or fn.rfind('.cxx') != -1 or fn.rfind('.cc') != -1:
        return 8
...
def MDK45Project(tree, target, script):
    ...
    for group in script:
        ...
        # get each group's definitions
        if 'ASDEFINES' in group and group['ASDEFINES']:
            if ASDEFINES:
                ASDEFINES += group['ASDEFINES']
            else:
                ASDEFINES = group['ASDEFINES']
    ...
    Define = tree.find('Targets/Target/TargetOption/TargetArmAds/Aads/VariousControls/Define')
    Define.text = ', '.join(set(ASDEFINES))

    Misc = tree.find('Targets/Target/TargetOption/TargetArmAds/LDads/Misc')

    if Misc.text is None:
        Misc.text = ', '.join(set(LINKFLAGS))
    else:
        Misc.text += LINKFLAGS
...
def ARMCC_Version():
    ...
    path = osconfig.COMPILER_PATH
    path = os.path.join(path, 'armcc.exe')

    if os.path.exists(path):
        cmd = path
    else:
        print('Error: get armcc version failed. Please update the KEIL MDK installation path in osconfig.py!')
        return "0.0"
