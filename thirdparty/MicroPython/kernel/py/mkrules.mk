...
vpath %.S . $(TOP_KERNEL) $(USER_C_MODULES)
$(BUILD)/%.o: %.S
	$(ECHO) "CC $<"
	$(Q)$(CC) $(CFLAGS) -c -o $@ $<
...
vpath %.c . $(TOP_KERNEL) $(USER_C_MODULES)
$(BUILD)/%.o: %.c
	$(call compile_c)
...
$(BUILD)/frozen_content.c: FORCE $(BUILD)/genhdr/qstrdefs.generated.h
	$(Q)$(MAKE_MANIFEST) -o $@ -v "MPY_DIR=$(TOP_USER)/machine/driver/unix" -v "MPY_LIB_DIR=$(MPY_LIB_DIR)" -v "PORT_DIR=$(shell pwd)" -v "BOARD_DIR=$(BOARD_DIR)" -b "$(BUILD)" $(if $(MPY_CROSS_FLAGS),-f"$(MPY_CROSS_FLAGS)",) $(FROZEN_MANIFEST)
...
ifneq ($(GIT_SUBMODULES),)
	$(Q)git submodule update --init $(addprefix $(TOP_KERNEL)/,$(GIT_SUBMODULES))
endif
.PHONY: submodules
