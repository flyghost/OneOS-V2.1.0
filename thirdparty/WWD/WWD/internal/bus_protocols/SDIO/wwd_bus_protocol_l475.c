...
wwd_result_t wwd_bus_init(void)
{
    ...
    mmcsd_set_bus_width(wwd_dev->card->host, 2);
...
wwd_result_t wwd_bus_sdio_transfer( wwd_bus_transfer_direction_t direction, wwd_bus_function_t function, uint32_t address, uint16_t data_size, /*@in@*/ /*@out@*/ uint8_t* data, sdio_response_needed_t response_expected )
{
    wwd_result_t result;
    ...
    do
    {
        ...
        else
        {
            ...
            while (left_size > 0)
            {
                int size = min(left_size, 512);
            
                void *_data = data + data_index;
            
                if (direction == BUS_READ)
                {
                    _data = os_malloc(size + 512);
                    if (_data == OS_NULL)
                        return WWD_IOCTL_FAIL;
                }

                result = wwd_bus_sdio_cmd53( direction, function, ( size > (uint16_t) 512 ) ? SDIO_BLOCK_MODE : SDIO_BYTE_MODE, address, size, _data, response_expected, NULL );

                if (direction == BUS_READ)
                {
                    memcpy(data + data_index, _data, size);
                    os_free(_data);
                }
...
static wwd_result_t wwd_bus_sdio_cmd53( wwd_bus_transfer_direction_t direction, wwd_bus_function_t function, sdio_transfer_mode_t mode, uint32_t address, uint16_t data_size, /*@in@*/ /*@out@*/ uint8_t* data, sdio_response_needed_t response_expected, /*@null@*/ uint32_t* response )
{
    ...
    {
        arg.cmd53.count = (unsigned int) ( ( data_size / (uint16_t)SDIO_512B_BLOCK ) & 0x0000001ff );
        if ( (uint32_t) ( arg.cmd53.count * (uint16_t)SDIO_512B_BLOCK ) < data_size )
        {
            ++arg.cmd53.count;
        }
    ...
    result = host_platform_sdio_transfer( direction, SDIO_CMD_53, mode, SDIO_512B_BLOCK, arg.value, (uint32_t*) data, data_size, response_expected, response );
