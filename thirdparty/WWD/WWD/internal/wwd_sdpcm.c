...
#define WWD_IOCTL_TIMEOUT_MS         (1000)
...
/*@null@*/ /*@exposed@*/ void* wwd_sdpcm_get_iovar_buffer( /*@special@*/ /*@out@*/ wiced_buffer_t* buffer, uint16_t data_length, const char* name )  /*@allocates *buffer@*/ /*@defines **buffer@*/
{
    ...
    if ( internal_host_buffer_get( buffer, WWD_NETWORK_TX, (unsigned short) ( IOCTL_OFFSET + data_length + name_length + name_length_alignment_offset ), (unsigned long) 0xFFFFFFFF ) == WWD_SUCCESS )
    {
...
{
    if ( internal_host_buffer_get( buffer, WWD_NETWORK_TX, (unsigned short) ( IOCTL_OFFSET + data_length ), (unsigned long) 0xFFFFFFFF ) == WWD_SUCCESS )
    {
...
int wwd_sdpcm_data_header_size(void)
{
    return sizeof(sdpcm_data_header_t);
}
