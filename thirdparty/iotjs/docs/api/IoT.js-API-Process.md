...
The `memstat` property holds IoT.js memory usage state.
The following memstat elements can be accessed:
* `total` total available bytes.
* `allocated` used bytes.
* `peak` peak memory used bytes.

**Example**

```js
console.log(process.memstat);
memstat:
  total : 262136
  allocated : 12056
  peak : 12352
