...
extern int tuv_work_running();

static iotjs_injection_func_t s_init_hook;
static iotjs_injection_func_t s_deinit_hook;
...
int iotjs_entry(int argc, char** argv) {
  ...
  while(tuv_work_running())
    os_task_tsleep(1);

  if (s_deinit_hook != NULL)
    s_deinit_hook();
...
void iotjs_injection_hook_register(iotjs_injection_func_t init, iotjs_injection_func_t deinit) {
  s_init_hook = init;
  s_deinit_hook = deinit;
}
