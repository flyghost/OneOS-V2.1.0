...
JS_FUNCTION(proc_memstat) {
  jerry_value_t stats = jerry_create_object();

  jmem_heap_stats_t heap_stats;
  jmem_heap_get_stats (&heap_stats);

  iotjs_jval_set_property_number(stats, "total",
                        heap_stats.size);

  iotjs_jval_set_property_number(stats, "allocated",
                        heap_stats.allocated_bytes);

  iotjs_jval_set_property_number(stats, "peak",
...
#if ENABLED (JERRY_MEM_STATS)
  iotjs_jval_set_method(process, IOTJS_MAGIC_STRING_MEMSTAT, proc_memstat);
#endif
#ifdef EXPOSE_GC
