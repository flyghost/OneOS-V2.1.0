...
jmem_heap_init (void)
{
  ...
  #if JERRY_GLOBAL_NON_STATIC
    jerry_global_heap.area = (uint8_t *)jerry_port_memalloc(JMEM_HEAP_AREA_SIZE);
  #endif
...
#endif /* !ENABLED (JERRY_SYSTEM_ALLOCATOR) */
#if JERRY_GLOBAL_NON_STATIC
  jerry_port_memfree(jerry_global_heap.area);
#endif
} /* jmem_heap_finalize */
...
#if !ENABLED (JERRY_SYSTEM_ALLOCATOR)
  JERRY_DEBUG_MSG ("  Heap size = %d bytes\n",
                   heap_stats->size);
#endif /* !ENABLED (JERRY_SYSTEM_ALLOCATOR) */
  JERRY_DEBUG_MSG ("  Allocated = %d bytes\n",
                   heap_stats->allocated_bytes);
  JERRY_DEBUG_MSG ("  Peak allocated = %d bytes\n",
                   heap_stats->peak_allocated_bytes);
  JERRY_DEBUG_MSG ("  Waste = %d bytes\n",
                   heap_stats->waste_bytes);
  JERRY_DEBUG_MSG ("  Peak waste = %d bytes\n",
                   heap_stats->peak_waste_bytes);
  JERRY_DEBUG_MSG ("  Allocated byte code data = %d bytes\n",
                   heap_stats->byte_code_bytes);
  JERRY_DEBUG_MSG ("  Peak allocated byte code data = %d bytes\n",
                   heap_stats->peak_byte_code_bytes);
  JERRY_DEBUG_MSG ("  Allocated string data = %d bytes\n",
                   heap_stats->string_bytes);
  JERRY_DEBUG_MSG ("  Peak allocated string data = %d bytes\n",
                   heap_stats->peak_string_bytes);
  JERRY_DEBUG_MSG ("  Allocated object data = %d bytes\n",
                   heap_stats->object_bytes);
  JERRY_DEBUG_MSG ("  Peak allocated object data = %d bytes\n",
                   heap_stats->peak_object_bytes);
  JERRY_DEBUG_MSG ("  Allocated property data = %d bytes\n",
                   heap_stats->property_bytes);
  JERRY_DEBUG_MSG ("  Peak allocated property data = %d bytes\n",
