...
input_file            : ../waterheater/out_codegeneration_merged.swagger.json
version of input_file :  
title of input_file   : server_waterheater
*/
...
app_init(void)
{
    ...
    ret |= oc_add_device("/oic/d", "oic.d.waterheater", "server_waterheater", 
        "ocf.2.2.2", /* icv value */
...
initialize_variables(void)
{
    ...
    g_temperature_temperature = 0; /* current value of property "temperature"  The current temperature setting or measurement. */
    strcpy(g_temperature_units, """");  /* current value of property "units" The unit for the conveyed temperature value, Note that when doing an UPDATE, the unit on the device does NOT change, it only indicates the unit of the conveyed value during the UPDATE operation. */
...
main(void)
{
    ...
    PRINT("Used input file : \"../waterheater/out_codegeneration_merged.swagger.json\"\n");
    PRINT("OCF Server name : \"server_waterheater\"\n");
    ...
    PRINT("OCF server \"server_waterheater\" running, waiting on incoming connections.\n");
