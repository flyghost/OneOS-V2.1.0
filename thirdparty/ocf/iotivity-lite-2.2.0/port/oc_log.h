...
#else
extern void print_to_mem(const char *fmt, ...);
#define PRINT(...) print_to_mem(__VA_ARGS__)
#endif
#endif
...
#define OC_LOG(level, ...)                                                     \
  do {                                                                         \
    PRINT("%08d %s: <%s:%d>: ", os_tick_get(), level,  __func__, __LINE__);    \
    PRINT(__VA_ARGS__);                                                        \
    PRINT("\n");                                                               \
  } while (0)
...
#define OC_ERR(...)		\
  do {                                                                         \
    PRINT("%08d ERROR : <%s:%d>: ", os_tick_get(), __func__, __LINE__);        \
    PRINT(__VA_ARGS__);                                                        \
    PRINT("\n");                                                               \
  } while (0)
#define OC_LOGipaddr(endpoint)
