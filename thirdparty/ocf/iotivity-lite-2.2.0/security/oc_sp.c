...
{
#ifdef OC_DYNAMIC_ALLOCATION
  sp = (oc_sec_sp_t *)oc_calloc(oc_core_get_num_devices(), sizeof(oc_sec_sp_t));
  if (!sp) {
    oc_abort("Insufficient memory");
  }
  sp_mfg_default =
    (oc_sec_sp_t *)oc_calloc(oc_core_get_num_devices(), sizeof(oc_sec_sp_t));
  if (!sp_mfg_default) {
    oc_abort("Insufficient memory");
  }
#endif /* OC_DYNAMIC_ALLOCATION */
