...
ssl_send(void *ctx, const unsigned char *buf, size_t len)
{
  ...
  message.data = oc_alloc(OC_PDU_SIZE);
  if (!message.data)
    return 0;
#endif /* OC_DYNAMIC_ALLOCATION */
...
oc_tls_add_peer(oc_endpoint_t *endpoint, int role, oc_message_t *message)
{
  oc_tls_peer_t *peer = oc_tls_get_peer(endpoint);
  if (!peer) {
    if (MBEDTLS_SSL_IS_SERVER == role)
    {
      if(message->data[0] != MBEDTLS_SSL_MSG_HANDSHAKE)
      {
        OC_DBG("bad client hello message, discard");
        return NULL;
      }
    }
    //zhw bug fix end
...
oc_tls_init_connection(oc_message_t *message)
{
  ...
  if (!peer) {
    peer = oc_tls_add_peer(&message->endpoint, MBEDTLS_SSL_IS_CLIENT, NULL);      //zhw bug fix
  }
...
{
  oc_tls_peer_t *peer =
    oc_tls_add_peer(&message->endpoint, MBEDTLS_SSL_IS_SERVER, message);       //zhw bug fix
